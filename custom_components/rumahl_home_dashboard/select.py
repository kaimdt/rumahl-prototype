"""Select platform for rumahl Home Dashboard integration."""
...
async def async_setup_entry(
    ...
    """Set up rumahl Home Dashboard select entities."""
    entry_data = hass.data[DOMAIN][entry.entry_id]
    client = entry_data.get(DATA_CLIENT)
    coordinator = entry_data[DATA_COORDINATOR]

    async_add_entities([
        RumahlDashboardThemeSelect(coordinator, entry, client),
        RumahlDashboardPageSelect(coordinator, entry, client),
    ])


class _BaseSelect(CoordinatorEntity, SelectEntity):
    """Base for rumahl Dashboard select entities."""
...
class RumahlDashboardThemeSelect(_BaseSelect):
...
class RumahlDashboardPageSelect(_BaseSelect):
