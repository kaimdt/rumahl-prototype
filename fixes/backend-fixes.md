...
**Date:** 2026-05-23  
**Branch/workspace:** `/Users/kaimdt/Documents/GitHub/home-assistant-dashb/iora-os/backend/`
...
**Files changed:**
- `services/iora-home/src/auth.rs`
- `shared/iora-shared/src/system_config.rs`
- `services/iora-home/src/main.rs`

**Changes:**

### `shared/iora-shared/src/system_config.rs`
- Modified `jwt_secret()` to use a 3-tier priority:
  1. `IORA_JWT_SECRET` environment variable (explicit override)
...
- Removed hardcoded default `"iora-dev-jwt-change-in-production"`
- Added `persist_jwt_secret(secret: &str)` function so `iora-home` can persist a generated secret to both the cache and the auto-fallback
...
**File changed:** `services/iora-home/src/main.rs`
...
**File changed:** `services/iora-home/src/theme_handler.rs`
...
- `handle_install_theme_inline` (line 679) now validates the incoming `ThemeDefinition` before installing:
  ...
  2. Calls `iora_shared::manifest_validator::validate_theme_manifest(&manifest_json)`
...
**File changed:** `shared/iora-shared/src/manifest_validator.rs`
...
- `cargo check -p iora-shared` — **passes** (3 pre-existing warnings, no new warnings)
- `cargo check -p iora-home` — **passes** (120 pre-existing warnings, no new warnings or errors)
- `cargo test -p iora-shared --lib -- manifest_validator::tests` — **6/6 pass** (test_valid_theme_minimal, test_missing_css_variables, test_missing_id, test_valid_app, test_valid_plugin, test_wrapped_theme)
...
2. Consider upgrading the UUID v4 fallback in `system_config::jwt_secret()` to use `getrandom`/`OsRng` for a 64-byte secret directly (would require adding `rand` + `base64` crates to `iora-shared`'s `Cargo.toml`)
