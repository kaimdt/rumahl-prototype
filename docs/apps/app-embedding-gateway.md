...
Installed rumahl apps run inside the rumahl desktop as iframes — on their **own
origin**, served by the **App Embedding Gateway**. The gateway is a
transparent reverse proxy for apps rumahl controls. It is **not**
server-side rendering and **not** the rumahl Browser: the app content is fully
...
| Surface | Origin |
| --- | --- |
| rumahl Desktop | `https://rumahl.local` |
| App runtime (iframe) | `https://<app-id>.apps.rumahl.local/` |
| Example: Nextcloud | `https://nextcloud.apps.rumahl.local/` |
| Example: Files | `https://files.apps.rumahl.local/` |

The desktop route `/app/<id>` (e.g. `https://rumahl.local/app/nextcloud`)
...
1. The browser loads `https://<app-id>.apps.rumahl.local/…` (Host header).
2. rumahl-home's gateway middleware resolves the app id from the subdomain.
...
- `Content-Security-Policy` is **not** stripped: every directive is kept,
  ...
  the rumahl desktop origin, e.g. `frame-ancestors https://rumahl.local`.
...
- Apps are **never same-origin** with the desktop (separate subdomains), so
  ...
  localStorage/sessionStorage or rumahl-internal objects.
- rumahl auth is Bearer-JWT based; the legacy `rumahl_token` cookie is
...
- `mode`: `embedded` (default — inside the rumahl App Runner iframe) or
...
implementation for `wss://<app-id>.apps.rumahl.local/socket`.
...
certificates, set `RUMAHL_GATEWAY_INSECURE_UPSTREAM_TLS=1` to accept them
...
- **„App außerhalb von rumahl OS aufrufen“** — per-app switch that opens the
...
## rumahl ↔ App communication (postMessage bridge)
...
- `RUMAHL_APPS_HOST_SUFFIX` (default `.apps.rumahl.local`) and
  `RUMAHL_DESKTOP_ORIGIN` override the host suffix / desktop origin used for
  `frame-ancestors` and redirect rewriting.
- `RUMAHL_GATEWAY_INSECURE_UPSTREAM_TLS=1` accepts self-signed certificates
...
- The nginx template no longer adds a global `X-Frame-Options` header and
  ...
  handles apps centrally; `*.apps.rumahl.local` is routed to rumahl-home by the
