#!/bin/bash
# ============================================================================
# rumahl-dev-selfheal.sh - idempotent self-healing fixes for the rumahl Dev VM
# ============================================================================
...
#
#   ./rumahl-dev-selfheal.sh --apply    one-time fixes (guard patch, SSH
#                                     hardening, network watchdog install)
#   ./rumahl-dev-selfheal.sh --net      network integrity check + repair
#                                     (no IPv4 on an UP interface -> renew)
#
# Called by:
#   - the dev-manager daemon when the VM comes up (guest_fixes_applied flag)
#   - rumahl-dev-compat.sh during provisioning
#   - the in-guest systemd timer rumahl-dev-net-check.timer (--net only)
# ============================================================================
set -u

LOG_TAG="rumahl-dev-selfheal"
log() { logger -t "$LOG_TAG" "$*" 2>/dev/null || echo "$LOG_TAG: $*"; }

MODE="${1:---apply}"

# -- build-deadlock autofix --------------------------------------------------
# All rumahl-* services run as `cargo run -p rumahl-X` and serialize on the cargo
...
build_watchdog() {
    ...
    local waiters
    waiters=$(pgrep -f 'cargo run -p rumahl-' 2>/dev/null | sort -n | head -1)
    [ -n "$waiters" ] || return 0
    local count
    count=$(pgrep -fc 'cargo run -p rumahl-' 2>/dev/null || echo 0)
    [ "$count" -gt 3 ] || return 0
    local age
...
# QEMU slirp user-net answers ARP for guest IPs, which made the rumahl DHCP
...
# -- guard patch -------------------------------------------------------------
# The rumahl DHCP conflict guard runs ARP-based Duplicate Address Detection.
...
patch_guard() {
    local guard=/usr/lib/ora/rumahl-dhcp-conflict-guard.sh
...
# root - but the Vite dev server runs as `ora` and needs write access to
...
fix_frontend_ownership() {
    local dir=/home/ora/ora/frontend/node_modules
    if [ -d "$dir" ] && [ "$(stat -c %U "$dir" 2>/dev/null)" != "ora" ]; then
        chown -R ora:ora "$dir" 2>/dev/null \
            && log "node_modules ownership fixed (ora)" \
...
# The rumahl firewall (INPUT policy DROP) only opens the known service ports.
...
install_net_watchdog() {
    local script=/usr/lib/ora/rumahl-dev-net-check.sh
    local timer=/etc/systemd/system/rumahl-dev-net-check.timer
    local unit=/etc/systemd/system/rumahl-dev-net-check.service
    local self
    self=$(readlink -f "$0" 2>/dev/null || echo "$0")
    if [ -f "$script" ] && grep -q 'rumahl-dev-selfheal' "$script"; then
...
#!/bin/sh
# rumahl Dev VM network integrity check (installed by rumahl-dev-selfheal.sh)
set -u
LOG_TAG="rumahl-dev-selfheal"
for iface in /sys/class/net/*; do
    name=$(basename "$iface")
...
NETCHECKEOF
        ...
        cat > "$unit" <<'UNITEOF'
[Unit]
Description=rumahl Dev VM network integrity check
After=network-online.target
[Service]
Type=oneshot
ExecStart=/usr/lib/ora/rumahl-dev-net-check.sh
UNITEOF
        cat > "$timer" <<'TIMEREOF'
[Unit]
Description=rumahl Dev VM network integrity timer
[Timer]
OnBootSec=60
OnUnitActiveSec=30
AccuracySec=5
...
TIMEREOF
        ...
        systemctl enable --now rumahl-dev-net-check.timer 2>/dev/null || true
        log "net watchdog installed and enabled"
    fi
}

# -- GStreamer runtime for rumahl-browserd (WebRTC) --------------------------
# The rumahl Browser renders via WebRTC (webrtcbin in plugins-bad, vp8enc in
...
gstreamer_check() {
    ...
    log "installing GStreamer packages for rumahl-browserd WebRTC…"
