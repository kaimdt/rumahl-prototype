...
apply_post_install_config() {
    ...
    if [ -n "$RUMAHL_ROOT_PW" ]; then
        ...
        local hash=$(echo "$RUMAHL_ROOT_PW" | openssl passwd -6 -stdin -salt "$salt" 2>/dev/null || true)
...
[Network]
Address=${RUMAHL_IP}/${RUMAHL_NETMASK:-24}
Gateway=${RUMAHL_GATEWAY:-}
DNS=${RUMAHL_DNS:-8.8.8.8}
NETEOF
    fi
...
screen_welcome() {
    if [ -n "$DIALOG_BIN" ]; then
                dlg --title " rumahl OS Setup " --msgbox "\
 Ready to deploy rumahl OS.
