#!/usr/bin/env bash
# ============================================================================
# dev-watch.sh – rumahl OS Dev-Loop (watch → build → mirror-deploy)
# ============================================================================
...
# (/home/ora/ora/.rumahl-dev/binaries). The in-VM hot-reload daemon
# (rumahl-hot-reload.service, build mode) installs them to /usr/bin/rumahl-*
...
#   ./dev-watch.sh --ssh-key FILE        SSH key (default <repo>/rumahl-os/.cache/rumahl-dev-key)
...
# ── Workspace detection ───────────────────────────────────────────────────
if [ -f "$REPO_ROOT/rumahl-os/backend/Cargo.toml" ]; then
    WORKSPACE="$REPO_ROOT/rumahl-os/backend"
elif [ -f "$REPO_ROOT/backend/Cargo.toml" ]; then
    WORKSPACE="$REPO_ROOT/backend"
else
    die "Cannot find Rust workspace (looked for rumahl-os/backend or backend)"
fi

# VM-side paths (for in-VM build strategy)
VM_WORKSPACE="/home/ora/ora/rumahl-os/backend"
...
# ── Defaults / args ───────────────────────────────────────────────────────
TARGET="${RUMAHL_DEV_TARGET:-x86_64-unknown-linux-gnu}"
VM_HOST="127.0.0.1"
VM_PORT=2222
...
# .rumahl-dev), so the mirror watcher never interferes with it.
MIRROR_DROPBOX="/home/ora/ora/.rumahl-dev/binaries"
MIRROR_NO_RESTART_FLAG="/etc/ora/.no-restart"
...
# Scan workspace for crates with binary targets named `rumahl-*`. We try
...
discover_services() {
    ...
    for base in "${dirs[@]}"; do
        ...
        for d in "$base"/*/; do
            ...
            local name
            name=$(basename "$d")
            # Only deploy crates whose binary will start with rumahl-
            [[ "$name" == rumahl-* ]] || continue
            out+=("$name")
        done
    done
...
log "Discovered ${#ALL_SERVICES[@]} rumahl-* crates"
...
# $MIRROR_DROPBOX, installs them to /usr/bin/rumahl-* and restarts the
...
#   * rumahl-home bootstrap role-fix is now idempotent and only sleeps when
...
deploy_many() {
    ...
    for entry in "${to_deploy[@]}"; do
        local svc="${entry%%:*}"
        local svc_short="${svc#rumahl-}"
        [ "$svc" = "rumahl-home" ] && home_changed=true
        remote_script+="install -m 0755 '$vm_target/$svc' '$MIRROR_DROPBOX/$svc' 2>/dev/null"$'\n'
        remote_script+="mkdir -p /etc/ora/db-credentials /tmp/rumahl-sandboxes /opt/rumahl/build/$svc/data /etc/systemd/system/$svc.service.d"$'\n'
        remote_script+="[ -f /etc/ora/db-credentials/$svc.env ] || echo 'DATABASE_URL=postgres://root:ora@localhost/rumahl_${svc_short}' > /etc/ora/db-credentials/$svc.env"$'\n'
        remote_script+="[ -f /etc/ora/$svc.env ] || printf 'DATABASE_URL=postgres://root:ora@localhost:5432/rumahl_${svc_short}\nRUST_LOG=${svc}=debug\n' > /etc/ora/$svc.env"$'\n'
        ...
        if [ "$svc" = "rumahl-home" ]; then
            remote_script+="grep -q RUMAHL_BOOTSTRAP_ADMIN_USER /etc/ora/rumahl-home.env 2>/dev/null || printf 'RUMAHL_BOOTSTRAP_ADMIN_USER=admin\nRUMAHL_BOOTSTRAP_ADMIN_PASSWORD=admin1234\n' >> /etc/ora/rumahl-home.env"$'\n'
        fi
    done
    if $DO_RESTART; then
    ...
    remote_script+="chown -R ora:ora '$MIRROR_DROPBOX' 2>/dev/null"$'\n'

    if ! ssh_vm "$remote_script" >/dev/null 2>&1; then
        warn "  deploy: mirror drop-box update reported errors (continuing)"
    fi

    # ── 4. Idempotent admin-role fix (only if rumahl-home changed and only
    ...
    if $home_changed; then
        (
            ...
            if ! ssh_vm "test -f /var/lib/ora/.admin-role-fixed" 2>/dev/null; then
                # Wait up to 8s for rumahl-home to finish bootstrap.
                local i
                for i in 1 2 3 4 5 6 7 8; do
                    if ssh_vm "su - postgres -c \"psql -tAc 'SELECT 1 FROM users LIMIT 1' rumahl_home\"" 2>/dev/null | grep -q 1; then
                        break
                    fi
                    sleep 1
                done
                ssh_vm "su - postgres -c \"psql rumahl_home -c \\\"UPDATE users SET role='admin' WHERE username='admin' AND role!='admin'\\\"\" >/dev/null 2>&1; mkdir -p /var/lib/ora && touch /var/lib/ora/.admin-role-fixed" 2>/dev/null || true
            fi
        ) &
    fi
...
build_rust() {
    ...
    if ! ssh_vm "su - ora -c 'test -f /home/ora/.cargo/bin/cargo && echo OK'" 2>/dev/null | grep -q OK; then
        log "Rust not found in VM. Installing..."
        ssh_vm "su - ora -c 'curl --proto =https --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain stable --profile minimal'" 2>&1 | tail -3
        ssh_vm "su - ora -c '/home/ora/.cargo/bin/cargo --version'" 2>/dev/null && ok "Rust installed" || { err "Rust install failed"; return 1; }
    fi

    # Fix target directory permissions (rsync may create as root)
    ssh_vm "chown -R ora:ora $VM_WORKSPACE/target 2>/dev/null; mkdir -p $VM_WORKSPACE/target && chown ora:ora $VM_WORKSPACE/target" 2>/dev/null || true
    ...
    rsync -az --delete \
        ...
        "$REPO_ROOT/" "root@$VM_HOST:/home/ora/ora/" 2>&1 | tail -3
    # Build only selected services (or full workspace)
    local cargo_cmd="CARGO_BUILD_JOBS=$OPTIMAL_JOBS /home/ora/.cargo/bin/cargo build"
    if [ -n "$SELECTED_SERVICES" ]; then
    ...
    ssh_vm "su - ora -c 'cd $VM_WORKSPACE && $cargo_cmd 2>&1'" || rc=$?
...
deploy_frontend() {
    ...
    ok "  frontend deployed → /opt/rumahl/build/dist"
}
    ...
    if command -v fswatch >/dev/null 2>&1; then
        ...
        (
            inotifywait -m -r -q -e modify,create,move,delete \
                ...
                --exclude '/(target|node_modules|\.git|dist|\.rumahl-dev)(/|$)' \
...
cat <<EOF
${B}╔══════════════════════════════════════════════════════════════════╗
║              rumahl OS Dev-Loop (intelligent)                       ║
╚══════════════════════════════════════════════════════════════════╝${N}
  Workspace : $WORKSPACE
...
_dash_overlay_health() {
    ...
    if ! $DASH_VM_ONLINE; then
        ...
        if ssh_vm "curl -sf --max-time 3 http://127.0.0.1:8126/api/health 2>/dev/null" 2>/dev/null | grep -q '"status":"ok"'; then
            printf '  %s✓ rumahl-home API: OK%s\n' "$G" "$N"
        else printf '  %s✗ rumahl-home API: unreachable%s\n' "$R" "$N"; fi
        printf '\n  %sFailed services:%s\n' "$B" "$N"
        ...
        case "${key:-}" in
            ...
            d|D)
                if $DASH_VM_ONLINE; then
                    ...
                    ssh_vm "rm -f /var/lib/ora/dev-binary-hashes/* 2>/dev/null || true" >/dev/null 2>&1 || true
