#!/usr/bin/env bash
# ============================================================================
# dev-sync.sh – rumahl Dev VM 1:1 Mirror Sync (once or live watch)
# ============================================================================
# Mirrors the host repository into the dev VM (/home/ora/ora) with
...
#   ./dev-sync.sh --ssh-key PATH   SSH key (default: ./.cache/rumahl-dev-key)
#   ./dev-sync.sh --with-binaries  Also mirror .rumahl-dev/binaries (build mode)
...
mkdir -p "$CACHE"
[ -n "$SSH_KEY" ] || SSH_KEY="$CACHE/rumahl-dev-key"
...
if $IS_WSL; then
    mkdir -p "$HOME/.ssh"
    if install -m 600 "$SSH_KEY" "$HOME/.ssh/rumahl-dev-key" 2>/dev/null; then
        SYNC_KEY="$HOME/.ssh/rumahl-dev-key"
    fi
fi
...
SSH_SOCK_DIR="$HOME/.cache/rumahl-sync-socks"
...
sync_now() {
    ...
    rsync -az "${EXCLUDES[@]}" \
        -e "ssh ${SSH_OPTS[*]}" \
        "$REPO_ROOT/" "root@$VM_HOST:/home/ora/ora/" || rc=1

    if $WITH_BINARIES && [ -d "$REPO_ROOT/.rumahl-dev/binaries" ]; then
        # Build-mode artifacts live INSIDE the mirror under .rumahl-dev/binaries
        rsync -az --delete \
            -e "ssh ${SSH_OPTS[*]}" \
            "$REPO_ROOT/.rumahl-dev/binaries/" "root@$VM_HOST:/home/ora/ora/.rumahl-dev/binaries/" || rc=1
    fi

    # Keep the tree owned by the ora user (cargo run / vite need write access)
    ssh "${SSH_OPTS[@]}" root@$VM_HOST "chown -R ora:ora /home/ora/ora 2>/dev/null || true" >/dev/null 2>&1 || true
    ...
    if [ "$rc" -eq 0 ]; then
        ...
        if [ -S "$CACHE/qga.sock" ] && command -v socat >/dev/null 2>&1; then
            dim "  VM control still available without SSH: $SCRIPT_DIR/qga.sh exec 'systemctl status rumahl-home'"
        fi
        sleep 5
        rsync -az "${EXCLUDES[@]}" -e "ssh ${SSH_OPTS[*]}" \
            "$REPO_ROOT/" "root@$VM_HOST:/home/ora/ora/" \
            && ssh "${SSH_OPTS[@]}" root@$VM_HOST "chown -R ora:ora /home/ora/ora 2>/dev/null || true" >/dev/null 2>&1 || true
    fi
    return 0
}
...
watch_fswatch() {
    fswatch -r -E \
        ...
        --exclude '/\.cache/' --exclude '\.rumahl-dev' --exclude '\.DS_Store$' \
...
watch_inotify() {
    inotifywait -m -r -q \
        ...
        --exclude '(\.git/|/target/|/node_modules/|/\.cache/|\.rumahl-dev/|\.DS_Store)' \
        "$REPO_ROOT" 2>/dev/null | while IFS= read -r _; do
            sleep 1
            sync_now
        done
...
$QUIET || dim "  Debounce: 1s | Mirror: /home/ora/ora | SSH port: $VM_PORT"
