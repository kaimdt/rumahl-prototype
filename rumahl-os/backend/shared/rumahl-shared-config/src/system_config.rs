//! rumahl System Configuration — NO .env files in production.
//! This module is isolated so unrelated shared-code changes do not rebuild it.
//!
//! In rumahl OS, ALL configuration lives in the Global Config system
...
//! let port = system_config::service_port("rumahl-home", 3001);
...
// ═══════════════════════════════════════════════════════════════════════
// Default port map — rumahl OS standard ports
// ═══════════════════════════════════════════════════════════════════════
...
const DEFAULT_PORTS: &[(&str, u16)] = &[
    ...
    ("rumahl-assist", 8092),
    ("rumahl-secrets", 8093),
    ("rumahl-watchdog", 8094),
    ("rumahl-security", 8095),
    ("rumahl-gateway", 8096),
    ("rumahl-supervisor", 8097),
    ("rumahl-appstore", 8098),
    ("rumahl-intelligence", 8099),
    ("rumahl-files", 8100),
    ("rumahl-api", 8101),
    ("rumahl-connector", 8102),
    ("rumahl-network-monitor", 8103),
    ("rumahl-domain-validator", 8104),
    ("rumahl-resource-manager", 8105),
    ("rumahl-updater", 8106),
    ("rumahl-backup", 8107),
    ("rumahl-nginx", 8108),
    ("rumahl-stt", 8110),
...
pub fn database_url() -> String {
    env_or("DATABASE_URL", "sqlite:./data/ora.db?mode=rwc")
}

/// Returns a service-specific database URL if set, otherwise the shared one.
/// Priority order:
/// 1. Service-specific credentials file (/etc/ora/db-credentials/{service}.env)
...
pub fn database_url_for(service: &str) -> String {
    // Try service-specific credentials file first (managed by rumahl-db-manager)
    let cred_file = format!("/etc/ora/db-credentials/{}.env", service);
...
/// 1. `RUMAHL_JWT_SECRET` environment variable
...
///
/// Shared JWT-secret file: rumahl-home persists the canonical secret here so
/// EVERY microservice on the host (rumahl-control, rumahl-files, rumahl-security,
...
///
/// `/etc/ora/jwt-secret` is the canonical host-level file provisioned by
/// `rumahl-config-sync`. `/var/lib/ora/jwt-secret` remains a writable fallback
...
pub fn jwt_secret_file() -> std::path::PathBuf {
    std::env::var_os("RUMAHL_JWT_SECRET_FILE")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|| std::path::PathBuf::from("/etc/ora/jwt-secret"))
}

fn read_jwt_secret_file() -> Option<String> {
    let primary = jwt_secret_file();
    let mut paths = vec![primary.clone()];
    if std::env::var_os("RUMAHL_JWT_SECRET_FILE").is_none() {
        paths.push(std::path::PathBuf::from("/var/lib/ora/jwt-secret"));
    }
    for path in paths {
...
fn write_jwt_secret_file(secret: &str) {
    ...
    if let Some((path, error)) = last_error {
        eprintln!(
            "[rumahl-shared-config] WARNING: could not persist shared JWT secret to {}: {error}",
            path.display()
        );
    }
}
...
pub fn jwt_secret() -> String {
    ...
    if let Some(secret) = env_optional("RUMAHL_JWT_SECRET") {
        return normalize_jwt_secret(&secret);
    }
    // 2. Settings cache (populated by rumahl-home from system_preferences table)
    ...
    // that start before rumahl-home persists the secret pick it up on the
...
/// Persist a generated JWT secret — called by rumahl-home after writing to the DB.
...
/// Returns the port for a given rumahl service. Checks:
/// 1. `{SERVICE}_PORT` env var (e.g. `RUMAHL_HOME_PORT`, `CORE_PORT`)
/// 2. `PORT` env var (generic fallback)
/// 3. Default port from the rumahl OS port map
...
/// itself — e.g. rumahl-home (PORT=8126) would proxy /api/files/* to
...
/// Builds the URL for another rumahl service.
/// Native rumahl OS services run as systemd units on the same device, so the
...
pub fn service_url(service: &str, default_port: u16) -> String {
    ...
    if std::env::var("RUMAHL_SERVICE_DNS").ok().as_deref() == Some("1")
        || std::env::var("RUMAHL_CONTAINER_MODE").ok().as_deref() == Some("1")
...
/// Backend API base URL (rumahl-home or the main API gateway).
pub fn backend_url() -> String {
    service_url("rumahl-home", 8126)
}

/// rumahl Assist / AI service URL.
pub fn assist_url() -> String {
    service_url("rumahl-assist", 8092)
}

/// Supervisor URL.
pub fn supervisor_url() -> String {
    service_url("rumahl-supervisor", 8097)
}
...
pub fn files_storage_dir() -> String {
    env_or("RUMAHL_FILES_STORAGE", "./data/file_storage")
}

pub fn files_max_size_bytes() -> usize {
    env_or("RUMAHL_FILES_MAX_SIZE", "104857600") // 100 MB
        .parse()
        .unwrap_or(104_857_600)
}

pub fn files_default_quota() -> i64 {
    env_or("RUMAHL_FILES_DEFAULT_QUOTA", "1073741824") // 1 GB
        .parse()
        .unwrap_or(1_073_741_824)
}

pub fn files_base_url(port: u16) -> String {
    env_or("RUMAHL_FILES_BASE_URL", &format!("http://localhost:{port}"))
}
...
pub fn local_apps_dir() -> String {
    env_or("RUMAHL_LOCAL_APPS_DIR", "./data/apps")
}
...
pub fn sandbox_dir() -> String {
    env_or("RUMAHL_SANDBOX_DIR", "./data/sandbox")
}
...
pub fn ai_provider() -> String {
    env_optional("RUMAHL_AI_PROVIDER")
        .or_else(|| env_optional("ASSIST_AI_PROVIDER"))
        .unwrap_or_else(|| "ollama".to_string())
}

pub fn ai_api_key() -> Option<String> {
    env_optional("RUMAHL_AI_API_KEY").or_else(|| env_optional("ASSIST_AI_API_KEY"))
}

pub fn ai_base_url() -> String {
    env_optional("RUMAHL_AI_BASE_URL")
        .or_else(|| env_optional("ASSIST_AI_BACKEND_URL"))
        .unwrap_or_default()
}

pub fn ai_model() -> Option<String> {
    env_optional("RUMAHL_AI_MODEL").or_else(|| env_optional("ASSIST_AI_MODEL"))
}

pub fn ai_api_version() -> Option<String> {
    env_optional("RUMAHL_AI_API_VERSION").or_else(|| env_optional("ASSIST_AI_API_VERSION"))
}
...
pub fn bootstrap_admin_user() -> Option<String> {
    env_optional("RUMAHL_BOOTSTRAP_ADMIN_USER")
}

pub fn bootstrap_admin_password() -> Option<String> {
    env_optional("RUMAHL_BOOTSTRAP_ADMIN_PASSWORD")
}

pub fn bootstrap_admin_display_name() -> Option<String> {
    env_optional("RUMAHL_BOOTSTRAP_ADMIN_DISPLAY_NAME")
}
...
pub fn recovery_mode() -> Option<String> {
    env_optional("RUMAHL_RECOVERY_MODE")
}

pub fn recovery_threshold() -> u32 {
    env_or("RUMAHL_RECOVERY_THRESHOLD", "3").parse().unwrap_or(3)
}

pub fn recovery_cooldown_secs() -> u64 {
    env_or("RUMAHL_RECOVERY_COOLDOWN_SECS", "300")
        .parse()
        .unwrap_or(300)
}
...
pub fn db_max_attempts() -> u32 {
    env_or("RUMAHL_HOME_DB_MAX_ATTEMPTS", "10")
        .parse()
        .unwrap_or(10)
}

pub fn download_host_allowlist() -> Vec<String> {
    env_optional("RUMAHL_DOWNLOAD_HOST_ALLOWLIST")
        .map(|s| s.split(',').map(|d| d.trim().to_string()).collect())
        .unwrap_or_default()
}

pub fn connector_domain() -> String {
    env_or("RUMAHL_CONNECTOR_DOMAIN", "localhost")
}

pub fn connector_relay_port() -> u16 {
    env_or("RUMAHL_CONNECTOR_RELAY_PORT", "0")
        .parse()
        .unwrap_or(0)
}
...
pub fn stt_service_url() -> String {
    env_or("RUMAHL_STT_URL", "http://localhost:8110")
}

/// TTS (Kokoro) service URL.
pub fn tts_service_url() -> String {
    env_or("RUMAHL_TTS_URL", "http://localhost:8111")
}
...
/// Thread-safe cache of settings fetched from rumahl-home's settings API.
...
/// Called by rumahl-home after bootstrapping to populate the cache.
...
mod jwt_secret_tests {
    ...
    #[test]
    fn canonical_path_is_etc_ora() {
        if std::env::var_os("RUMAHL_JWT_SECRET_FILE").is_none() {
            assert_eq!(
                super::jwt_secret_file(),
                std::path::PathBuf::from("/etc/ora/jwt-secret")
            );
        }
    }
}
