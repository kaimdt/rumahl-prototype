//! rumahl environment detection and install marker.
//! This module is isolated from unrelated application-domain shared code.
//!
//! The system distinguishes between:
//! - **production** – self-installed (marker file present) or `RUMAHL_ENV=production`
...
//! call [`RumahlEnv::detect()`] at startup to determine the current mode.
//!
//! Setup completion is detected via a dual-flag strategy:
//! - Primary flag on data partition: `/mnt/data/ora/.setup-complete`
//! - Secondary flag on rootfs (survives data-partition mount failures):
//!   `/etc/ora/.setup-complete`
//!   If EITHER flag exists, the system is considered set up.

use std::path::{Path, PathBuf};

/// Runtime environment of the rumahl system.
...
impl RumahlEnv {
    ...
    /// 1. `RUMAHL_ENV` environment variable (`production` | `development`)
    /// 2. Presence of the install marker file (`<data_dir>/installed.marker`)
    /// 3. Fallback to [`Development`](RumahlEnv::Development)
    pub fn detect() -> Self {
        // 1. Explicit env var
        if let Ok(val) = std::env::var("RUMAHL_ENV") {
    ...
    pub fn is_setup_complete() -> bool {
        let flags = [
            Path::new("/mnt/data/ora/.setup-complete"),
            Path::new("/etc/ora/.setup-complete"),
        ];
        ...
        let setup_artifacts = [
            Path::new("/mnt/data/ora/docker-compose.yml"),
            Path::new("/etc/ora/db.password"),
        ];
    ...
    fn marker_path() -> PathBuf {
        let base = std::env::var("RUMAHL_DATA_DIR").unwrap_or_else(|_| {
            if cfg!(target_os = "windows") {
                "./data".to_string()
            } else {
                "/var/lib/ora".to_string()
            }
        });
...
impl std::fmt::Display for RumahlEnv {
