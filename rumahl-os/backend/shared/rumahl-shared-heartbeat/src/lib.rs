//! Heartbeat / self-registration client used by every rumahl service.
//!
//! Goal: make the rumahl service mesh dynamic and self-healing. Every
//! service binary embeds a small background task that:
//!
//!   1. Registers itself with `rumahl-core` on startup (idempotent — the
//!      core upserts on every heartbeat).
//!   2. POSTs a status snapshot (`ServiceHeartbeat`) to
//!      `rumahl-core` every `RUMAHL_HEARTBEAT_INTERVAL_SECS` seconds (default
...
//!
//! The receiver side lives in `rumahl-core` (`/api/core/services/heartbeat`).
...
//!
//! ```no_run
//! use rumahl_shared_heartbeat::{HeartbeatClient, HeartbeatConfig};
//!
//! # async fn run() {
...
use rumahl_shared_types::HealthStatus;
...
pub struct HeartbeatConfig {
    ...
    pub version: String,
    /// Core base URL. Honors `$RUMAHL_CORE_URL`, default `http://127.0.0.1:8090`.
    pub core_url: String,
    /// Heartbeat interval. Honors `$RUMAHL_HEARTBEAT_INTERVAL_SECS`,
...
impl Default for HeartbeatConfig {
    fn default() -> Self {
        let interval_secs: u64 = std::env::var("RUMAHL_HEARTBEAT_INTERVAL_SECS")
            .ok()
        ...
        Self {
            ...
            core_url: std::env::var("RUMAHL_CORE_URL")
...
/// be `http://127.0.0.1:{port}` because in rumahl OS all services run on
...
async fn run_loop(cfg: HeartbeatConfig, state: Arc<RwLock<HeartbeatState>>) {
    ...
    let client = match reqwest::Client::builder()
        ...
        .user_agent(format!("rumahl-heartbeat/{}", cfg.version))
        .build()
    {
        Ok(c) => c,
        Err(e) => {
