...
impl Default for SystemCapabilities {
    fn default() -> Self {
        Self {
            ...
            min_rumahl_version: "2.0.0".to_string(),
        }
    }
}
...
pub fn validate_dependencies(
    ...
    // Check rumahl version requirement
    if let Some(min_version) = manifest
        .get("store_metadata")
        .and_then(|m| m.get("min_rumahl_version"))
        .and_then(|v| v.as_str())
    {
        if !is_version_compatible(min_version, &context.system_capabilities.min_rumahl_version) {
            result.add_error(
                "store_metadata.min_rumahl_version",
                &format!(
                    "App benötigt rumahl Version {} aber System hat Version {}.",
                    min_version, context.system_capabilities.min_rumahl_version
                ),
                Some("Aktualisiere rumahl oder verwende eine kompatible App-Version."),
            );
        }
    }
    ...
    if has_bundle && !context.system_capabilities.supports_bundles {
        result.add_error(
            ...
            Some("Aktualisiere rumahl auf eine Version mit Bundle-Unterstützung."),
        );
    }
    ...
    if !policy.allow_untrusted_apps {
        ...
        if installation_source == "manual_upload" {
            result.add_warning(
                ...
                Some("Lade die App aus dem offiziellen rumahl App Store herunter.")
            );
        }
    }
}
