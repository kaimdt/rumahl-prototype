//! App Manifest Schema - defines the structure for rumahl apps
//!
...
pub struct AppManifest {
    ...
    ///
    /// Controls how the app is presented inside the rumahl desktop (App
    ...
    /// AI tools (functions) the app/plugin exposes to rumahl Assist and pi.dev
...
pub struct AppWidget {
    ...
    /// URL to the widget component (will be proxied through rumahl)
...
pub struct DockerConfig {
    /// Use auto-build (rumahl generates Dockerfile) or pre-built image
    #[serde(default)]
    pub auto_build: bool,
    ...
    /// app's own origin (`https://<app-id>.apps.rumahl.local/`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// When an app defines `bundle` in its manifest, rumahl manages
...
pub struct AppBundle {
    ...
    /// Minimum rumahl version required for this bundle format
    #[serde(skip_serializing_if = "Option::is_none")]
    pub min_rumahl_version: Option<String>,
}
...
/// How an installed app is presented inside the rumahl desktop.
///
...
/// (`https://<app-id>.apps.rumahl.local/`) or opened externally, the isolation
...
pub enum AppDisplayMode {
    /// App runs inside the rumahl desktop App Runner as an iframe.
    #[default]
    Embedded,
...
pub struct AppDisplayConfig {
    /// How the app is presented in the rumahl desktop.
    #[serde(default)]
...
pub struct StoreMetadata {
    ...
    /// Minimum rumahl version required
    #[serde(skip_serializing_if = "Option::is_none")]
    pub min_rumahl_version: Option<String>,
...
pub enum InstallationSource {
    /// From rumahl App Store - CANNOT use Developer Mode
    AppStore,

    /// Manually uploaded (ZIP, direct install) - CAN use Developer Mode
    ManualUpload,

    /// Special rumahl Developer App - exclusive hot-reload access
    DeveloperApp,
}
