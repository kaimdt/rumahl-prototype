//! Generic rumahl settings registry.
//!
//! Goal: every configuration value that rumahl cares about (HA URL, MQTT broker, JWT secret,
...
//! * a generic REST API exposed by `rumahl-home`
//!
...
//!
//! Adding a new rumahl setting is therefore a single-file change: register a new
...
pub struct SettingDefinition {
    ...
    pub wizard: bool,
    /// Required to start rumahl at all (wizard cannot be skipped).
    pub required: bool,
    /// Setting back-end services need to be restarted to pick up changes.
    /// Contains systemd unit names (e.g. `rumahl-home.service`).
...
/// Built-in rumahl settings. Every service is encouraged to call this once at startup
...
/// All the rumahl defaults in one place. **Add new system-wide settings here** – they will
...
pub fn default_settings() -> Vec<SettingDefinition> {
    ...
    vec![
        ...
        SettingBuilder::new("system.hostname", "Hostname", System, String)
            .description("Network hostname for this rumahl device.")
            .default(serde_json::json!("ora"))
            .wizard(true)
            .required(true)
        ...
        SettingBuilder::new("privacy.telemetry", "Send anonymous telemetry", Privacy, Bool)
            .description("Help us improve rumahl by sending anonymous usage statistics.")
            .default(serde_json::json!(false))
            .build(),
        ...
        SettingBuilder::new("developer.log_level", "Log level", Developer, Enum)
            ...
            .restart(&["rumahl-home.service", "rumahl-core.service"])
            .build(),
        ...
        SettingBuilder::new("backend.url", "Backend API URL", System, Url)
            .description("Base-URL des rumahl Backends. Vom System automatisch erkannt.")
            .default(serde_json::json!(""))
            .required(true)
            .restart(&["rumahl-home.service"])
            .tags(&["network", "api", "auto"])
            .build(),
        SettingBuilder::new("backend.assist_url", "rumahl Assist URL", System, Url)
            .description("URL des rumahl AI Assistant. Vom System automatisch erkannt.")
            .default(serde_json::json!(""))
            .restart(&["rumahl-home.service"])
            .tags(&["ai", "api", "auto"])
            .build(),
        ...
        SettingBuilder::new("database.url", "Database URL", System, String)
            ...
            .default(serde_json::json!("sqlite:./data/ora.db?mode=rwc"))
            .required(true)
        ...
        SettingBuilder::new("ai.provider", "AI Provider", Integrations, Enum)
            .description("KI-Backend für rumahl Assist.")
        ...
        SettingBuilder::new("ai.base_url", "AI Base URL", Integrations, Url)
            .description("Basis-URL des KI-API-Endpunkts, aus Sicht von rumahl/rumahl-assist erreichbar.")
            .default(serde_json::json!(""))
            .tags(&["ai"])
            .build(),
