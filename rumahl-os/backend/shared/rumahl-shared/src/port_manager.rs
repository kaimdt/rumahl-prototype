//! Port Manager - Dynamic port allocation for rumahl apps
//!
...
/// rumahl system services port range
pub const RUMAHL_SERVICES_RANGE_START: u16 = 8080;
pub const RUMAHL_SERVICES_RANGE_END: u16 = 8099;
...
/// rumahl reserved ports for system services
const RUMAHL_RESERVED_PORTS: &[u16] = &[
    80,   // rumahl-nginx (HTTP)
    443,  // rumahl-nginx (HTTPS)
    8080, // rumahl-home
    8090, // rumahl-core
    8091, // rumahl-control
    8092, // rumahl-assist
    ...
    8098, // rumahl-appstore
    8099, // Reserved for future rumahl services
];
...
impl PortManager {
    pub fn new() -> Self {
        ...
        reserved.extend(RUMAHL_RESERVED_PORTS.iter().copied());
    ...
    pub async fn get_stats(&self) -> PortStats {
        ...
        // Count all reserved ports (well-known + rumahl)
        let mut reserved_set = HashSet::new();
        reserved_set.extend(WELL_KNOWN_PORTS.iter().copied());
        reserved_set.extend(RUMAHL_RESERVED_PORTS.iter().copied());
