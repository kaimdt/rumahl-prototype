...
#[cfg(not(target_os = "linux"))]
fn main() -> Result<()> {
    anyhow::bail!("rumahl-runtime-sensor is a Linux-only service: it consumes kernel eBPF telemetry over a Unix socket")
}
...
mod linux {
    ...
    const INGEST_SOCKET: &str = "/run/ora/runtime-sensor/ebpf-events.sock";
    ...
    pub(crate) async fn run() -> Result<()> {
        tracing_subscriber::fmt()
            ...
            .init();
        tokio::fs::create_dir_all("/run/ora/runtime-sensor").await?;
        ...
        let app = Router::new()
            ...
            .with_state(state);
        let port = rumahl_shared_config::system_config::service_port("rumahl-runtime-sensor", 8106);
        let listener = tokio::net::TcpListener::bind(("127.0.0.1", port)).await?;
        let _heartbeat = rumahl_shared_heartbeat::spawn_default(
            "rumahl-runtime-sensor",
            port,
            "Read-only eBPF runtime telemetry",
        );
        info!(port, "rumahl runtime sensor consumer started");
    ...
    mod tests {
        ...
        async fn oversized_hash_is_explicitly_skipped() {
            let path = std::env::temp_dir().join(format!("rumahl-large-{}", std::process::id()));
