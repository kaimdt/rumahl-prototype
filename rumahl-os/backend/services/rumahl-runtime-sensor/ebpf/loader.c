...
/*
 * rumahl-runtime-sensor-ebpf-loader — observation-only CO-RE loader.
 *
 ...
 * event as one JSON line to rumahl-runtime-sensor over its Unix socket.
 *
...
static const char *g_sock_path = "/run/ora/runtime-sensor/ebpf-events.sock";
...
int main(int argc, char **argv)
{
	...
	if (!obj) {
		fprintf(stderr, "rumahl-ebpf-loader: cannot open %s: %s\n",
			argv[2], strerror(errno));
		goto out;
	}
	if (bpf_object__load(obj)) {
		fprintf(stderr, "rumahl-ebpf-loader: program load failed\n");
		goto out;
	}
	map = bpf_object__find_map_by_name(obj, "events");
	if (!map) {
		fprintf(stderr, "rumahl-ebpf-loader: ring buffer map 'events' missing\n");
		goto out;
	}
	rb = ring_buffer__new(bpf_map__fd(map), handle_event, NULL, NULL);
	if (!rb) {
		fprintf(stderr, "rumahl-ebpf-loader: ring buffer setup failed\n");
		goto out;
	}
	signal(SIGTERM, on_signal);
	signal(SIGINT, on_signal);
	fprintf(stderr, "rumahl-ebpf-loader: ready (socket=%s)\n", g_sock_path);
	while (!g_stop) {
		int polled = ring_buffer__poll(rb, 200);
		if (polled < 0 && polled != -EINTR) {
			fprintf(stderr, "rumahl-ebpf-loader: poll error: %s\n",
				strerror(-polled));
			sleep(1);
		}
	}
