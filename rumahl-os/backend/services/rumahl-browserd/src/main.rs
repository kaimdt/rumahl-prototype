//! rumahl-browserd — rumahl Browser service.
//!
//! The rumahl Browser is NOT an iframe: a real headless Chromium runs on rumahl
//! and its rendered surface (CDP `Page.startScreencast` JPEG frames) is
//! streamed to the rumahl Browser app over WebSocket. Input flows back through
//! CDP `Input.*` commands. Websites therefore never see an embedding —
//! X-Frame-Options / CSP frame-ancestors are irrelevant.
//!
//! The service is GLOBAL: any rumahl app can open a remote browsing surface
//! against the same API (create tabs, feed input, receive frames). The rumahl
//! Browser store app is just one consumer.
//!
//! Security: both the HTTP API and the Chromium CDP endpoint bind to
//! 127.0.0.1 only. Apps reach the service through the rumahl-home app
...
async fn spawn_chromium() -> anyhow::Result<()> {
    ...
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/ora".to_string());
    cmd.env("HOME", &home);
    cmd.args([
        "--headless=new",
        "--no-sandbox", // rumahl runs as root
...
async fn main() -> anyhow::Result<()> {
    ...
    let state = AppState {
        ...
        webrtc_candidate_host: std::env::var("RUMAHL_WEBRTC_CANDIDATE_HOST")
            .ok()
    ...
    tracing::info!("rumahl-browserd listening on http://{addr}");
