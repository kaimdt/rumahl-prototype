...
enum RecoveryMode {
    ...
    Systemd,
    /// Call the Docker daemon via `docker restart rumahl-<name>` (for rumahl-os).
    Docker,
...
/// How many lines of log tail to gather and forward to rumahl-home when a
/// service can't be recovered automatically.
const LOG_TAIL_LINES: usize = 200;

impl RecoveryMode {
    fn from_env() -> Self {
        match std::env::var("RUMAHL_RECOVERY_MODE").as_deref() {
...
struct AppState {
    ...
    /// `rumahl-home` base URL for posting system notifications, e.g.
    /// `http://127.0.0.1:8126`. Read from `RUMAHL_HOME_URL`.
    rumahl_home_url: Option<String>,
    /// Shared secret matched against `RUMAHL_INTERNAL_TOKEN` on rumahl-home.
...
async fn health_check_loop(state: AppState) {
    ...
    loop {
        ...
        for (name, mut service) in services {
            ...
            // Check for rumahl-core specifically
            if name == "rumahl-core" {
                ...
                *core_down = !is_healthy;
                ...
                if !was_core_down && *core_down {
                    error!("CRITICAL: rumahl-core is down! Watchdog taking over event bus");
                    let _ = state.events_tx.send(WatchdogEvent {
                        event_type: "core_down".to_string(),
                        service_name: Some("rumahl-core".to_string()),
                        severity: "critical".to_string(),
                    ...
                    info!("rumahl-core recovered - returning event bus control");
                    let _ = state.events_tx.send(WatchdogEvent {
...
async fn core_service_sync_loop(state: AppState) {
    let core_url = system_config::service_url("rumahl-core", 8090);
...
///    POST a system notification (with the captured log tail) to `rumahl-home`
...
async fn attempt_recovery(state: &AppState, service_name: &str) {
    ...
    match result {
        ...
        Err(detail) => {
            ...
            // Escalate further: post a system notification on rumahl-home and
...
async fn attempt_self_fix(state: &AppState, service: &str) {
    ...
    {
        let container = format!("rumahl-{service}");
        ...
        && (lower.contains("postgres") || lower.contains("5432"))
    {
        let pg_restart = match mode {
            ...
            RecoveryMode::Docker => run_blocking_cmd("docker", &["restart", "rumahl-postgres"]).await,
...
async fn gather_service_logs(mode: RecoveryMode, service: &str) -> String {
    ...
    let result: Result<String, String> = tokio::task::spawn_blocking(move || {
        ...
        match mode {
            ...
            RecoveryMode::Docker => {
                let candidates = [format!("rumahl-{svc}"), svc.clone()];
...
fn guess_default_port(service: &str) -> Option<u16> {
    match service {
        "rumahl-home" => Some(8126),
        "rumahl-core" => Some(8090),
        "rumahl-assist" => Some(8092),
        "rumahl-secrets" => Some(8093),
        "rumahl-watchdog" => Some(8094),
        "rumahl-security" => Some(8095),
        "rumahl-gateway" => Some(8096),
        "rumahl-supervisor" => Some(8097),
        "rumahl-appstore" => Some(8098),
        "rumahl-files" => Some(8100),
        "rumahl-connector" => Some(8102),
        "rumahl-network-monitor" => Some(8103),
        "rumahl-domain-validator" => Some(8104),
        "rumahl-resource-manager" => Some(8105),
        _ => None,
    }
}
...
/// POST an `admin_system_notifications` row to rumahl-home so the user sees a
...
async fn escalate_to_system_notification(
    ...
    let (Some(base), Some(token)) = (state.rumahl_home_url.as_ref(), state.internal_token.as_ref())
    else {
        warn!(
            "Cannot escalate {} to system notification: RUMAHL_HOME_URL or RUMAHL_INTERNAL_TOKEN unset",
            service
        );
        return;
    };
    ...
    let res = reqwest::Client::new()
        .post(&url)
        .header("X-rumahl-Internal-Token", token.as_str())
        .json(&body)
    ...
    match res {
        Ok(r) if r.status().is_success() => {
            info!("escalated {} to rumahl-home system notification", service);
            let _ = state.events_tx.send(WatchdogEvent {
                event_type: "recovery_escalated".to_string(),
                service_name: Some(service.to_string()),
                severity: "critical".to_string(),
                message: format!("Posted system notification to rumahl-home for {service}"),
                timestamp: Utc::now().to_rfc3339(),
            });
        }
        Ok(r) => warn!(
...
fn run_recovery_command(mode: RecoveryMode, service: &str) -> Result<String, String> {
    ...
    match mode {
        ...
        RecoveryMode::Docker => {
            // Container naming convention used by rumahl-supervisor / rumahl-os.
            let candidates = [format!("rumahl-{service}"), service.to_string()];
...
async fn main() -> anyhow::Result<()> {
    ...
    let state = AppState {
        services: Arc::new(RwLock::new(HashMap::from([(
            "rumahl-core".to_string(),
            ServiceStatus {
                name: "rumahl-core".to_string(),
                url: system_config::service_url("rumahl-core", 8090),
                status: "unknown".to_string(),
                response_time_ms: None,
                last_check: Utc::now().to_rfc3339(),
                last_success: None,
        ...
        rumahl_home_url: std::env::var("RUMAHL_HOME_URL")
            .ok()
            .filter(|v| !v.is_empty()),
        internal_token: std::env::var("RUMAHL_INTERNAL_TOKEN")
            .ok()
            .filter(|v| !v.is_empty()),
    };
    ...
    let port = system_config::service_port("rumahl-watchdog", 8094).to_string();
    let addr = format!("0.0.0.0:{}", port);

    info!("👁️  rumahl-watchdog starting on {}", addr);
