...
use rumahl_shared::app_manifest::{AppManifest, TrustLevel};
use rumahl_shared::port_manager::PortManager;
...
/// rumahl App Store Service
///
/// Manages app discovery, installation, and lifecycle.
/// Integrates with store.rumahl.com and supports ZIP uploads.
...
async fn search_apps(query: web::Query<SearchQuery>, data: web::Data<AppState>) -> impl Responder {
    // This would query the remote store.rumahl.com
...
async fn install_app(req: web::Json<InstallRequest>, data: web::Data<AppState>) -> impl Responder {
    ...
    if let Some(ref docker) = manifest.docker {
        for internal_port in &docker.internal_ports {
            let protocol = if internal_port.protocol == "udp" {
                rumahl_shared::port_manager::PortProtocol::Udp
            } else {
                rumahl_shared::port_manager::PortProtocol::Tcp
            };

            let mode = if internal_port.assignment_mode == "fixed" {
                rumahl_shared::port_manager::PortAssignmentMode::Fixed
            } else {
                rumahl_shared::port_manager::PortAssignmentMode::Random
            };
    ...
    for assignment in &port_assignments {
        let mode_str = match assignment.assignment_mode {
            rumahl_shared::port_manager::PortAssignmentMode::Random => "random",
            rumahl_shared::port_manager::PortAssignmentMode::Fixed => "fixed",
        };
...
async fn main() -> std::io::Result<()> {
    ...
    info!("Starting rumahl App Store v{}", env!("CARGO_PKG_VERSION"));

    // Connect to database
    let database_url = system_config::database_url_for("rumahl-appstore");
    ...
    if let Ok(assignments) = sqlx::query_as::<_, PortAssignment>("SELECT * FROM port_assignments")
        ...
        .await
    {
        let port_assignments: Vec<rumahl_shared::port_manager::PortAssignment> = assignments
            .into_iter()
            .map(|a| rumahl_shared::port_manager::PortAssignment {
                ...
                assignment_mode: rumahl_shared::port_manager::PortAssignmentMode::default(),
    ...
    let appstore_url = std::env::var("RUMAHL_APPSTORE_REMOTE_URL")
        .unwrap_or_else(|_| "https://store.rumahl.com".to_string());
    let http_client = reqwest::Client::builder()
        .timeout(std::time::Duration::from_secs(30))
        .user_agent(concat!("rumahl-appstore/", env!("CARGO_PKG_VERSION")))
        .build()
    ...
    let port = system_config::service_port("rumahl-appstore", 8098);

    info!("Starting HTTP server on 0.0.0.0:{}", port);

    let _hb =
        rumahl_shared_heartbeat::spawn_default("rumahl-appstore", port, "App store / app catalog");
