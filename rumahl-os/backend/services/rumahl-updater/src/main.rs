// rumahl-updater — replaces the old /opt/rumahl/update/check-update.sh.
//
// Responsibilities:
//   * Ask update.rumahl.com whether an update is available.
...
//     rumahl release public key stored under /etc/ora/rumahl-release.pub.
...
//     the scheme to https:// and the host to *.rumahl.com by default.  An
//     operator can override with RUMAHL_DOWNLOAD_HOST_ALLOWLIST.
...
const DEFAULT_UPDATE_SERVER: &str = "https://update.rumahl.com";
const VERSION_FILE: &str = "/etc/rumahl-version";
const BUILD_INFO_FILE: &str = "/etc/ora/build-info.json";
const MACHINE_ID_FILE: &str = "/etc/machine-id";
const PUBKEY_FILE: &str = "/etc/ora/rumahl-release.pub";
const TMPDIR: &str = "/tmp/rumahl-update";
const LOGFILE: &str = "/var/log/rumahl-update.log";

#[derive(Parser, Debug)]
#[command(name = "rumahl-updater", about = "rumahl OS update installer")]
struct Cli {
    ...
    #[arg(long, env = "RUMAHL_UPDATE_SERVER")]
...
async fn main() -> Result<()> {
    ...
    // Refuse to do anything on rumahl OS Dev builds.  Such images have no
    ...
    if let Ok(buf) = tokio::fs::read_to_string(BUILD_INFO_FILE).await {
        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&buf) {
            ...
            {
                eprintln!("rumahl-updater: this is an rumahl OS Dev build (variant=os-dev).");
                eprintln!("Updates are disabled because the image has no unique version.");
                eprintln!("Rebuild with `rumahl-os/build.sh` (without --dev) to get a");
    ...
    let url = format!(
        "{server}/v1/ora/os/check?version={current_version}&channel={}&arch={arch}&device_id={device_id}",
        cli.channel
    );
    ...
    let bundle_path = PathBuf::from(TMPDIR).join(format!("rumahl-update-{latest}.raucb"));
    ...
    let _ = client
        .post(format!("{server}/v1/ora/os/report"))
...
/// suffixes.  Default allowlist is `rumahl.com`; override with a comma-
/// separated list in RUMAHL_DOWNLOAD_HOST_ALLOWLIST.
