//! rumahl Intelligence Engine — Predictive health scoring, self-repair orchestration,
//! anomaly detection, and proactive system optimization.
//!
//! This service watches all rumahl components and:
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rumahl_intelligence=info".into()),
        )
        .init();

    let db_url = system_config::database_url_for("rumahl-intelligence");
    let watchdog_url = system_config::service_url("rumahl-watchdog", 8094);
    let core_url = system_config::service_url("rumahl-core", 8090);
    let supervisor_url = system_config::supervisor_url();
    let port: u16 = system_config::service_port("rumahl-intelligence", 8099);
    ...
    info!("rumahl Intelligence Engine starting on {}", addr);
