//! CalDAV interface for rumahl API.
//!
    ...
    let token = match extract_token(headers) {
        ...
        None => {
            return Response::builder()
                ...
                .header("WWW-Authenticate", "Basic realm=\"rumahl CalDAV\"")
...
async fn caldav_propfind(
    ...
    // Fetch calendar entities from HA via rumahl Home
    let resp = state
        .http_client
        .get(format!("{}/api/calendars", state.rumahl_home_url))
    ...
    if path == "/" || path.is_empty() {
        ...
        xml.push_str("        <D:displayname>rumahl Calendars</D:displayname>\n");
        xml.push_str(
        ...
        if depth != "0" {
            ...
            let events_resp = state
                ...
                .get(format!(
                    ...
                    state.rumahl_home_url, entity_id, start, end
                ))
                .header("Authorization", format!("Bearer {}", token))
                .send()
                .await;
...
async fn caldav_get(state: &crate::AppState, path: &str, token: &str) -> Response {
    ...
    let uid = format!("{}-{}@ora", cal_path, event_idx);
