//! rumahl API – Extended interface layer providing multiple protocol access
//! to the rumahl ecosystem:
//!
...
//!
//! All interfaces share the same JWT authentication from rumahl Home.
...
pub struct AppState {
    ...
    /// Base URL of the local rumahl-home instance
    pub rumahl_home_url: String,
    /// Base URL of the local rumahl-files instance
    pub rumahl_files_url: String,
    /// Home Assistant URL (for MQTT and calendar)
    pub ha_url: String,
    pub ha_token: String,
    pub http_client: reqwest::Client,
    pub graphql_schema: graphql::RumahlSchema,
}
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rumahl_api=info,tower_http=info".into()),
        )
        .init();

    let database_url = system_config::database_url_for("rumahl-api");
    ...
    let port: u16 = system_config::service_port("rumahl-api", 8101);
    let rumahl_home_url = system_config::backend_url();
    let rumahl_files_url = system_config::service_url("rumahl-files", 8100);
    ...
    info!("rumahl API gateway listening on {}", addr);
    let _hb = rumahl_shared_heartbeat::spawn_default("rumahl-api", addr.port(), "REST v2 API surface");
...
async fn health_check(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    Json(serde_json::json!({
        ...
        "rumahl_home": state.rumahl_home_url,
    }))
}
...
async fn v2_list_entities(
    ...
    let url = format!("{}/api/states", state.rumahl_home_url);
    let resp = state
        .http_client
        .get(&url)
...
async fn v2_get_entity(
    ...
    let url = format!("{}/api/states/{}", state.rumahl_home_url, entity_id);
    let resp = state
        .http_client
        .get(&url)
        .header("Authorization", format!("Bearer {}", token))
...
async fn v2_list_services(
    ...
    let resp = state
        .http_client
        .get(format!("{}/api/admin/ha/services", state.rumahl_home_url))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, e.to_string()))?;
...
async fn v2_call_service(
    ...
    let url = format!(
        "{}/api/services/{}/{}",
        state.rumahl_home_url, domain, service
    );
    let resp = state
        .http_client
        .post(&url)
...
async fn v2_list_automations(
    ...
    let resp = state
        .http_client
        .get(format!("{}/api/admin/ha/automations", state.rumahl_home_url))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, e.to_string()))?;
...
async fn v2_trigger_automation(
    ...
    let url = format!("{}/api/services/automation/trigger", state.rumahl_home_url);
...
async fn v2_list_areas(
    ...
    let resp = state
        .http_client
        .get(format!("{}/api/admin/ha/areas", state.rumahl_home_url))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, e.to_string()))?;
...
async fn v2_list_devices(
    ...
    let resp = state
        .http_client
        .get(format!("{}/api/admin/ha/devices", state.rumahl_home_url))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, e.to_string()))?;
...
async fn v2_list_users(
    ...
    let resp = state
        .http_client
        .get(format!("{}/api/auth/users", state.rumahl_home_url))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
        .map_err(|e| (StatusCode::BAD_GATEWAY, e.to_string()))?;
...
async fn v2_batch_request(
    ...
    for (idx, request) in body.iter().enumerate() {
        ...
        let url = format!("{}{}", state.rumahl_home_url, path);
