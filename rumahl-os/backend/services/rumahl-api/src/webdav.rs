//! WebDAV interface for rumahl Files.
//!
//! Provides standard WebDAV protocol access to the rumahl file storage,
    ...
    let token = match extract_token(headers) {
        ...
        None => {
            return Response::builder()
                ...
                .header("WWW-Authenticate", "Basic realm=\"rumahl WebDAV\"")
...
async fn webdav_propfind(
    ...
    // Fetch file listing from rumahl-files
    let url = format!("{}/api/files?folder_path={}", state.rumahl_files_url, path);
    let resp = state
        .http_client
        .get(&url)
    ...
    let files: serde_json::Value = match resp {
        Ok(r) if r.status().is_success() => {
            r.json().await.unwrap_or(serde_json::json!({"files": []}))
        }
    ...
    xml.push_str(&format!(
        ...
        path,
        path.split('/').next_back().unwrap_or("rumahl Files"),
    ));
...
async fn webdav_get(state: &crate::AppState, path: &str, token: &str) -> Response {
    ...
    let url = format!("{}/api/files?search={}", state.rumahl_files_url, filename);
    let resp = state
        .http_client
        .get(&url)
        .header("Authorization", format!("Bearer {}", token))
    ...
    let download_url = format!("{}/api/files/{}/download", state.rumahl_files_url, file_id);
...
async fn webdav_put(state: &crate::AppState, path: &str, token: &str, body: Body) -> Response {
    ...
    // Upload via multipart to rumahl-files
    ...
    let resp = state
        .http_client
        .post(format!("{}/api/files/upload", state.rumahl_files_url))
        .header("Authorization", format!("Bearer {}", token))
        .multipart(form)
        .send()
        .await;
...
async fn webdav_delete(state: &crate::AppState, path: &str, token: &str) -> Response {
    ...
    if let Some(file) = files["files"].as_array().and_then(|a| a.first()) {
        ...
        let _ = state
            .http_client
            .delete(format!("{}/api/files/{}", state.rumahl_files_url, file_id))
            .header("Authorization", format!("Bearer {}", token))
            .send()
            .await;
    }
...
async fn webdav_mkcol(state: &crate::AppState, path: &str, token: &str) -> Response {
    ...
    let resp = state
        .http_client
        .post(format!("{}/api/files/folders", state.rumahl_files_url))
        .header("Authorization", format!("Bearer {}", token))
        .json(&body)
        .send()
        .await;
...
async fn webdav_move(
    ...
    let rename_url = format!("{}/api/files/{}/rename", state.rumahl_files_url, file_id);
    let resp = state
        .http_client
        .put(&rename_url)
        .header("Authorization", format!("Bearer {}", token))
...
async fn webdav_copy(
    ...
    let bytes = match dl {
        Ok(r) if r.status().is_success() => match r.bytes().await {
            ...
            Err(e) => {
                return Response::builder()
                    .status(StatusCode::INTERNAL_SERVER_ERROR)
                    .body(Body::from(format!("Download error: {}", e)))
                    .unwrap()
            }
        },
        _ => {
            return Response::builder()
                .status(StatusCode::INTERNAL_SERVER_ERROR)
                .body(Body::from("Failed to download source"))
                .unwrap()
        }
    };
    ...
    let form = reqwest::multipart::Form::new().part("file", part);
    let upload_url = format!("{}/api/files/upload", state.rumahl_files_url);
    let resp = state
        .http_client
        .post(&upload_url)
        .header("Authorization", format!("Bearer {}", token))
