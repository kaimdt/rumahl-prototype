...
/// Publish an MQTT message via the rumahl Home MQTT bridge.
pub async fn mqtt_publish(
    ...
    // Forward to rumahl Home's MQTT publish endpoint
    let url = format!("{}/api/admin/mqtt/publish", state.rumahl_home_url);
                    ...
                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&text) {
                        ...
                        match msg_type {
                            "subscribe" => {
                                ...
                                // Subscribe via rumahl-home, which holds the persistent MQTT
                                // broker connection on behalf of all microservices.
                                let sub_url =
                                    format!("{}/api/admin/mqtt/subscribe", state.rumahl_home_url);
                            ...
                            "publish" => {
                                ...
                                let pub_url =
                                    format!("{}/api/admin/mqtt/publish", state.rumahl_home_url);
...
pub async fn list_topics(
    ...
    let resp = state
        .http_client
        .get(format!("{}/api/states", state.rumahl_home_url))
        .header("Authorization", format!("Bearer {}", token))
        .send()
        .await
