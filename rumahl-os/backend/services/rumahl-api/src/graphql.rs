//! GraphQL interface for the rumahl ecosystem.
//!
...
pub type RumahlSchema = Schema<QueryRoot, MutationRoot, SubscriptionRoot>;
...
impl QueryRoot {
    ...
    async fn entities(
        ...
        let resp = gql_ctx
            .http_client
            .get(format!("{}/api/states", gql_ctx.rumahl_home_url))
    ...
    async fn entity(&self, ctx: &Context<'_>, entity_id: String) -> Result<Option<Entity>> {
        ...
        let resp = gql_ctx
            ...
            .get(format!(
                "{}/api/states/{}",
                gql_ctx.rumahl_home_url, entity_id
            ))
            .header("Authorization", format!("Bearer {}", token))
            .send()
            .await?;
    ...
    async fn areas(&self, ctx: &Context<'_>) -> Result<Vec<Area>> {
        ...
        let resp = gql_ctx
            .http_client
            .get(format!("{}/api/admin/ha/areas", gql_ctx.rumahl_home_url))
            .header("Authorization", format!("Bearer {}", token))
            .send()
            .await?;
...
impl MutationRoot {
    ...
    async fn call_service(
        ...
        let url = format!(
            "{}/api/services/{}/{}",
            gql_ctx.rumahl_home_url, domain, service
        );
...
impl SubscriptionRoot {
    ...
    ///
    /// Polls rumahl-home `/api/states` periodically and emits an `Entity` whenever
