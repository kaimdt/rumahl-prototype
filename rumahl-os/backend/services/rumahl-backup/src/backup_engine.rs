...
//!
//! Designed to run inside the rumahl-os container with read access to the
...
impl BackupEngine {
    ...
    pub fn create_backup(&self, name: &str, opts: &BackupOptions) -> anyhow::Result<BackupResult> {
        ...
        for (flag, label, candidates) in [
            (
                ...
                "docker_volumes",
                vec!["/var/lib/ora/volumes", "/var/ora/volumes"],
            ),
            (
                opts.include_system_config,
                "system_config",
                vec!["/etc/ora", "/var/lib/ora/config"],
            ),
            (
                opts.include_user_data,
                "user_data",
                vec!["/var/lib/ora/data", "/data/ora"],
            ),
            (
                opts.include_apps,
                "apps",
                vec!["/var/lib/ora/apps", "/data/ora/apps"],
            ),
        ] {
            if !flag {
                continue;
    ...
    ///
    /// Hardened via [`rumahl_shared_upload::SecureUploadStore`]: every
    ...
    pub fn restore_backup(&self, archive_path: &Path) -> anyhow::Result<PathBuf> {
        use rumahl_shared_upload::{SecureUploadStore, TarExtractLimits};
