...
async fn main() -> Result<()> {
    ...
    //   2. /etc/ora/secrets-master.key (auto-generated on first boot).
    //
    ...
    const KEY_FILE: &str = "/etc/ora/secrets-master.key";
    ...
    let master_key_hex = if env_usable {
        ...
        // Best effort: ensure /etc/ora exists, write 0600 file.
    ...
    let port = system_config::service_port("rumahl-secrets", 8093).to_string();
    let addr = format!("0.0.0.0:{}", port);

    info!("🔐 rumahl-secrets starting on {}", addr);
