//! rumahl Cloud Connector — Cloud Relay Service (Nabu Casa-style)
//!
//! ## Architecture (like Home Assistant Cloud / Nabu Casa)
//!
//! ```text
//! ┌─────────────────────────┐          WebSocket Tunnel         ┌────────────────────┐
//! │  rumahl Cloud Relay       │◄──────────────────────────────►│  rumahl Home (Client) │
//! │  (rumahl-connector)       │    Single persistent WS conn    │                    │
...
//! 2. The rumahl Home client connects OUTBOUND via WebSocket to the cloud relay
//! 3. The relay assigns a unique subdomain: `https://<tunnel-id>.ora.cloud`
//! 4. All HTTP requests to that subdomain are proxied through the WS tunnel to rumahl Home
...
// ─── Tunnel Protocol ────────────────────────────────────────────────────────
// Messages sent over the WebSocket between client (rumahl Home) and cloud relay.
//
// Client → Relay:
...
pub struct CloudTunnel {
    ...
    pub rumahl_version: Option<String>,
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rumahl_connector=info,tower_http=info".into()),
        )
        .init();

    let database_url = system_config::database_url_for("rumahl-connector");
    let jwt_secret = system_config::jwt_secret();
    let http_port: u16 = system_config::service_port("rumahl-connector", 8102);
    ...
    let http_app = Router::new()
        ...
        // Public proxy: routes external traffic to rumahl Home via WS tunnel
        .fallback(any(proxy_http_request))
        .layer(cors.clone())
        .with_state(state.clone());

    // ── Relay API (WebSocket port) — accepts tunnel connections from rumahl Home clients ──
...
// This is the CORE of the cloud relay. rumahl Home clients connect here via
...
async fn handle_tunnel_connection(state: Arc<AppState>, mut ws: WebSocket) {
    ...
    // Step 4: Main loop — read from WS (responses from rumahl Home) and write to WS (requests from internet)
    let mut heartbeat_interval = tokio::time::interval(std::time::Duration::from_secs(30));
    let mut last_activity = Instant::now();

    loop {
        tokio::select! {
            // Incoming message from rumahl Home (HTTP responses, heartbeats)
            ws_msg = ws.recv() => {
                match ws_msg {
            ...
            // Outgoing HTTP request from internet → forward to rumahl Home via WS
...
// ─── HTTP Proxy: Route external requests to rumahl Home via WS tunnel ────────
...
async fn proxy_http_request(
    ...
    // Extract subdomain from host header: <subdomain>.ora.cloud → subdomain
...
async fn create_pairing_token(
    ...
    let user_id = extract_admin_user(&headers, &rumahl_shared_config::system_config::jwt_secret())?;
    ...
    .bind(&tunnel_id).bind(body.label.as_deref().unwrap_or("rumahl Home"))
...
async fn admin_auth_middleware(
    ...
    verify_admin_jwt(token, &rumahl_shared_config::system_config::jwt_secret())?;
