// System Guard – Protection & Control for ORA AI
...
pub enum SystemState {
    ...
    Paused,
    EmergencyStop,
    PartialPause { paused_agents: Vec<String> },
}
...
pub struct RecoveryAction {
    ...
    pub adjusted_input: String,
    pub reason: String,
    pub loop_pattern: Vec<String>,
}
...
impl SystemGuard {
    pub fn new() -> Self {
        Self {
            ...
            rules: RwLock::new(vec![
                ProtectionRule {
                    id: "max-failures".into(),
                    name: "Max failures".into(),
                    rule_type: ProtectionRuleType::MaxConsecutiveFailures,
                    threshold: 5,
                    action: ProtectionAction::AdjustAndRetry,
                    enabled: true,
                },
                ProtectionRule {
                    id: "tool-rate".into(),
                    name: "Tool rate limit".into(),
                    rule_type: ProtectionRuleType::MaxToolCallsPerMinute,
                    threshold: 30,
                    action: ProtectionAction::PauseAgent,
                    enabled: true,
                },
                ProtectionRule {
                    id: "max-time".into(),
                    name: "Max session time".into(),
                    rule_type: ProtectionRuleType::MaxSessionDuration,
                    threshold: 3600,
                    action: ProtectionAction::PauseAgent,
                    enabled: true,
                },
                ProtectionRule {
                    id: "repetitive".into(),
                    name: "Repetitive pattern".into(),
                    rule_type: ProtectionRuleType::RepetitivePattern,
                    threshold: 3,
                    action: ProtectionAction::StopAgent,
                    enabled: true,
                },
                ProtectionRule {
                    id: "empty-output".into(),
                    name: "Empty output".into(),
                    rule_type: ProtectionRuleType::EmptyOutput,
                    threshold: 3,
                    action: ProtectionAction::AdjustAndRetry,
                    enabled: true,
                },
    ...
    pub fn pause_system(&self) {
        *self.state.write() = SystemState::Paused;
        info!("SystemGuard: PAUSED");
    }
    pub fn resume_system(&self) {
        *self.state.write() = SystemState::Running;
        info!("SystemGuard: RESUMED");
    }
    pub fn emergency_stop(&self) {
        *self.state.write() = SystemState::EmergencyStop;
        error!("SystemGuard: EMERGENCY STOP");
    }
    ...
    pub fn pause_agent(&self, agent_id: &str) {
        ...
        *state = SystemState::PartialPause {
    ...
    pub fn resume_agent(&self, agent_id: &str) {
        ...
        if let SystemState::PartialPause { paused_agents } = &*state {
            let agents: Vec<String> = paused_agents
                .iter()
                .filter(|a| *a != agent_id)
                .cloned()
                .collect();
    ...
    pub fn can_proceed(&self, agent_id: Option<&str>) -> bool {
        match &*self.state.read() {
            ...
            SystemState::PartialPause { paused_agents } => {
                agent_id.is_none_or(|id| !paused_agents.contains(&id.to_string()))
            }
        }
    }
    ...
    pub fn record_tool_call(
        ...
        activity.tool_calls.push(ToolCallRecord {
        ...
        if activity.call_history.len() > 20 {
            activity.call_history.pop_front();
        }
    ...
    fn detect_loops(&self, agent_id: &str, activity: &AgentActivity) -> Option<LoopDetection> {
        ...
        if let Some(rule) = rules
            .iter()
            .find(|r| r.rule_type == ProtectionRuleType::MaxConsecutiveFailures && r.enabled)
        {
            let recent: Vec<_> = activity
                .tool_calls
                .iter()
                .rev()
                .take(rule.threshold as usize)
                .filter(|tc| !tc.success)
                .collect();
            if recent.len() >= rule.threshold as usize
                && recent
                    .iter()
                    .all(|tc| (now - tc.timestamp).num_seconds() < 120)
            {
                ...
                let detection = LoopDetection {
                    ...
                    repetitions: rule.threshold,
                    total_failures: activity.failed_tool_calls,
                    suggestion: format!(
                        "Failed tools: {}. Try a different approach.",
                        pattern.join(" → ")
                    ),
                };
                if rule.action == ProtectionAction::AdjustAndRetry {
                    self.recovery_queue.write().push_back(RecoveryAction {
                        agent_id: agent_id.to_string(),
                        original_input: activity.current_task.clone(),
                        adjusted_input: format!(
                            "Previous attempt failed. Try a DIFFERENT strategy. Failed: {}",
                            pattern.join(", ")
                        ),
                        reason: "consecutive failures".into(),
        ...
        if let Some(rule) = rules
            .iter()
            .find(|r| r.rule_type == ProtectionRuleType::RepetitivePattern && r.enabled)
        {
            if activity.call_history.len() >= (rule.threshold * 2) as usize {
                ...
                if let Some((pat, reps)) = find_repeating_pattern(&hist, rule.threshold as usize) {
                    let detection = LoopDetection {
                        ...
                        total_failures: activity.failed_tool_calls,
                        suggestion: format!(
                            "Repeating {:?} ({}x). Break into smaller steps.",
                            pat, reps
                        ),
                    };
                    self.loop_detections.write().push_back(detection.clone());
    ...
    pub fn get_rules(&self) -> Vec<ProtectionRule> {
    ...
    pub fn get_loop_detections(&self) -> Vec<LoopDetection> {
        self.loop_detections.read().iter().cloned().collect()
    }
    pub fn get_agent_activities(&self) -> Vec<AgentActivity> {
        self.agent_activities.read().values().cloned().collect()
    }
    pub fn get_recovery_actions(&self) -> Vec<RecoveryAction> {
        self.recovery_queue.read().iter().cloned().collect()
    }
    pub fn take_recovery_action(&self) -> Option<RecoveryAction> {
        self.recovery_queue.write().pop_front()
    }
