...
/// Honors `$RUMAHL_HOME_URL` env var; falls back to `system_config::service_url`.
fn rumahl_home_url() -> String {
    std::env::var("RUMAHL_HOME_URL").unwrap_or_else(|_| system_config::service_url("rumahl-home", 8126))
}
...
impl ContextBuilder {
    ...
    /// Fetch current smart home context from rumahl-home
    pub async fn fetch_context(
        ...
        // User preferences: pull dashboard_settings from rumahl-home
    ...
    /// Fetch all entities from rumahl-home
    async fn fetch_entities(
        &self,
    ) -> Result<Vec<EntityState>, Box<dyn std::error::Error + Send + Sync>> {
        let response = self
            .client
            .get(format!("{}/api/states", rumahl_home_url()))
            .send()
            .await?;
