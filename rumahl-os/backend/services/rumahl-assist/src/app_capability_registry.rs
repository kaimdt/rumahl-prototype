...
//!
//! Runtime registry for capabilities that installed rumahl apps / plugins expose
//! to ORA Assist and the pi.dev agents. rumahl-home pushes an app's
...
//! rumahl-home's app index and is repopulated whenever rumahl-home (re)pushes an
//! app on install or on its own startup. This keeps rumahl-assist stateless with
//! respect to app persistence.

use std::collections::HashMap;
use std::sync::Arc;

use rumahl_shared::app_capabilities::{AssistToolDefinition, ServiceExport};
...
pub struct AppCapabilities {
    ...
    #[serde(default)]
    pub app_name: String,
    /// AI tools the app exposes to ORA Assist / pi.dev agents.
    #[serde(default)]
