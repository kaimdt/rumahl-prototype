...
impl ReflectionEngine {
        ...
        for (category, errors) in failure_patterns {
            if errors.len() >= 2 {
                ...
                reflections.push(SelfReflection {
                    ...
                    action_items: Some(
                        serde_json::to_value(self.generate_action_items(&category, &errors))
                            .unwrap_or_default(),
                    ),
                ...
                reflections.push(SelfReflection {
                    ...
                    action_items: Some(serde_json::Value::Array(vec![
                        serde_json::Value::String(format!(
                            "Document {} execution pattern as best practice",
                            task_type
                        )),
                        serde_json::Value::String(
                            "Store optimization strategy in knowledge base".to_string(),
                        ),
                    ])),
    ...
    pub async fn process_feedback(
        ...
        let reflection = SelfReflection {
            ...
            lesson_learned: if rating < 3.0 {
                ...
                Some(format!(
                    "User satisfied ({} rating). Current approach is effective.",
                    rating
                ))
            },
            action_items: if rating < 3.0 {
                Some(serde_json::Value::Array(vec![
                    ...
                    serde_json::Value::String(format!(
                        "Address user concern: '{}'",
                        truncate(feedback, 50)
                    )),
                ]))
            } else {
                None
    ...
    pub async fn run_self_assessment(&self) -> Result<SelfAssessmentReport, String> {
        ...
        let kb_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM knowledge_base")
            .fetch_one(&self.db_pool)
            .await
            .map_err(|e| e.to_string())?;
    ...
    fn generate_recommendations(&self, report: &SelfAssessmentReport) -> Vec<String> {
        ...
        if report.success_rate < 0.90 {
            recommendations.push(
                "Priority: Improve task success rate through better error handling and retry logic"
                    .to_string(),
            );
        }
        ...
        if report.avg_tokens_per_task > 4000 {
            recommendations.push(
                "Reduce token usage by optimizing prompts and reducing unnecessary context"
                    .to_string(),
            );
        }
        ...
        if report.knowledge_entries < 10 {
            recommendations.push(
                "Expand knowledge base - more systematic learning will improve future performance"
                    .to_string(),
            );
        }
        ...
        recommendations
            .push("Schedule regular self-assessment cycles (weekly recommended)".to_string());
        recommendations
            .push("Review and update system prompts based on learned patterns".to_string());
