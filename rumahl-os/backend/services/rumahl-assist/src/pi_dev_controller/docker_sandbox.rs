...
impl DockerSandbox {
    ...
    pub async fn create_container(&self, config: &SandboxConfig) -> Result<String, String> {
        ...
        let env_vars: Vec<String> = vec![
            ...
            "RUMAHL_MANAGED=true".to_string(),
        ];

        // Plugin install + pi.dev startup command
        //
        // When `RUMAHL_PI_EXTENSION_PATH` is set on the host, the ORA bridge
        ...
        let ext_flag = match std::env::var("RUMAHL_PI_EXTENSION_PATH") {
            Ok(host_path) if !host_path.trim().is_empty() => {
                binds.push(format!("{}:/opt/rumahl/rumahl-bridge.ts:ro", host_path));
                " -e /opt/rumahl/rumahl-bridge.ts".to_string()
            }
            _ => String::new(),
        };
