// ORA STT Provider – calls the rumahl-stt microservice (faster-whisper)
use super::{
...
pub struct RumahlSttProvider {
    client: Client,
    config: ProviderConfig,
}

impl RumahlSttProvider {
...
#[async_trait]
impl AIProvider for RumahlSttProvider {
    fn name(&self) -> &str {
        "ORA STT (faster-whisper)"
    }
    ...
    async fn chat(
        ...
        Err("ORA STT provider does not support chat. Use for transcription only.".into())
    }
    ...
    async fn synthesize_speech(
        ...
        Err("ORA STT provider does not support TTS. Use ORA TTS for synthesis.".into())
    }
}
