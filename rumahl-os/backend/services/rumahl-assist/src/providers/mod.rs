...
impl ProviderType {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::RumahlStt => "rumahl_stt",
            Self::RumahlTts => "rumahl_tts",
        }
    }
}
...
pub fn provider_type_from_str(value: &str) -> Option<ProviderType> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        "rumahl_stt" | "rumahl-stt" | "faster-whisper" | "faster_whisper" => {
            Some(ProviderType::RumahlStt)
        }
        "rumahl_tts" | "rumahl-tts" | "kokoro" => Some(ProviderType::RumahlTts),
        _ => None,
    }
}
...
pub fn create_provider(provider_type: ProviderType, config: ProviderConfig) -> Box<dyn AIProvider> {
    match provider_type {
        ...
        ProviderType::RumahlStt => Box::new(rumahl_stt::RumahlSttProvider::new(config)),
        ProviderType::RumahlTts => Box::new(rumahl_tts::RumahlTtsProvider::new(config)),
    }
}
