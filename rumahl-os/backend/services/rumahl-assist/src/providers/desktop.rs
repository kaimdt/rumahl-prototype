// Desktop AI Provider Implementation (via ORA Desktop client)
use super::{
...
impl AIProvider for DesktopAIProvider {
    ...
    async fn is_available(&self) -> bool {
        if let Some(base_url) = &self.config.base_url {
            // Try to ping the desktop AI proxy (LM Studio via ORA Desktop)
    ...
    async fn chat(
        ...
        let base_url = self
            ...
            .base_url
            .as_ref()
            .ok_or("Desktop AI base URL not configured (should point to ORA Desktop proxy)")?;
