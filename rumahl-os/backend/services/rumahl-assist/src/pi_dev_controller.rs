...
// Full control layer: ORA manages pi.dev lifecycle, monitors activity, enforces policy
...
/// Version of the ORA pi.dev bridge protocol. Mirrors the LocalUp bridge so
...
/// Live, aggregated state of a pi.dev session — the ORA equivalent of the
...
impl PiDevController {
    ...
    ///
    /// `provider`/`model` select the model backend (e.g. `ora` for ORA Assist,
    ...
    pub async fn send_control_command(
        ...
        match cmd.as_str() {
            "ping" => {
                ...
                message = if healthy {
                    "pong — ORA bridge active".to_string()
                } else {
