...
impl ConversationManager {
    ...
    /// Deliver pending notifications by forwarding them to rumahl-home's notification
    ...
    async fn deliver_pending_notifications(
        ...
        let home_url =
            std::env::var("RUMAHL_HOME_URL").unwrap_or_else(|_| "http://127.0.0.1:8126".to_string());
        ...
        for notification in notifications {
            ...
            match client.post(&url).json(&payload).send().await {
                ...
                Ok(resp) => {
                    tracing::warn!(
                        "rumahl-home notification dispatch returned {} for notification {}",
                        resp.status(),
                        notification.id
                    );
                }
                Err(e) => {
                    tracing::error!(
                        "Failed to forward notification {} to rumahl-home: {}",
                        notification.id,
                        e
                    );
                }
