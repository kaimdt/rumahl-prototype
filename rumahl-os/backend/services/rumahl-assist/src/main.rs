// rumahl-assist exposes a large public API surface. Many types, struct fields, and
...
const DEFAULT_SYSTEM_PROMPT: &str =
    "You are ORA Assist, an AI assistant integrated into the ORA smart home system. \
...
async fn chat(State(state): State<AppState>, Json(req): Json<ChatRequest>) -> impl IntoResponse {
    ...
    if let Some(ref agent_id) = req.agent_id {
        if !agent_id.trim().is_empty() {
            tracing::debug!(target: "rumahl-assist", "chat agent_id={} model={:?}", agent_id, req.model);
        }
    }
...
async fn insights(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let messages = vec![
        crate::providers::ChatMessage {
            role: "system".into(),
            content: "You are ORA Assist. Output JSON: {\"insights\":[string,...]} (3-5 entries, each <=120 chars). No prose.".into(),
        },
        crate::providers::ChatMessage {
            role: "user".into(),
    ...
    Json(serde_json::json!({
        ...
        "available_providers": [
            ...
            },
            {
                ...
                "capabilities": ["chat", "stt", "tts", "models"],
                "requires_api_key": false,
                "description": "Fetches all OpenAI-compatible models from the active ORA Desktop local AI proxy (e.g. LM Studio).",
            },
            {
                "name": "pi.dev",
                "id": "pidev",
...
// These always use the local ORA STT/TTS microservices regardless of
// which chat AI provider is currently active.

/// Transcribe audio using the local ORA STT (faster-whisper) service.
/// Falls back to the current provider if STT service is not available.
async fn transcribe_local(
    State(state): State<AppState>,
    mut multipart: Multipart,
) -> impl IntoResponse {
    // Try the dedicated ORA STT provider first
    let stt_provider = {
        let providers = state.orchestrator.list_providers().await;
        if providers.iter().any(|p| p == "rumahl_stt") {
            let stt_config = ProviderConfig {
                base_url: Some(rumahl_shared_config::system_config::stt_service_url()),
                ..Default::default()
            };
            Some(providers::create_provider(
                providers::ProviderType::RumahlStt,
                stt_config,
            ))
        } else {
            None
    ...
    if !provider.is_available().await {
        return (
            ...
            Json(serde_json::json!({
                ...
                "message": "Neither local ORA STT nor current AI provider are available.",
            })),
        );
    }
...
/// Synthesize speech using the local ORA TTS (Kokoro) service.
...
async fn synthesize_local(
    ...
    // Try the dedicated ORA TTS provider first
    let tts_provider = {
        let providers = state.orchestrator.list_providers().await;
        if providers.iter().any(|p| p == "rumahl_tts") {
            let tts_config = ProviderConfig {
                base_url: Some(rumahl_shared_config::system_config::tts_service_url()),
                model: req.voice.clone(),
                api_version: req.lang.clone(),
                ..Default::default()
            };
            Some(providers::create_provider(
                providers::ProviderType::RumahlTts,
                tts_config,
            ))
        } else {
            None
        ...
        return Response::builder()
            ...
            .body(Body::from(
                serde_json::json!({
                    ...
                    "message": "Neither local ORA TTS nor current AI provider are available.",
                })
                .to_string(),
            ))
            .unwrap();
...
async fn list_stt_models(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let stt = providers::create_provider(providers::ProviderType::RumahlStt, stt_config);
...
async fn list_tts_voices(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let tts = providers::create_provider(providers::ProviderType::RumahlTts, tts_config);
...
async fn openai_chat_completions(
    ...
    // Konvertiere Messages in rumahl-Format (ohne system)
...
async fn workspace_git_create_branch(
    ...
    let name = req
        .get("name")
        .and_then(|v| v.as_str())
        .unwrap_or("feature/rumahl-agent");
...
async fn github_create_pr(
    ...
    let body = req["body"].as_str().unwrap_or("");
    let head = req["head"].as_str().unwrap_or("feature/rumahl-agent");
...
/// provides. Called by rumahl-home on app install and on its own startup.
...
/// Remove all capabilities for an app. Called by rumahl-home on uninstall.
...
async fn list_collab_plans(
    State(state): State<AppState>,
) -> Json<Vec<rumahl_features::multi_agent::CollaborationPlan>> {
...
) -> Json<rumahl_features::multi_agent::CollaborationPlan> {
...
async fn get_collab_plan(
    State(state): State<AppState>,
    axum::extract::Path(id): axum::extract::Path<String>,
) -> Result<Json<rumahl_features::multi_agent::CollaborationPlan>, StatusCode> {
    state
        .collaboration
        .get_plan(&id)
        .map(Json)
...
async fn get_health_checks(
    State(state): State<AppState>,
) -> Json<Vec<rumahl_features::self_healing::HealthCheck>> {
...
async fn get_cost_records(
    State(state): State<AppState>,
) -> Json<Vec<rumahl_features::cost_intel::CostRecord>> {
...
async fn get_mistakes(
    State(state): State<AppState>,
) -> Json<Vec<rumahl_features::mistake_learner::Mistake>> {
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rumahl_assist=debug,info".parse().unwrap()),
        )
        .init();
...

    {
        ...
        info!("Starting ORA AI (ORA Assist) with provider: {}", p.name());
    }
    ...
    // ─── Auto-register ORA STT (faster-whisper) ───────────────────────
    let stt_url = rumahl_shared_config::system_config::stt_service_url();
    let stt_config = ProviderConfig {
        base_url: Some(stt_url.clone()),
        ..Default::default()
    };
    let stt_provider = providers::create_provider(providers::ProviderType::RumahlStt, stt_config);
    if stt_provider.is_available().await {
        orchestrator
            .register_provider("rumahl_stt".to_string(), stt_provider)
            .await;
        info!("ORA STT (faster-whisper) registered at {}", stt_url);
    } else {
        info!("ORA STT not available at {} – skipping", stt_url);
    }

    // ─── Auto-register ORA TTS (Kokoro) ───────────────────────────────
    let tts_url = rumahl_shared_config::system_config::tts_service_url();
    let tts_config = ProviderConfig {
        base_url: Some(tts_url.clone()),
        ..Default::default()
    };
    let tts_provider = providers::create_provider(providers::ProviderType::RumahlTts, tts_config);
    if tts_provider.is_available().await {
        orchestrator
            .register_provider("rumahl_tts".to_string(), tts_provider)
            .await;
        info!("ORA TTS (Kokoro) registered at {}", tts_url);
    } else {
        info!("ORA TTS not available at {} – skipping", tts_url);
    }
    ...
    let acp_router = Arc::new(AcpRouter::new(
        "rumahl-assist",
        "Primary ORA AI Assistant agent for the ORA ecosystem",
        "primary",
    ));
    ...
    let sandbox_base =
        std::env::var("RUMAHL_SANDBOX_BASE").unwrap_or_else(|_| "/tmp/rumahl-sandboxes".to_string());
    ...
    event_bus.system_event(
        "startup",
        "ORA Assist system event bus initialized",
        "info",
    );
    ...
    let app = Router::new()
        ...
        .route("/api/assist/health", get(health))
        .route("/api/assist/health/checks", get(get_health_checks))
    ...
    let port = system_config::service_port("rumahl-assist", 8092);

    let addr = std::net::SocketAddr::from(([0, 0, 0, 0], port));
    info!("ORA AI (rumahl-assist) listening on {}", addr);
    let _hb = rumahl_shared_heartbeat::spawn_default("rumahl-assist", addr.port(), "ORA AI assistant");
