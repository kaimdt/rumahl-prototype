/**
 * ORA Bridge Extension v1.0 — pi.dev <-> ORA Assist live bridge & control
 *
 * This is the ORA Assist counterpart to the LocalUp agent extension. It makes
 * pi.dev and ORA Assist work together for interactive sessions (`pi serve`):
 *
 * - Monitoring (extension -> ORA): every 2 seconds a `BridgeState` snapshot is
 *   pushed via `ctx.ui.setStatus("rumahl_bridge", JSON)`. ORA Assist mirrors the
 *   same state server-side from the headless JSON event stream, so the dashboard
 *   sees identical data in both modes.
 * - Control (ORA -> extension): the `rumahl_control` tool accepts commands
 *   (ping, pause, resume, set_model, set_thinking, get_state, compact, reset)
 *   so the ORA Assist dashboard can drive a running pi.dev session.
 *
 * The protocol intentionally matches the ORA Assist `BridgeState` Rust struct
 * (see services/rumahl-assist/src/pi_dev_controller.rs).
 *
 * Install: pi -e rumahl-bridge.ts   (wired automatically when the container is
 * started with the RUMAHL_PI_EXTENSION_PATH host path set).
 */
...
export default function register(pi: ExtensionAPI): void {
  ...
  function pushBridge(ctx?: ExtensionContext) {
    ...
    try {
      target.ui.setStatus("rumahl_bridge", JSON.stringify(buildState()));
    } catch (error) {
      console.error("ORA bridge push error:", error);
    }
  }
  ...
  pi.registerTool({
    name: "rumahl_bridge_status",
    label: "ORA Bridge Status",
    description:
      "Get the full ORA bridge state: working dir, model, file changes, and session metrics.",
  ...
  pi.registerTool({
    name: "rumahl_control",
    label: "ORA Remote Control",
    description:
      "Receive control commands from ORA Assist to drive this pi.dev session (pause, resume, set model, set thinking, compact, reset).",
    parameters: {
      type: "object",
      properties: {
        command: {
    ...
    async execute(_id: string, params: any, _onUpdate: any, ctx: any) {
      ...
      try {
        switch (cmd) {
          case "ping":
            return {
              content: [{ type: "text", text: "pong — ORA bridge active" }],
          ...
          case "reset":
            ...
            pushBridge();
            return {
              content: [{ type: "text", text: "ORA bridge state reset" }],
              details: { success: true },
            };
          default:
            return {
  ...
  pi.registerCommand("rumahl-health", {
    description: "Show ORA bridge health and session metrics",
    async run(_args: string, ctx: ExtensionContext) {
      const state = buildState();
      const lines = [
        `ORA bridge v${EXTENSION_VERSION}`,
  ...
  pi.registerCommand("rumahl-help", {
    description: "List ORA bridge tools and commands",
    async run(_args: string, ctx: ExtensionContext) {
      const help = [
        "ORA Bridge commands:",
        "  /rumahl-health  — show bridge health & metrics",
        "  /rumahl-help    — this help",
        "Tools:",
        "  rumahl_bridge_status — full bridge state",
        "  rumahl_control       — ping/pause/resume/set_model/set_thinking/get_state/compact/reset",
      ].join("\n");
