...
use chrono::Utc;
use hmac::{Hmac, Mac};
use rumahl_shared_config::env::RumahlEnv;
...
fn compute_event_hash(key: &[u8; 32], event: &SecurityEvent) -> String {
    ...
    mac.update(b"rumahl-AUDIT-V1\0");
...
mod audit_tests {
    ...
    fn event() -> SecurityEvent {
        SecurityEvent {
            ...
            severity: "high".into(),
            source_ip: None,
            service_name: Some("rumahl-security".into()),
            user_id: Some("admin".into()),
            event_data: Some("encrypted-payload".into()),
            hash: String::new(),
            prev_hash: Some("previous".into()),
        }
    }
    ...
    fn keyed_audit_hash_detects_payload_and_identity_changes() {
        ...
        changed.event_data = Some("replacement".into());
        assert_ne!(expected, compute_event_hash(&key, &changed));
        changed = original.clone();
        changed.user_id = Some("attacker".into());
        assert_ne!(expected, compute_event_hash(&key, &changed));
...
async fn check_database_connections(state: &AppState) -> Result<()> {
    ...
    for row in rows {
        ...
        // Known rumahl service users are always authorized – never block ourselves
        let is_known_service_user = username
            .as_deref()
            .is_some_and(|u| matches!(u, "ora" | "postgres") || u.starts_with("rumahl_"));
...
async fn main() -> Result<()> {
    ...
    // Detect environment
    let rumahl_env = RumahlEnv::detect();
    info!("rumahl environment: {}", rumahl_env);

    // Connect to PostgreSQL using the shared DATABASE_URL.
    // In production the ora user already has the required privileges.
    ...
    let postgres_admin = match Pool::<Postgres>::connect(&postgres_url).await {
        ...
        Err(e) if rumahl_env.is_development() => {
    ...
    let app = Router::new()
        ...
        .route(
            ...
            get(security_center::overview),
        )
        .route(
            "/api/security/center/providers",
            get(security_center::providers).post(security_center::configure_provider),
        )
        .route(
            "/api/security/center/policies",
            get(security_center::policies).post(security_center::save_policy),
        )
        .route(
            "/api/security/center/scans",
            post(security_center::start_scan),
        )
        .route(
            "/api/security/center/quarantine/restore",
            post(security_center::restore_quarantine),
        )
        .route(
            "/api/security/center/firewall",
            post(security_center::apply_firewall),
        )
        .layer(CorsLayer::permissive())
        .layer(tower_http::trace::TraceLayer::new_for_http())
        .with_state(state.clone());

    let port = system_config::service_port("rumahl-security", 8095).to_string();
    // Loopback-only: the Security Center API is reached exclusively through
    // the authenticated rumahl-home admin proxy, which attaches the trusted
    ...
    // rumahl binaries are tampered with (evidence from rumahl-integrity.service).
