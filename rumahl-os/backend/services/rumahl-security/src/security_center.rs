...
#[cfg(target_os = "linux")]
const HELPER_SOCKET: &str = "/run/ora/security-helper.sock";
...
pub async fn overview(
    ...
    headers: HeaderMap,
) -> Result<impl IntoResponse, CenterError> {
    require_permission(&headers, "security_read")?;
    let helper = helper_call(serde_json::json!({"action":"status"}))
        .await
        .unwrap_or_else(|error| serde_json::json!({"ok":false,"message":error.to_string()}));
    let policies = sqlx::query("SELECT COUNT(*) AS count FROM security_policies WHERE enabled = 1")
        .fetch_one(&*state.security_db)
        .await?
        .get::<i64, _>("count");
    let scans =
        sqlx::query("SELECT COUNT(*) AS count FROM scan_jobs WHERE status IN ('queued','running')")
            .fetch_one(&*state.security_db)
            .await?
            .get::<i64, _>("count");
    let quarantine =
        sqlx::query("SELECT COUNT(*) AS count FROM quarantine_items WHERE released_at IS NULL")
            .fetch_one(&*state.security_db)
            .await?
            .get::<i64, _>("count");
    ...
    Ok(Json(
        serde_json::json!({"health":health,"health_reasons":health_reasons,"helper":helper,"audit_chain_valid":audit_chain_valid,"enabled_policies":policies,"active_scans":scans,"quarantine_items":quarantine,"default_response":"detect_alert_contain_confirm","automated_response":crate::automated_response::status(),"integrity_response":crate::integrity_response::status()}),
    ))
}

fn classify_health(
    helper: &serde_json::Value,
    ...
    if helper.get("ok").and_then(|value| value.as_bool()) != Some(true) {
        reasons.push("helper_unavailable");
    }
    if helper
        .pointer("/data/firewall_effective")
        .and_then(|value| value.as_bool())
        != Some(true)
    {
        reasons.push("firewall_ineffective");
    }
    if !reasons.is_empty() {
    ...
    let scanners = helper
        ...
        || helper
            ...
            .and_then(|value| value.as_bool())
            == Some(true);
    let integrity = helper
        .pointer("/data/integrity_monitor_fresh")
        .and_then(|value| value.as_bool())
        == Some(true);
    if !scanners {
    ...
    if helper
        ...
        == Some(true)
        && helper
            .pointer("/data/clamav_definitions_age_seconds")
            .and_then(|value| value.as_u64())
            .is_none_or(|age| age > 172_800)
    {
        reasons.push("definitions_outdated");
    }
    if reasons.is_empty() {
        ("healthy", reasons)
    } else {
...
async fn verify_audit_chain(state: &AppState) -> Result<bool> {
    ...
    for row in rows {
        ...
        if event.prev_hash != previous || (keyed != event.hash && legacy != event.hash) {
            return Ok(false);
        }
...
pub async fn providers(
    ...
    Ok(Json(
        serde_json::json!({"providers":providers,"selection":"resource_aware"}),
    ))
}
...
pub async fn configure_provider(
    ...
    if !["internal", "yara", "clamav"].contains(&input.provider_id.as_str())
        || !["auto", "always", "disabled"].contains(&input.mode.as_str())
    {
        return Err(anyhow::anyhow!("invalid provider configuration").into());
    }
...
pub async fn save_policy(
    ...
    require_permission(&headers, "security_policy_manage")?;
    validate_token(&input.id)?;
    validate_token(&input.threat_type)?;
    if input.actions.is_empty()
        || !["info", "low", "medium", "high", "critical"].contains(&input.minimum_severity.as_str())
    {
        return Err(anyhow::anyhow!("invalid policy").into());
    }
...
pub async fn start_scan(
    ...
    for provider in &providers {
        results.push(
            helper_call(serde_json::json!({"action":"scan","provider":provider,"path":input.path}))
                .await
                .unwrap_or_else(|e| serde_json::json!({"ok":false,"message":e.to_string()})),
        );
    }
    let infected = results.iter().any(|result| {
        result.get("message").and_then(|v| v.as_str()) == Some("threat detected")
            || result.pointer("/data/infected").and_then(|v| v.as_bool()) == Some(true)
    });
    ...
    sqlx::query("UPDATE scan_jobs SET status=?,result=?,completed_at=? WHERE id=?")
        .bind(status)
        ...
        .await?;
    Ok(Json(
        serde_json::json!({"id":id,"status":status,"providers":providers,"infected":infected,"results":results}),
    ))
}

pub async fn apply_firewall(
    headers: HeaderMap,
    Json(input): Json<FirewallInput>,
) -> Result<impl IntoResponse, CenterError> {
    require_permission(&headers, "security_firewall_manage")?;
    if input.ruleset.contains("flush ruleset") || !input.ruleset.contains("rumahl_security") {
        return Err(anyhow::anyhow!("unsafe firewall policy").into());
    }
    Ok(Json(
        helper_call(serde_json::json!({"action":"apply_firewall","ruleset":input.ruleset})).await?,
    ))
}
...
pub async fn restore_quarantine(
    ...
    let actor = headers
        ...
        .unwrap_or("admin");
    let destination = format!("/var/lib/rumahl-security/restored/{}", input.destination_name);
    let response = helper_call(
        serde_json::json!({"action":"restore_quarantine","id":input.id,"destination":destination}),
    )
    .await?;
    if response.get("ok").and_then(|value| value.as_bool()) != Some(true) {
        return Err(anyhow::anyhow!("quarantine restore rejected").into());
    }
    ...
    crate::log_security_event(
        ...
        Some(actor),
        Some(&serde_json::json!({"id":input.id,"destination":destination}).to_string()),
    )
    .await?;
    Ok(Json(response))
}
...
async fn select_providers(state: &AppState, requested: &[String]) -> Result<Vec<String>> {
    if !requested.is_empty() {
        for item in requested {
            if !["internal", "yara", "clamav"].contains(&item.as_str()) {
                bail!("unknown scanner provider")
            }
        }
    ...
    Ok(rows
        .into_iter()
        .filter_map(|r| {
            let id = r.get::<String, _>("provider_id");
            let mode = r.get::<String, _>("mode");
            if mode == "always" || id != "clamav" || clamav_safe {
                Some(id)
            } else {
                None
            }
...
fn meminfo_kib(contents: &str, key: &str) -> u64 {
    contents
        .lines()
...
fn memory_pressure_avg10(contents: &str) -> f64 {
    contents
        .lines()
...
pub(crate) async fn helper_call(value: serde_json::Value) -> Result<serde_json::Value> {
    let mut stream = UnixStream::connect(HELPER_SOCKET)
        .await
        .context("security helper unavailable")?;
    stream
...
fn require_permission(headers: &HeaderMap, permission: &str) -> Result<()> {
    let trusted_proxy =
        headers.get("x-rumahl-proxy").and_then(|v| v.to_str().ok()) == Some("rumahl-home");
    let permissions = headers
        .get("x-rumahl-permissions")
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    if !trusted_proxy
        || !(permissions.split(',').any(|p| p.trim() == permission)
            || permissions.split(',').any(|p| p.trim() == "security_admin"))
    {
        bail!("missing security permission: {permission}")
    }
    Ok(())
}
...
impl IntoResponse for CenterError {
...
mod tests {
    ...
    fn resource_parser_uses_available_not_total_memory() {
        ...
        assert_ne!(
            meminfo_kib(data, "MemAvailable"),
            meminfo_kib(data, "MemTotal")
        );
    }
    ...
    fn pressure_parser_is_fail_closed_for_reported_pressure() {
        assert_eq!(
            memory_pressure_avg10("some avg10=12.50 avg60=4.0 total=1\n"),
            12.5
        );
    ...
    fn direct_requests_never_gain_permissions() {
        ...
        headers.insert("x-rumahl-permissions", "security_admin".parse().unwrap());
    ...
    fn health_distinguishes_degraded_from_compromised() {
        ...
        assert_eq!(
            classify_health(&effective, true),
            ("degraded", vec!["scanner_unavailable"])
        );
