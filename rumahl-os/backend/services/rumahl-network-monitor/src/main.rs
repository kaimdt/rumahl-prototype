...
// 8104 per the rumahl port map (8103 belongs to rumahl-files) - read via
// system_config::service_port so PORT / RUMAHL_NETWORK_MONITOR_PORT win.
...
async fn health() -> Json<serde_json::Value> {
    Json(serde_json::json!({
        "service": "rumahl-network-monitor",
        "status": "healthy",
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG").unwrap_or_else(|_| "rumahl_network_monitor=info".to_string()),
        )
        .init();

    info!("Starting rumahl Network Monitor service...");
    ...
    let port = system_config::service_port("rumahl-network-monitor", DEFAULT_PORT);
    let addr = std::net::SocketAddr::from(([0, 0, 0, 0], port));
    info!("rumahl-network-monitor listening on {}", addr);
