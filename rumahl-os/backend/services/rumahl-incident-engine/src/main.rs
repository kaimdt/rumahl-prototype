...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        ...
        .init();
    let journal = std::env::var("RUMAHL_INCIDENT_JOURNAL")
        .unwrap_or_else(|_| "/var/lib/rumahl-incidents/incidents.jsonl".into());
    let state = AppState {
    ...
    let app = Router::new()
        ...
        .with_state(state);
    let port = rumahl_shared_config::system_config::service_port("rumahl-incident-engine", 8109);
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", port)).await?;
    let _heartbeat = rumahl_shared_heartbeat::spawn_default(
        "rumahl-incident-engine",
        port,
        "Read-only runtime incident correlation",
    );
    info!(port, "rumahl incident engine started");
