...
async fn duckduckgo_search(
    ...
    let resp = client
        .get(&url)
        .header("User-Agent", "rumahl-gateway/1.0")
        .send()
        .await?;
    if !resp.status().is_success() {
...
async fn main() -> Result<()> {
    ...
    let port = system_config::service_port("rumahl-gateway", 8096).to_string();
    let addr = format!("0.0.0.0:{}", port);

    info!("🌐 rumahl-gateway starting on {}", addr);
