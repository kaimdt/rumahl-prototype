...
use rumahl_shared::{
    ...
    system_config,
    types::{HealthStatus, RumahlEvent, ServiceHealth},
...
struct AppState {
    ...
    events_tx: broadcast::Sender<RumahlEvent>,
...
fn send_event(tx: &broadcast::Sender<RumahlEvent>, event: RumahlEvent) {
    if let Err(e) = tx.send(event) {
        tracing::warn!(
            "rumahl-core: event broadcast failed (channel full or no receivers): {:?}",
            e
        );
    }
}
...
async fn register_service(
    ...
    let event = RumahlEvent {
        event_type: "service.registered".to_string(),
        source: "rumahl-core".to_string(),
...
async fn install_plugin(
    ...
    // supervisor (Docker apps) or in the rumahl-home plugin host (sandboxed JS
    ...
    struct RegisteredPlugin(rumahl_shared::plugin::PluginMetadata);
    #[async_trait::async_trait]
    impl rumahl_shared::plugin::IPlugin for RegisteredPlugin {
        fn metadata(&self) -> &rumahl_shared::plugin::PluginMetadata {
            &self.0
        }
        ...
        async fn on_load(&self) -> anyhow::Result<()> {
            tracing::info!(
                ...
                "plugin metadata registered with rumahl-core"
            );
            Ok(())
        }
        ...
        async fn run_sandboxed(
            ...
            // rumahl-core only stores metadata. The actual execution path is
            // supervisor (for container apps) or rumahl-home (for sandboxed
            // JS plugins). Surface this so callers do not silently no-op.
            anyhow::bail!(
                "plugin '{}' has no inline executor in rumahl-core; \
                 invoke it via the supervisor or rumahl-home plugin host",
                self.0.id
            )
        }
    }

    let plugin = Arc::new(RegisteredPlugin(req.metadata)) as Arc<dyn rumahl_shared::plugin::IPlugin>;
...
async fn watch_heartbeat_freshness(state: AppState) {
    loop {
        ...
        for name in newly_stale {
            tracing::warn!(
                "rumahl-core: service '{}' missed heartbeats; marking stale",
                name
            );
            send_event(
                &state.events_tx,
...
async fn run_core_migrations(pool: &DbPool) -> anyhow::Result<()> {
    ...
    for (name, sql) in migrations {
        ...
        if result.is_none() {
            tracing::info!("rumahl-core: applying migration {}", name);
            ...
            let mut tx = pool.begin().await.map_err(|e| {
                tracing::error!(
                    "rumahl-core: failed to begin transaction for migration {}: {}",
                    name,
                    e
                );
                e
            ...
            if has_multiple_statements {
                ...
                for stmt in &statements {
                    sqlx::query(stmt.as_str())
                        ...
                        .map_err(|e| {
                            tracing::error!(
                                "rumahl-core: migration {} failed on statement (rolling back): {}",
                                name,
                                e
                            );
                            e
                ...
                if !statement_trimmed.is_empty() && !statement_trimmed.starts_with("--") {
                    sqlx::query(statement_trimmed)
                        ...
                        .map_err(|e| {
                            tracing::error!(
                                "rumahl-core: migration {} failed (rolling back): {}",
                                name,
                                e
                            );
                            e
            ...
            tx.commit().await.map_err(|e| {
                tracing::error!("rumahl-core: failed to commit migration {}: {}", name, e);
                e
            })?;
        }
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rumahl_core=debug,info".parse().unwrap()),
        )
        .init();
    ...
    let db = if let Ok(db_url) = std::env::var("DATABASE_URL") {
        match sqlx::PgPool::connect(&db_url).await {
            Ok(pool) => {
                tracing::info!("rumahl-core: connected to PostgreSQL");
                if let Err(e) = run_core_migrations(&pool).await {
                    tracing::warn!("rumahl-core: migration warning: {}", e);
                }
                Some(Arc::new(pool))
            }
            Err(e) => {
                tracing::warn!(
                    "rumahl-core: could not connect to PostgreSQL ({}), running without DB",
                    e
                );
                None
            }
        ...
        tracing::info!("rumahl-core: DATABASE_URL not set, running without persistent storage");
        None
    };
    ...
    let port: u16 = system_config::service_port("rumahl-core", 8090);
    ...
    info!("rumahl-core listening on {}", addr);
