...
The Resource Manager service monitors Docker container resource usage and intelligently reallocates CPU and memory resources based on actual utilization patterns. It ensures efficient resource distribution across all rumahl services and apps.
...
1. Fetches stats for all running rumahl containers
...
1. **List Containers**: Filters for rumahl containers (prefix: `rumahl-`)
...
**Before**:
- `rumahl-watchdog`: 2048 CPU shares, 1024 MB RAM
...
**Before**:
- `rumahl-core`: 1024 CPU shares, 512 MB RAM
...
- **rumahl-appstore**: Notified when app containers need more resources during installation
- **rumahl-supervisor**: Coordinates with supervisor for container lifecycle management
- **rumahl-watchdog**: Alerted when resource reallocation fails or limits reached
- **rumahl-nginx**: Not affected by resource changes (proxies remain stable)
