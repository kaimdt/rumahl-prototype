        ...
        NasPlanRequest::CreateShare {
            ...
            let commands = match protocol.as_str() {
                "smb" => vec![PlannedCommand {
                    program: "rumahl-storage-share".into(),
                    arguments: vec![
                        "apply-smb".into(),
                        "--name".into(),
                        name.clone(),
            ...
            Ok(NasOperationPlan {
                ...
                warnings: vec![
                    "The target path must remain inside an rumahl-managed storage volume.".into(),
...
fn validate_managed_path(value: &str) -> Result<String, (StatusCode, Json<Value>)> {
    ...
    let configured =
        std::env::var("RUMAHL_STORAGE_ROOTS").unwrap_or_else(|_| "/srv/ora:/mnt/ora".into());
    if configured
        .split(':')
        ...
        .any(|root| Path::new(&value).starts_with(root))
    {
        ...
        Err(api_error(
            StatusCode::FORBIDDEN,
            "Storage path is outside the configured rumahl-managed roots".into(),
        ))
    }
}
...
mod tests {
    ...
    fn creates_non_executable_volume_plan() {
        let plan = build_plan(NasPlanRequest::CreateVolume {
            backend: "btrfs".into(),
            pool: "/srv/ora/storage".into(),
            name: "family".into(),
            quota_bytes: Some(1_000_000),
        })
        .unwrap();
        assert_eq!(plan.resource, "/srv/ora/storage/family");
