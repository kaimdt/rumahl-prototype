...
pub async fn inventory() -> Json<Value> {
    let lsblk = command_json("lsblk", &["--bytes", "--json", "--output", "NAME,KNAME,PATH,TYPE,SIZE,MODEL,SERIAL,VENDOR,TRAN,ROTA,RM,RO,FSTYPE,FSVER,LABEL,UUID,MOUNTPOINTS,FSAVAIL,FSUSED,FSUSE%"])
        .await;
    let block_devices = lsblk
        .as_ref()
        .ok()
        .and_then(|value| value.get("blockdevices"))
        .cloned()
        .unwrap_or_else(|| json!([]));
...
fn capability(error: Option<&String>) -> Capability {
    Capability {
        ...
        detail: error.cloned().unwrap_or_else(|| "Available".into()),
    }
}
...
fn physical_disk_paths(devices: &Value) -> Vec<String> {
    devices
        ...
        .into_iter()
        .flatten()
        .filter(|device| device.get("type").and_then(Value::as_str) == Some("disk"))
        .filter_map(|device| {
            device
                .get("path")
...
async fn collect_smart(paths: &[String]) -> HashMap<String, Value> {
    if !command_exists("smartctl").await {
        return HashMap::new();
    }
    let futures = paths.iter().map(|path| async move {
        let output = Command::new("smartctl")
            .args(["--json", "--health", "--attributes", path])
            .output()
            .await
            .ok()?;
...
pub async fn plan(
    Json(request): Json<RaidPlanRequest>,
) -> Result<Json<RaidOperationPlan>, (StatusCode, Json<Value>)> {
    Ok(Json(build_plan(request).await?))
}

pub async fn build_plan(
    request: RaidPlanRequest,
) -> Result<RaidOperationPlan, (StatusCode, Json<Value>)> {
    let mdstat = tokio::fs::read_to_string("/proc/mdstat")
        .await
        .map_err(|error| {
            api_error(
                StatusCode::SERVICE_UNAVAILABLE,
                format!("Linux MD status is unavailable: {error}"),
            )
        })?;
            ...
            let current = arrays
                ...
                .ok_or_else(|| {
                    api_error(
                        ...
                        format!("RAID array '{array}' was not found"),
                    )
                })?;
            if !current.active {
                return Err(api_error(
                    StatusCode::CONFLICT,
                    "Inactive arrays cannot be scrubbed".into(),
                ));
            }
            RaidOperationPlan {
                operation: "scrub".into(),
                array: format!("/dev/{array}"),
                summary: format!("Request a consistency check for /dev/{array}"),
                warnings: vec![
                    "A scrub increases disk I/O and can affect application performance.".into(),
                ],
                commands: vec![PlannedCommand {
                    program: "sh".into(),
                    arguments: vec![
                        "-c".into(),
                        format!("echo check > /sys/block/{array}/md/sync_action"),
                    ],
                    purpose: "Start the kernel MD consistency check".into(),
                }],
                confirmation_phrase: format!("SCRUB {array}"),
                executable: false,
            }
        }
        RaidPlanRequest::ReplaceMember {
            array,
            failed_member,
            replacement,
        } => {
            let array = validate_array_name(&array)?;
            let failed_member = failed_member
                .as_deref()
                .map(validate_device_path)
                .transpose()?;
            ...
            if current.health != "degraded" {
                return Err(api_error(
                    StatusCode::CONFLICT,
                    "Member replacement is only planned for degraded arrays".into(),
                ));
            }
            if failed_member.as_ref().is_some_and(|failed| {
                !current.members.iter().any(|member| {
                    Path::new(failed).file_name().and_then(|name| name.to_str())
                        == Some(member.as_str())
                })
            }) {
                return Err(api_error(
                    StatusCode::BAD_REQUEST,
                    "The failed member does not belong to this array".into(),
                ));
            }
            let replacement_info = inspect_replacement(&replacement).await?;
            if replacement_info.mounted {
                return Err(api_error(
                    StatusCode::CONFLICT,
                    "The replacement device or one of its children is mounted".into(),
                ));
            }
            if replacement_info.has_signatures {
                return Err(api_error(
                    StatusCode::CONFLICT,
                    "The replacement device contains a filesystem or RAID signature".into(),
                ));
            }
            let required_bytes =
                member_required_bytes(&current.members)
                    .await
                    .ok_or_else(|| {
                        api_error(
                            StatusCode::SERVICE_UNAVAILABLE,
                            "Could not determine the RAID member capacity".into(),
                        )
                    })?;
            if replacement_info.size_bytes < required_bytes {
                return Err(api_error(StatusCode::CONFLICT, format!("Replacement capacity is {} bytes but at least {required_bytes} bytes are required", replacement_info.size_bytes)));
            }
            RaidOperationPlan {
                operation: "replace_member".into(),
                array: format!("/dev/{array}"),
                summary: failed_member.as_ref().map_or_else(
                    || format!("Add {replacement} to the degraded /dev/{array}"),
                    |failed| format!("Replace {failed} with {replacement} in /dev/{array}"),
                ),
                warnings: vec![
                    "Verify the physical disk serial number before executing this plan.".into(),
                    "The replacement will be overwritten when it is added to the array.".into(),
                    "rumahl will monitor the kernel rebuild before declaring the array healthy."
                        .into(),
                ],
                commands: {
                    ...
                    if let Some(failed) = failed_member {
                        commands.push(PlannedCommand {
                            ...
                            purpose: "Mark the failed member".into(),
                        });
                        commands.push(PlannedCommand {
                            program: "mdadm".into(),
                            arguments: vec![format!("/dev/{array}"), "--remove".into(), failed],
                            purpose: "Remove the failed member".into(),
                        });
                    }
                    commands.push(PlannedCommand {
                        program: "mdadm".into(),
                        arguments: vec![format!("/dev/{array}"), "--add".into(), replacement],
                        purpose: "Add the verified replacement and start rebuild".into(),
                    });
                    commands
                },
                confirmation_phrase: format!("REPLACE {array}"),
                executable: false,
            }
        }
    };
...
async fn inspect_replacement(path: &str) -> Result<ReplacementInfo, (StatusCode, Json<Value>)> {
    let value = command_json(
        ...
        &[
            ...
            "--json",
            "--output",
            "PATH,SIZE,FSTYPE,MOUNTPOINTS,TYPE",
            path,
        ],
    )
    .await
    ...
    let device = value
        ...
        .ok_or_else(|| {
            api_error(
                StatusCode::NOT_FOUND,
                "Replacement device was not found".into(),
            )
        })?;
    ...
    while let Some(entry) = stack.pop() {
        mounted |= entry
            .get("mountpoints")
            .and_then(Value::as_array)
            .is_some_and(|points| points.iter().any(|point| !point.is_null()));
        has_signatures |= entry
            .get("fstype")
            .and_then(Value::as_str)
    ...
    Ok(ReplacementInfo {
        size_bytes: device.get("size").and_then(Value::as_u64).unwrap_or(0),
        mounted,
        has_signatures,
    })
}
...
fn validate_array_name(value: &str) -> Result<String, (StatusCode, Json<Value>)> {
            ...
            .all(|character| character.is_ascii_alphanumeric() || character == '_')
    {
        ...
        Err(api_error(
            StatusCode::BAD_REQUEST,
            "Invalid Linux MD array name".into(),
        ))
    }
}
...
fn validate_device_path(value: &str) -> Result<String, (StatusCode, Json<Value>)> {
        ...
        })
    {
        ...
        Err(api_error(
            StatusCode::BAD_REQUEST,
            "Invalid block device path".into(),
        ))
    }
}
...
pub fn parse_mdstat(input: &str) -> Vec<RaidArray> {
    ...
    while index < lines.len() {
        ...
        let pattern = detail
            .split_whitespace()
            .find(|token| token.starts_with('[') && token.contains(['U', '_']));
        let operation_line = lines.get(index + 2).map(|line| line.trim()).unwrap_or("");
        let operation = ["recovery", "resync", "reshape", "check"]
            .into_iter()
            .find(|name| operation_line.contains(name))
            .map(str::to_string);
        let progress_percent = operation.as_ref().and_then(|_| {
            operation_line
                .split_whitespace()
                .find_map(|token| token.strip_suffix('%')?.parse().ok())
        });
        let (expected_devices, active_devices) = counts.unwrap_or((None, None));
        let degraded = pattern.is_some_and(|pattern| pattern.contains('_'))
            || matches!((expected_devices, active_devices), (Some(expected), Some(current)) if current < expected);
...
mod tests {
    ...
    fn validates_array_and_device_identifiers() {
        ...
        assert_eq!(
            validate_device_path("/dev/disk/by-id/test_disk-1").unwrap(),
            "/dev/disk/by-id/test_disk-1"
        );
