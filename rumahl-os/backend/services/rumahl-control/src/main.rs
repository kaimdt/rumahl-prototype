...
/// Returns the rumahl-core base URL.
/// Honors `$RUMAHL_CORE_URL` env var; falls back to system_config.
fn rumahl_core_url() -> String {
    std::env::var("RUMAHL_CORE_URL")
        .unwrap_or_else(|_| rumahl_shared_config::system_config::service_url("rumahl-core", 8090))
}

/// Returns the rumahl-home base URL.
/// Honors `$RUMAHL_HOME_URL` env var; falls back to system_config.
fn rumahl_home_url() -> String {
    std::env::var("RUMAHL_HOME_URL")
        .unwrap_or_else(|_| rumahl_shared_config::system_config::service_url("rumahl-home", 8126))
}
...
impl AppState {
    fn new() -> Self {
        // Use the shared rumahl JWT secret (same source as rumahl-home) so tokens
        // issued by rumahl-home's /api/auth/login are accepted here. The generic
...
async fn dashboard_overview(State(state): State<AppState>) -> Json<serde_json::Value> {
    let services = async {
        state
            .http
            .get(format!("{}/api/core/services", rumahl_core_url()))
            .send()
            .await
            .ok()?
    ...
    let plugins = async {
        state
            .http
            .get(format!("{}/api/core/plugins", rumahl_core_url()))
            .send()
            .await
            .ok()?
            .json::<serde_json::Value>()
...
async fn list_services(State(state): State<AppState>) -> impl IntoResponse {
        ...
        .await
    {
        ...
        Err(e) => (
            ...
            Json(serde_json::json!({ "error": format!("rumahl-core unreachable: {}", e) })),
        )
            .into_response(),
    }
}
...
async fn aggregate_logs() -> Json<serde_json::Value> {
    ...
    let services: Vec<(&str, String)> = vec![
        ("rumahl-home", rumahl_home_url()),
        ("rumahl-core", rumahl_core_url()),
        ("rumahl-control", "http://localhost:8123".to_string()),
        (
            "rumahl-assist",
            std::env::var("RUMAHL_ASSIST_URL")
                .unwrap_or_else(|_| "http://localhost:8129".to_string()),
        ),
        ("rumahl-supervisor", "http://localhost:8097".to_string()),
        ("rumahl-watchdog", "http://localhost:8095".to_string()),
    ];
        ...
        for (name, base) in &services {
            ...
            let candidates = [
                ...
                format!(
                    ...
                    name.trim_start_matches("rumahl-")
                ),
            ];
            let mut got = false;
...
async fn install_plugin(
    ...
    match state
        .http
        .post(format!("{}/api/core/plugins", rumahl_core_url()))
        .json(&body)
        .send()
        .await
    {
...
async fn remove_plugin(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    match state
        .http
        .delete(format!("{}/api/core/plugins/{}", rumahl_core_url(), id))
        .send()
        .await
    {
        Ok(resp) => {
...
async fn list_users(State(state): State<AppState>) -> impl IntoResponse {
    match state
        .http
        .get(format!("{}/api/users", rumahl_home_url()))
        .send()
        .await
    {
        Ok(resp) => match resp.json::<serde_json::Value>().await {
        ...
        Err(e) => (
            StatusCode::SERVICE_UNAVAILABLE,
            Json(serde_json::json!({ "error": format!("rumahl-home unreachable: {}", e) })),
        )
            .into_response(),
    }
}
...
async fn list_network_interfaces() -> Json<serde_json::Value> {
    // Try rumahl-netctl first (available on rumahl OS hosts).
    let netctl_output = tokio::process::Command::new("/usr/bin/rumahl-netctl")
        .args(["status"])
        .output()
        .await;
    ...
    // Fallback: use sysinfo only (non-rumahl-OS or rumahl-netctl not installed).
    let networks = Networks::new_with_refreshed_list();
...
async fn set_hostname(Json(req): Json<HostnameRequest>) -> impl IntoResponse {
    ...
    if !success {
        ...
        return (
            ...
            Json(serde_json::json!({
                "success": false,
                "error": "Failed to apply hostname (requires root or rumahl OS)",
            })),
        )
            .into_response();
    }
...
async fn os_reboot(Json(req): Json<PowerRequest>) -> impl IntoResponse {
    ...
    if scheduled {
        ...
        (
            ...
            Json(serde_json::json!({
                "success": false,
                "error": "Failed to schedule reboot (requires root or rumahl OS)",
            })),
        )
            .into_response()
    }
...
async fn os_shutdown(Json(req): Json<PowerRequest>) -> impl IntoResponse {
    ...
    if scheduled {
        ...
        (
            ...
            Json(serde_json::json!({
                "success": false,
                "error": "Failed to schedule shutdown (requires root or rumahl OS)",
            })),
        )
            .into_response()
    }
...
/// POST /api/control/os/network/set — Apply network configuration via rumahl-netctl.
/// Only works on rumahl OS hosts where /usr/bin/rumahl-netctl is installed.
pub async fn set_network_config(Json(config): Json<serde_json::Value>) -> Json<serde_json::Value> {
    ...
    let mut child = match tokio::process::Command::new("/usr/bin/rumahl-netctl")
        .args(["set", "-"])
        ...
        .spawn()
    {
        ...
        Err(e) => {
            return Json(serde_json::json!({
                "ok": false,
                "error": format!("Cannot run rumahl-netctl: {}", e),
                "note": "This API requires rumahl OS with rumahl-netctl installed"
            }));
        }
    };
    ...
    match child.wait_with_output().await {
        ...
        Ok(out) => {
            ...
            Json(serde_json::json!({
                "ok": false,
                "error": format!("rumahl-netctl failed: {}", stderr)
            }))
        }
        Err(e) => Json(serde_json::json!({
            "ok": false,
            "error": format!("rumahl-netctl error: {}", e)
        })),
    }
}
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rumahl_control=debug,info".parse().unwrap()),
        )
        .init();
    ...
    let protected_routes = Router::new()
        ...
        // OS-level management (only useful when running on rumahl OS)
    ...
    info!("rumahl-control listening on {}", addr);
