...
const BACKUP_DIR_DEFAULT: &str = "/var/backups/ora";
const STATE_DIR: &str = "/var/lib/ora";

#[derive(Parser)]
#[command(name = "rumahl-installer")]
#[command(about = "rumahl Installation and Update Manager", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
#[derive(Subcommand)]
enum Commands {
    /// Perform a fresh installation of rumahl
    Install {
        /// Skip interactive prompts
        #[arg(long)]
        non_interactive: bool,
    },
    /// Update rumahl to the latest version
    Update {
    ...
    Config,
    /// Show status of all rumahl services
    Status,
...
fn install(non_interactive: bool) -> Result<()> {
    println!("{}", "🚀 rumahl Installation".bright_blue().bold());
    println!();
    ...
    // Create rumahl user
    println!();
    println!("{}", "Creating rumahl system user...".bright_cyan());
    create_rumahl_user()?;

    // Install rumahl binaries
    println!();
    println!("{}", "Installing rumahl programs...".bright_cyan());
    install_rumahl_binaries()?;
    ...
    // Start services
    println!();
    println!("{}", "Starting rumahl services...".bright_cyan());
    start_services()?;
    ...
    match rumahl_shared_config::env::RumahlEnv::write_install_marker() {
        Ok(path) => println!(
...
fn check_debian_version() -> Result<()> {
    ...
    if version_num >= 11.0 {
        ...
        anyhow::bail!(
            "rumahl requires Debian 11 or later (detected: {})",
            version.trim()
        );
    }
}
...
fn create_rumahl_user() -> Result<()> {
    // Check if user exists
    let exists = Command::new("id").arg("ora").status().is_ok();

    if !exists {
        Command::new("useradd")
            .args(["-r", "-s", "/bin/false", "-d", "/opt/rumahl", "ora"])
            .status()
            .context("Failed to create ora user")?;
        println!("  ✓ Created system user 'ora'");
    } else {
        println!("  ✓ User 'ora' already exists");
    }

    Ok(())
}

fn install_rumahl_binaries() -> Result<()> {
    let pb = create_progress_bar("Building rumahl programs");

    // Create installation directory
    std::fs::create_dir_all("/opt/rumahl/bin")?;

    // In a real implementation, this would either:
    // 1. Download pre-built binaries
    // 2. Build from source using cargo
    println!("  ℹ️  Binaries should be built with: cargo build --release");
    println!("  ℹ️  Then copied to /opt/rumahl/bin/");
...
fn setup_databases() -> Result<()> {
    ...
    let databases = vec!["rumahl_home", "rumahl_core", "rumahl_secrets"];
...
fn generate_configuration(non_interactive: bool) -> Result<()> {
    ...
    let configs = vec![
        (
            ...
            "DATABASE_URL=postgres://ora:rumahl_password@localhost:5432/rumahl_home\n\
             PORT=8126\n"
                .to_string(),
        ),
        (
            "/etc/ora/rumahl-core.env",
            "DATABASE_URL=postgres://ora:rumahl_password@localhost:5432/rumahl_core\n\
             PORT=8090\n"
                .to_string(),
        ),
        (
            "/etc/ora/rumahl-secrets.env",
            format!(
                "DATABASE_URL=postgres://ora:rumahl_password@localhost:5432/rumahl_secrets\n\
             SECRETS_MASTER_KEY={}\n\
             PORT=8093\n",
                master_key_hex
            ),
        ),
        ("/etc/ora/rumahl-watchdog.env", "PORT=8094\n".to_string()),
    ];
    ...
    println!("  ⚠️  Master key saved to /etc/ora/rumahl-secrets.env - KEEP THIS SECURE!");
...
fn create_systemd_services() -> Result<()> {
    ...
    for (name, port) in MANAGED_SERVICES {
        // rumahl-core is a hard dependency for most downstream services; keep
        // the dependency graph explicit so the unit order is stable.
        let wants = if *name == "rumahl-core" {
            "".to_string()
        } else {
            "Wants=rumahl-core.service\nAfter=rumahl-core.service\n".to_string()
        };

        let unit_content = format!(
            "[Unit]
Description=rumahl {name} Service
Documentation=https://github.com/rumahl/home-assistant-dashb
...
User=ora
Group=ora
WorkingDirectory=/opt/rumahl
EnvironmentFile=-/etc/ora/{name}.env
ExecStart=/opt/rumahl/bin/{name}
ExecStartPre=/bin/sh -c 'test -x /opt/rumahl/bin/{name}'
Restart=always
RestartSec=5
TimeoutStartSec=60
TimeoutStopSec=30
...
ReadWritePaths=/var/log/ora /var/lib/ora /tmp /mnt/data
# Logging
StandardOutput=journal
StandardError=journal
SyslogIdentifier={name}
# Expose port for healthchecks
Environment=RUMAHL_SERVICE_PORT={port}
...
Description=rumahl Health Enforcer
After=multi-user.target

[Service]
Type=oneshot
ExecStart=/opt/rumahl/bin/rumahl-installer validate --enforce
User=root
StandardOutput=journal
StandardError=journal
";
    std::fs::write("/etc/systemd/system/rumahl-health.service", health_service)?;

    let health_timer = "[Unit]
Description=rumahl Health Enforcer Timer
...
";
    std::fs::write("/etc/systemd/system/rumahl-health.timer", health_timer)?;

    Command::new("systemctl").arg("daemon-reload").status()?;
    let _ = Command::new("systemctl")
        .args(["enable", "--now", "rumahl-health.timer"])
        .status();
...
fn start_services() -> Result<()> {
    // rumahl-core must come up first so dependent services find the event bus.
    let mut ordered: Vec<&str> = MANAGED_SERVICES.iter().map(|(n, _)| *n).collect();
    ordered.sort_by_key(|n| if *n == "rumahl-core" { 0 } else { 1 });
...
fn update(version: Option<String>) -> Result<()> {
    println!("{}", "🔄 rumahl Update".bright_blue().bold());

    let url =
        std::env::var("RUMAHL_UPDATER_URL").unwrap_or_else(|_| "http://127.0.0.1:8101".to_string());
    ...
    match resp {
        ...
        Err(e) => {
            println!(
                "  ⚠ rumahl-updater is not reachable ({}). Falling back to manual instructions.",
                e
            );
            println!("  ℹ️  Pull the latest images and run `systemctl restart rumahl-*` manually.");
            Ok(())
        }
    }
}
...
fn rollback() -> Result<()> {
    println!("{}", "⏪ rumahl Rollback".bright_yellow().bold());
    ...
    let mut entries: Vec<PathBuf> = std::fs::read_dir(backup_dir)?
        ...
        .filter(|p| {
            p.extension().and_then(|s| s.to_str()) == Some("gz")
                && p.file_name()
                    .and_then(|n| n.to_str())
                    .map(|n| n.starts_with("rumahl-backup-"))
                    .unwrap_or(false)
        })
        .collect();
    if entries.is_empty() {
...
fn config() -> Result<()> {
    println!("{}", "⚙️  rumahl Configuration".bright_blue().bold());
    let config_path =
        std::env::var("RUMAHL_CONFIG_FILE").unwrap_or_else(|_| "/etc/ora/ora.toml".to_string());
    ...
    if !Path::new(&config_path).exists() {
        std::fs::create_dir_all(
            Path::new(&config_path)
                .parent()
                .unwrap_or(Path::new("/etc/ora")),
        )?;
        std::fs::write(&config_path, "# rumahl configuration\n")?;
...
fn status() -> Result<()> {
    println!("{}", "📊 rumahl Status".bright_blue().bold());
    println!();
...
fn backup(_output: Option<String>) -> Result<()> {
    println!("{}", "💾 rumahl Backup".bright_blue().bold());
    let url =
        std::env::var("RUMAHL_BACKUP_URL").unwrap_or_else(|_| "http://127.0.0.1:8084".to_string());
...
fn restore(backup_file: String) -> Result<()> {
    println!("{}", "📦 rumahl Restore".bright_yellow().bold());
    let url =
        std::env::var("RUMAHL_BACKUP_URL").unwrap_or_else(|_| "http://127.0.0.1:8084".to_string());
