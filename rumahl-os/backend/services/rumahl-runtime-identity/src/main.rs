...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .with_state(state);
    let port = rumahl_shared_config::system_config::service_port("rumahl-runtime-identity", 8107);
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", port)).await?;
    let _heartbeat = rumahl_shared_heartbeat::spawn_default(
        "rumahl-runtime-identity",
        port,
        "Read-only runtime attribution",
    );
    info!(port, "rumahl runtime identity resolver started");
...
async fn refresh_containers(state: &AppState) -> Result<()> {
    let url = rumahl_shared_config::system_config::service_url("rumahl-supervisor", 8097);
...
async fn resolve(event: &RuntimeEvent, state: &AppState) -> IdentitySnapshot {
    ...
    if let Some(container_id) = event.identity.container_id.as_deref() {
        ...
        if let Some(record) = find_container(container_id, &containers) {
            ...
            evidence.push(Evidence {
                ...
                source: "rumahl-supervisor:/api/supervisor/apps".into(),
    ...
    if let Some(path) = event.process.executable.path.as_deref() {
        ...
        if path.starts_with("/opt/rumahl/build/") {
            let id = path
                .trim_start_matches("/opt/rumahl/build/")
                .split('/')
                .next()
                .unwrap_or("unknown")
                .to_owned();
    ...
    async fn conflicting_high_quality_evidence_is_explicit() {
        ...
        let snapshot = resolve(
            &event(
                ...
                Some("/system.slice/rumahl-home.service"),
                None,
                1,
                "x",
                0,
