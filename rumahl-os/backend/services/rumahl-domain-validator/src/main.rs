//! rumahl Domain Validator Service
//!
...
async fn health() -> Json<serde_json::Value> {
    Json(serde_json::json!({
        "service": "rumahl-domain-validator",
        "status": "healthy",
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG").unwrap_or_else(|_| "rumahl_domain_validator=info".to_string()),
        )
        .init();

    info!("Starting rumahl Domain Validator service...");
    ...
    let port = system_config::service_port("rumahl-domain-validator", DEFAULT_PORT);
    let addr = std::net::SocketAddr::from(([0, 0, 0, 0], port));
    info!("rumahl-domain-validator listening on {}", addr);
