...
/// rumahl Supervisor - Docker orchestration for rumahl OS
///
/// This service runs as a privileged container with access to the Docker socket.
/// It manages all rumahl service containers, handles health checks, restarts,
/// updates, and coordinates the entire rumahl ecosystem.
...
async fn check_developer_mode_access(
    ...
    if !developer_mode {
        return Err(HttpResponse::Forbidden().json(serde_json::json!({
            ...
            "message": "Enable Developer Mode in rumahl Control Center to access this endpoint"
        })));
    }

    // Get app container and check installation source
    let container_name = format!("rumahl-app-{}", app_id);
    ...
    let installation_source = labels
        .as_ref()
        .and_then(|l| l.get("ora.app.installation_source"));
        ...
        Some("manual_upload") | Some("developer_app") => {
            ...
            if data.environment == "production" {
                ...
                let allow_in_prod = labels
                    .as_ref()
                    .and_then(|l| l.get("ora.app.developer_mode.allow_in_production"))
...
async fn get_status(data: web::Data<AppState>) -> impl Responder {
    ...
    let containers = match data
        ...
        .list_containers(Some(ListContainersOptions::<String> {
            ...
            filters: {
                ...
                filters.insert("label".to_string(), vec!["ora.managed=true".to_string()]);
                filters
            },
            ..Default::default()
        }))
    ...
    HttpResponse::Ok().json(serde_json::json!({
        ...
        "setup_complete": rumahl_shared_config::env::RumahlEnv::is_setup_complete(),
    }))
}
...
async fn configure_network(req: web::Json<NetworkConfigRequest>) -> impl Responder {
    ...
    let backend = std::env::var("RUMAHL_NETCONF_BACKEND").unwrap_or_else(|_| "auto".to_string());
...
async fn list_apps(data: web::Data<AppState>) -> impl Responder {
    let containers = match data
        ...
        .list_containers(Some(ListContainersOptions::<String> {
            ...
            filters: {
                let mut filters = HashMap::new();
                filters.insert("label".to_string(), vec!["ora.type=app".to_string()]);
                filters
            },
            ..Default::default()
        }))
    ...
    // Deduplicate by `ora.app.id`: stale containers of a previously
    ...
    let mut apps: Vec<serde_json::Value> = best_by_id
        ...
        .map(|c| {
            ...
            serde_json::json!({
                "id": labels.and_then(|l| l.get("ora.app.id")).unwrap_or(&"unknown".to_string()),
                "name": labels.and_then(|l| l.get("ora.app.name")).unwrap_or(&"unknown".to_string()),
                "version": labels.and_then(|l| l.get("ora.app.version")).unwrap_or(&"unknown".to_string()),
                "description": labels.and_then(|l| l.get("ora.app.description")).unwrap_or(&"".to_string()),
                "state": c.state.clone().unwrap_or_else(|| "unknown".to_string()),
                "status": c.status.clone().unwrap_or_else(|| "unknown".to_string()),
                "image": c.image.clone().unwrap_or_else(|| "unknown".to_string()),
...
async fn install_app(
    ...
    labels.insert("ora.app.name".to_string(), app.name.clone());
    labels.insert("ora.app.version".to_string(), app.version.clone());
    labels.insert("ora.app.description".to_string(), app.description.clone());
    labels.insert("ora.app.author".to_string(), app.author.clone());

    // Create container configuration
    let container_name = format!("rumahl-app-{}", app.id);
    let config = Config {
...
async fn uninstall_app(
    ...
    let container_name = format!("rumahl-app-{}", req.app_id);
...
fn compose_project_dir(req: &ComposeProjectRequest) -> Result<std::path::PathBuf, String> {
    ...
    // rumahl-home may use either the canonical /var/lib/ora/local-apps or its own
    // RUMAHL_LOCAL_APPS_DIR (/var/lib/ora/rumahl-home/local-apps) — accept both so
    // compose projects from either layout work.
    let base_dir_raw = std::env::var("RUMAHL_LOCAL_APPS_DIR")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| std::path::PathBuf::from("/var/lib/ora/local-apps"));
    let base_dir = base_dir_raw.canonicalize().unwrap_or(base_dir_raw.clone());
    let home_apps_dir = std::path::PathBuf::from("/var/lib/ora/rumahl-home/local-apps");
...
/// standalone Compose v1 binary used by older rumahl development images.
...
async fn compose_status_for_project(
    ...
    // rumahl-home keeps its apps under /var/lib/ora/rumahl-home/local-apps while
    // this service defaults to /var/lib/ora/local-apps — try the default and
    // fall back to the rumahl-home layout so status checks work for both.
    let mut compose_dirs: Vec<std::path::PathBuf> = Vec::new();
    if let Ok(dir) = compose_project_dir(&req) {
        compose_dirs.push(dir);
    }
    let home_dir = std::path::PathBuf::from("/var/lib/ora/rumahl-home/local-apps")
...
async fn remove_developer_app(docker: &Docker) -> Result<(), Box<dyn std::error::Error>> {
    const DEVELOPER_APP_CONTAINER: &str = "rumahl-app-io.rumahl.developer-app";
...
async fn ensure_developer_app_installed(docker: &Docker) -> Result<(), Box<dyn std::error::Error>> {
    const DEVELOPER_APP_ID: &str = "io.rumahl.developer-app";
    const DEVELOPER_APP_IMAGE: &str = "rumahl-developer-app:local";
    const DEVELOPER_APP_CONTAINER: &str = "rumahl-app-io.rumahl.developer-app";
    ...
    if images.is_empty() {
        ...
        // Resolve the build context path. In a typical rumahl OS deployment the
        // build context is mounted into the supervisor container via the
        // RUMAHL_BUILD_CONTEXT environment variable. If unset, fall back to
        // the conventional /opt/rumahl location used by the OS image.
        let build_context =
            std::env::var("RUMAHL_BUILD_CONTEXT").unwrap_or_else(|_| "/opt/rumahl".to_string());
        let dockerfile_rel = std::env::var("RUMAHL_DEVELOPER_APP_DOCKERFILE")
        ...
        if context_available {
            ...
            let _build_options = BuildImageOptions {
                ...
                buildargs: {
                    ...
                    args.insert(
                        "RUMAHL_DEVELOPER_APP_OFFICIAL".to_string(),
                        "true".to_string(),
                    );
                    args
                },
            ...
            // RUMAHL_DEVELOPER_APP_LIVE_BUILD=1 is set.
            info!("Build context ready – live image build is handled by the system updater.");
        } else {
            info!(
                "No build context at {} (set RUMAHL_BUILD_CONTEXT to enable live builds).",
                build_context
            );
        }

        info!("Falling back to the pre-built image shipped with the rumahl system update.");

        let main_images = docker
            .list_images(Some(ListImagesOptions::<String> {
                filters: {
                    let mut filters = HashMap::new();
                    filters.insert("reference".to_string(), vec!["rumahl-backend:*".to_string()]);
                    filters
                },
                ..Default::default()
            }))
        ...
        if main_images.is_empty() {
            return Err("Developer App image not found. Please rebuild rumahl system with: docker build -t rumahl-backend:latest --target rumahl-developer-app backend/".into());
        }

        info!("Using Developer App image from main rumahl build");
    } else {
    ...
    let mut labels = HashMap::new();
    labels.insert("ora.type".to_string(), "app".to_string());
    labels.insert("ora.app.id".to_string(), DEVELOPER_APP_ID.to_string());
    labels.insert("ora.app.name".to_string(), "rumahl Developer".to_string());
    labels.insert("ora.app.version".to_string(), "0.1.0".to_string());
    labels.insert(
        "ora.app.installation_source".to_string(),
        "developer_app".to_string(),
    );
    labels.insert("ora.app.permissions".to_string(), "DeveloperAccess,InterAppCommunication,LiveMetrics,DirectDeploy,DebugAccess,LiveLogs,HotReload".to_string());

    let env = vec![
        "RUST_LOG=info".to_string(),
        "SUPERVISOR_URL=http://rumahl-supervisor:8097".to_string(),
        "RUMAHL_API_URL=http://rumahl-api:8080".to_string(),
    ];

    let host_config = HostConfig {
        binds: Some(vec![
            "/var/run/docker.sock:/var/run/docker.sock:ro".to_string(),
            "rumahl-developer-data:/app/data".to_string(),
        ]),
        ..Default::default()
    };
    ...
    for container in containers {
        ...
        let app_info = DetailedAppInfo {
            id: labels
                ...
                .unwrap_or_else(|| "unknown".to_string()),
            name: labels
                .and_then(|l| l.get("ora.app.name"))
                .cloned()
                .unwrap_or_else(|| "unknown".to_string()),
            version: labels
                .and_then(|l| l.get("ora.app.version"))
                .cloned()
                .unwrap_or_else(|| "unknown".to_string()),
            description: labels
                .and_then(|l| l.get("ora.app.description"))
                .cloned()
                .unwrap_or_default(),
            author: labels
                .and_then(|l| l.get("ora.app.author"))
                .cloned()
                .unwrap_or_default(),
            image: container
                .image
            ...
            volumes,
            permissions: labels
                .and_then(|l| l.get("ora.app.permissions"))
...
async fn inter_app_call(
    ...
    let container_name = format!("rumahl-app-{}", req.target_app_id);
...
async fn main() -> std::io::Result<()> {
    ...
    info!("Starting rumahl Supervisor v{}", env!("CARGO_PKG_VERSION"));
    ...
    let port = system_config::service_port("rumahl-supervisor", 8097);
...
mod tests {
    ...
    fn test_parse_conflict_container_id() {
        let stderr = "ERROR: for rumahl-app-rumahl-astro-forge_rumahl-astro-forge_1 Cannot create container for service rumahl-astro-forge: Conflict. The container name \"/rumahl-app-rumahl-astro-forge_rumahl-astro-forge_1\" is already in use by container \"c9360eadf521e430d5958fa1903771146cf82880e127981754fc14eec6306964\". You have to remove (or rename) that container to be able to reuse that name.";
        assert_eq!(
    ...
    /// Mutex to serialize tests that modify the RUMAHL_LOCAL_APPS_DIR env var.
    ...
    fn test_default_dir_uses_base_plus_app_id() {
        ...
        let tmp = env::temp_dir().join("rumahl-test-default");
        fs::create_dir_all(&tmp).unwrap();
        env::set_var("RUMAHL_LOCAL_APPS_DIR", tmp.to_str().unwrap());
    ...
    fn test_valid_dir_within_base() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let tmp = env::temp_dir().join("rumahl-test-valid");
        fs::create_dir_all(&tmp).unwrap();
        env::set_var("RUMAHL_LOCAL_APPS_DIR", tmp.to_str().unwrap());
    ...
    fn test_path_traversal_rejected() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let tmp = env::temp_dir().join("rumahl-test-traversal");
        fs::create_dir_all(&tmp).unwrap();
        env::set_var("RUMAHL_LOCAL_APPS_DIR", tmp.to_str().unwrap());
    ...
    fn test_absolute_path_outside_base_rejected() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let tmp = env::temp_dir().join("rumahl-test-outside");
        fs::create_dir_all(&tmp).unwrap();
        env::set_var("RUMAHL_LOCAL_APPS_DIR", tmp.to_str().unwrap());
    ...
    fn test_empty_compose_dir_uses_default() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let tmp = env::temp_dir().join("rumahl-test-empty");
        fs::create_dir_all(&tmp).unwrap();
        env::set_var("RUMAHL_LOCAL_APPS_DIR", tmp.to_str().unwrap());
