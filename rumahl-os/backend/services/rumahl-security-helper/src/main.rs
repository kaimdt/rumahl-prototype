//! Privileged rumahl Security enforcement helper.
//!
...
fn main() -> anyhow::Result<()> {
    anyhow::bail!("rumahl-security-helper is a Linux-only privileged service: it must run as root on the rumahl host")
}
...
mod linux {
    ...
    const SOCKET_PATH: &str = "/run/ora/security-helper.sock";
    const POLICY_PATH: &str = "/etc/ora/security/firewall.nft";
    const QUARANTINE_DIR: &str = "/var/lib/rumahl-security/quarantine";
    ...
    pub(crate) async fn serve() -> Result<()> {
        tracing_subscriber::fmt()
            ...
            .init();
        tokio::fs::create_dir_all("/run/ora").await?;
        if Path::new(SOCKET_PATH).exists() {
            tokio::fs::remove_file(SOCKET_PATH).await?;
        }
        let listener = UnixListener::bind(SOCKET_PATH).context("bind security helper socket")?;
        let security_uid = account_id("/etc/passwd", "rumahl-security")
            .context("dedicated rumahl-security service account missing")?;
        let security_gid = account_id("/etc/group", "rumahl-security")
            .context("dedicated rumahl-security service group missing")?;
        ...
        info!(socket = SOCKET_PATH, "rumahl Security helper ready");
        ...
        match request {
            Request::Status => Ok(success(
                ...
                serde_json::json!({
                    ...
                    "firewall_effective": probe("/usr/sbin/nft", &["list", "table", "inet", "rumahl_security"]).await,
            ...
            Request::StopService { service } => {
                ...
                if service == "rumahl-security.service"
                    || service == "rumahl-security-helper.service"
                    || service == "rumahl-security-firewall.service"
                {
            ...
            Request::Lockdown { reason } => {
                ...
                run("/usr/sbin/nft", &["-f", "/etc/ora/security/lockdown.nft"]).await?;
                Ok(success(
                    "lockdown activated",
    ...
    async fn apply_firewall(ruleset: &str) -> Result<Response> {
        if ruleset.len() > 262_144 || !ruleset.contains("table inet rumahl_security") {
            bail!("invalid rumahl ruleset");
        }
        tokio::fs::create_dir_all("/etc/ora/security").await?;
    ...
    async fn scan(provider: Provider, path: PathBuf) -> Result<Response> {
        ...
        let result = match provider {
            ...
            Provider::Yara => {
                command_scan(
                    "/usr/bin/yara",
                    &["-r", "/etc/ora/security/yara", snapshot.to_str().unwrap()],
                )
                .await
            }
    ...
    async fn secure_snapshot(path: &Path) -> Result<PathBuf> {
        ...
        tokio::fs::create_dir_all("/run/ora/security-scans").await?;
        let snapshot = Path::new("/run/ora/security-scans").join(format!(
            "{}-{}",
            std::process::id(),
            opened.ino()
        ));
    ...
    async fn restore_quarantine(id: &str, destination: &str) -> Result<Response> {
        ...
        let recovery = Path::new("/var/lib/rumahl-security/restored");
    ...
    fn checked_scan_path(value: &str) -> Result<PathBuf> {
        ...
        let allowed = ["/opt/rumahl", "/var/lib/ora", "/srv/ora", "/tmp/rumahl-scans"];
    ...
    fn integrity_monitor_fresh() -> bool {
        std::fs::metadata("/run/ora/integrity-last-result")
    ...
    mod tests {
        ...
        fn identifiers_cannot_become_arguments() {
            for value in [
                ...
                if value == "rumahl-security.service" {
        ...
        fn symlink_components_are_rejected() {
            let root =
                std::env::temp_dir().join(format!("rumahl-helper-test-{}", std::process::id()));
