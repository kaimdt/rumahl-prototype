...
fn privileged_helper_has_no_shell_escape_surface() {
    ...
    for forbidden in ["/bin/sh", "sh -c", "bash -c", "Command::new(binary)"] {
        if forbidden == "Command::new(binary)" {
            ...
            assert!(
                ...
                "forbidden shell surface: {forbidden}"
            );
        }
    }
    ...
    assert!(
        source.contains("/usr/bin/docker\", &[\"stop\""),
        "container isolation must cover host and custom networks"
    );
}

#[test]
fn firewall_policy_cannot_flush_other_tables() {
    let source = include_str!("../../../../board/rumahl/post-build.sh");
    let baseline = source
        .split("firewall.nft\" <<'NFT'")
        .nth(1)
        .unwrap()
        .split("\nNFT")
        .next()
    ...
    assert!(
        source.contains("tcp dport 8126"),
        "lockdown must preserve rate-limited administrative recovery"
    );
    assert!(
        source.contains("rumahl-security:x:919:919"),
        "security service needs a dedicated UID"
    );
    assert!(source.contains("write_rumahl_service \"rumahl-security\" \"8095\" \"rumahl-security\""));
