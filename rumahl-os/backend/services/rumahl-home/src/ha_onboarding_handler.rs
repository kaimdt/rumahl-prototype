//! Guided Home Assistant discovery and onboarding for rumahl Home.
...
fn normalize_url(raw: &str) -> Result<String, ApiError> {
    ...
    let parsed = reqwest::Url::parse(&with_scheme)
        .map_err(|_| (StatusCode::BAD_REQUEST, "Invalid Home Assistant URL".into()))?;
...
pub async fn discover(
...
) -> Json<Vec<DiscoveredHomeAssistant>> {
    ...
    let urls: Vec<String> = candidates
        ...
        .collect();
    let results =
        futures_util::future::join_all(urls.into_iter().map(|url| probe(&state, url))).await;
    Json(results)
}
...
pub async fn onboard(
    ...
    if token.is_empty() {
        return Err((
            StatusCode::BAD_REQUEST,
            "Home Assistant access token is required".into(),
        ));
    }
    ...
    let response = state
        ...
        .map_err(|error| {
            (
                ...
                format!("Could not connect to Home Assistant: {error}"),
            )
        })?;
    ...
    let config: serde_json::Value = response.json().await.map_err(|error| {
        (
            StatusCode::BAD_GATEWAY,
            format!("Invalid Home Assistant response: {error}"),
        )
    })?;
    ...
    for (key, value) in [("ha.url", json!(url)), ("ha.token", json!(token))] {
        state
            .config_repo
            .save_system_preference(SaveSystemPreferenceRequest {
                preference_key: key.into(),
                preference_value: value.clone(),
            })
            .await
            .map_err(|error| {
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    format!("Could not save Home Assistant configuration: {error}"),
                )
            })?;
        rumahl_shared_config::system_config::update_cached_setting(key.into(), value.to_string());
    }
...
mod tests {
    ...
    fn normalizes_host_and_removes_trailing_slash() {
        assert_eq!(
            normalize_url("homeassistant.local:8123/").unwrap(),
    ...
    fn rejects_non_http_urls() {
        assert_eq!(
            normalize_url("ftp://homeassistant.local").unwrap_err().0,
            StatusCode::BAD_REQUEST
        );
    }
}
