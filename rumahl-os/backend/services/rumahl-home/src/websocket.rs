            ...
            match msg {
                Message::Text(text) => {
                    if text.len() > MAX_INBOUND_FRAME_BYTES {
                        warn!(
                            "[WS #{}] oversize frame ({} bytes), closing",
                            client_id,
                            text.len()
                        );
                        break;
                    ...
                    match ws_msg {
                        ...
                        WSMessage::Auth { token } => {
                            match auth::verify_token(&token) {
                                Ok(claims) => {
                                    reader_ws_manager
                                        .mark_authenticated(client_id, &claims)
                                        .await;
                        ...
                        WSMessage::CallService {
                            ...
                            if command_count > MAX_COMMANDS_PER_SECOND {
                                warn!(
                                    "[WS #{}] command rate limited ({}/s)",
                                    client_id, command_count
                                );
                                let _ = send_ws(
                                    &reader_out,
                                    &WSMessage::Error {
                                        id: None,
                                        message: "Rate limit exceeded. Max 30 commands per second."
                                            .to_string(),
                                    },
                                );
                                continue;
                            ...
                            info!(
                                "[WS #{}] cmd {}.{} entity={}",
                                client_id, domain, service, entity_id
                            );
                            if reader_ha_ws.is_connected() {
                                ...
                                info!(
                                    "[WS #{}] HA-WS down, REST fallback {}.{}",
                                    client_id, domain, service
                                );
                                ...
                                tokio::spawn(async move {
                                    if let Err(e) =
                                        client.call_service_fast(&d, &s, call_data).await
                                    {
        ...
        loop {
            match rx.recv().await {
                Ok(changed) => {
                    ...
                    if !send_ws(&state_tx, &WSMessage::StateUpdate { changed }) {
                        break;
                    }
                }
    ...
    let config_send_task = tokio::spawn(async move {
        loop {
            match config_rx.recv().await {
                Ok(changes) => {
                    if !send_ws(&config_tx_clone, &WSMessage::ConfigChanged { changes }) {
                        break;
                    }
                }
                Err(broadcast::error::RecvError::Lagged(n)) => {
            ...
            match event_rx.recv().await {
                Ok(event) => {
                    if let Ok(json) = serde_json::to_string(&event) {
                        if event_tx_clone.send(Message::Text(json)).is_err() {
                            break;
                        }
                    }
                }
