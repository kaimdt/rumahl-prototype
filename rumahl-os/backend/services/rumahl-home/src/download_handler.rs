...
/// feature; axum's Multipart extractor on the rumahl-files side accepts this).
...
pub async fn start_download(
    ...
    // The background task needs the user's token to upload into rumahl-files.
    let token = headers
    ...
    let files_base = crate::microservice_url("RUMAHL_FILES_URL", "rumahl-files", 8100);
...
/// Resolve the user's Downloads folder id via rumahl-files (create on first use).
...
/// Background download: stream → temp file → upload to rumahl-files → job done.
...
async fn run_download_task(
    ...
    let result: Result<serde_json::Value, String> = async {
        ...
        if let Some((folder_id, folder_name)) = folder {
            let boundary = format!("----ora{}", Uuid::new_v4().simple());
