...
//!
//! When RUMAHL_FRONTEND_DEV_URL is set (e.g., http://localhost:5173), this module
...
/// Check if frontend dev proxy mode is enabled via RUMAHL_FRONTEND_DEV_URL
pub fn is_dev_proxy_enabled() -> Option<String> {
    std::env::var("RUMAHL_FRONTEND_DEV_URL")
        .ok()
...
///
/// This handler forwards the request to the dev server specified by RUMAHL_FRONTEND_DEV_URL,
    ...
    let dev_url = match is_dev_proxy_enabled() {
        ...
        None => {
            return (
                ...
                "RUMAHL_FRONTEND_DEV_URL not configured",
            )
                .into_response()
        }
    };
