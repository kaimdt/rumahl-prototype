...
//! ```
//!
//! When a system event is recorded, rumahl-home resolves each app's runtime
...
use rumahl_shared::app_capabilities::LifecycleEvent;
...
pub async fn dispatch_to_apps(store: &LocalAppStore, event: &SystemEventHookEvent) {
    ...
    for app in apps {
        ...
        let hooks: Option<rumahl_shared::app_capabilities::LifecycleHooks> = app
            .manifest
            .extra
            .get("lifecycle_hooks")
