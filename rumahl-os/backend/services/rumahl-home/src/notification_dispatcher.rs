//! rumahl Notification Dispatcher
//!
//! Routes outbound notifications to multiple channels:
//! - **rumahl** – internal dashboard notification (DB + WebSocket broadcast)
//! - **HA Mobile** – Home Assistant `notify.mobile_app_*` service (push to smartphones)
//! - **Desktop** – rumahl Desktop Client (WebSocket push)
//!
...
impl NotificationDispatcher {
    ...
    /// Always dispatches to the rumahl internal channel regardless of `channels` filter.
    pub async fn dispatch(&self, req: DispatchRequest) -> (String, Vec<ChannelResult>) {
        let id = Uuid::new_v4().to_string();
        let now = chrono::Utc::now();

        // 1. Always persist to rumahl internal notification table
        let rumahl_result = self.dispatch_ora(&id, &req, now).await;
        let mut results = vec![ChannelResult {
            channel_id: "ora".to_string(),
            channel_type: "ora".to_string(),
            status: if rumahl_result.is_ok() { "ok" } else { "error" }.to_string(),
            error: rumahl_result.err(),
        }];
    ...
    async fn dispatch_ora(
        ...
        info!("[notif] rumahl: [{}] {}", req.level, req.title);
        Ok(())
    }
    ...
    /// Push a notification directly to the rumahl Desktop Client via WebSocket.
