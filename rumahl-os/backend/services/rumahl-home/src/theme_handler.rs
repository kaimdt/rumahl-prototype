...
fn pick_writable_theme_dir(requested: PathBuf) -> PathBuf {
    ...
    if cfg!(target_os = "linux") {
        candidates.push(PathBuf::from("/var/lib/ora/rumahl-home/themes"));
    }
    if !cfg!(target_os = "linux") {
        if let Ok(cwd) = std::env::current_dir() {
            candidates.push(cwd.join("data").join("themes"));
        }
        candidates.push(std::env::temp_dir().join("rumahl-home").join("themes"));
    }
    ...
    if cfg!(target_os = "linux") {
        PathBuf::from("/var/lib/ora/rumahl-home/themes")
    } else {
        std::env::temp_dir().join("rumahl-home").join("themes")
    }
}
...
impl ThemeState {
    ...
    pub fn extract_zip(
        &self,
        zip_data: &[u8],
    ) -> anyhow::Result<rumahl_shared::theme::ThemeDefinition> {
        use std::io::Cursor;
    ...
    pub async fn store_theme(
        &self,
        def: rumahl_shared::theme::ThemeDefinition,
    ) -> anyhow::Result<rumahl_shared::theme::ThemeDefinition> {
        // DB insert
    ...
    pub async fn install_inline(
        &self,
        mut def: rumahl_shared::theme::ThemeDefinition,
    ) -> anyhow::Result<()> {
    ...
    pub async fn set_user_theme(
        ...
        user_id: &str,
        profile_id: &str,
        sel: rumahl_shared::theme::UserThemeSelection,
    ) -> anyhow::Result<()> {
        let now = chrono::Utc::now();
    ...
    pub async fn get_user_theme(
        &self,
        profile_id: &str,
    ) -> anyhow::Result<Option<rumahl_shared::theme::UserThemeSelection>> {
        let row = sqlx::query(
        ...
        .fetch_optional(&self.db_pool)
        .await?;
        Ok(row.map(|r| rumahl_shared::theme::UserThemeSelection {
    ...
    pub async fn get_theme_css1(
        &self,
        profile_id: &str,
    ) -> anyhow::Result<rumahl_shared::theme::ThemeCssResponse> {
        ...
        if let Some((row, cached_at)) = cache.get(theme_id) {
            ...
            let fonts: Vec<rumahl_shared::theme::ThemeFont> = row
                .fonts_json
                .as_ref()
            ...
            if let Some(ref parent_id) = row.parent_theme {
                if !parent_id.is_empty() && parent_id != "auto" && parent_id != "default" {
                    if let Some((parent_row, _)) = cache.get(parent_id) {
                        let parent_fonts: Vec<rumahl_shared::theme::ThemeFont> = parent_row
                            .fonts_json
                            .as_ref()
                            .and_then(|j| match serde_json::from_str(j) {
                                Ok(v) => Some(v),
            ...
            let icon_font: Option<rumahl_shared::theme::ThemeIconConfig> = row
                .icon_font_json
                .as_ref()
                .and_then(|j| match serde_json::from_str(j) {
                    Ok(v) => Some(v),
            ...
            let mut resolved_icon_font: Option<rumahl_shared::theme::ThemeIconConfig> = None;
            ...
            let capabilities: Option<rumahl_shared::theme::ThemeCapabilities> = row
                .capabilities_json
                .as_ref()
                .and_then(|j| match serde_json::from_str(j) {
                    Ok(v) => Some(v),
            ...
            let mut widget_templates: Vec<rumahl_shared::theme::WidgetTemplate> = row
            ...
            if let Some(ref parent_id) = row.parent_theme {
                if !parent_id.is_empty() && parent_id != "auto" && parent_id != "default" {
                    if let Some((parent_row, _)) = cache.get(parent_id) {
                        let parent_wts: Vec<rumahl_shared::theme::WidgetTemplate> = parent_row.widget_templates_json.as_ref()
                            .and_then(|j| match serde_json::from_str(j) {
                                Ok(v) => Some(v),
                                Err(e) => {
        ...
        Ok(rumahl_shared::theme::ThemeCssResponse {
...
fn builtin_themes() -> Vec<rumahl_shared::theme::ThemeDefinition> {
    macro_rules! t {
        ($id:expr, $name:expr, $desc:expr, $icon:expr, $order:expr) => {
            rumahl_shared::theme::ThemeDefinition {
                id: $id.into(),
                name: $name.into(),
                version: "1.0.0".into(),
                developer: "rumahl".into(),
...
pub async fn list_themes(
...
) -> Result<Json<rumahl_shared::theme::ThemeListResponse>, (StatusCode, String)> {
    ...
    let installed: Vec<rumahl_shared::theme::InstalledTheme> = rows
        .iter()
        .map(|r| {
            ...
            rumahl_shared::theme::InstalledTheme {
                id: r.id,
                name: r.name,
                version: r.version,
        ...
        .collect();
    Ok(Json(rumahl_shared::theme::ThemeListResponse {
        builtin,
        installed,
    }))
}
...
pub async fn handle_install_theme_inline(
    ...
    let validation = rumahl_shared::manifest_validator::validate_theme_manifest(&merged);
...
pub async fn set_user_theme(
    ...
    let sel = rumahl_shared::theme::UserThemeSelection {
...
pub async fn get_theme_css(
    State(gs): State<AppState>,
    Path(profile_id): Path<String>,
) -> Result<Json<rumahl_shared::theme::ThemeCssResponse>, (StatusCode, String)> {
    let css = gs
        .theme_manager
...
pub async fn get_default_theme(
    State(gs): State<AppState>,
) -> Result<Json<rumahl_shared::theme::DefaultThemeConfig>, (StatusCode, String)> {
    ...
    let config: rumahl_shared::theme::DefaultThemeConfig = match raw {
        Some(val) => serde_json::from_str(&val).unwrap_or_default(),
        None => rumahl_shared::theme::DefaultThemeConfig::default(),
    };
    Ok(Json(config))
}
...
pub async fn set_default_theme(
    State(gs): State<AppState>,
    Json(config): Json<rumahl_shared::theme::DefaultThemeConfig>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
