// Local app store — works without `rumahl-appstore`.
//
// This is the minimum we need to let users install ZIP-packaged apps and
// plugins on a fresh / dashboard-only image. The proper rumahl-appstore
...
// frontend changes when rumahl-appstore is missing.
//
// Storage layout:
//
//   <base_dir>/
...
//       <… extracted ZIP …>
//
// Defaults to `/var/lib/ora/rumahl-home/local-apps` on Linux, falls back to a
// `rumahl-local-apps` folder under the current working directory on
// Windows / unprivileged dev envs.

use anyhow::{anyhow, Context as _, Result};
use rumahl_shared::manifest_validator::{self, ValidationSeverity};
...
pub struct AppManifest {
    ...
    pub display: Option<rumahl_shared::app_manifest::AppDisplayConfig>,
...
impl LocalAppStore {
    ...
    pub async fn open() -> Result<Arc<Self>> {
        let base = std::env::var("RUMAHL_LOCAL_APPS_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                if cfg!(target_os = "linux") {
                    PathBuf::from("/var/lib/ora/rumahl-home/local-apps")
                } else {
                    std::env::current_dir()
                        .unwrap_or_else(|_| PathBuf::from("."))
                        .join("data")
                        .join("rumahl-local-apps")
                }
            });
    ...
    pub async fn set_developer_app(&self, present: bool) -> Result<()> {
        let id = "rumahl-developer-app";
        self.set_system_app_inner(id, present, || InstalledApp {
            id: id.to_string(),
            name: "rumahl Developer App".to_string(),
            version: env!("CARGO_PKG_VERSION").to_string(),
            developer: "rumahl Project".to_string(),
            description:
    ...
    /// Register rumahl Share as a built-in system app.
    pub async fn set_share_app(&self, present: bool) -> Result<()> {
        let id = "io.rumahl.share";
        self.set_system_app_inner(id, present, || InstalledApp {
            id: id.to_string(),
            name: "rumahl Share".to_string(),
            version: env!("CARGO_PKG_VERSION").to_string(),
            developer: "rumahl Project".to_string(),
    ...
    /// Register rumahl Streaming as a built-in system app.
    pub async fn set_streaming_app(&self, present: bool) -> Result<()> {
        let id = "io.rumahl.streaming";
        self.set_system_app_inner(id, present, || InstalledApp {
            id: id.to_string(),
            name: "rumahl Streaming".to_string(),
            version: env!("CARGO_PKG_VERSION").to_string(),
            developer: "rumahl Project".to_string(),
            description: "Streame Kamera, Mikrofon oder Bildschirm live über rumahl — niedrige Latenz, ideal für Überwachungskameras oder Präsentationen.".to_string(),
            ...
            custom_pages: vec![CustomPageEntry {
                ...
                title: "rumahl Streaming".to_string(),
    ...
    async fn run_install_inner(
        ...
        // away - without this a freshly installed local app (e.g. the rumahl
        // Browser on :8102) answers "Keine konfigurierte URL für diese App"
        // until the next rumahl-home restart (reload_index recovery).
...
fn extract_zip(
    ...
    // Hard caps shared with [`rumahl_shared_upload`] — protect against
...
mod tests {
    ...
    /// local (non-Docker) app like the rumahl Browser declares its service port
    ...
    fn manifest_top_level_ports_survive_flatten_parse() {
        ...
        assert_eq!(parsed.id, "rumahl-browser");
