...
fn embedded_fallback_config() -> DocsConfig {
    DocsConfig {
        title: "rumahl OS Dokumentation".to_string(),
        description: "Eingebettete Basisdokumentation für rumahl OS".to_string(),
        navigation: vec![
            DocsSection {
...
pub async fn get_doc_file(
    ...
    let search_bases = vec!["/opt/rumahl/docs", "docs"];
...
pub async fn list_docs() -> Result<Json<Vec<String>>, (StatusCode, String)> {
    let search_dirs = vec!["/opt/rumahl/docs", "docs"];
