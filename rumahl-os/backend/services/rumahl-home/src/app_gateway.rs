...
//!
//! Serves installed rumahl apps on their own origin:
//!
//! ```text
//! https://<app-id>.apps.rumahl.local/
//! ```
//!
//! The rumahl desktop (e.g. `https://rumahl.local`) embeds these origins as
//! iframes inside its App Runner. This module is a **transparent reverse
//! proxy / app gateway** — it is NOT server-side rendering and NOT the rumahl
//! Browser. The app content is fully rendered and executed by the user's
//! browser; the gateway only:
//!
//! 1. Resolves `<app-id>.apps.rumahl.local` against the **live** app lifecycle
...
//!      only the rumahl desktop origin may frame the app,
...
pub const DEFAULT_APPS_HOST_SUFFIX: &str = ".apps.rumahl.local";

pub fn apps_host_suffix() -> String {
    std::env::var("RUMAHL_APPS_HOST_SUFFIX")
        .ok()
...
/// Optional override for the rumahl desktop origin used in `frame-ancestors`
...
pub fn desktop_origin_override() -> Option<String> {
    std::env::var("RUMAHL_DESKTOP_ORIGIN")
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
}
...
pub async fn resolve_lifecycle_state(app: &InstalledApp) -> AppLifecycleState {
    ...
    if needs_docker {
        ...
        if let Some(status) = app_lifecycle::docker_compose_status(&app.id).await {
            ...
            if status.services.contains_key("rumahl-supervisor") {
...
pub struct RuntimeTarget {
    /// Host the app is reachable on from the rumahl host (127.0.0.1).
    pub host: String,
    /// Published (host) port.
    pub port: u16,
...
/// Extract the app id from a `Host: <app-id>.apps.rumahl.local[:port]` header.
...
/// `https://nextcloud.apps.rumahl.local:3001` → `https://rumahl.local:3001`.
/// Falls back to the configured `RUMAHL_DESKTOP_ORIGIN` when present.
pub fn desktop_origin_for_host(host: &str, scheme: &str) -> String {
    ...
    // hostname = <app-id>.apps.rumahl.local → desktop = rumahl.local. The apps
...
/// Rewrite an app response so it can be embedded by the rumahl desktop:
...
(function () {{
  ...
  function send(type) {{
    window.parent.postMessage({{ source: 'rumahl-app', appId: appId, type: type }}, '*');
  }}
...
/// (`RUMAHL_GATEWAY_INSECURE_UPSTREAM_TLS=1`). LAN-internal apps often use
/// self-signed certificates; this must be enabled explicitly.
pub fn upstream_insecure_tls() -> bool {
    std::env::var("RUMAHL_GATEWAY_INSECURE_UPSTREAM_TLS")
        .ok()
...
/// default, or an accept-any verifier when `RUMAHL_GATEWAY_INSECURE_UPSTREAM_TLS`
/// is enabled.
...
/// upstream connections to rumahl-controlled apps on the local host.
#[derive(Debug)]
...
async fn proxy_http(
    ...
    if let Ok(value) = HeaderValue::from_str(app_id) {
        parts.headers.insert("x-rumahl-app", value);
    }
...
/// hostname so certificates issued for `https://<app-id>.apps.rumahl.local/`
/// verify correctly.
...
/// `wss://<app-id>.apps.rumahl.local/socket`.
...
/// outermost layer so app subdomain traffic never reaches rumahl desktop
...
/// matching the production rumahl OS gateway; the dev VM overrides both via
/// `RUMAHL_APPS_PUBLIC_SCHEME` (e.g. `http`) and `RUMAHL_APPS_PUBLIC_PORT`
/// (e.g. `8126`, the forwarded host port that reaches rumahl-home).
pub fn public_app_runtime_url(app_id: &str, base: &str) -> String {
    let scheme = std::env::var("RUMAHL_APPS_PUBLIC_SCHEME")
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "https".to_string())
        .to_ascii_lowercase();
    let port = std::env::var("RUMAHL_APPS_PUBLIC_PORT")
        .ok()
        .map(|s| s.trim().to_string())
...
pub async fn runtime_info(
    ...
    // OS cannot rely on *.apps.rumahl.local DNS/wildcard routing being
    ...
    let subdomains_configured = std::env::var("RUMAHL_APPS_PUBLIC_SCHEME")
        .ok()
    ...
    let runtime_url = if running && subdomains_configured {
        resolve_runtime_target(&state.local_appstore, app)
            ...
            .map(|_| {
                ...
                // and port are configurable (RUMAHL_APPS_PUBLIC_SCHEME /
                // RUMAHL_APPS_PUBLIC_PORT): the dev VM serves the gateway on
                // http via a forwarded host port because *.apps.rumahl.local
...
mod tests {
    ...
    fn public_url_defaults_to_https_without_port() {
        std::env::remove_var("RUMAHL_APPS_PUBLIC_SCHEME");
        std::env::remove_var("RUMAHL_APPS_PUBLIC_PORT");
        assert_eq!(
            public_app_runtime_url("nextcloud", "apps.rumahl.local"),
            "https://nextcloud.apps.rumahl.local/"
        );
    }

    #[test]
    fn public_url_honours_dev_scheme_and_port() {
        std::env::set_var("RUMAHL_APPS_PUBLIC_SCHEME", "http");
        std::env::set_var("RUMAHL_APPS_PUBLIC_PORT", "8126");
        assert_eq!(
            public_app_runtime_url("nextcloud", "apps.rumahl.local"),
            "http://nextcloud.apps.rumahl.local:8126/"
        );
    }
    ...
    fn public_url_omits_default_ports() {
        std::env::set_var("RUMAHL_APPS_PUBLIC_SCHEME", "http");
        std::env::set_var("RUMAHL_APPS_PUBLIC_PORT", "80");
        assert_eq!(
            public_app_runtime_url("files", ".apps.rumahl.local"),
            "http://files.apps.rumahl.local/"
        );
        std::env::remove_var("RUMAHL_APPS_PUBLIC_PORT");
        std::env::remove_var("RUMAHL_APPS_PUBLIC_SCHEME");
    }
    ...
    fn subdomain_parsing() {
        let suffix = ".apps.rumahl.local";
        assert_eq!(
            apps_subdomain_app_id("nextcloud.apps.rumahl.local", suffix).as_deref(),
            Some("nextcloud")
        );
        assert_eq!(
            apps_subdomain_app_id("nextcloud.apps.rumahl.local:3001", suffix).as_deref(),
            Some("nextcloud")
        );
        assert_eq!(
            apps_subdomain_app_id("files.apps.rumahl.local", suffix).as_deref(),
            Some("files")
        );
        // Bare base domain / wrong suffix / multi-label prefixes are rejected.
        assert_eq!(apps_subdomain_app_id("rumahl.local", suffix), None);
        assert_eq!(apps_subdomain_app_id("apps.rumahl.local", suffix), None);
        assert_eq!(apps_subdomain_app_id("example.com", suffix), None);
        assert_eq!(
            apps_subdomain_app_id("sub.nextcloud.apps.rumahl.local", suffix),
            None
        );
        assert_eq!(
            apps_subdomain_app_id("nextcloud.apps.rumahl.local.evil.com", suffix),
            None
        );
        // Custom suffix.
        assert_eq!(
    ...
    fn desktop_origin_derivation() {
        assert_eq!(
            desktop_origin_for_host("nextcloud.apps.rumahl.local", "https"),
            "https://rumahl.local"
        );
        assert_eq!(
            desktop_origin_for_host("nextcloud.apps.rumahl.local:3001", "http"),
            "http://rumahl.local:3001"
        );
        assert_eq!(
            desktop_origin_for_host("files.apps.rumahl.local:8443", "https"),
            "https://rumahl.local:8443"
        );
    }
    ...
    fn csp_frame_ancestors_none_replaced() {
        ...
        assert!(csp.contains("frame-ancestors https://rumahl.local"));
    ...
    fn csp_frame_ancestors_added_when_missing() {
        ...
        let rewritten = sanitize_csp(policy, "https://rumahl.local");
        ...
        assert!(rewritten.contains("frame-ancestors https://rumahl.local"));
    ...
    fn location_rewritten_to_public_origin() {
        ...
        assert_eq!(
            ...
            "https://nextcloud.apps.rumahl.local/login"
        );
        assert_eq!(
            rewrite_location(
            ...
            "https://nextcloud.apps.rumahl.local/apps/files?x=1#top"
        );
        ...
        assert_eq!(
            ...
            "https://nextcloud.apps.rumahl.local/index.php"
        );
        ...
        assert_eq!(
            rewrite_location("/login", &t, "https://nextcloud.apps.rumahl.local"),
            "/login"
        );
        assert_eq!(
            rewrite_location(
...
mod integration_tests {
    ...
    fn gateway_req(method: &str, path: &str) -> axum::extract::Request {
        axum::extract::Request::builder()
            .method(method)
            .uri(path)
            .header(header::HOST, "nextcloud.apps.rumahl.local")
            .body(Body::empty())
            .unwrap()
    }
    ...
    async fn rewrites_internal_redirect_to_public_origin() {
        ...
        let location = resp
            ...
            .unwrap();
        assert!(
            location.starts_with("https://nextcloud.apps.rumahl.local/"),
            "got {location}"
        );
...
mod gateway_more_tests {
    ...
    async fn state_page_for_stopped_app_has_start_action() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        // The state page may only be framed by the rumahl desktop origin.
        let csp = resp
            .headers()
            .get(header::CONTENT_SECURITY_POLICY)
            .unwrap()
    ...
    async fn state_page_restricts_framing_to_desktop_origin() {
        ...
        let csp = resp
            ...
            .to_str()
            .unwrap();
        assert!(csp.contains("frame-ancestors http://rumahl.local:3001"));
