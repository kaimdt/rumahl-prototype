...
impl AppTerminalManager {
    async fn resolve_target(
        ...
        for prefix in ["rumahl-app-", "rumahl-bundle-"] {
...
pub struct AppState {
    ...
    /// Generic settings registry – schema for all user-configurable rumahl values.
    /// See [`rumahl_shared::settings`].
    pub settings_registry: Arc<SettingsRegistry>,
    /// Filesystem-backed app store used when rumahl-appstore isn't deployed
    ...
    /// Snapshot of `/etc/ora/os-dev-mode` markers — drives the
...
#[openapi(
    info(
        title = "rumahl API",
        version = "2.0.0",
        description = "Backend API for rumahl – Interface for Optimized Residential Autonomy – a smart home ecosystem with glass-morphism UI, multi-user support, and device terminal modes.",
        license(name = "MIT")
    ),
    paths(
        api_doc_health,
    ...
    tags(
        ...
        (name = "streaming", description = "Streaming server – create and manage live video streams for rumahl dashboard"),
    ),
...
async fn main() -> anyhow::Result<()> {
    // Initialize tracing with our custom RumahlLogLayer for log capture
    ...
    // Detect environment
    let rumahl_env = rumahl_shared_config::env::RumahlEnv::detect();
    info!("rumahl environment: {}", rumahl_env);

    // Check setup completion status for diagnostics
    let setup_complete = rumahl_shared_config::env::RumahlEnv::is_setup_complete();
    info!("First-boot setup completed: {}", setup_complete);
    if rumahl_env.is_production() && !setup_complete {
        warn!(
            "First-boot setup has NOT been completed. The setup wizard should be \
             running on port 8080. rumahl-home is starting regardless to be ready \
             when setup finishes."
        );
    }
    ...
    // through PostgreSQL's startup window on a freshly-booted rumahl OS.
    // Without this, `rumahl-home` exits 1 immediately if pg isn't ready,
    ...
    //
    // On a fresh rumahl OS install no Home Assistant is configured yet — the
    ...
    if !ha_configured {
        info!(
            "Home Assistant integration is OFF \u{2014} configure HA from the rumahl \
        ...
        .unwrap_or(true)
    {
        ...
        if let Err(e) = config_repo.save_system_preference(save_req).await {
            ...
            rumahl_shared_config::system_config::persist_jwt_secret(&secret);
        ...
        // quotes: otherwise rumahl-home would sign with the quoted value while
    ...
    // The setup wizard (board/rumahl/rumahl-setup/setup-server.py) writes the
    // chosen rumahl Home web-admin credentials to either an env-file consumed
    // by systemd OR a JSON file at /mnt/data/ora/rumahl-home-bootstrap.json.
    ...
    let tor_manager = Arc::new(tor_manager::TorManager::new(std::path::PathBuf::from(
        "/opt/rumahl/data/rumahl-home/tor",
    )));
    ...
    let local_appstore = match local_appstore::LocalAppStore::open().await {
        ...
        Err(e) => {
            ...
            // starts.
            std::env::set_var(
                "RUMAHL_LOCAL_APPS_DIR",
                std::env::temp_dir().join("rumahl-local-apps"),
            );
    ...
    // ── plugin_sandbox init ─────────────────────────────────────────
    let plugin_sandbox_base = std::env::var("RUMAHL_LOCAL_APPS_DIR")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| std::path::PathBuf::from("/var/lib/ora/rumahl-home/local-apps"));
    let plugin_sandbox = match plugin_sandbox::PluginSandbox::new(&plugin_sandbox_base).await {
        ...
        Err(e) => {
            ...
            plugin_sandbox::PluginSandbox::new(&std::env::temp_dir().join("rumahl-plugin-sandbox"))
                .await
    ...
    let state = AppState {
        ...
        settings_registry: Arc::new(rumahl_shared::settings::default_registry()),
        local_appstore,
        ...
        // Theme system
        theme_manager: {
            let data_dir = std::env::var("RUMAHL_THEMES_DIR")
                .map(std::path::PathBuf::from)
                .unwrap_or_else(|_| {
                    if cfg!(target_os = "linux") {
                        std::path::PathBuf::from("/var/lib/ora/rumahl-home")
                    } else {
                        std::env::current_dir()
    ...
    //    without waiting for a toggle.
    {
        ...
        tokio::spawn(async move {
            ...
            if dev_image.is_os_dev && !enabled {
                ...
                for unit in ["rumahl-developer-app.service", "rumahl-dev-bridge.service"] {
            ...
            // Legacy cleanup: the old "rumahl Browser" (webbrowser,
            // jlesage/firefox container) was removed from the store catalog
            // and replaced by the global rumahl-browserd based "rumahl Browser"
            // (rumahl-browser). VMs provisioned before that change can still
            // hold the legacy app entry + its compose containers, which
            // makes the browser appear as installed multiple times (once as
            // webbrowser, once as rumahl-browser, plus stale containers).
            // Remove the legacy entry once on every boot (idempotent).
            {
                ...
                if legacy_installed {
                    if let Err(e) = store.uninstall_force("webbrowser").await {
                        ...
                        info!(
                            "bootstrap: removed legacy 'webbrowser' app (replaced by rumahl-browser)"
                        );
                    }
                }
                for prefix in ["rumahl-app-", "rumahl-bundle-"] {
    ...
    // Sorgt dafür, dass:
    //  (a) der persistierte Status nach einem Crash von rumahl-home mit der
    ...
    // Ein einziger gemeinsamer Container (`rumahl-plugin-sandbox`) hostet alle
    ...
    let admin_routes = Router::new()
        ...
        // rumahl Control Center: services, tasks, control mode
        ...
        // Generic passthrough proxy to rumahl-control:8091 for SSH and OS-level
        // management endpoints (e.g. /api/admin/rumahl-control/ssh/status,
        // /api/admin/rumahl-control/os/disks, /api/admin/rumahl-control/os/reboot).
        // Uses a different prefix than /api/admin/control/ to avoid clashing
        // with the dedicated handlers above.
        .route(
            "/api/admin/rumahl-control/*path",
            get(admin_rumahl_control_proxy)
                .post(admin_rumahl_control_proxy)
                .put(admin_rumahl_control_proxy)
                .delete(admin_rumahl_control_proxy)
                .patch(admin_rumahl_control_proxy),
        )
        // rumahl Log System & Metrics
        ...
        // the trusted proxy marker is attached for rumahl-security.
        .route(
    ...
    let auth_routes = Router::new()
        ...
        .route("/api/os/control/*path", any(user_rumahl_control_proxy))
    ...
    let data_routes = Router::<AppState>::new()
        ...
        // Generic settings API (schema-driven – see rumahl_shared::settings).
        ...
        // rumahl microservices (rumahl-supervisor, rumahl-core). When those
        ...
        // App-Store: served locally by rumahl-home so ZIP installs and the
        // installed-apps list work even when the dedicated `rumahl-appstore`
        ...
        // Restart a known rumahl-* service from the Control Center.
        .route(
        ...
        // ── Proxies to external rumahl microservices ────────────────────────
        ...
        // rumahl-assist (Port 8092). Browser clients must use this proxy or
        ...
        // rumahl-domain-validator (Port 8104; falls back to env override)
        .route(
        ...
        // rumahl-resource-manager (Port 8105)
        ...
        // rumahl-network-monitor (Port 8103)
        ...
        // rumahl-cloud (Port 8120, optional external)
        .route(
            "/api/admin/rumahl-cloud/config",
            get(proxy_rumahl_cloud)
                .post(proxy_rumahl_cloud)
                .put(proxy_rumahl_cloud),
        )
    ...
    let app = Router::new()
        ...
        // page or a "rumahl Home not configured" placeholder. No auth needed
        ...
        // Internal: service-to-service system-notification ingest (e.g. rumahl-watchdog
        // escalating a failed auto-recovery). Auth via shared `RUMAHL_INTERNAL_TOKEN`
        // header `X-rumahl-Internal-Token`. Not exposed in OpenAPI.
        .route(
        ...
        // Path resolved at startup from RUMAHL_HOME_DIST / ../dist / ./dist / /opt/rumahl/rumahl-home/dist.
        .nest_service(
            "/icons",
        ...
        // matches `<app-id><suffix>` (e.g. nextcloud.apps.rumahl.local) are
        // routed to the app gateway before any rumahl route matching, so app
    ...
    // Start server.  Port is configurable via the PORT or RUMAHL_HOME_PORT env
    // var (set in /etc/ora/rumahl-home.env on rumahl OS to 8126).  Defaults to
    // 3001 to preserve the legacy dev behaviour when run from `cargo run`.
    let port: u16 = system_config::service_port("rumahl-home", 3001);
    ...
    // immediately visible in `journalctl -u rumahl-home` instead of users
    ...
    match resolve_dist_dir() {
        ...
        None => info!("Dashboard frontend dist NOT FOUND — serving embedded fallback page. Set RUMAHL_HOME_DIST=/path/to/dist to enable the React UI."),
    }
...
async fn detect_setup_wizard() -> (Option<String>, bool) {
    if !rumahl_shared_config::env::RumahlEnv::detect().is_production()
...
async fn health_check(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let rumahl_env = rumahl_shared_config::env::RumahlEnv::detect();
    let raw_setup_complete = rumahl_shared_config::env::RumahlEnv::is_setup_complete();
    let setup_required = rumahl_env.is_production() && !raw_setup_complete;
    ...
    Json(serde_json::json!({
        ...
        "rumahl_env": rumahl_env.to_string(),
...
///   1. `/mnt/data/ora/rumahl-home-bootstrap.json` — written by the rumahl OS
///      first-boot setup wizard (board/rumahl/rumahl-setup/setup-server.py).
...
///   2. `RUMAHL_BOOTSTRAP_ADMIN_USER` + `RUMAHL_BOOTSTRAP_ADMIN_PASSWORD` env
...
/// JSON is owner-only at /mnt/data/ora and is consumed and deleted on
/// the very next rumahl-home start, so the only window in which it can
...
async fn bootstrap_admin_user(
    ...
    let is_os_dev = std::path::Path::new("/etc/ora/os-dev-mode").exists();
    ...
    // The systemd unit for rumahl-home sets ProtectSystem=strict and only
    // whitelists /var/lib/ora/rumahl-home + /var/log/ora as writable.
    // /mnt/data/ora is therefore READ-ONLY for the rumahl-home process —
    // we cannot put bootstrap state there. The wizard runs as root and
    // hands us credentials via:
    //   * /var/lib/ora/rumahl-home/bootstrap.json   (chowned ora:ora 0640)
    //   * RUMAHL_BOOTSTRAP_ADMIN_{USER,PASSWORD,DISPLAY_NAME} env vars,
    //     loaded from /etc/ora/rumahl-home.env by systemd before the
    //     privilege drop
    // Plus a legacy fallback at /mnt/data/ora/rumahl-home-bootstrap.json
    // for old installs (read-only attempt).
    let primary_json = std::path::PathBuf::from("/var/lib/ora/rumahl-home/bootstrap.json");
    let legacy_json = std::path::PathBuf::from("/mnt/data/ora/rumahl-home-bootstrap.json");
        ...
        if json_present && creds.is_none() {
            match tokio::fs::read_to_string(json_path).await {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
                    warn!("Bootstrap JSON at {} exists but is not readable by this process (EACCES). Wizard should chmod it ora:ora 0640 (or 0644).", json_path.display());
                }
    ...
    info!(
        "Bootstrap: env RUMAHL_BOOTSTRAP_ADMIN_USER {} ({}), RUMAHL_BOOTSTRAP_ADMIN_PASSWORD {} ({} bytes)",
    ...
    if creds.is_none() {
        if let (Some(u), Some(p)) = (env_user, env_pass) {
            ...
            if !u.is_empty() && p.len() >= min_password_len {
                let d = std::env::var("RUMAHL_BOOTSTRAP_ADMIN_DISPLAY_NAME").ok();
    ...
    if creds.is_none() {
        if let (Ok(u), Ok(p)) = (
            std::env::var("RUMAHL_BOOTSTRAP_ADMIN_USER"),
            std::env::var("RUMAHL_BOOTSTRAP_ADMIN_PASSWORD"),
        ) {
            let u = u.trim().to_string();
            if !u.is_empty() && p.len() >= min_password_len {
                let d = std::env::var("RUMAHL_BOOTSTRAP_ADMIN_DISPLAY_NAME").ok();
            ...
            .await
            {
                ...
                Ok(rows) => {
                    ...
                    if orphans.is_empty() {
                        ...
                        warn!(
                            "Bootstrap: NO credentials supplied (env vars unset, JSON missing/unreadable) BUT user(s) {:?} have NULL password_hash. They cannot log in. Re-run the rumahl setup wizard to seed a password, or set RUMAHL_BOOTSTRAP_ADMIN_USER + RUMAHL_BOOTSTRAP_ADMIN_PASSWORD in /etc/ora/rumahl-home.env and restart rumahl-home.",
                            orphans
                        );
                    }
                }
    ...
    // Sentinel: the env vars stay in /etc/ora/rumahl-home.env across
    ...
    //
    // Stored under StateDirectory (writable for the rumahl-home service).
    let sentinel_path = std::path::PathBuf::from("/var/lib/ora/rumahl-home/.bootstrap-applied");
...
///   1. `RUMAHL_HOME_DIST` env var (absolute path, set by /etc/ora/rumahl-home.env on rumahl OS)
///   2. `../dist`              (legacy: cargo run from backend/rumahl-home/)
...
fn resolve_icons_dir() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("RUMAHL_ICONS_DIR") {
    ...
    for rel in [
        ...
        "./frontend/public/icons",
        "/home/ora/ora/frontend/public/icons",
        "/opt/rumahl/rumahl-home/icons",
    ] {
...
fn resolve_dist_dir() -> Option<std::path::PathBuf> {
    if let Ok(p) = std::env::var("RUMAHL_HOME_DIST") {
        let pb = std::path::PathBuf::from(p);
        if pb.join("index.html").is_file() {
            return Some(pb);
        }
    }
    for rel in ["../dist", "./dist", "/opt/rumahl/rumahl-home/dist"] {
        let pb = std::path::PathBuf::from(rel);
        if pb.join("index.html").is_file() {
            return Some(pb);
        }
...
///
/// When RUMAHL_FRONTEND_DEV_URL is set, requests are proxied to the Vite dev server
    ...
    if path.starts_with("/api/") || path.starts_with("/ws/") {
        return (
            ...
            serde_json::json!({
                ...
                "path": path,
                "available": false,
                "hint": "Dieser Endpunkt wird von einem anderen rumahl-Microservice bereitgestellt, der auf diesem System nicht läuft."
            }).to_string(),
        ).into_response();
    }

    // Frontend Dev Proxy Mode: When RUMAHL_FRONTEND_DEV_URL is set, proxy to Vite dev server
...
> = std::sync::LazyLock::new(|| {
    ...
    m.insert(
        ...
        Value::String("rumahl befindet sich im Wartungsmodus.".into()),
    );
...
/// Used by the
/// frontend so the Overview page can be replaced with a "rumahl Home not
...
// and the Admin Control Center (everything not `Hidden`). New rumahl settings
// are added once in `rumahl_shared::settings::default_settings()` and surface
...
//
// In a stripped-down build (or before rumahl-supervisor / rumahl-core have
...
/// Proxy to rumahl-intelligence for health overview.
...
async fn proxy_intelligence_overview(State(state): State<AppState>) -> Json<Value> {
    let intel_url = std::env::var("INTELLIGENCE_URL")
        .ok()
        .filter(|v| !v.is_empty())
        .unwrap_or_else(|| system_config::service_url("rumahl-intelligence", 8099));
...
/// Proxy to rumahl-intelligence for maintenance tasks.
...
async fn proxy_secrets(
    ...
    let base = microservice_url("RUMAHL_SECRETS_URL", "rumahl-secrets", 8093);
...
async fn proxy_files(
    ...
    let base = microservice_url("RUMAHL_FILES_URL", "rumahl-files", 8100);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_files_share(
    ...
    // /api/share/:token in rumahl-home maps to /api/files/shared/:token in rumahl-files
    use axum::body::Body;
    let base = microservice_url("RUMAHL_FILES_URL", "rumahl-files", 8100);
    ...
    // rumahl-files performs its own authorization and inspects no trusted
...
async fn proxy_gateway(
    ...
    let base = microservice_url("RUMAHL_GATEWAY_URL", "rumahl-gateway", 8096);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_assist(
    ...
    let base = microservice_url("RUMAHL_ASSIST_URL", "rumahl-assist", 8092);
    forward_request_to(&state, &base, req).await
}
    ...
    if let Some(object) = payload.as_object_mut() {
        object.insert(
            ...
            json!({
                ...
                "via": "rumahl-home-app-assist-bridge",
            }),
        );
    }
...
async fn proxy_watchdog(
    ...
    let base = microservice_url("RUMAHL_WATCHDOG_URL", "rumahl-watchdog", 8094);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_connector(
    ...
    let base = microservice_url("RUMAHL_CONNECTOR_URL", "rumahl-connector", 8102);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_network_monitor(
    ...
    let base = microservice_url("RUMAHL_NETWORK_MONITOR_URL", "rumahl-network-monitor", 8103);
    forward_request_to(&state, &base, req).await
}

async fn proxy_rumahl_cloud(
    State(state): State<AppState>,
    req: axum::extract::Request,
) -> axum::response::Response {
    let base = microservice_url("RUMAHL_CLOUD_URL", "rumahl-cloud", 8120);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_supervisor(
    ...
    let base = microservice_url("RUMAHL_SUPERVISOR_URL", "rumahl-supervisor", 8097);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_appstore(
    ...
    let base = microservice_url("RUMAHL_APPSTORE_URL", "rumahl-appstore", 8098);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_core(
    ...
    let base = microservice_url("RUMAHL_CORE_URL", "rumahl-core", 8090);
    forward_request_to(&state, &base, req).await
}
...
async fn proxy_core_security(
    ...
    use axum::body::Body;
    let base = microservice_url("RUMAHL_SECURITY_URL", "rumahl-security", 8095);
    let path = req.uri().path().to_string();
    ...
    // markers first, then attach the marker that rumahl-security trusts.
    parts.headers.remove("x-rumahl-proxy");
    parts.headers.remove("x-rumahl-permissions");
    parts.headers.insert(
        "x-rumahl-proxy",
        axum::http::HeaderValue::from_static("rumahl-home"),
    );
    parts.headers.insert(
        "x-rumahl-permissions",
...
async fn local_appstore_app_delete(
    ...
    // Best-effort: drop any capabilities this app registered with rumahl-assist.
    ...
    // every container labelled `ora.type=app`) cannot resurrect the app as
    ...
    {
        let filter = format!("label=ora.app.id={app_id}");
...
///
/// Request body: `{ "app_id": "rumahl-developer-app" }`
/// Supported app_ids:
///   - `rumahl-developer-app` — The rumahl Developer App (dev tools, bridge, debug APIs)
///
...
// ── Control Center: restart a known rumahl-* service ──────────────────────────
...
async fn admin_control_restart_service(
    ...
    let soft_restart_home = || async {
        ...
        Ok::<Json<Value>, ErrorResponse>(Json(json!({
            ...
            "service": "rumahl-home",
            "soft_restart": true,
            "message": "rumahl-home Soft-Restart ausgeführt (HA-Verbindung neu initialisiert, Prozess bleibt online).",
            "ha_configured": ha_config.is_configured(),
            "url": if ha_config.is_configured() { ha_config.url } else { String::new() },
        })))
    };

    // Soft-restart: for rumahl-home itself, just reconnect HA as a lightweight
    // alternative to a full systemctl restart (which would drop WebSocket clients).
    if name == "rumahl-home-ha" || name == "rumahl-home" {
        ...
        .await
    {
            ...
            let hint = if stderr.contains("not found") || stderr.contains("No such") {
                ...
                format!(
                    "Keine Berechtigung zum Neustart von '{unit}'. Der Dienst läuft vermutlich ohne root/systemd-Rechte. \
                     Nutze bei rumahl-home den Soft-Restart (Service 'rumahl-home')."
                )
            } else {
        ...
        Err(e) => Err(ErrorResponse::service_unavailable(format!(
            "systemctl nicht verfügbar ({e}). Auf einer Dev-Workstation ist Service-Neustart per UI nicht möglich. \
             Nutze 'rumahl-home-ha' als Service-Namen für einen Soft-Neustart der HA-Verbindung."
        ))),
    }
}
    ...
    for (a, (needs_docker, docker_status)) in candidates.into_iter().zip(probes) {
        let status = docker_status
            ...
            .map(|state| {
                ...
                if state.services.contains_key("rumahl-supervisor") {
                    a.status.as_str()
...
async fn supervisor_apps_start(
    ...
    // only) — e.g. the rumahl Browser opens a fresh tab. Then a quick port
            ...
            Some(Ok(msg)) => {
                ...
                match app_lifecycle::wait_until_running(&app_id_bg, 60).await {
                    ...
                    Err(verify_err) => {
                        ...
                        if !verify_err.contains("rumahl-supervisor") {
...
/// the rumahl Browser opens a fresh tab on start and closes all tabs on stop.
...
async fn ensure_app_stopped_for_uninstall(
    ...
    if let Some(status) = app_lifecycle::docker_compose_status(app_id).await {
        ...
        // "rumahl-supervisor" service (total 1) — that is NOT a running
        ...
        let real_services: Vec<&String> = status
            .services
            .keys()
            .filter(|name| name.as_str() != "rumahl-supervisor")
            .collect();
...
fn generate_compose_yaml(app_id: &str, bundle: &serde_json::Value) -> String {
    ...
    let net_name = network_config
        ...
        .unwrap_or(&format!("rumahl-bundle-{}", app_id))
        .to_string();
    ...
    let mut yaml = format!(
        "# rumahl App Bundle: {app_id}\n# Bundled components:\n#\n",
        app_id = app_id
    );

    yaml.push_str(&format!(
        "version: '{version}'\nname: rumahl-bundle-{app_id}\n\nservices:\n",
        version = version,
        app_id = app_id
    ));
...
fn generate_app_compose_yaml(app_id: &str, docker: &serde_json::Value) -> String {
    ...
    let image_tag = if auto_build {
        docker
            ...
            .unwrap_or_else(|| format!("rumahl-app-{}:local", app_id))
    } else {
        image.to_string()
    };
    ...
    if auto_build {
        let context = docker
            ...
            .and_then(|v| v.as_str())
            .unwrap_or(".");
        let dockerfile = explicit_dockerfile.unwrap_or(".rumahl-generated.Dockerfile");
        yaml.push_str(&format!(
            "    build:\n      context: {context}\n      dockerfile: {dockerfile}\n"
        ));
    }
...
fn generated_auto_build_dockerfile(docker: &serde_json::Value) -> String {
    ...
    format!(
        "FROM {base_image}\n{rust_build_deps}WORKDIR {working_dir}\nCOPY . .\nRUN if [ -d .rumahl-sdks/rust ]; then mkdir -p /sdks && cp -a .rumahl-sdks/rust /sdks/rust; fi\nRUN {install_cmd}\n"
    )
}
...
fn candidate_rust_sdk_paths() -> Vec<std::path::PathBuf> {
    let mut paths = vec![
        std::path::PathBuf::from("/opt/rumahl/sdks/rust"),
        std::path::PathBuf::from("/usr/share/ora/sdks/rust"),
        std::path::PathBuf::from("/var/lib/ora/sdks/rust"),
    ];
...
fn should_vendor_rust_sdk(compose_dir: &std::path::Path) -> bool {
    ...
    cargo_toml.contains("rumahl-sdk") && cargo_toml.contains("../../sdks/rust")
}
...
async fn ensure_vendored_rust_sdk(compose_dir: &std::path::Path) -> Result<(), String> {
    ...
    let Some(source) = candidate_rust_sdk_paths()
        ...
        return Err("Rust-App benoetigt rumahl-sdk via ../../sdks/rust, aber der lokale Rust-SDK wurde nicht gefunden".to_string());
    };
    let target = compose_dir.join(".rumahl-sdks/rust");
...
async fn write_compose_support_files(
    ...
    if let Some(docker) = &app.docker_config {
        ...
        if auto_build && dockerfile.is_none() {
            ...
            tokio::fs::write(
                compose_dir.join(".rumahl-generated.Dockerfile"),
...
async fn try_docker_compose_up(
    ...
    let project_name = format!("rumahl-app-{}", app_id);
    if let Some(result) =
    ...
    // Local development fallback only. On rumahl OS, rumahl-home intentionally
    // has no Docker socket permission; the privileged rumahl-supervisor path
...
async fn supervisor_compose_up(
    ...
    if status.is_success() {
        Some(Ok(format!(
            "rumahl-supervisor compose up erfolgreich: {text}"
        )))
    } else {
        Some(Err(format!(
            "rumahl-supervisor compose up fehlgeschlagen ({status}): {text}"
        )))
    }
}
...
async fn supervisor_compose_prepare(
    ...
    if status.is_success() {
        Some(Ok(format!(
            "rumahl-supervisor compose {} erfolgreich: {text}",
            prepare_mode
        )))
    } else {
        Some(Err(format!(
            "rumahl-supervisor compose {} fehlgeschlagen ({status}): {text}",
            prepare_mode
        )))
    }
}
...
/// rumahl-assist so the agent can discover and call them. Best-effort: failures
    ...
    tokio::spawn(async move {
        ...
        match state.http_client.post(&url).json(&body).send().await {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Pushed app capabilities for '{}' to rumahl-assist", app.id);
            }
            Ok(resp) => {
                tracing::warn!(
                    "rumahl-assist rejected capabilities for '{}': HTTP {}",
                    app.id,
                    resp.status()
                );
            }
            Err(err) => {
                tracing::warn!(
                    "Failed to push app capabilities for '{}' to rumahl-assist: {}",
                    app.id,
                    err
                );
            }
...
/// Remove an app's capabilities from rumahl-assist on uninstall (best-effort).
fn spawn_remove_app_capabilities(state: AppState, app_id: String) {
    tokio::spawn(async move {
        ...
        if let Err(err) = state.http_client.delete(&url).send().await {
            tracing::warn!(
                "Failed to remove app capabilities for '{}' from rumahl-assist: {}",
                app_id,
                err
            );
        }
        ...
        for _ in 0..300 {
            ...
            match job.status {
                local_appstore::InstallStatus::Succeeded => {
                    ...
                    // rumahl-assist so the agent can discover and call them. This
...
async fn supervisor_compose_down(
    ...
    if status.is_success() {
        Some(Ok(format!(
            "rumahl-supervisor compose down erfolgreich: {text}"
        )))
    } else {
        Some(Err(format!(
            "rumahl-supervisor compose down fehlgeschlagen ({status}): {text}"
        )))
    }
}

/// Try running docker-compose down for an app.
/// Probiert BEIDE Project-Prefixes (`rumahl-app-`, `rumahl-bundle-`) und sammelt
...
/// This allows the iframe widget to load app content through the rumahl backend.
...
/// Transparent WebSocket tunnel from the rumahl desktop client to the app
...
/// bidirectionally (the rumahl Browser UI streams its screencast/WebRTC
/// signaling over /ws).
...
async fn app_proxy_handler(
    ...
    match app {
        ...
        Some(app) => {
            ...
            // ports (local non-Docker apps like the rumahl Browser).
            let proxy_url = app
                .custom_pages
                .first()
                .map(|p| p.url.clone())
            ...
            match proxy_url {
                Some(base_url) => {
                    ...
                    // service (the rumahl Browser UI streams frames + WebRTC
...
///      under the install dir (`/var/lib/ora/local-apps/<id>/<icon>`).
...
const input = JSON.parse(process.env.RUMAHL_PLUGIN_INPUT || '{{}}');
...
"#,
                ...
                .env("RUMAHL_PLUGIN_INPUT", &input_str)
...
# Read input from env
input_data = json.loads(os.environ.get('RUMAHL_PLUGIN_INPUT', '{{}}'))
...
async fn core_sandbox_status(State(state): State<AppState>) -> Json<Value> {
    ...
    Json(json!({
        ...
        "container": "rumahl-plugin-sandbox",
...
async fn plugins_register(
    ...
    let tmp = std::env::temp_dir().join(format!("rumahl-plugin-{plugin_id}.js"));
...
fn read_current_rumahl_version() -> String {
    // Tries the standard version file first, then env var fallback,
    // then a build-time constant.
    if let Ok(v) = std::fs::read_to_string("/etc/rumahl-version") {
        let trimmed = v.trim();
        if !trimmed.is_empty() {
            return trimmed.to_string();
        }
    }
    if let Ok(v) = std::env::var("RUMAHL_VERSION") {
        if !v.is_empty() {
            return v;
        }
    }
...
async fn core_updates_check(
    ...
    let current = read_current_rumahl_version();
    let server = std::env::var("RUMAHL_UPDATE_SERVER")
        .unwrap_or_else(|_| "https://update.rumahl.com".to_string());
    let channel = std::env::var("RUMAHL_UPDATE_CHANNEL").unwrap_or_else(|_| "stable".to_string());

    let mut updates: Vec<Value> = Vec::new();
    let now = chrono::Utc::now().to_rfc3339();

    // Best-effort check against the rumahl update server. If unreachable we
...
async fn core_updates_install(
    ...
    require_user_os_permission(&state, &identity, "os.updates").await?;
    let id = uuid::Uuid::new_v4().to_string();
    let from_version = read_current_rumahl_version();

    // Spawn the rumahl-updater binary (--yes for non-interactive). If it isn't
    // installed we record the failure in update_history and return 503.
    let updater_bin =
        std::env::var("RUMAHL_UPDATER_BIN").unwrap_or_else(|_| "rumahl-updater".to_string());
    ...
    let (status, error_message): (&str, Option<String>) = match spawn_result {
        ...
        Err(e) => (
            "failed",
            Some(format!("Failed to spawn rumahl-updater: {}", e)),
        ),
    };
    ...
    if status == "failed" {
        return Err(ErrorResponse::service_unavailable(
            error_message.unwrap_or_else(|| "rumahl-updater not available".into()),
        ));
    }
...
async fn admin_settings_put(
    ...
    if matches!(
        def.visibility,
        rumahl_shared::settings::SettingVisibility::ReadOnly
    ) {
        return Err(ErrorResponse::bad_request(format!(
    ...
    rumahl_shared_config::system_config::update_cached_setting(key.clone(), body.value.to_string());
    ...
    // the rumahl-developer-app systemd unit so dev-only API endpoints become
    ...
    if key == "developer.mode" {
        ...
        match tokio::process::Command::new("systemctl")
            .arg(action)
            .arg("rumahl-developer-app.service")
            .output()
            .await
        {
            Ok(out) if out.status.success() => {
                info!("developer.mode toggle: systemctl {action} rumahl-developer-app.service OK");
            }
            Ok(out) => {
                warn!(
                    "developer.mode toggle: systemctl {action} rumahl-developer-app.service failed: {}",
                    String::from_utf8_lossy(&out.stderr).trim()
                );
            }
            Err(e) => {
                warn!("developer.mode toggle: could not exec systemctl ({e}) – rumahl-developer-app may need manual start.");
            }
        }
    }
...
async fn auth_login(
    ...
    let password_hash = match &user.password_hash {
        ...
        None => {
            ...
            // the next rumahl-home start by `bootstrap_admin_user()` when
...
async fn upload_background_image(
        ...
        .map_err(|e| ErrorResponse::bad_request(format!("Invalid multipart data: {}", e)))?
    {
        ...
        rumahl_shared_upload::atomic_write_async(&disk_path_buf, &bytes)
            .await
...
/// rumahl microservices (e.g. rumahl-developer-app) to delegate auth to rumahl-home.
...
struct RumahlLogLayer;

impl<S: tracing::Subscriber> tracing_subscriber::Layer<S> for RumahlLogLayer {
...
// ─── rumahl Metrics System ────────────────────────────────────────────

/// Global metrics counters
pub(crate) struct RumahlMetrics {
...
pub(crate) static METRICS: std::sync::LazyLock<RumahlMetrics> =
    std::sync::LazyLock::new(|| RumahlMetrics {
...
async fn admin_database_info(State(state): State<AppState>) -> Result<Json<Value>, ErrorResponse> {
    ...
    let db_name: String = sqlx::query_scalar("SELECT current_database()")
        .fetch_one(&state.db_pool)
        .await
        .unwrap_or_else(|_| "ora".to_string());
...
/// {
///   "name": "My Pixel 8",
///   "channel_type": "ha_mobile",   // "ora" | "ha_mobile" | "desktop"
...
async fn notification_channel_create(
    ...
    let channel_type = match body.get("channel_type").and_then(|v| v.as_str()) {
        Some(t) if matches!(t, "ora" | "ha_mobile" | "desktop") => t.to_string(),
        _ => return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "channel_type must be one of: ora, ha_mobile, desktop"}))).into_response(),
    };
    let target_id = body
        .get("target_id")
        .and_then(|v| v.as_str())
...
// ── rumahl Control Center handlers ─────────────────────────────────────────────

/// Fetch service health overview from all rumahl subsystems
async fn admin_control_services(State(state): State<AppState>) -> Json<Value> {
    ...
    let services = vec![
        (
            "rumahl-home",
            system_config::service_url("rumahl-home", 3001),
            "Dashboard Backend, API, Auth, Streaming",
        ),
        (
            "rumahl-core",
            system_config::service_url("rumahl-core", 8090),
            "Service Registry, Tasks, Plugins",
        ),
        (
            "rumahl-control",
            system_config::service_url("rumahl-control", 8091),
            "Dashboard Aggregation, System Monitor",
        ),
        (
            "rumahl-assist",
            system_config::service_url("rumahl-assist", 8092),
            "AI Chat, Automation Suggestions",
        ),
        (
            "rumahl-secrets",
            system_config::service_url("rumahl-secrets", 8093),
            "Secret & Credential Management",
        ),
        (
            "rumahl-watchdog",
            system_config::service_url("rumahl-watchdog", 8094),
            "Service Monitoring & Alerting",
        ),
        (
            "rumahl-security",
            system_config::service_url("rumahl-security", 8095),
            "Security Monitoring, Audit Logging",
        ),
        (
            "rumahl-gateway",
            system_config::service_url("rumahl-gateway", 8096),
            "API Gateway, External Integrations",
        ),
        (
            "rumahl-supervisor",
            system_config::service_url("rumahl-supervisor", 8097),
            "App-Container, Docker und Laufzeitverwaltung",
        ),
        (
            "rumahl-appstore",
            system_config::service_url("rumahl-appstore", 8098),
            "App Store, Pakete und Signaturen",
        ),
        (
            "rumahl-intelligence",
            system_config::service_url("rumahl-intelligence", 8099),
            "KI-gestützte Systemanalyse & Health Intelligence",
        ),
        (
            "rumahl-files",
            system_config::service_url("rumahl-files", 8100),
            "Dateien, Freigaben und App-Artefakte",
        ),
        (
            "rumahl-connector",
            system_config::service_url("rumahl-connector", 8102),
            "Remote-Zugriff und Cloud-Verbindung",
        ),
        (
            "rumahl-network-monitor",
            system_config::service_url("rumahl-network-monitor", 8103),
            "Netzwerk-Scan und Geräteerkennung",
        ),
        (
            "rumahl-domain-validator",
            system_config::service_url("rumahl-domain-validator", 8104),
            "Domain-Whitelist und DNS-Prüfung",
        ),
        (
            "rumahl-resource-manager",
            system_config::service_url("rumahl-resource-manager", 8105),
            "CPU-, RAM- und Speicherüberwachung",
        ),
        (
            "rumahl-updater",
            system_config::service_url("rumahl-updater", 8106),
            "System- und App-Updates",
        ),
        (
            "rumahl-backup",
            system_config::service_url("rumahl-backup", 8107),
            "Backups und Wiederherstellung",
        ),
        (
            "rumahl-nginx",
            system_config::service_url("rumahl-nginx", 8108),
    ...
    let checks = services.into_iter().map(|(name, url, description)| {
            ...
            .send().await
        {
            ...
            // as a red "Offline" alarm. rumahl-home itself is special-cased:
            // if we're running this handler, rumahl-home is obviously up,
            // so treat its own failure as a real outage.
            Err(_) if name == "rumahl-home" => ("offline", 0u64, json!({ "note": "self check failed" })),
...
/// List background tasks from rumahl-core
...
static CONTROL_MODE_KEY: &str = "rumahl_control_mode";
...
/// Generic proxy: forwards /api/admin/rumahl-control/<path> to
/// rumahl-control's internal service URL. The endpoint is
...
async fn user_rumahl_control_proxy(
    State(state): State<AppState>,
    Extension(identity): Extension<middleware::AuthIdentity>,
    ...
    match user_has_os_permission(&state, identity.user_id(), required_permission).await {
        Ok(true) => {
            admin_rumahl_control_proxy(State(state), method, Path(path), headers, raw_query, body)
                .await
        }
...
async fn user_backup_proxy(
    ...
    let base = microservice_url("RUMAHL_BACKUP_URL", "rumahl-backup", 8107);
...
async fn admin_rumahl_control_proxy(
    State(state): State<AppState>,
    method: axum::http::Method,
    Path(path): Path<String>,
    headers: HeaderMap,
    ...
    let qs = raw_query
        ...
        .map(|q| format!("?{}", q))
        .unwrap_or_default();
    let base = std::env::var("RUMAHL_CONTROL_URL")
        .ok()
        .filter(|v| !v.is_empty())
        .unwrap_or_else(|| system_config::service_url("rumahl-control", 8091));
    ...
    for (k, v) in headers.iter() {
        ...
        // upstream rumahl-control is reached on the internal rumahl service network
    ...
    match req.timeout(std::time::Duration::from_secs(15)).send().await {
        ...
        Err(e) => (
            ...
            Json(json!({
                "error": format!("rumahl-control upstream error: {}", e),
                "url": url,
            })),
        )
            .into_response(),
...
async fn deliver_webhook_payload(
    ...
    let mut req = client
        .post(url)
        .header("Content-Type", "application/json")
        .header("User-Agent", "rumahl-HOME-Dashboard-Webhook/2.0");
...
/// Internal: ingest a system notification from another rumahl microservice
/// (e.g. `rumahl-watchdog` after auto-recovery + self-fix have both failed).
///
/// Authentication: shared secret in the `X-rumahl-Internal-Token` header,
/// compared against the `RUMAHL_INTERNAL_TOKEN` environment variable.
/// If `RUMAHL_INTERNAL_TOKEN` is unset the endpoint is disabled.
#[derive(Debug, Deserialize)]
struct InternalSystemNotificationRequest {
    ...
    /// Originating subsystem, e.g. "rumahl-watchdog".
...
async fn internal_create_system_notification(
    ...
    // Token check.
    let expected = match std::env::var("RUMAHL_INTERNAL_TOKEN") {
...
async fn handle_theme_zip_install(
    ...
    if !validation.is_valid() {
        let errors: Vec<String> = validation
            .issues
            .iter()
            .filter(|i| i.severity == rumahl_shared::manifest_validator::ValidationSeverity::Error)
...
async fn handle_validate_manifest(
    ...
    let result = rumahl_shared::manifest_validator::validate_manifest(&json);
