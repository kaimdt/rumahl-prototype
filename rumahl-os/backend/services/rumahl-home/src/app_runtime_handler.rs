...
pub async fn run_job(
    ...
    let job = AppRuntimeJob {
        ...
        result: None,
        error: None,
        logs: vec!["Job accepted by rumahl app runtime".to_string()],
    };
...
pub async fn app_http_request(
    ...
    let mut request = state
        ...
        .header("User-Agent", format!("rumahl-AppRuntime/1.0 app={app_id}"));
...
async fn run_job_inner(state: AppState, app_id: String, job_id: String) {
    ...
    let result = if let Some(url) = job.target_url.clone() {
        ...
        Ok(json!({
            "status": "completed",
            "message": "Action recorded by rumahl app runtime",
        }))
    };
