...
//!
//! This module provides a secure intermediary between rumahl Desktop clients and Home Assistant.
...
pub async fn get_desktop_extensions() -> Result<Json<DesktopExtensionManifest>, StatusCode> {
    Ok(Json(DesktopExtensionManifest {
        custom_elements: vec![DesktopCustomElement {
            ...
            description: "A custom desktop status widget for rumahl Home pages.".to_string(),
        ...
        custom_pages: vec![DesktopCustomPage {
            ...
            description: "A desktop-specific settings page exposed to rumahl Home.".to_string(),
        }],
    }))
}
...
pub async fn receive_metrics(
    ...
    let sensors = vec![
        (
            format!("sensor.rumahl_desktop_{}_cpu", device_name),
            metrics.cpu_usage,
            "%",
            "CPU Usage",
        ),
        (
            format!("sensor.rumahl_desktop_{}_memory", device_name),
            metrics.memory_percent,
            "%",
            "Memory Usage",
        ),
        (
            format!("sensor.rumahl_desktop_{}_disk", device_name),
            metrics.disk_percent,
            "%",
            "Disk Usage",
        ),
        (
            format!("sensor.rumahl_desktop_{}_network_rx", device_name),
            metrics.network_rx_mb as f32,
            "MB",
            "Network RX",
        ),
        (
            format!("sensor.rumahl_desktop_{}_network_tx", device_name),
            metrics.network_tx_mb as f32,
            "MB",
            "Network TX",
        ),
        (
            format!("sensor.rumahl_desktop_{}_uptime", device_name),
    ...
    if let Some(battery_pct) = metrics.battery_percent {
        let entity_id = format!("sensor.rumahl_desktop_{}_battery", device_name);
        let state_data = json!({
            "state": battery_pct,
            "attributes": {
                "unit_of_measurement": "%",
