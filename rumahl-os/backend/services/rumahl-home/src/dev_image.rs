...
// where `/etc/ora/os-dev-mode` is dropped by `board/rumahl/post-build.sh`.
//
// On those images:
//   * the Developer Mode setting is forced ON and cannot be disabled
//     from the UI,
//   * `rumahl-developer-app.service` and `rumahl-dev-bridge.service` are
...
impl DevImageInfo {
    pub fn detect() -> Self {
        let is_os_dev = Path::new("/etc/ora/os-dev-mode").exists();
        let dev_token_present = Path::new("/etc/ora/dev-token").exists();
        let bridge_unit_installed = Path::new("/etc/systemd/system/rumahl-dev-bridge.service")
            .exists()
            || Path::new("/lib/systemd/system/rumahl-dev-bridge.service").exists()
            || Path::new("/usr/lib/systemd/system/rumahl-dev-bridge.service").exists();
        let developer_app_unit_installed =
            Path::new("/etc/systemd/system/rumahl-developer-app.service").exists()
                || Path::new("/lib/systemd/system/rumahl-developer-app.service").exists()
                || Path::new("/usr/lib/systemd/system/rumahl-developer-app.service").exists();
        let build_id = std::fs::read_to_string("/etc/ora/build-id")
            .ok()
