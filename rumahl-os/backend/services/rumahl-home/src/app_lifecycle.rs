...
//! - Beim Start von `rumahl-home` den persistierten Status mit der Docker-Realität abgleichen.
...
/// (`rumahl-app-`, `rumahl-bundle-`), damit Bundles und einfache Apps gleichermaßen funktionieren.
///
...
async fn supervisor_compose_status(app_id: &str) -> Option<AppDockerStatus> {
    let base = std::env::var("RUMAHL_SUPERVISOR_URL").unwrap_or_else(|_| {
        rumahl_shared_config::system_config::service_url("rumahl-supervisor", 8097)
    });
    let url = format!(
...
pub async fn wait_until_running(
    ...
    loop {
        ...
        // key "rumahl-supervisor"). That is not a real container state — keep
        // polling instead of failing and tearing the healthy container down.
        if status.any_failed() && !status.services.contains_key("rumahl-supervisor") {
            return Err(format!(
...
/// Beim Start von `rumahl-home` den persistierten Status mit Docker abgleichen.
...
///   werden korrekt als `running` geflaggt (Konsistenz nach Crash von rumahl-home).
...
struct CrashTracker {
    ...
    /// Gesamt-Crashes seit Start von rumahl-home (Telemetrie).
    total_crashes: u64,
}
