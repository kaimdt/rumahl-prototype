// rumahl-dev-bridge — rumahl OS Dev hot-reload bridge. ROOT ONLY. NO SECURITY.
//
...
//
// This binary ONLY ships on images built with `RUMAHL_OS_DEV=1`.
// On production builds: /usr/bin/rumahl-dev-bridge does NOT exist,
// the systemd unit is NOT installed, /etc/ora/os-dev-mode is ABSENT.
//
// Auth: rumahl dashboard credentials (username/password → validated
// against rumahl-home /api/auth/login, admin role required). Falls back
// to static token file at /var/lib/ora/dev-token for legacy compat.
//
// Endpoints:
...
const DEV_MODE_FILE: &str = "/etc/ora/os-dev-mode";
/// Persistent build workspace. We keep one directory per component so
/// cargo's `target/` cache survives across uploads — a `rumahl-watchdog`
...
/// switches.
const PERSISTENT_BUILD_ROOT: &str = "/var/lib/rumahl-dev/builds";
/// Older releases used `/tmp/rumahl-dev-build-*` per request; clean those
/// up on startup so we don't fill /tmp on long-running dev images.
const LEGACY_BUILD_PREFIX: &str = "rumahl-dev-build-";
/// URL of the local rumahl-core (used by /dev/services to surface live
/// heartbeats from every rumahl service in one place). Override via
/// `$RUMAHL_CORE_URL`.
const DEFAULT_CORE_URL: &str = "http://127.0.0.1:8090";
const DEV_TOKEN_FILE: &str = "/etc/ora/dev-token";
/// Writable fallback (always on the persistent overlay, even on a
/// read-only RAUC slot). Preferred when present and readable; the
/// systemd unit pre-populates it via /usr/lib/ora/rumahl-dev-bridge-prepare.sh.
const DEV_TOKEN_FILE_WRITABLE: &str = "/var/lib/ora/dev-token";
const VERSION_FILE: &str = "/etc/rumahl-version";
const COMPOSE_DIR: &str = "/mnt/data/ora";
const REMOTE_BUILD_IMAGE: &str = "rust:1.90";

/// Resolve the path of the dev-token file, honoring `$RUMAHL_DEV_TOKEN_FILE`
/// when set. Without override, prefers the writable copy under /var/lib.
fn resolve_dev_token_path() -> String {
    if let Ok(p) = std::env::var("RUMAHL_DEV_TOKEN_FILE") {
        if !p.trim().is_empty() {
            return p;
        }
    }
...
#[derive(Parser, Debug)]
#[command(name = "rumahl-dev-bridge")]
struct Cli {
    /// Address to bind (default 0.0.0.0:8101 or $RUMAHL_DEV_BIND).
    /// Note: 8099 is reserved for rumahl-api; the dev bridge uses 8101.
    #[arg(long, env = "RUMAHL_DEV_BIND")]
...
async fn main() -> Result<()> {
    ...
    #[cfg(unix)]
    {
        ...
        if uid != 0 {
            tracing::error!(
                "rumahl-dev-bridge MUST run as root (current uid={uid}). \
                 Set User=root in the systemd unit."
            );
            anyhow::bail!("rumahl-dev-bridge requires root (uid=0), got uid={uid}");
        }
    ...
    if !std::path::Path::new(DEV_MODE_FILE).exists() {
        anyhow::bail!(
            "{DEV_MODE_FILE} not found — rumahl-dev-bridge refuses to run on a non-OS-dev image"
        );
    }
    ...
    let core_url = std::env::var("RUMAHL_CORE_URL").unwrap_or_else(|_| DEFAULT_CORE_URL.to_string());
    let http = reqwest::Client::builder()
        .timeout(std::time::Duration::from_secs(5))
        .user_agent("rumahl-dev-bridge")
        .build()
    ...
    let app = Router::new()
        ...
        // Auth via rumahl dashboard credentials (username/password)
        ...
        // Global Config (system_preferences in rumahl_home)
        ...
        // Full PostgreSQL access (all rumahl databases)
    ...
    tracing::info!("rumahl-dev-bridge listening on http://{addr}");

    // mDNS advertisement so the local `rumahl-dev-deploy` CLI can find this
    // device on the LAN.  We register the service `_rumahl-dev._tcp.local.`
    // with TXT records identifying the variant, build, hostname and port.
    // The advertiser is only ever started here — `rumahl-dev-bridge` itself
...
fn start_mdns_advertiser(port: u16, build_id: &str) -> Result<mdns_sd::ServiceDaemon> {
    ...
    let host = if host.is_empty() {
        "rumahl-dev".to_string()
    } else {
        host
    };
    let instance = format!("rumahl-dev-{host}");
    ...
    tracing::info!("mDNS: registered _rumahl-dev._tcp as {instance} on port {port}");
    Ok(daemon)
}
...
pub(crate) fn check_auth(
    ...
    // 2. Bearer token (session from POST /dev/auth via rumahl-home login)
    ...
    Err((
        ...
        "missing or invalid auth. Use POST /dev/auth with ora username/password".into(),
    ))
}
...
// ─── /dev/auth — rumahl dashboard credentials → dev session token ──────
...
/// Try to reach rumahl-home on standard ports (3001 dev, 8126 production).
/// Returns the first successful response or the last error.
async fn try_rumahl_home_login(
    http: &reqwest::Client,
    username: &str,
    password: &str,
) -> Result<(String, serde_json::Value), String> {
    // Standard ports: 3001 (dev), 8126 (production rumahl OS)
    const RUMAHL_HOME_PORTS: &[u16] = &[3001, 8126];
    let mut last_err = String::new();

    for port in RUMAHL_HOME_PORTS {
    ...
    Err(format!(
        "Cannot reach rumahl-home on any port. Tried ports: {:?}. Last error: {}",
        RUMAHL_HOME_PORTS, last_err
    ))
}

async fn try_rumahl_home_verify(
    http: &reqwest::Client,
    jwt: &str,
    ...
    Err(format!(
        "Cannot verify token on any rumahl-home port. Tried: {:?}. Last error: {}",
        RUMAHL_HOME_PORTS, last_err
    ))
}
...
/// This is a reliable fallback when rumahl-home is down.
...
async fn dev_auth(
    ...
    // ── Method 1: Try rumahl-home auth (dashboard credentials) ─────────
    let rumahl_auth_ok = match try_rumahl_home_login(&state.http, &body.username, &body.password).await
    {
        Ok((jwt, _login_json)) => match try_rumahl_home_verify(&state.http, &jwt).await {
            Ok(user_info) => {
    ...
    if rumahl_auth_ok {
        auth_method = "rumahl-home";
        tracing::info!(
            "dev-auth: user '{}' authenticated via rumahl-home (admin)",
            username
        );
    }

    // ── Method 2: Fall back to OS-level auth (emergency access) ──────
    // This works even when rumahl-home is down. OS root/admin users can
    ...
    if auth_method == "none" {
        return (StatusCode::UNAUTHORIZED,
            "Authentication failed. Try your rumahl dashboard credentials or your OS (root) password."
        ).into_response();
    }
...
/// Aggregated, live service map. Talks to the local rumahl-core's
...
async fn dev_services(State(s): State<AppState>, headers: HeaderMap) -> impl IntoResponse {
    ...
    match s.http.get(&url).send().await {
        ...
        Err(e) => (
            StatusCode::BAD_GATEWAY,
            Json(serde_json::json!({
                "error": format!("cannot reach rumahl-core at {}: {e}", *s.core_url),
            })),
        )
            .into_response(),
    }
...
///
/// Auth: accepts (1) `x-rumahl-dev-token` header with static dev token,
...
async fn build_replace(
    ...
    // Native rumahl OS dev images build on-device. Docker is intentionally not
    // used unless explicitly enabled via RUMAHL_DEV_ALLOW_DOCKER_BUILD=1.
    ...
    let build = if command_exists("cargo") {
        ...
        emit_bridge_log(
            &s.event_tx,
            &log_service,
            "native cargo is not available on this rumahl OS Dev image".to_string(),
        );
        CmdResult {
            ok: false,
            code: -1,
            stdout: String::new(),
            stderr: "native cargo/rustc missing on device; rebuild the rumahl OS Dev image with RUMAHL_OS_DEV=1 so BR2_PACKAGE_RUMAHL_DEV_TOOLCHAIN is included".into(),
        }
    };
...
// ─── Allowlists ─────────────────────────────────────────────────────────────
//
// We only act on units / paths that belong to rumahl so the dev bridge can't
...
fn extract_backend_bundle_sync(
    ...
    use rumahl_shared_upload::{extract_tar_gz_into, TarExtractLimits};
...
async fn ensure_tooling_locally(
    ...
    if !command_exists("apt-get") {
        return CmdResult {
            ...
            stdout: String::new(),
            stderr: format!(
                "apt-get is missing; cannot auto-install required tooling on Buildroot rumahl OS. Missing: {}. Rebuild the Dev image with RUMAHL_OS_DEV=1 / BR2_PACKAGE_RUMAHL_DEV_TOOLCHAIN=y",
...
fn allow_docker_build_fallback() -> bool {
    std::env::var("RUMAHL_DEV_ALLOW_DOCKER_BUILD").ok().as_deref() == Some("1")
}

/// ─── Self-Update ─────────────────────────────────────────────────────────
//
// POST /dev/self-update – Ersetzt das rumahl-dev-bridge Binary und startet
...
async fn self_update(
    ...
    let self_path = "/usr/bin/rumahl-dev-bridge";
    let self_unit = "rumahl-dev-bridge.service";
    let tmp = "/tmp/rumahl-dev-bridge-update";
    let started = std::time::Instant::now();
...
async fn self_update_ssh(body: &SelfUpdateRequest, output: &str) -> CmdResult {
    ...
    let remote_path = match &body.path {
        Some(p) => p,
        None => "/usr/bin/rumahl-dev-bridge",
    };
    let port = body.port;
...
async fn install_binary_and_restart(
    ...
    let restart_result = if let Some(u) = unit {
        if is_allowed_unit(u) {
            if u == "rumahl-dev-bridge.service" {
                ...
                CmdResult {
                    ok: true,
                    code: 0,
                    stdout: "rumahl-dev-bridge restart scheduled after response".into(),
                    stderr: String::new(),
                }
            } else {
