...
#[derive(Parser)]
#[command(name = "rumahl-db-manager")]
#[command(about = "rumahl PostgreSQL Database Manager - User creation, rotation, and security")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
    ...
    #[arg(long, default_value = "/etc/ora/db-config.toml")]
    config: PathBuf,
...
enum Commands {
    ...
    Create {
        ...
        /// Database name (default: rumahl_{service})
        #[arg(long)]
    ...
    Backup {
        /// Backup directory
        #[arg(long, default_value = "/var/backups/rumahl-db")]
        dir: PathBuf,
    },
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Create {
            ...
            let db_name = database.unwrap_or_else(|| format!("rumahl_{}", service));
            manager
