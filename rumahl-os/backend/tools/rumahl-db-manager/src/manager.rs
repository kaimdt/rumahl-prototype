...
impl DatabaseManager {
    ...
    async fn ensure_tracking_tables(pool: &PgPool) -> Result<()> {
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS _rumahl_db_users (
            ...
            CREATE TABLE IF NOT EXISTS _rumahl_password_history (
                ...
                FOREIGN KEY (service) REFERENCES _rumahl_db_users(service) ON DELETE CASCADE
            );

            CREATE INDEX IF NOT EXISTS idx_password_history_service
                ON _rumahl_password_history(service);
            CREATE INDEX IF NOT EXISTS idx_password_history_valid
                ON _rumahl_password_history(service, valid_until)
    ...
    async fn track_user(
        ...
        sqlx::query(
            ...
            ON CONFLICT (service) DO UPDATE SET
                ...
                last_rotated = NOW(),
                next_rotation = $5,
                rotation_count = _rumahl_db_users.rotation_count + 1,
                conn_limit = $6
            "#,
        )
        .bind(service)
        ...
        sqlx::query(
            r#"
            INSERT INTO _rumahl_password_history (service, password_hash)
            VALUES ($1, $2)
            "#,
        )
        .bind(service)
    ...
    async fn write_connection_string(
        ...
        // Write to /etc/ora/db-credentials/{service}.env
    ...
    pub async fn drop_service(&self, service: &str) -> Result<()> {
        let user_info: Option<(String, String)> =
            sqlx::query_as("SELECT username, database_name FROM _rumahl_db_users WHERE service = $1")
                .bind(service)
        ...
        if let Some((username, db_name)) = user_info {
            ...
            sqlx::query("DELETE FROM _rumahl_db_users WHERE service = $1")
                .bind(service)
                .execute(&self.pool)
                .await?;

            // Remove credentials file
            let cred_file = Path::new("/etc/ora/db-credentials").join(format!("{}.env", service));
    ...
    pub async fn export_connection_strings(&self, format: &str) -> Result<String> {
        let users: Vec<(String, String, String)> = sqlx::query_as(
            "SELECT service, username, database_name FROM _rumahl_db_users ORDER BY service",
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn backup_all_databases(&self, backup_dir: &Path) -> Result<Vec<String>> {
        let databases: Vec<(String,)> =
            sqlx::query_as("SELECT datname FROM pg_database WHERE datname LIKE 'rumahl_%'")
                .fetch_all(&self.pool)
                .await?;
