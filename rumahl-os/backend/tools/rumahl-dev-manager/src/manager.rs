...
impl Manager {
    pub fn discover(explicit: Option<PathBuf>) -> Result<Self> {
        let start = explicit
            .or_else(|| std::env::var_os("RUMAHL_OS_ROOT").map(PathBuf::from))
            .unwrap_or(std::env::current_dir()?);
        let mut root = find_rumahl_root(&start).context(
            "no rumahl-os root found; run from the repository root/rumahl-os or set RUMAHL_OS_ROOT",
        )?;
    ...
    pub fn start(&mut self, mode: NetworkMode, mappings: &[PortMapping]) -> Result<()> {
        ...
        if mode == NetworkMode::Bridge {
            let tap = std::env::var("RUMAHL_DEV_TAP").unwrap_or_else(|_| "rumahl-tap0".into());
        ...
        let disk = cache.join("rumahl-dev-vm.qcow2");
        if !disk.exists() {
        ...
        let accel_override = std::env::var("RUMAHL_DEV_ACCEL")
            .ok()
            .map(|value| value.trim().to_string())
            .filter(|value| !value.is_empty());
        let skip_whpx = std::env::var("RUMAHL_DEV_SKIP_WHPX")
            .ok()
            .map(|value| {
                matches!(
        ...
        let memory = std::env::var("RUMAHL_DEV_RAM")
            .unwrap_or_else(|_| format!("{}G", self.config.default_ram_gb.max(4)));
        let cpus = std::env::var("RUMAHL_DEV_CPUS")
        ...
        let network = match mode {
            ...
            NetworkMode::Bridge => {
                ...
                format!(
                    ...
                    std::env::var("RUMAHL_DEV_TAP").unwrap_or_else(|_| "rumahl-tap0".into())
                )
            }
        };
        ...
        let firmware = if architecture == "aarch64" {
            let firmware = find_aarch64_firmware().context(
                "AArch64 QEMU firmware was not found; set RUMAHL_DEV_FIRMWARE to its path",
            )?;
        ...
        let seed = cache.join("rumahl-dev-seed.iso");
    ...
    fn spawn_dev_local_bootstrap(&self, rebuild: bool) -> Result<()> {
        ...
        if !script.exists() {
            anyhow::bail!(
                ...
                self.root.join(".cache/rumahl-dev-vm.qcow2").display()
            );
        }
        // Remove the Mark-of-the-Web from every script under rumahl-os so
    ...
    pub fn create_golden_snapshot(&mut self) -> Result<()> {
        ...
        let disk = self.root.join(".cache/rumahl-dev-vm.qcow2");
        let golden = self.root.join(".cache/rumahl-dev-golden.qcow2");
    ...
    pub async fn disk_info(&self) -> Value {
        let disk = self
            ...
            .vm_disk
            .clone()
            .unwrap_or_else(|| self.root.join(".cache/rumahl-dev-vm.qcow2"));
        if !disk.exists() {
...
"#;
    ...
    pub fn open_ssh(&self) -> Result<()> {
        ...
        let key = self.root.join(".cache/rumahl-dev-key");
        if key.exists() {
...
/// `RUMAHL_DEV_FORWARD`). `Manager::start` additionally merges the configured
/// extra ports from `dev-manager.json`.
pub fn extra_ports() -> Vec<u16> {
    std::env::var("RUMAHL_DEV_FORWARD")
...
/// their command lines. Used to find rumahl Dev VMs started outside this
...
fn parse_qemu_process(pid: u32, command_line: String) -> Option<QemuProcessInfo> {
    ...
    let is_rumahl_dev = command_line.contains("rumahl-dev-vm")
        || command_line.contains("-name rumahl-Dev")
        || command_line.contains("rumahl-dev");
...
pub fn environment_notes(root: &Path) -> Vec<String> {
    ...
    let qemu_ok = if cfg!(windows) {
        windows_qemu_dir("qemu-system-x86_64.exe").is_some()
            || std::env::var("RUMAHL_DEV_QEMU").is_ok_and(|p| Path::new(&p).exists())
    } else {
    ...
    if !qemu_ok {
        notes.push(format!(
            "QEMU not found ({qemu}). Install it (winget install SoftwareFreedomConservancy.QEMU) or set RUMAHL_DEV_QEMU."
        ));
    }
    ...
    let disk = root.join(".cache/rumahl-dev-vm.qcow2");
    if !disk.exists() {
        notes.push(format!(
...
/// Find the VM disk path: start at the first `rumahl-dev-vm` occurrence and
/// expand to the whole token (bounded by space/quote/comma).
fn disk_after(input: &str) -> Option<String> {
    let relative = input.find("rumahl-dev-vm")?;
...
fn forwarding_plan_with(
    ...
    // WebRTC (rumahl Browser): the browser receives the GStreamer media over
    // UDP. It connects to 127.0.0.1:40000 on the host; inside the guest
    // rumahl-browserd runs a socat hop from :40000 to the session port.
...
fn find_rumahl_root(start: &Path) -> Option<PathBuf> {
    let mut candidates = vec![start.to_path_buf(), start.join("rumahl-os")];
    let mut current = start.parent();
    for _ in 0..8 {
        if let Some(parent) = current {
            candidates.push(parent.to_path_buf());
            candidates.push(parent.join("rumahl-os"));
...
fn resolve_qemu() -> String {
    if let Ok(explicit) = std::env::var("RUMAHL_DEV_QEMU") {
        return explicit;
    }
...
fn resolve_qemu_img() -> String {
    if let Ok(explicit) = std::env::var("RUMAHL_DEV_QEMU_IMG") {
        return explicit;
    }
    let executable = if cfg!(windows) {
...
fn cpu_model(acceleration: &str) -> String {
    std::env::var("RUMAHL_DEV_CPU").unwrap_or_else(|_| {
        if acceleration == "tcg" {
            "max".into()
...
fn find_aarch64_firmware() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("RUMAHL_DEV_FIRMWARE").map(PathBuf::from) {
...
mod tests {
    ...
    fn native_start_reports_missing_disk_when_bootstrap_script_is_unavailable() {
        let root = std::env::temp_dir().join(format!("rumahl-dev-manager-{}", std::process::id()));
    ...
    fn forwarding_plan_deduplicates_overlapping_host_ports() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                // rumahl Browser WebRTC: UDP hop for the GStreamer media stream.
    ...
    #[test]
    fn qemu_cmdline_parsing_finds_rumahl_dev_vm() {
        // A real dev-local.ps1 command line (as seen in the field).
        let command_line = r#"C:\Program Files\qemu\qemu-system-x86_64.exe -name rumahl-Dev -m 16G -smp 16 -machine q35,accel=whpx -drive file=C:\tmp\home-assistant-dashb\rumahl-os\.cache\rumahl-dev-vm.qcow2,format=qcow2,if=none,id=rumahl-disk -device virtio-blk-pci,drive=rumahl-disk,bootindex=1 -drive file=C:\tmp\home-assistant-dashb\rumahl-os\.cache\rumahl-dev-seed.iso,format=raw,media=cdrom,if=none,id=rumahl-seed -device ide-cd,drive=rumahl-seed,bootindex=2 -netdev user,id=n0,hostfwd=tcp::2222-:22,hostfwd=tcp::8126-:8126,hostfwd=tcp::5173-:5173 -device virtio-net-pci,netdev=n0 -chardev socket,id=qga0,host=127.0.0.1,port=8109,server=on,wait=off -device virtserialport,chardev=qga0,id=qga0,name=org.qemu.guest_agent.0 -qmp tcp:127.0.0.1:8130,server=on,wait=off -vnc 127.0.0.1:1"#;
        let info = parse_qemu_process(4242, command_line.into()).unwrap();
        assert!(info.is_rumahl_dev);
        ...
        assert!(info.disk.as_deref().unwrap().contains("rumahl-dev-vm.qcow2"));
    }
