...
impl Daemon {
    ...
    pub fn refresh_status_cache(&self) {
        ...
        *self.status_cache.lock().unwrap() = json!({
            ...
            "access": json!({
                ...
                "ssh": format!("ssh -i {} -p {ssh} root@{host}", root.join(".cache/rumahl-dev-key").display()),
                "keyPath": root.join(".cache/rumahl-dev-key").display().to_string(),
                "webUrl": format!("http://{host}:{home}"),
                "webUser": "admin",
                "webPassword": "admin1234",
                "webPin": "0000",
                // SFTP link for the file browser: same SSH channel + key.
                // sftp://<user>@<host>:<port>/ - import .cache/rumahl-dev-key
                // into WinSCP / FileZilla / any SFTP client.
                "sftpUser": config.sftp_user.clone(),
                "sftp": format!("sftp://{}@{}:{}/", config.sftp_user, host, ssh),
                "sftpKey": root.join(".cache/rumahl-dev-key").display().to_string(),
            }),
            "config": config,
        })
    }
    ...
    pub async fn services(&self) -> Result<Vec<Value>> {
        ...
        let output = manager
            .guest(
                "systemctl list-units --type=service --all --no-legend --no-pager --plain 'rumahl-*'",
            )
            .await?;
    ...
    /// guest and rebuild/restart the affected rumahl services. Runs detached
    ...
    pub async fn adopt_foreign(&self, pid: u32) -> Result<()> {
        ...
        if !info.is_rumahl_dev {
            anyhow::bail!("PID {pid} does not look like an rumahl Dev VM (no rumahl-dev-vm disk)");
        }
        {
            let mut manager = self.manager.lock().await;
            manager.adopt(info)?;
    ...
    pub async fn reset_network(&self) -> Result<String> {
        ...
        let info = processes
            .iter()
            .find(|process| process.is_rumahl_dev)
            .context("no rumahl Dev VM found - is the VM running?")?;
    ...
    async fn refresh_stats(&self) {
        ...
        let disk_size = std::fs::metadata(manager.root.join(".cache/rumahl-dev-vm.qcow2"))
            .map(|meta| meta.len())
            .unwrap_or(0);
    ...
    async fn watchdog_tick(&self, last_lifecycle: &mut String) {
        ...
        *self.last_probe.lock().unwrap() = probe.clone();
            ...
            if note_due {
                *self.last_provision_note.lock().unwrap() = Some(Instant::now());
                ...
                let message = if let Some(percent) = download["percent"].as_u64() {
                    ...
                    "Vorbereitung: der erste Start lädt das Debian Cloud-Image (~400MB) herunter und installiert die rumahl-Runtime (5-15 min). Das Dashboard bleibt erreichbar; die rumahl-Web-UI erscheint, sobald die VM bereit ist."
                        .to_string()
                };
        ...
        // Open forwarded ports only once the guest is fully up: the rumahl
        ...
        if probe.qga {
            ...
            // Build-deadlock autofix: if the rumahl-* cargo services stall on
            ...
            if build_check_due {
                *self.last_build_check.lock().unwrap() = Some(Instant::now());
                ...
                let _ = channels::guest_exec(
                    qga_port,
                    &socket,
                    "[ -f /tmp/rumahl-dev-selfheal.sh ] && bash /tmp/rumahl-dev-selfheal.sh --build || true",
                )
                .await;
            }
        ...
        let restart = {
                ...
                && !desired.ever_ready
            {
                ...
                // source-mode first boot where rumahl-home still compiles)
    ...
    /// The rumahl guest firewall (iptables INPUT policy DROP) only allows a
    ...
    pub async fn add_mapping(&self, host: u16, guest: u16, label: Option<String>) -> Result<()> {
        ...
        if matches!(host, 2222 | 8126) {
            anyhow::bail!("host port {host} is reserved for SSH / rumahl-home");
        }
        {
    ...
    const DEFAULT_MAPPINGS: &[(u16, u16, &str)] = &[
        (3001, 3001, "rumahl-home (nginx)"),
        (5432, 5432, "postgres"),
        (8101, 8101, "dev bridge"),
        // rumahl-files web UI: gives the Dev Manager a direct HTTP window into
        // every file of rumahl OS (alternative to the SFTP link in Access).
        (8100, 8100, "rumahl-files"),
    ];
        ...
        let updated = content
            ...
            .map(|line| {
                if line.starts_with("VITE_BACKEND_URL=")
                    || line.starts_with("VITE_RUMAHL_ASSIST_URL=")
                {
...
",
        ...
        let command = format!(
            "echo {} | base64 -d > /home/ora/ora/frontend/.env.development && chown ora:ora /home/ora/ora/frontend/.env.development && systemctl restart rumahl-frontend-dev.service",
            encoded
        );
    ...
    fn ssh_args(&self, manager: &Manager) -> Vec<String> {
        let key = manager.root.join(".cache/rumahl-dev-key");
        vec![
            "-o".into(),
    ...
    /// Whether the guest installation finished (`/etc/ora/dev-vm-provisioned`
    ...
    async fn installation_marker(&self) -> Option<bool> {
        ...
        let result = channels::guest_exec(
            qga_port,
            &socket,
            "test -f /etc/ora/dev-vm-provisioned && echo YES || echo NO",
        )
        .await;
    ...
    async fn seed_progress(&self) -> Option<Value> {
        ...
        if due {
            ...
            if let Ok(output) = channels::guest_exec(
                qga_port,
                &socket,
                "du -sm /home/ora/.cargo/registry 2>/dev/null | cut -f1",
            )
            .await
            {
    ...
    async fn apply_guest_fixes(&self) {
        ...
        let script = root.join("rumahl-dev-selfheal.sh");
        let Ok(content) = std::fs::read_to_string(&script) else {
            self.emit(
                "error",
                "self-heal script missing: rumahl-os/rumahl-dev-selfheal.sh",
            );
            return;
        };
        let command = format!(
...
"#;
    ...
    async fn network_remediation(&self) {
        ...
        let command = "if [ -f /tmp/rumahl-dev-selfheal.sh ]; then bash /tmp/rumahl-dev-selfheal.sh --net; else for n in $(ls /sys/class/net | grep -vE '^(lo|docker|br-|veth|vnet|virbr|tun|tap|bond|sit)'); do [ -d /sys/class/net/$n/device ] || continue; grep -q 'state UP' /sys/class/net/$n/operstate 2>/dev/null || continue; ip -4 addr show dev $n 2>/dev/null | grep -q ' inet ' || dhclient -1 $n >/dev/null 2>&1 || true; done; fi; ip -4 route get 1.1.1.1 >/dev/null 2>&1 && echo NET_OK || echo NET_STILL_DOWN";
...
pub fn spawn(daemon: Arc<Daemon>) {
    ...
    tokio::spawn(async move {
        ...
        // neither running nor being (re)provisioned, bring it up.
        {
            ...
            tokio::spawn(async move {
                ...
                let (should_start, mode) = {
                    ...
                    // RUMAHL_DEV_NO_AUTOSTART=1 wins over the config file so
                    // headless/CI invocations can always opt out.
                    let no_autostart = std::env::var("RUMAHL_DEV_NO_AUTOSTART")
                        .ok()
                        .map(|value| {
                            matches!(
        ...
        // disk is missing). Disable with RUMAHL_DEV_NO_AUTOSTART=1.
        let autostart_enabled = std::env::var("RUMAHL_DEV_NO_AUTOSTART")
            .ok()
            .map(|value| {
                !matches!(
...
fn port_label(port: u16) -> Option<&'static str> {
    match port {
        ...
        8080 => Some("HTTP alt"),
        8126 => Some("rumahl-home API"),
        8090..=8098 => Some("rumahl service"),
        _ => None,
    }
}
...
mod tests {
    ...
    fn parses_plain_systemctl_lines() {
        let output = "rumahl-api.service loaded active running rumahl API\n\nrumahl-home.service loaded failed failed rumahl Home\n";
        let services = parse_services(output);
        assert_eq!(services.len(), 2);
        assert_eq!(services[0]["unit"], "rumahl-api.service");
        assert_eq!(services[0]["description"], "rumahl API");
