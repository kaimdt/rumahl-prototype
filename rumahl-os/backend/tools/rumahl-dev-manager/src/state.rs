...
/// Persistent Dev Manager configuration (`dev-manager.json` in the rumahl-os
...
pub struct DevManagerConfig {
    ...
    /// RAM for the VM when no RUMAHL_DEV_RAM env override is set (GB).
    pub default_ram_gb: u64,
    /// vCPU count when no RUMAHL_DEV_CPUS env override is set.
