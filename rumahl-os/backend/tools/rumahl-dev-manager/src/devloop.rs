...
fn relevant(path: &Path) -> bool {
    ...
    // Host-only development tooling under rumahl-os/tools (vscode-rumahl-dev,
    // rumahl-dev-deploy) must NOT be mirrored: the guest-side timestamp map
    // only tracks rumahl-os/backend/tools. Without this exclusion those files
    ...
    // imported module".
    {
        let comps: Vec<&str> = path
            ...
            .collect();
        if let Some(i) = comps.iter().position(|name| *name == "rumahl-os") {
...
async fn remote_timestamps(
    ...
    let command = "cd /home/ora/ora && find frontend rumahl-os/backend/services rumahl-os/backend/shared rumahl-os/backend/tools custom_components -type f \\( -name '*.ts' -o -name '*.tsx' -o -name '*.css' -o -name '*.rs' -o -name '*.sql' -o -name '*.json' -o -name '*.service' -o -name '*.html' -o -name '*.js' -o -name '*.yaml' -o -name '*.yml' -o -name 'Cargo.toml' -o -name 'vite.config.ts' -o -name 'vite.config.js' \\) -printf '%T@ %s %p\\n' 2>/dev/null";
...
async fn verify_and_sync_inner(
    ...
    let remote = match remote_timestamps(os_root, state).await {
        ...
        // yet (the /home/ora/ora mirror does not exist) — the incremental
...
async fn apply_changes(
    ...
    if frontend_restart {
        ssh_run(
            state,
            os_root,
            "systemctl restart rumahl-frontend-dev && systemctl is-active --quiet rumahl-frontend-dev",
        )
        .await?;
    }
...
fn ssh_args(state: &RuntimeState, os_root: &Path) -> Vec<String> {
    ...
    let key = os_root.join(".cache/rumahl-dev-key");
    vec![
        "-o".into(),
...
async fn bulk_sync(
    ...
    ssh.args(ssh_args(state, os_root))
        ...
        .arg("tar -xf - -C /home/ora/ora --touch && chown -R ora:ora /home/ora/ora/frontend /home/ora/ora/rumahl-os/backend /home/ora/ora/custom_components")
        .stdin(Stdio::piped());
...
async fn sync_path(repo: &Path, os_root: &Path, state: &RuntimeState, path: &Path) -> Result<()> {
    ...
    let remote = Path::new("/home/ora/ora").join(relative);
    ...
    if path.exists() && path.is_file() {
        ...
        if false && bytes.len() <= 1_048_576 {
            ...
            let temporary = format!("{}.rumahl-sync", remote.display());
...
/// skew and missed watcher events), then rebuild all affected rumahl services
...
pub async fn force_full_sync(
    ...
    // GStreamer packages rumahl-browserd needs for WebRTC (webrtcbin/vp8enc/
    // jpegdec + -dev for cargo) on guests that predate the provisioning
    // change, and applies the other one-time fixes.
    if let Ok(content) = std::fs::read_to_string(os_root.join("rumahl-dev-selfheal.sh")) {
        let command = format!(
            "echo {} | base64 -d > /tmp/rumahl-dev-selfheal.sh && chmod 755 /tmp/rumahl-dev-selfheal.sh && bash /tmp/rumahl-dev-selfheal.sh --apply",
    ...
    //    so environment/unit changes (e.g. rumahl-nginx NGINX_TEMPLATE_PATH)
    ...
    let _ = ssh_run(
        state,
        os_root,
        "systemctl restart rumahl-frontend-dev 2>/dev/null || true",
    )
    .await;
    let summary = format!(
...
async fn run_guest_build(
    ...
    let log = "/tmp/rumahl-dev-build.log";
    ...
    ssh_run(
        ...
        os_root,
        &format!(
            "rm -f {log} && nohup sudo -u ora bash -c 'cd /home/ora/ora/rumahl-os/backend && HOME=/home/ora /home/ora/.cargo/bin/cargo build {spec} --offline' >{log} 2>&1 & echo started"
        ),
    )
    .await?;
...
/// A cargo package counts as a deployable rumahl service when its manifest
...
fn native_unit(service: &str) -> String {
    // rumahl-nginx renders /etc/nginx/nginx.conf from a template. In the dev
    // VM the template must come from the synced sources (services/rumahl-nginx/
    ...
    let environment = match service {
        "rumahl-nginx" => {
            "\nEnvironment=NGINX_TEMPLATE_PATH=/home/ora/ora/rumahl-os/backend/services/rumahl-nginx/nginx-config/nginx.conf.template"
        }
        // rumahl-browserd WebRTC in the dev VM: the guest IP is unreachable
        ...
        "rumahl-browserd" => "\nEnvironment=RUMAHL_WEBRTC_CANDIDATE_HOST=127.0.0.1",
        _ => "",
    };
    format!(
        "[Unit]\nDescription=rumahl development service {service}\nAfter=network-online.target postgresql.service\nWants=network-online.target\n\n[Service]\nType=simple\nUser=ora\nWorkingDirectory=/home/ora/ora/rumahl-os/backend{environment}\nEnvironment=RUMAHL_ENV=development\nExecStart=/home/ora/ora/rumahl-os/backend/target/debug/{service}\nRestart=on-failure\nRestartSec=2\n\n[Install]\nWantedBy=multi-user.target\n"
    )
}
...
mod tests {
    ...
    fn base64_matches_qga_payload() {
        assert_eq!(encode_base64(b"rumahl"), "SU9SQQ==");
    }
    #[test]
    fn service_manifest_detection_is_separator_agnostic() {
        ...
        assert!(is_service_manifest(
            r"C:\repo\rumahl-os\backend\services\rumahl-home\Cargo.toml"
        ));
        assert!(is_service_manifest(
            "C:/repo/rumahl-os/backend/services/rumahl-home/Cargo.toml"
        ));
        assert!(is_service_manifest(
            r"C:\repo\rumahl-os\backend\apps\system\rumahl-developer-app\Cargo.toml"
        ));
        assert!(!is_service_manifest(
            "C:/repo/rumahl-os/backend/tools/rumahl-dev-manager/Cargo.toml"
        ));
        assert!(!is_service_manifest(
            "C:/repo/rumahl-os/backend/shared/rumahl-shared/Cargo.toml"
        ));
    }
    #[test]
    ...
    fn ignores_buildroot_trees() {
        ...
        assert!(!relevant(Path::new(
            "rumahl-os/buildroot-2024.02/output/build/linux-headers-6.6.15/tools/perf/pmu-events/others.json"
        )));
        assert!(!relevant(Path::new(
            "rumahl-os/buildroot-2024.02/output/build/linux-headers-6.6.15/tools/arch/powerpc/Makefile"
        )));
    }
    #[test]
    fn generated_service_unit_runs_like_rumahl_os_under_systemd() {
        let unit = native_unit("rumahl-example");
        assert!(unit.contains("User=ora"));
        assert!(unit.contains("After=network-online.target postgresql.service"));
        assert!(unit.contains("target/debug/rumahl-example"));
    }
    #[test]
