...
/// rumahl CLI - Command-line interface for controlling rumahl OS and services
#[derive(Parser)]
#[command(name = "ora")]
#[command(about = "rumahl - rumahl Command-Line Interface (short form)", long_about = None)]
#[command(version)]
struct Cli {
    /// Base URL for rumahl services
    #[arg(long, env = "RUMAHL_URL", default_value = "http://localhost")]
    url: String,
...
enum Commands {
    ...
    /// Update rumahl OS (shortcut for `ora system update`)
    Update {
...
async fn handle_system(cmd: SystemCommands) -> Result<()> {
    match cmd {
        SystemCommands::Info => {
            ...
            if let Ok(output) = std::fs::read_to_string("/etc/rumahl-version") {
                println!("  rumahl OS: {}", output.trim());
            }
...
async fn show_status(base_url: &str, verbose: bool) -> Result<()> {
    ...
    println!("{}", "rumahl System Status".bright_blue().bold());
    println!();

    // System info
    if let Ok(version) = std::fs::read_to_string("/etc/rumahl-version") {
        println!("  rumahl OS: {}", version.trim());
    }
...
async fn handle_system_update(check_only: bool, assume_yes: bool, channel: &str) -> Result<()> {
    let update_server = std::env::var("RUMAHL_UPDATE_SERVER")
        .unwrap_or_else(|_| "https://update.rumahl.com".to_string());

    let hw = detect_hardware();
    let current_version = std::fs::read_to_string("/etc/rumahl-version")
        .ok()
    ...
    let url = format!(
        "{}/v1/ora/os/check?version={}&channel={}&arch={}&device_id={}&platform={}",
        update_server, current_version, channel, arch_for_server, device_id, hw.platform
    );

    println!("{}", "Checking for rumahl OS updates...".bright_cyan());
    ...
    // with rumahl-os/post-build.sh so we reuse the signature/checksum logic.
    let helper = "/opt/rumahl/update/check-update.sh";
    if std::path::Path::new(helper).exists() {
        println!(
            "{}",
            "Handing over to /opt/rumahl/update/check-update.sh...".bright_cyan()
        );
        ...
        eprintln!(
            "{}",
            "Update helper /opt/rumahl/update/check-update.sh not found \
             (this system may not be rumahl OS)."
                .yellow()
        );
        eprintln!(
            "  Download URL: {}",
        ...
        "app" | "" => {
            ...
            if !yes {
                let confirmed = dialoguer::Confirm::new()
                    .with_prompt(format!("Install app `{name}` from the rumahl app store?"))
                    .default(true)
                    .interact()
                    .unwrap_or(false);
                if !confirmed {
            ...
            let body = serde_json::json!({
                "metadata": {
                    ...
                    "author": "app-store",
                    "icon": null,
                    "image": format!("rumahl-apps/{name}:latest"),
                    "ports": [],
                    "environment": {},
                    "volumes": [],
                    "permissions": [],
...
async fn handle_dev(cmd: DevCommands) -> Result<()> {
    match cmd {
        DevCommands::Status => {
            let is_dev = std::path::Path::new("/etc/ora/os-dev-mode").exists();
            let bridge = std::path::Path::new("/usr/bin/rumahl-dev-bridge").exists();
            ...
            println!("      the rumahl Developer App.  Does NOT touch the OS.");
            println!("    • rumahl OS Dev mode                  — baked into the");
            println!("      image at build time via  build.sh --dev  only.");
            println!();
            println!("  OS-dev marker (/etc/ora/os-dev-mode): {}", yesno(is_dev));
            ...
            if !is_dev {
                ...
                println!("  {}", "via `rumahl-os/build.sh --dev`.".yellow());
                return Ok(());
            }
            ...
            match client.get("http://127.0.0.1:8099/dev/status").send().await {
                ...
                Err(_) => {
                    ...
                    println!(
                        "  {}",
                        "Bridge not reachable — is rumahl-dev-bridge.service running?".yellow()
                    );
                }
            }
            Ok(())
...
async fn print_version_overview() {
    println!("{}", "rumahl OS".bright_blue().bold());
    println!();

    // --- OS / build metadata -------------------------------------------------
    match std::fs::read_to_string("/etc/ora/build-info.json") {
            ...
            Ok(info) => {
                ...
                println!(
                    ...
                    if updates {
                        ...
                        "disabled (rumahl OS Dev build)".yellow()
                    }
                );
            }
        ...
        Err(_) => {
            match std::fs::read_to_string("/etc/rumahl-version") {
                Ok(v) => println!("  Version:  {}", v.trim()),
                Err(_) => println!("  Version:  unknown"),
            }
            println!(
                "  {}",
                "(no build-info.json — running outside rumahl OS?)".bright_black()
            );
        }
    }
    ...
    let base = std::env::var("RUMAHL_URL").unwrap_or_else(|_| "http://localhost".into());
    ...
    let services: &[(&str, u16)] = &[
        ...
        ("rumahl-files", 8098),
        ("rumahl-appstore", 8100),
        ("rumahl-developer-app", 8101),
    ];
    let client = reqwest::Client::builder()
