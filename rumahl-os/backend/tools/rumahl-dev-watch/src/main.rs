//! rumahl Dev Watch — Build, deploy & monitor the Dev VM.
//!
...
mod channel_tests {
    ...
    fn decodes_qga_output() {
        assert_eq!(
            decode_base64("aW9yYS1ob21lIGFjdGl2ZQo="),
            "rumahl-home active\n"
        );
    }
    ...
    fn keeps_vite_source_changes_on_hmr_but_restarts_for_configuration() {
        assert_eq!(
            classify_frontend_change(Path::new("src/App.tsx")),
            (false, true)
        );
        assert_eq!(
            classify_frontend_change(Path::new("vite.config.ts")),
            (true, false)
        );
    }
}
...
struct Args {
    ...
    #[arg(long)]
    initial_build: bool,
    /// Bridge port (rumahl-dev-bridge). Set to 0 to disable bridge mode.
...
impl Backend {
    ...
    async fn qga_exec(&self, cmd: &str) -> Result<String> {
        ...
        let pid = start
            ...
            .context("QGA did not return a guest PID")?;
        for _ in 0..120 {
    ...
    async fn fetch_service_status(&self) -> Vec<(String, bool)> {
        ...
        // Probe multiple bin directories — rumahl OS may use /usr/local/bin or
        // /opt/rumahl/bin depending on the image. Reporting "binary missing"
        // when it's just installed elsewhere caused phantom auto-builds.
        let script = format!(
            "for s in {}; do \
                bin=no; \
                for d in /usr/bin /usr/local/bin /opt/rumahl/bin /home/ora/.cargo/bin; do \
    ...
    fn build_rust_cmd(&self, only: Option<&HashSet<String>>) -> String {
        let mut cmd = format!(
            "cd {} && CARGO_BUILD_JOBS=$(nproc) /home/ora/.cargo/bin/cargo build",
            self.vm_workspace
        );
    ...
    async fn sync_sources(&self, tx: &mpsc::UnboundedSender<AppEvent>) -> Result<()> {
        ...
        args.push(format!("root@{}:/home/ora/ora/", self.vm_host));
        ...
        self.ssh_exec("chown -R ora:ora /home/ora/ora 2>/dev/null")
            .await?;
        Ok(())
    }
    ...
    async fn sync_changed_sources(
        ...
        for path in paths {
            ...
            let remote = format!("/home/ora/ora/{relative}");
            if path.is_file() {
        ...
        self.ssh_exec("chown -R ora:ora /home/ora/ora/rumahl-os/backend 2>/dev/null")
            .await?;
        Ok(())
    }
    ...
    async fn build_rust(
        ...
        let _ = self.ssh_exec("su - ora -c 'test -f /home/ora/.cargo/bin/cargo || curl --proto =https --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain stable --profile minimal' 2>&1").await;

        // Check disk space — clean only incremental cache (not deps/)
        let disk_check = self
            .ssh_exec(
                "df -BG /home/ora/ora/rumahl-os/backend | tail -1 | awk '{print $4}' | tr -d 'G'",
            )
            .await;
        ...
        if free_gb < 5 {
            ...
            let _ = self.ssh_exec("find /home/ora/ora/rumahl-os/backend/target -type d -name incremental -exec rm -rf {} + 2>/dev/null; find /home/ora/ora/rumahl-os/backend/target -name '*.d' -delete 2>/dev/null; echo OK").await;
        }
        ...
        let full = format!("su - ora -c '{}' 2>&1", cmd);
    ...
    /// remove every rumahl-* binary from the standard install dirs. Used by the
    ...
    async fn wipe_hashes_and_bins(&self, tx: &mpsc::UnboundedSender<AppEvent>) {
        let _ = tx.send(AppEvent::Log(
            "[DEPLOY] Wiping bin-hashes + removing rumahl-* from /usr/bin /usr/local/bin /opt/rumahl/bin".into(),
        ));
        let script = r#"set +e
rm -rf /var/lib/ora/.bin-hashes 2>/dev/null
for d in /usr/bin /usr/local/bin /opt/rumahl/bin; do
    [ -d "$d" ] && find "$d" -maxdepth 1 -name 'rumahl-*' -type f -delete 2>/dev/null
done
echo "wiped"
"#;
    ...
    async fn deploy_binaries(&self, tx: mpsc::UnboundedSender<AppEvent>) {
        ...
        let cmd = format!(
            "cat > /tmp/rumahl-deploy.sh << 'DEPLOYEOF'\n{}\nDEPLOYEOF\nbash /tmp/rumahl-deploy.sh; rc=$?; rm -f /tmp/rumahl-deploy.sh; exit $rc",
            script
        );
        use tokio::io::{AsyncBufReadExt, BufReader};
        let mut args = self.ssh_args();
    ...
    fn deploy_script(&self) -> String {
        ...
        format!(
            r#"#!/bin/bash
set +e
mkdir -p /var/lib/ora/.bin-hashes
# Pick a writable system bin dir. /usr/bin is read-only on the rumahl OS
# (buildroot squashfs) image; /usr/local/bin is the persistent overlay.
INSTALL_DIR=/usr/bin
if ! touch "$INSTALL_DIR/.rumahl-write-test" 2>/dev/null; then
    INSTALL_DIR=/usr/local/bin
    mkdir -p "$INSTALL_DIR"
fi
rm -f "$INSTALL_DIR/.rumahl-write-test" 2>/dev/null
...
for svc in {svc_list}; do
    ...
    prev=$(cat "/var/lib/ora/.bin-hashes/$svc" 2>/dev/null)
    ...
    # Dev-VMs can boot rumahl OS units with hardening drop-ins copied from the
    ...
    mkdir -p "/etc/systemd/system/$svc.service.d" "/etc/ora/db-credentials" "/opt/rumahl/build/$svc/data" /tmp/rumahl-sandboxes /var/log/ora "/var/lib/ora/$svc"
...
NSDROP
    ...
    if ! install -m 0755 "$bin" "$INSTALL_DIR/$svc" 2>/tmp/rumahl-install.err; then
        echo "install FAILED $svc: $(cat /tmp/rumahl-install.err 2>/dev/null)"
        rm -f /tmp/rumahl-install.err
        continue
    fi
    rm -f /tmp/rumahl-install.err
    [ -f "/etc/ora/db-credentials/$svc.env" ] || echo "DATABASE_URL=postgres://root:ora@localhost/rumahl_$short" > "/etc/ora/db-credentials/$svc.env"
    if [ ! -f "/etc/ora/$svc.env" ]; then
        if [ "$svc" = "rumahl-home" ]; then
            printf 'DATABASE_URL=postgres://root:ora@localhost:5432/rumahl_%s\nRUST_LOG=%s=debug\nRUMAHL_BOOTSTRAP_ADMIN_USER=admin\nRUMAHL_BOOTSTRAP_ADMIN_PASSWORD=admin1234\n' "$short" "$svc" > "/etc/ora/$svc.env"
        else
            printf 'DATABASE_URL=postgres://root:ora@localhost:5432/rumahl_%s\nRUST_LOG=%s=debug\n' "$short" "$svc" > "/etc/ora/$svc.env"
        fi
    fi
...
[Unit]
Description=$svc (auto-installed by rumahl-dev-watch)
After=network-online.target

[Service]
EnvironmentFile=-/etc/ora/$svc.env
EnvironmentFile=-/etc/ora/db-credentials/$svc.env
...
UNIT
    ...
    echo "$cur" > "/var/lib/ora/.bin-hashes/$svc"
    restart_list="$restart_list $svc"
    [ "$svc" = "rumahl-home" ] && home_changed=1
    deployed=$((deployed+1))
...
wait
if [ "$home_changed" = "1" ] && [ ! -f /var/lib/ora/.admin-role-fixed ]; then
    for i in 1 2 3 4 5 6 7 8; do
        if su - postgres -c "psql -tAc 'SELECT 1 FROM users LIMIT 1' rumahl_home" 2>/dev/null | grep -q 1; then break; fi
        sleep 1
    done
    su - postgres -c "psql rumahl_home -c \"UPDATE users SET role='admin' WHERE username='admin' AND role!='admin'\"" >/dev/null 2>&1
    touch /var/lib/ora/.admin-role-fixed
fi
...
"#
    ...
    async fn deploy_frontend(&self, tx: mpsc::UnboundedSender<AppEvent>) {
        ...
        let remote = "set -e; mkdir -p /opt/rumahl/build; \
            tmp=$(mktemp -d /opt/rumahl/build/.dist-XXXXXX); \
            tar xzf - -C \"$tmp\"; \
            rm -rf /opt/rumahl/build/dist; \
            mv \"$tmp\" /opt/rumahl/build/dist; \
    ...
    ///
    /// Currently handles:
    ///   1. `rumahl-dev-bridge` is started if its unit exists but is not active.
    ///   2. Any failed `rumahl-*` systemd unit is reset and restarted.
...
missing=""
mkdir -p /var/lib/ora/.bin-hashes
# 0) Detect missing rumahl-* binaries across every standard bin directory.
#    Always include the bridge so it gets auto-bootstrapped even when no
#    services were discovered yet.
for b in rumahl-dev-bridge {svc_list}; do
    found=""
    for d in /usr/bin /usr/local/bin /opt/rumahl/bin /home/ora/.cargo/bin; do
    ...
    if [ -z "$found" ]; then
        ...
        rm -f "/var/lib/ora/.bin-hashes/$b"
    fi
done
if [ -n "$missing" ]; then
    out="${{out}}missing: ${{missing}}\n"
fi
# 0b) rumahl OS production units are strongly sandboxed. In the dev VM this can
...
for svc in {svc_list}; do
    if systemctl list-unit-files "$svc.service" --no-legend 2>/dev/null | grep -q "^$svc\.service"; then
        mkdir -p "/etc/systemd/system/$svc.service.d" /tmp/rumahl-sandboxes /var/log/ora "/var/lib/ora/$svc" "/opt/rumahl/build/$svc/data"
        tmp="/tmp/rumahl-dev-namespaces-$svc.conf"
        cat > "$tmp" <<'NSDROP'
[Service]
ProtectSystem=no
ProtectHome=no
...
# 1) rumahl-dev-bridge: start it if a unit file exists but it's not active.
bridge_state=$(systemctl is-active rumahl-dev-bridge 2>/dev/null)
if [ "$bridge_state" != "active" ]; then
    if systemctl list-unit-files rumahl-dev-bridge.service 2>/dev/null | grep -q rumahl-dev-bridge; then
        systemctl reset-failed rumahl-dev-bridge 2>/dev/null
        systemctl start rumahl-dev-bridge 2>/dev/null
        sleep 1
        new=$(systemctl is-active rumahl-dev-bridge 2>/dev/null)
        out="${{out}}bridge ${{bridge_state:-missing}} -> ${{new}}\n"
    else
        bridge_bin=""
        for d in /usr/bin /usr/local/bin /opt/rumahl/bin; do
            if [ -x "$d/rumahl-dev-bridge" ]; then bridge_bin="$d/rumahl-dev-bridge"; break; fi
        done
        if [ -n "$bridge_bin" ]; then
            cat > /etc/systemd/system/rumahl-dev-bridge.service <<UNIT
[Unit]
Description=rumahl Dev Bridge (auto-installed by rumahl-dev-watch)
After=network-online.target
[Service]
...
UNIT
            systemctl daemon-reload 2>/dev/null
            systemctl enable --now rumahl-dev-bridge 2>/dev/null
            new=$(systemctl is-active rumahl-dev-bridge 2>/dev/null)
...
enum AppEvent {
    ...
    /// Self-heal detected one or more rumahl-* binaries missing on the VM
...
mod delta_sync_tests {
    ...
    fn portable_paths_use_forward_slashes() {
        assert_eq!(
            portable_relative_path(Path::new("rumahl-os/backend/Cargo.toml")).as_deref(),
            Some("rumahl-os/backend/Cargo.toml")
        );
    }
...
fn find_repo_root() -> Result<PathBuf> {
    ...
    for a in std::env::current_exe()?.ancestors() {
        if a.join("rumahl-os/backend/Cargo.toml").exists() || a.join("backend/Cargo.toml").exists() {
            return Ok(a.into());
        }
    }
...
fn find_workspace(r: &PathBuf) -> Result<PathBuf> {
    for c in &[r.join("rumahl-os/backend"), r.join("backend")] {
...
fn discover_services(w: &PathBuf) -> Vec<String> {
    let mut v = Vec::new();
    // Only long-running rumahl OS systemd targets belong here. CLI/tools crates
    // such as rumahl-cli, rumahl-sign or rumahl-db-manager are buildable packages,
            ...
            for en in e.flatten() {
                ...
                if p.is_dir() && p.join("Cargo.toml").exists() {
                    if let Some(n) = p.file_name().and_then(|n| n.to_str()) {
                        if n.starts_with("rumahl-") && n != "rumahl-dev-watch" {
                            v.push(n.into());
                        }
                    }
                }
...
fn service_manifest_paths(workspace: &Path) -> HashMap<String, PathBuf> {
    ...
    for sub in &["services", "apps/system", "dev"] {
        ...
        for entry in entries.flatten() {
            ...
            if name.starts_with("rumahl-") && manifest.is_file() {
...
fn cargo_metadata_impacts(
    ...
    for package in packages {
        names.insert(
            package.get("id")?.as_str()?.to_string(),
    ...
    for (package_id, package_name) in &names {
        if !package_name.starts_with("rumahl-shared") {
            continue;
        }
        let mut affected = HashSet::new();
        ...
        while let Some(current) = pending.pop() {
            ...
            for dependent in reverse.get(&current).into_iter().flatten() {
                if let Some(name) = names.get(dependent) {
                    if service_set.contains(name.as_str()) {
    ...
    let mut w: RecommendedWatcher =
        notify::recommended_watcher(move |res: notify::Result<notify::Event>| {
            ...
            for p in &ev.paths {
                ...
                if s.contains("rumahl-dev-watch") {
                    continue;
                }
                if s.ends_with(".rs") {
                ...
                if frontend_config {
                    ...
                    let _ = tx.send(AppEvent::Log(format!(
                        "[HMR] {} synchronized; Vite handles reload",
                        p.display()
                    )));
                }
            }
...
fn spawn_journal(state: &AppState, tx: mpsc::UnboundedSender<AppEvent>) {
    ...
    tokio::spawn(async move {
        if let Ok(l) = backend
            .ssh_exec("journalctl -u rumahl-home --no-pager -n 30 2>/dev/null")
            .await
        {
            for line in l.lines() {
...
fn export_log(state: &AppState) {
    let path = state
        .backend
        .repo_root
        .join("rumahl-os/.cache/dev-watch-log.txt");
...
fn handle_key(state: &mut AppState, key: KeyEvent, tx: &mpsc::UnboundedSender<AppEvent>) {
    ...
    match key.code {
        ...
        KeyCode::Char('x') | KeyCode::Char('X') => {
            export_log(state);
            state.push_log("[EXPORT] Log written to rumahl-os/.cache/dev-watch-log.txt");
        }
...
fn render_deploy(f: &mut ratatui::Frame, area: Rect, s: &AppState) {
    ...
    for i in 0..n {
        let svc = &s.backend.services[i];
        let label = svc.strip_prefix("rumahl-").unwrap_or(svc);
...
fn render_help(f: &mut ratatui::Frame, area: Rect, s: &AppState) {
    ...
    let keys = vec![
        ...
        Line::from("  J          Journal (rumahl-home log)"),
...
async fn main() -> Result<()> {
    ...
    let cache_dir = repo_root.join("rumahl-os/.cache");
    std::fs::create_dir_all(&cache_dir).ok();
    let ssh_key = args
        .ssh_key
        .clone()
        .map(PathBuf::from)
        .unwrap_or_else(|| cache_dir.join("rumahl-dev-key"));
    ...
    let backend = Backend {
        ...
        vm_workspace: "/home/ora/ora/rumahl-os/backend".into(),
    ...
    let (input_tx, mut input_rx) = mpsc::unbounded_channel::<CtEvent>();
    {
        ...
        std::thread::Builder::new()
            .name("rumahl-dev-watch-input".into())
            .spawn(move || loop {
    ...
    loop {
        tokio::select! {
            ...
            // 7. Periodic self-heal: auto-start bridge, reset failed rumahl-* units,
    ...
    match ev {
        ...
        ConnEvent::Offline => {
            ...
            if was_connected {
                ...
                // single `systemctl start rumahl-dev-bridge` fixes everything.
                if !state.building && !heal_in_flight.load(Ordering::Relaxed) {
                    heal_in_flight.store(true, Ordering::Relaxed);
                    let backend = state.backend.clone();
                    let tx = app_tx.clone();
