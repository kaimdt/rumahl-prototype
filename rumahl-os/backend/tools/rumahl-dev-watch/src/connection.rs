//! Persistent bridge connection for rumahl-dev-watch.
//!
//! Replaces ad-hoc SSH polling with a dual-channel approach:
//!   1. HTTP to `rumahl-dev-bridge` (port 8101) for status, health, SSE events
...
fn ssh_control_path() -> String {
    let dir = std::path::PathBuf::from("/tmp/rumahl-ssh");
...
/// in /tmp/rumahl-ssh (our namespace) and clean them all.
#[cfg(unix)]
