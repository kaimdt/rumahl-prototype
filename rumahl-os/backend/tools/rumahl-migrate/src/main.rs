...
#[derive(Parser)]
#[command(name = "rumahl-migrate")]
#[command(about = "rumahl Database Migration Tool - Run and track migrations across all services")]
struct Cli {
    #[command(subcommand)]
    command: Commands,

    /// Migrations directory (default: /opt/rumahl/migrations or ./migrations)
    #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    let migrations_dir = cli
        .migrations_dir
        .or_else(|| std::env::var("RUMAHL_MIGRATIONS_DIR").ok().map(PathBuf::from))
        .unwrap_or_else(|| {
            if std::path::Path::new("/opt/rumahl/migrations").exists() {
                PathBuf::from("/opt/rumahl/migrations")
            } else {
...
fn load_service_db_url() -> Option<String> {
    // Try to load from /etc/ora/db-credentials/{current-service}.env
    if let Ok(exe) = std::env::current_exe() {
        if let Some(name) = exe.file_stem() {
            let service_name = name.to_string_lossy();
            if let Some(service) = service_name.strip_prefix("rumahl-") {
                let cred_file = format!("/etc/ora/db-credentials/rumahl-{}.env", service);
