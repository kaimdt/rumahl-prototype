...
impl MigrationTracker {
    ...
    async fn ensure_tracking_table(&self) -> Result<()> {
        sqlx::query(
            r#"
            CREATE TABLE IF NOT EXISTS _rumahl_migrations (
            ...
            CREATE INDEX IF NOT EXISTS idx_migrations_service
                ON _rumahl_migrations(service);
            CREATE INDEX IF NOT EXISTS idx_migrations_applied
                ON _rumahl_migrations(applied_at DESC);
            "#,
        )
        .execute(&self.pool)
        .await?;
    ...
    pub async fn is_applied(&self, service: &str, migration: &str) -> Result<bool> {
        let exists: bool = sqlx::query_scalar(
            r#"
            SELECT EXISTS(
                SELECT 1 FROM _rumahl_migrations
    ...
    pub async fn mark_applied(
        ...
        sqlx::query(
            r#"
            INSERT INTO _rumahl_migrations
    ...
    pub async fn remove_migration(&self, service: &str, migration: &str) -> Result<()> {
        sqlx::query("DELETE FROM _rumahl_migrations WHERE service = $1 AND migration = $2")
            .bind(service)
            .bind(migration)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn get_rollback_sql(&self, service: &str, migration: &str) -> Result<Option<String>> {
        let sql: Option<String> = sqlx::query_scalar(
            "SELECT rollback_sql FROM _rumahl_migrations WHERE service = $1 AND migration = $2",
        )
        .bind(service)
        .bind(migration)
    ...
    pub async fn validate_checksum(
        ...
        let stored: Option<String> = sqlx::query_scalar(
            "SELECT checksum FROM _rumahl_migrations WHERE service = $1 AND migration = $2",
        )
        .bind(service)
        .bind(migration)
        .fetch_optional(&self.pool)
    ...
    pub async fn update_checksum(
        ...
        sqlx::query(
            "UPDATE _rumahl_migrations SET checksum = $3 WHERE service = $1 AND migration = $2",
        )
        .bind(service)
        .bind(migration)
        .bind(checksum)
