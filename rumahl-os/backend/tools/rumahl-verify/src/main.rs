// rumahl-verify — boot-time integrity check for rumahl OS
//
...
const MANIFEST_PATH: &str = "/etc/ora/manifest.json";
const SIGNATURE_PATH: &str = "/etc/ora/manifest.json.sig";
const PUBKEY_PATH: &str = "/etc/ora/rumahl-release.pub";
const TAMPER_FLAG: &str = "/run/rumahl-tamper";
const TAMPER_LOG: &str = "/var/log/rumahl-tamper.log";
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => {
            eprintln!("[rumahl-verify] integrity check passed");
            ExitCode::SUCCESS
        }
        Err(e) => {
