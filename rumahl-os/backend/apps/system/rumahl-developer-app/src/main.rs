...
const DEVELOPER_APP_TOKEN_BUILDTIME: Option<&str> = option_env!("RUMAHL_DEVELOPER_APP_TOKEN");
...
fn developer_app_token() -> &'static str {
    ...
    TOKEN
        .get_or_init(|| {
            ...
            if let Ok(t) = std::env::var("RUMAHL_DEVELOPER_APP_TOKEN") {
                if !t.is_empty() {
                    return t;
                }
            }
            ...
            let token: String = (0..48)
                ...
                .collect();
            warn!(
                "RUMAHL_DEVELOPER_APP_TOKEN not configured; generated ephemeral token: {}",
                token
            );
            token
        })
...
async fn check_developer_mode(
    ...
    // Validate API key by forwarding to rumahl-api: a 200 response indicates a
        ...
        if let Ok(key_str) = api_key.to_str() {
            if !key_str.is_empty() {
                let validate_url = format!(
                    "{}/api/auth/validate",
                    data.rumahl_api_url.trim_end_matches('/')
                );
...
async fn get_info(data: web::Data<AppState>) -> impl Responder {
    ...
    HttpResponse::Ok().json(serde_json::json!({
        ...
        "version": env!("CARGO_PKG_VERSION"),
        "description": "Official rumahl development tool",
...
/// Returns whether the underlying rumahl OS image was built with
/// `build.sh --dev` and, if so, the capabilities exposed by
/// `rumahl-dev-bridge` on 127.0.0.1:8101.  On a production image this
...
async fn detect_os_dev_mode() -> serde_json::Value {
    let marker = std::path::Path::new("/etc/ora/os-dev-mode").exists();
    if !marker {
...
async fn main() -> std::io::Result<()> {
    ...
    info!("Starting rumahl Developer App v{}", env!("CARGO_PKG_VERSION"));
    info!(
        "Developer App Token: {}",
        ...
        } else if std::env::var("RUMAHL_DEVELOPER_APP_TOKEN")
            .map(|t| !t.is_empty())
            .unwrap_or(false)
        {
    ...
    let supervisor_url = std::env::var("SUPERVISOR_URL")
        .unwrap_or_else(|_| "http://rumahl-supervisor:8097".to_string());
    let rumahl_api_url =
        std::env::var("RUMAHL_API_URL").unwrap_or_else(|_| "http://rumahl-api:8080".to_string());

    info!("Supervisor URL: {}", supervisor_url);
    info!("rumahl API URL: {}", rumahl_api_url);
