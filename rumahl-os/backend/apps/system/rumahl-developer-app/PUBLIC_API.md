# rumahl Developer App - Public API Documentation

This document describes the public APIs provided by the rumahl Developer App that can be accessed by other apps when Developer Mode is enabled.

## Overview

The Developer App exposes several **public endpoints** that allow other rumahl apps to access development features when running in Developer Mode. These endpoints do not require the exclusive Developer App token and can be called using a standard API key.
...
async with rumahlClient("http://localhost:8080") as client:
    status = await client.get("/apps/io.rumahl.developer-app/api/public/developer-status")
...
**Example**:
```python
async with rumahlClient("http://localhost:8080", api_key="your-key") as client:
    info = await client.get(
        "/apps/io.rumahl.developer-app/api/public/deployment-info/com.example.my-app"
    )
...
async with rumahlClient("http://localhost:8080", api_key="your-key") as client:
    metrics = await client.get(
        "/apps/io.rumahl.developer-app/api/public/app-metrics/com.example.my-app"
    )
...
async def check_my_status():
    async with rumahlClient(RUMAHL_URL, api_key=API_KEY) as client:
        ...
        dev_status = await client.get(
            "/apps/io.rumahl.developer-app/api/public/developer-status"
        )
        ...
        my_info = await client.get(
            f"/apps/io.rumahl.developer-app/api/public/deployment-info/{MY_APP_ID}"
        )

        # Get my metrics
        my_metrics = await client.get(
            f"/apps/io.rumahl.developer-app/api/public/app-metrics/{MY_APP_ID}"
        )
...
async def create_dev_dashboard():
    async with rumahlClient(RUMAHL_URL, api_key=API_KEY) as client:
        ...
        for app in apps:
            ...
            info = await client.get(
                f"/apps/io.rumahl.developer-app/api/public/deployment-info/{app['id']}"
            )

            # Get metrics for each app
            metrics = await client.get(
                f"/apps/io.rumahl.developer-app/api/public/app-metrics/{app['id']}"
            )
...
async def ide_deployment_workflow():
    async with rumahlClient(RUMAHL_URL, api_key=API_KEY) as client:
        ...
        before = await client.get(
            f"/apps/io.rumahl.developer-app/api/public/deployment-info/{APP_ID}"
        )
...
```
http://localhost:8080/apps/io.rumahl.developer-app/swagger-ui/
```
