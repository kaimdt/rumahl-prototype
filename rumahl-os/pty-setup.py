...
send("echo 'root:ora' | chpasswd", 1)
send("sed -i 's/PasswordAuthentication no/PasswordAuthentication yes/' /etc/ssh/sshd_config", 1)
send("systemctl restart sshd", 4)
send("useradd -m -s /bin/bash -G sudo ora 2>/dev/null; echo 'ora:ora' | chpasswd", 1)
send("mkdir -p /etc/ora && touch /etc/ora/ssh-ready && echo SETUP-OK", 1)
