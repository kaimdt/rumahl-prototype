#!/bin/bash
#
# rumahl OS Complete Image Builder
...
FALLBACK_MARKER="${OUTPUT_DIR}/rumahl-os.fallback"
...
write_payload_metadata() {
    ...
    elif [ -f "${OUTPUT_DIR}/rumahl-os.bootmode" ]; then
        payload_boot_mode=$(cat "${OUTPUT_DIR}/rumahl-os.bootmode" 2>/dev/null | tr -d '\r\n' || echo "dual")
...
OPTIONS:
    ...
    --publish              Publish release to rumahl update server (requires RUMAHL_UPDATE_API_KEY)
...
configure_buildroot() {
    log_info "Configuring Buildroot for rumahl OS..."

    cd "${BUILD_DIR}"
    local config_key="${RUMAHL_DEFCONFIG:-rumahl_defconfig}|dev=${RUMAHL_OS_DEV:-0}"
    local config_marker=".rumahl-config-key"
    local current_key=""
    [ -f "${config_marker}" ] && current_key="$(cat "${config_marker}" 2>/dev/null || true)"

    if [ -f .config ] && [ "${current_key}" = "${config_key}" ] && [ "${RUMAHL_FORCE_RECONFIGURE:-0}" != "1" ]; then
        log_info "Reusing existing Buildroot .config (${config_key}); running olddefconfig only."
        if [ "${RUMAHL_OS_DEV:-0}" = "1" ] && ! grep -q '^BR2_PACKAGE_RUMAHL_DEV_TOOLCHAIN=y$' .config 2>/dev/null; then
            log_info "Enabling native build toolchain for reused rumahl OS Dev config..."
            cat >> .config <<'EOF'
BR2_PACKAGE_RUMAHL_DEV_TOOLCHAIN=y
EOF
        fi
        PATH="${BUILDROOT_SAFE_PATH}" FORCE_UNSAFE_CONFIGURE=1 make BR2_EXTERNAL="${SCRIPT_DIR}" olddefconfig
    else
        PATH="${BUILDROOT_SAFE_PATH}" FORCE_UNSAFE_CONFIGURE=1 make BR2_EXTERNAL="${SCRIPT_DIR}" "${RUMAHL_DEFCONFIG:-rumahl_defconfig}"
        if [ "${RUMAHL_OS_DEV:-0}" = "1" ]; then
            log_info "Enabling native build toolchain for rumahl OS Dev image..."
            if ! grep -q '^BR2_PACKAGE_RUMAHL_DEV_TOOLCHAIN=y$' .config 2>/dev/null; then
                cat >> .config <<'EOF'
BR2_PACKAGE_RUMAHL_DEV_TOOLCHAIN=y
EOF
            fi
...
# ── rumahl: System Go bootstrap staging ───────────────────────────────────────
...
# ── rumahl: Go bootstrap stage1 mk patcher ─────────────────────────────────────
...
build_base_image() {
    log_info "Building rumahl OS base image (this may take 1-2 hours)..."
    ...
    if [ "${PROGRESS}" = true ]; then
        PATH="${BUILDROOT_SAFE_PATH}" \
            ...
            FORCE_UNSAFE_CONFIGURE=1 RUMAHL_POST_IMAGE_MODE="${POST_IMAGE_MODE}" RUMAHL_UNATTENDED="${UNATTENDED}" \
...
# =============================================================================
# Compile rumahl service binaries and embed them in the rootfs overlay
# =============================================================================
# All rumahl system containers are self-built on the device (no registry pulls).
...
# post-build.sh can bundle them into /opt/rumahl/build/<service>/bin/.
#
...
#      native rumahl service will stay inactive because ConditionPathExists on
#      /opt/rumahl/build/<svc>/bin/<svc> will fail. Setup wizard on :8080 may
#      also be unreachable if it depends on the native stack.

# ─── Frontend (React/Vite dashboard) ─────────────────────────────────────────
# The rumahl dashboard frontend lives in the repo root (one level up from
# this rumahl-os/ folder): package.json + vite.config.ts + src/. rumahl-home
...
# rootfs overlay at /opt/rumahl/rumahl-home/dist/, and let rumahl-home read the
# path from the RUMAHL_HOME_DIST env var (set in /etc/ora/rumahl-home.env).
#
...
# RUMAHL_REBUILD_FRONTEND is not set to 1. Otherwise rebuilds.
#
# Skip with RUMAHL_SKIP_FRONTEND=1 if you only want the backend image.
build_frontend_bundle() {
    if [ "${RUMAHL_SKIP_FRONTEND:-0}" = "1" ]; then
        log_info "RUMAHL_SKIP_FRONTEND=1 → skipping frontend bundle"
        return 0
    fi

    # Locate the frontend source. The repo layout puts package.json at the
    # repo root (one level up from rumahl-os/). Older sibling layouts placed
    ...
    if [ -z "${FRONTEND_DIR}" ]; then
        ...
        log_warn "rumahl-home will serve the embedded fallback page on :8126."
        return 0
    fi
    ...
    local rebuild=1
    if [ "${RUMAHL_REBUILD_FRONTEND:-0}" != "1" ] && [ -f "${FRONTEND_DIR}/dist/index.html" ]; then
        local newest_src
    ...
    if [ "${rebuild}" = "1" ]; then
        ...
        if ! command -v npm >/dev/null 2>&1; then
            ...
            if command -v apt-get >/dev/null 2>&1; then
                if [ "$(id -u)" = "0" ]; then
                    apt-get update -qq >/tmp/rumahl-apt.log 2>&1 || true
                    apt-get install -y --no-install-recommends nodejs npm >>/tmp/rumahl-apt.log 2>&1 || true
                else
                    sudo -n apt-get update -qq >/tmp/rumahl-apt.log 2>&1 || true
                    sudo -n apt-get install -y --no-install-recommends nodejs npm >>/tmp/rumahl-apt.log 2>&1 || true
                fi
            fi
        fi
        if ! command -v npm >/dev/null 2>&1; then
            ...
            log_error "Or skip the frontend with: RUMAHL_SKIP_FRONTEND=1 sudo ./build.sh all"
            log_error "Without the frontend, :8126 will only show the rumahl placeholder page."
            return 1
        fi
        ...
        if ! ( cd "${FRONTEND_DIR}" && npm "${install_cmd}" --no-audit --no-fund --prefer-offline ) >/tmp/rumahl-npm-install.log 2>&1; then
            log_error "  npm ${install_cmd} FAILED — last 30 lines of /tmp/rumahl-npm-install.log:"
            tail -n 30 /tmp/rumahl-npm-install.log 2>/dev/null | sed 's/^/      /' || true
            log_error "  Image will be built without the dashboard UI (:8126 → rumahl placeholder page)."
            log_error "  Set RUMAHL_SKIP_FRONTEND=1 to silence this error, or fix npm and re-run."
            return 1
        fi

        log_info "  npm run build..."
        if ! ( cd "${FRONTEND_DIR}" && npm run build ) >/tmp/rumahl-npm-build.log 2>&1; then
            log_error "  npm run build FAILED — last 40 lines of /tmp/rumahl-npm-build.log:"
            tail -n 40 /tmp/rumahl-npm-build.log 2>/dev/null | sed 's/^/      /' || true
            log_error "  Image will be built without the dashboard UI (:8126 → rumahl placeholder page)."
            return 1
        fi
    ...
    # Stage dist/ into the rootfs overlay at /opt/rumahl/rumahl-home/dist/.
    local FRONTEND_DEST="${SCRIPT_DIR}/board/rumahl/rootfs-overlay/opt/rumahl/rumahl-home/dist"
    rm -rf "${FRONTEND_DEST}"
...
build_service_binaries() {
    local OVERLAY="${SCRIPT_DIR}/board/rumahl/rootfs-overlay/opt/rumahl/build"
    # Resolve the backend/ source tree. We try the in-tree location first
    # (rumahl-os/backend/) — that's where the workspace lives in this repo and
    ...
    if [ -z "${BACKEND_DIR}" ]; then
        ...
        log_warn "Without native service binaries, rumahl-core/rumahl-home/... will NOT start on boot."
        return 0
    fi
    ...
    if ! command -v cargo >/dev/null 2>&1 && [ "${RUMAHL_AUTO_INSTALL_RUST:-1}" = "1" ]; then
        log_warn "cargo not found on PATH. Installing rustup non-interactively…"
        if command -v curl >/dev/null 2>&1; then
            curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs \
                | sh -s -- -y --default-toolchain stable --profile minimal --no-modify-path \
                >/tmp/rumahl-rustup-install.log 2>&1 || \
                log_warn "rustup install failed (see /tmp/rumahl-rustup-install.log)"
    ...
    local SERVICES="rumahl-core rumahl-home rumahl-control rumahl-assist rumahl-secrets \
                    ...
                    rumahl-runtime-sensor rumahl-runtime-identity rumahl-runtime-policy rumahl-incident-engine"
    # CLI tools as `package:binary` pairs (binary may differ from crate name —
    # rumahl-cli ships its binary as `ora`, the user-facing command).
    local CLI_TOOLS="rumahl-cli:ora rumahl-sign:rumahl-sign rumahl-verify:rumahl-verify"

    # ── Workspace consistency check ─────────────────────────────────────────
    # Verify that every workspace member whose name starts with "rumahl-"
...
" 2>/dev/null); do
    ...
    # RUMAHL_BUILD_BACKEND lets the user force a specific path:
    #   auto    – pick the safest (default)
    #   native  – always cargo on host
    #   docker  – always Dockerfile / Alpine
    local _RUMAHL_BUILD_BACKEND="${RUMAHL_BUILD_BACKEND:-auto}"
    local _host_glibc
    ...
    local _RUMAHL_AUTO_MUSL=0
    if [ "${_RUMAHL_BUILD_BACKEND}" = "auto" ]; then
        if ! command -v ldd >/dev/null 2>&1; then
            ...
            if command -v docker >/dev/null 2>&1; then
                ...
                log_warn "Override with RUMAHL_BUILD_BACKEND=native (musl) or RUMAHL_RUST_TRIPLE=...."
                _RUMAHL_BUILD_BACKEND="docker"
            elif [ -z "${RUMAHL_RUST_TRIPLE:-}" ] \
                && command -v rustup >/dev/null 2>&1 \
                && [ "${RUMAHL_ARCH:-x86_64}" = "x86_64" ]; then
                log_warn "Host glibc ${_host_glibc} is newer than target glibc 2.38."
    ...
    if [ "${_RUMAHL_BUILD_BACKEND}" != "docker" ] && command -v cargo >/dev/null 2>&1; then
        log_info "Pre-compiling rumahl service binaries natively with cargo..."
        log_info "Backend source: ${BACKEND_DIR}"
        log_info "cargo: $(command -v cargo) ($(cargo --version 2>/dev/null || echo unknown))"

        local RUST_TRIPLE="${RUMAHL_RUST_TRIPLE:-x86_64-unknown-linux-gnu}"
        case "${RUMAHL_ARCH:-x86_64}" in
        ...
        if command -v rustup >/dev/null 2>&1; then
            ...
            rustup default stable >/tmp/rumahl-rustup-default.log 2>&1 || \
                log_warn "rustup default stable failed (see /tmp/rumahl-rustup-default.log — last 10 lines below)"
            [ -f /tmp/rumahl-rustup-default.log ] && \
                tail -n 10 /tmp/rumahl-rustup-default.log 2>/dev/null | sed 's/^/    /' || true
            if ! rustup target add "${RUST_TRIPLE}" >/tmp/rumahl-rustup-target.log 2>&1; then
        ...
        if [ "${_RUMAHL_BUILD_BACKEND}" = "docker" ]; then
            log_info "Docker build backend selected; skipping native cargo build."
        else
            # Use -p <package> instead of --bin: each rumahl service lives in its
        ...
        local _persistent_target="${HOME}/.rumahl-cache/cargo-target"
        mkdir -p "${_persistent_target}"
        local CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-${_persistent_target}}"
        export CARGO_TARGET_DIR
        : >/tmp/rumahl-cargo-build.log
        local CARGO_LOG_DIR="/tmp/rumahl-cargo-logs"
        rm -rf "${CARGO_LOG_DIR}"
        mkdir -p "${CARGO_LOG_DIR}"

        # ── sccache – Rust compiler cache ───────────────────────────────────
        # Like ccache, but for rustc.  Persisted in ~/.rumahl-cache/sccache.
        # Even when rumahl-shared changes, unchanged translation units in
        # dependent crates still hit the cache (3-10× speedup on rebuiilds).
        if [ "${RUMAHL_SCCACHE:-1}" = "1" ] && command -v sccache >/dev/null 2>&1; then
            export SCCACHE_DIR="${HOME}/.rumahl-cache/sccache"
            mkdir -p "${SCCACHE_DIR}"
            export RUSTC_WRAPPER="sccache"
            # Prefer local disk cache only (no S3/GCS/…).
            sccache --start-server >/dev/null 2>&1 || true
            log_info "sccache active → cache dir: ${SCCACHE_DIR}"
        elif [ "${RUMAHL_SCCACHE:-1}" = "1" ]; then
        ...
        if command -v ccache >/dev/null 2>&1; then
            local CCACHE_DIR="${HOME}/.rumahl-cache/ccache"
            mkdir -p "${CCACHE_DIR}"
            export CCACHE_DIR
        ...
        # (rumahl-shared, tokio, axum, …) are compiled exactly once.
        #
        ...
        if ( cd "${BACKEND_DIR}" && \
             ...
             >"${_ws_log}" 2>&1; then
            cat "${_ws_log}" >>/tmp/rumahl-cargo-build.log
            ...
            for entry in ${SERVICES} ${CLI_TOOLS}; do
                ...
                if ( cd "${BACKEND_DIR}" && \
                     ...
                     cargo build ${_cargo_profile_flag} --target "${RUST_TRIPLE}" -p "${svc}" \
                         --message-format=short ) \
                     >"${svc_log}" 2>&1; then
                    cat "${svc_log}" >>/tmp/rumahl-cargo-build.log
                    built_ok="${built_ok} ${svc}"
                    continue
                fi
                log_warn "    ${svc}: cross-compile failed."
                if [ "${_RUMAHL_AUTO_MUSL:-0}" = "1" ]; then
                    log_warn "    ${svc}: BUILD FAILED (musl) — first 5 errors:"
                    grep -m 5 -E "^error" "${svc_log}" 2>/dev/null \
                        | sed 's/^/        /' || true
                    log_warn "    Hint: re-run with RUMAHL_BUILD_BACKEND=docker to use the"
        ...
        if [ -n "${built_fail}" ]; then
            ...
            log_warn "Combined log: /tmp/rumahl-cargo-build.log"
        ...
        if [ "${failed}" -gt 0 ]; then
            ...
            log_warn "(ConditionPathExists on /opt/rumahl/build/<svc>/bin/<svc> will fail)."
        else
            log_success "All rumahl service binaries embedded in rootfs overlay (native build)."
        fi

        # ── CLI tools → /usr/bin ───────────────────────────────────────────
        local cli_dest="${SCRIPT_DIR}/board/rumahl/rootfs-overlay/usr/bin"
        mkdir -p "${cli_dest}"
                ...
                if [ "${RUMAHL_GLIBC_AUDIT_NO_RECOVER:-0}" != "1" ] \
                    ...
                    if rumahl_docker_rebuild_services "${_too_new_svcs}"; then
                        ...
                        if [ -z "${_still_bad}" ]; then
                            ...
                            if [ "${RUMAHL_GLIBC_AUDIT_FATAL:-0}" = "1" ]; then
                                log_error "RUMAHL_GLIBC_AUDIT_FATAL=1 — aborting."
                                return 1
                            fi
                        fi
                    else
                    ...
                    log_warn "Re-run with RUMAHL_BUILD_BACKEND=docker, or install Docker."
                    if [ "${RUMAHL_GLIBC_AUDIT_FATAL:-0}" = "1" ]; then
                        log_error "RUMAHL_GLIBC_AUDIT_FATAL=1 is set — aborting build."
                        return 1
                    fi
                fi
            else
    ...
    if [ "${_RUMAHL_BUILD_BACKEND}" != "docker" ]; then
        ...
        log_warn "or set RUMAHL_AUTO_INSTALL_RUST=1 and ensure curl is available."
    fi
    ...
    if [ ! -f "${BACKEND_DIR}/Dockerfile" ]; then
        ...
        log_warn "Without binaries, native rumahl services will NOT start on boot."
        return 0
    fi
    ...
    log_info "Pre-compiling rumahl service binaries via Docker builder image..."
    log_info "Backend source: ${BACKEND_DIR}"

    local BUILDER_TAG="rumahl-builder-tmp:$(date +%s)"

    # Optional cache-busting flags. Set RUMAHL_DOCKER_NOCACHE=1 if a previous
    # build cached an older Dockerfile/source layout and you want to force
    # rebuild everything from scratch. RUMAHL_DOCKER_PULL=1 also re-pulls the
    # base image (rust:1.77-alpine) in case the registry has updates.
    local _docker_extra=""
    if [ "${RUMAHL_DOCKER_NOCACHE:-0}" = "1" ]; then
        _docker_extra="${_docker_extra} --no-cache"
        log_info "  RUMAHL_DOCKER_NOCACHE=1 → using --no-cache"
    fi
    if [ "${RUMAHL_DOCKER_PULL:-0}" = "1" ]; then
        _docker_extra="${_docker_extra} --pull"
        log_info "  RUMAHL_DOCKER_PULL=1 → using --pull"
    fi
    ...
    if ! docker build \
            ...
            "${BACKEND_DIR}"; then
        log_warn "Failed to build rumahl-builder image; skipping binary embedding."
        return 0
    fi
    ...
    if [ "${failed}" -gt 0 ]; then
        ...
        log_success "All rumahl service binaries embedded in rootfs overlay."
    fi
}
...
rumahl_docker_rebuild_services() {
    local svcs="$1"
    [ -z "${svcs}" ] && return 0

    local BUILDER_TAG="rumahl-builder-recover:$(date +%s)"
...
create_iso_image() {
    ...
    if [ ! -f "${RELEASE_DIR}/rumahl-os.img.xz" ]; then
        log_warn "rumahl-os.img.xz not found in release directory, skipping ISO"
        return
    fi
    ...
    if command -v xorriso &> /dev/null; then
        ...
        mark_skipped "rumahl-os-installer.iso (missing xorriso/genisoimage/mkisofs)"
        return
    fi
    ...
    cp "${RELEASE_DIR}/rumahl-os.img.xz" "${ISO_STAGE_DIR}/"
    write_payload_metadata "${ISO_STAGE_DIR}"

    cat > "${ISO_STAGE_DIR}/INSTALL.txt" <<'EOF'
rumahl OS installer/archive ISO

This ISO contains:
- rumahl-os.img.xz (compressed raw image)
...
EOF
    ...
    if [ "${iso_tool}" = "xorriso" ]; then
        xorriso -as mkisofs -r -J -V "RUMAHL_OS" -o "${RELEASE_DIR}/rumahl-os-installer.iso" "${ISO_STAGE_DIR}" >/dev/null 2>&1
    else
        "${iso_tool}" -r -J -V "RUMAHL_OS" -o "${RELEASE_DIR}/rumahl-os-installer.iso" "${ISO_STAGE_DIR}" >/dev/null 2>&1
    fi

    rm -rf "${ISO_STAGE_DIR}"

    local size
    size=$(du -h "${RELEASE_DIR}/rumahl-os-installer.iso" | cut -f1)
    log_success "ISO created: rumahl-os-installer.iso (${size})"
    mark_created "rumahl-os-installer.iso"
}
...
show_boot_splash() {
    ...
    # rumahl palette (256-color, matches the dashboard accent)
    ...
    # rumahl wordmark (pure ASCII -- box-drawing/Braille mojibake on fbcon)
    printf "${ACCENT}"
    cat <<'SPLASH'
...
SPLASH
    ...
    printf "\n${WHITE}        Starting rumahl OS Installer${RESET}\n"
...
if [ -f /etc/ora/os-dev-mode ]; then
    BACKTITLE="rumahl OS Installer  *** DEV BUILD -- INTERNAL USE ONLY ***"
else
    BACKTITLE="rumahl OS Installer  |  Use Tab/Arrow keys to navigate, Enter to confirm"
fi
RUMAHL_HOSTNAME="ora"
RUMAHL_TIMEZONE="Europe/Berlin"
RUMAHL_NETWORK="dhcp"
RUMAHL_LOCALE="en_US.UTF-8"
RUMAHL_KEYBOARD="de"
RUMAHL_INSTALL_MODE="guided"
RUMAHL_USER=""
RUMAHL_USER_PW=""
RUMAHL_USER_FULLNAME=""
RUMAHL_USER_SUDO=true
RUMAHL_VIRT_TYPE="none"
RUMAHL_VIRT_VENDOR=""
RUMAHL_VIRT_CONTAINER="none"
RUMAHL_VIRT_LABEL="Bare metal"
...
# rumahl OS installer -- rumahl brand theme (cyan/yellow on blue)
aspect = 0
separate_widget = ""
tab_len = 4
visit_items = ON
...
# Sets global vars:
#   RUMAHL_VIRT_TYPE   = none | kvm | qemu | vmware | virtualbox | hyperv |
#                      xen | bochs | parallels | microsoft | innotek
#   RUMAHL_VIRT_VENDOR = human-friendly string (e.g. "Proxmox / KVM", "VMware")
#   RUMAHL_VIRT_CONTAINER = none | lxc | docker | systemd-nspawn | openvz | wsl
#   RUMAHL_VIRT_LABEL  = short label for UI ("Bare metal", "VM: VMware", ...)
detect_virtualization() {
    ...
    if [ -f /.dockerenv ] || grep -qa 'docker\|containerd' /proc/1/cgroup 2>/dev/null; then
        RUMAHL_VIRT_CONTAINER="docker"
    elif [ -n "${container:-}" ]; then
        case "$container" in
            lxc|lxc-libvirt) RUMAHL_VIRT_CONTAINER="lxc" ;;
            systemd-nspawn)  RUMAHL_VIRT_CONTAINER="systemd-nspawn" ;;
            docker)          RUMAHL_VIRT_CONTAINER="docker" ;;
            podman)          RUMAHL_VIRT_CONTAINER="podman" ;;
            *)               RUMAHL_VIRT_CONTAINER="$container" ;;
        esac
    elif grep -qa 'lxc\|lxcfs' /proc/1/cgroup 2>/dev/null; then
        RUMAHL_VIRT_CONTAINER="lxc"
    elif [ -d /proc/vz ] && [ ! -d /proc/bc ]; then
        RUMAHL_VIRT_CONTAINER="openvz"
    elif grep -qi 'microsoft\|wsl' /proc/sys/kernel/osrelease 2>/dev/null; then
        RUMAHL_VIRT_CONTAINER="wsl"
    fi
    ...
    case "${sys_vendor} ${product} ${vendor}" in
        *VMware*|*"VMware, Inc."*)
            RUMAHL_VIRT_TYPE="vmware"
            RUMAHL_VIRT_VENDOR="VMware (${product:-vSphere/Workstation})" ;;
        *VirtualBox*|*innotek*|*Oracle*VirtualBox*)
            RUMAHL_VIRT_TYPE="virtualbox"
            RUMAHL_VIRT_VENDOR="Oracle VirtualBox" ;;
        *QEMU*)
            RUMAHL_VIRT_TYPE="qemu"
            RUMAHL_VIRT_VENDOR="QEMU/KVM${product:+ (${product})}" ;;
        *Xen*)
            RUMAHL_VIRT_TYPE="xen"
            RUMAHL_VIRT_VENDOR="Xen${product:+ (${product})}" ;;
        *Microsoft*Hyper-V*|*"Microsoft Corporation Virtual Machine"*|*Hyper-V*)
            RUMAHL_VIRT_TYPE="hyperv"
            RUMAHL_VIRT_VENDOR="Microsoft Hyper-V" ;;
        *Parallels*)
            RUMAHL_VIRT_TYPE="parallels"
            RUMAHL_VIRT_VENDOR="Parallels" ;;
        *Bochs*)
            RUMAHL_VIRT_TYPE="bochs"
            RUMAHL_VIRT_VENDOR="Bochs/QEMU (TCG)" ;;
    esac

    # Proxmox VE heuristic: QEMU with ovmf/seabios + often product="Standard PC (Q35 + ICH9, 2009)"
    if [ "$RUMAHL_VIRT_TYPE" = "qemu" ]; then
        if [ -r /sys/class/dmi/id/bios_version ]; then
            case "$(tr -d '\0' < /sys/class/dmi/id/bios_version 2>/dev/null)" in
                *pve*|*proxmox*|*Proxmox*)
                    RUMAHL_VIRT_VENDOR="Proxmox VE (KVM)" ;;
            esac
        fi
    fi

    # -- Fallback: hypervisor CPUID flag in /proc/cpuinfo --
    if [ "$RUMAHL_VIRT_TYPE" = "none" ] && grep -qa '^flags.*\bhypervisor\b' /proc/cpuinfo 2>/dev/null; then
        RUMAHL_VIRT_TYPE="kvm"  # best-effort default when no DMI info is present
        RUMAHL_VIRT_VENDOR="Unknown hypervisor (CPUID hypervisor flag set)"
    fi
    ...
    if [ -r /sys/hypervisor/type ]; then
        ...
        case "$hv" in
            xen) RUMAHL_VIRT_TYPE="xen"; RUMAHL_VIRT_VENDOR="Xen" ;;
        esac
    fi

    # -- Build label --
    if [ "$RUMAHL_VIRT_CONTAINER" != "none" ]; then
        RUMAHL_VIRT_LABEL="Container: ${RUMAHL_VIRT_CONTAINER}"
    elif [ "$RUMAHL_VIRT_TYPE" != "none" ]; then
        RUMAHL_VIRT_LABEL="VM: ${RUMAHL_VIRT_VENDOR:-$RUMAHL_VIRT_TYPE}"
    else
        RUMAHL_VIRT_LABEL="Bare metal"
    fi

    export RUMAHL_VIRT_TYPE RUMAHL_VIRT_VENDOR RUMAHL_VIRT_CONTAINER RUMAHL_VIRT_LABEL
}
...
repair_disk_and_bootloader() {
    ...
    local _menu_suffix=""
    if [ -f "${target}/etc/ora/os-dev-mode" ]; then
...
menuentry "rumahl OS${_menu_suffix}" {
...
menuentry "rumahl OS (second slot)${_menu_suffix}" {
...
menuentry "rumahl OS Recovery${_menu_suffix}" {
    search --no-floppy --set=root --fs-uuid ${fsuuid_a:-00000000-0000-0000-0000-000000000000}
    if [ -f /boot/vmlinuz ]; then
...
EFIREDIR
    ...
    if chroot "$target" /bin/sh -c "command -v grub-install >/dev/null 2>&1" 2>/dev/null; then
        ...
        if [ -d /usr/lib/grub/i386-pc ] && command -v grub-mkimage >/dev/null 2>&1 \
           && command -v grub-bios-setup >/dev/null 2>&1; then
            ...
            local core_img="/tmp/rumahl-core.img"
    ...
    if [ "$efi_mounted" = true ]; then
        ...
        # efibootmgr: register rumahl OS boot entry if running in UEFI installer
...
install_bootloader_fallback() {
    ...
    if chroot "$target" /bin/sh -c "command -v grub-install >/dev/null 2>&1" >/dev/null 2>&1; then
        ...
        if [ "$efi_mounted" = true ]; then
            ...
            if [ -d /sys/firmware/efi ]; then
                chroot "$target" /bin/sh -c \
                    "grub-install --target=x86_64-efi --efi-directory=/boot/efi --boot-directory=/boot --bootloader-id=rumahl --recheck /dev/${disk}" \
                    >/dev/null 2>&1 || true
            fi
        fi
    ...
    if [ "$uefi_install_ok" = false ] && [ "$efi_mounted" = true ] \
       ...
       && [ -d /usr/lib/grub/x86_64-efi ]; then
        mkdir -p "${target}/boot/efi/EFI/BOOT" "${target}/boot/efi/EFI/rumahl" 2>/dev/null || true
        if grub-mkimage -O x86_64-efi -o "${target}/boot/efi/EFI/BOOT/BOOTX64.EFI" \
             -p /boot/grub \
             part_gpt part_msdos fat ext2 normal configfile linux \
             search search_label search_fs_uuid search_fs_file echo \
            ...
            cp -f "${target}/boot/efi/EFI/BOOT/BOOTX64.EFI" \
                  "${target}/boot/efi/EFI/rumahl/grubx64.efi" 2>/dev/null || true
            mkdir -p "${target}/boot/grub/x86_64-efi" 2>/dev/null || true
            cp -a /usr/lib/grub/x86_64-efi/*.mod "${target}/boot/grub/x86_64-efi/" 2>/dev/null || true
        fi
    fi
    ...
    if [ "$efi_mounted" = true ]; then
        ...
        if [ ! -f "${target}/boot/efi/EFI/BOOT/BOOTX64.EFI" ]; then
            for _src in \
                "${target}/boot/efi/EFI/rumahl/grubx64.efi" \
                "${target}/boot/efi/EFI/ubuntu/grubx64.efi" \
                "${target}/boot/efi/EFI/debian/grubx64.efi" \
                "${target}/boot/efi/EFI/grub/grubx64.efi" \
                "${target}/boot/efi/EFI/GRUB/grubx64.efi"; do
        ...
        if [ -f "${target}/boot/efi/EFI/BOOT/BOOTX64.EFI" ] \
           || [ -f "${target}/boot/efi/EFI/rumahl/grubx64.efi" ] \
           || [ -f "${target}/boot/efi/EFI/ubuntu/grubx64.efi" ] \
           || [ -f "${target}/boot/efi/EFI/debian/grubx64.efi" ]; then
            efi_has_loader=true
        fi

        # Register an rumahl OS NVRAM entry when running on UEFI firmware.
           ...
           && command -v efibootmgr >/dev/null 2>&1; then
            ...
            if [ -n "$efi_part_num" ]; then
                efibootmgr --create --disk "/dev/${disk}" --part "$efi_part_num" \
                    --label "rumahl OS" --loader '\EFI\BOOT\BOOTX64.EFI' \
                    >/dev/null 2>&1 || true
            fi
        fi
    fi
    ...
    if [ ! -s "${target}/boot/grub/grub.cfg" ] \
       || ! grep -q 'rumahl_slot=' "${target}/boot/grub/grub.cfg" 2>/dev/null; then
...
menuentry "rumahl OS${_menu_suffix2}" {
...
menuentry "rumahl OS (Partition B)${_menu_suffix2}" {
...
menuentry "rumahl OS Recovery${_menu_suffix2}" {
    search --no-floppy --set=root --fs-uuid ${_fsuuid_a:-00000000-0000-0000-0000-000000000000}
    if [ -f /boot/vmlinuz ]; then
...
rumahl_set_account_password() {
    ...
    # Requires /bin/sh inside the target; the rumahl rootfs always has it.
...
apply_post_install_config() {
    ...
    # Set hostname
    if [ -n "$RUMAHL_HOSTNAME" ] && [ "$RUMAHL_HOSTNAME" != "ora" ]; then
        echo "$RUMAHL_HOSTNAME" > "${target}/etc/hostname" 2>/dev/null || true
        if [ -f "${target}/etc/hosts" ]; then
            sed -i "s/ora/${RUMAHL_HOSTNAME}/g" "${target}/etc/hosts" 2>/dev/null || true
        fi
    fi

    # Set timezone
    if [ -n "$RUMAHL_TIMEZONE" ] && [ -f "${target}/usr/share/zoneinfo/${RUMAHL_TIMEZONE}" ]; then
        ln -sf "/usr/share/zoneinfo/${RUMAHL_TIMEZONE}" "${target}/etc/localtime" 2>/dev/null || true
        echo "$RUMAHL_TIMEZONE" > "${target}/etc/timezone" 2>/dev/null || true
    fi
    ...
    if [ -n "$RUMAHL_ROOT_PW" ]; then
        rumahl_set_account_password "$target" "root" "$RUMAHL_ROOT_PW" \
 ...
 Repair menu (Reset root password)."
    ...
    if [ -n "$RUMAHL_USER" ] && [ -f "${target}/etc/passwd" ]; then
        if ! grep -q "^${RUMAHL_USER}:" "${target}/etc/passwd" 2>/dev/null; then
            local uid=1000
            while grep -q ":${uid}:" "${target}/etc/passwd" 2>/dev/null; do
                uid=$((uid + 1))
            done
            local gid="$uid"
            echo "${RUMAHL_USER}:x:${uid}:${gid}:${RUMAHL_USER_FULLNAME:-${RUMAHL_USER}}:/home/${RUMAHL_USER}:/bin/sh" \
                >> "${target}/etc/passwd"
            echo "${RUMAHL_USER}:x:${gid}:" >> "${target}/etc/group" 2>/dev/null || true
            ...
            # verifies the result.
            echo "${RUMAHL_USER}:!:19000:0:99999:7:::" >> "${target}/etc/shadow" 2>/dev/null || true
            if [ -n "$RUMAHL_USER_PW" ]; then
                rumahl_set_account_password "$target" "$RUMAHL_USER" "$RUMAHL_USER_PW" \
                    || dlg_msg " Password Warning " "\
 Could not set the password for '${RUMAHL_USER}'.\n\
 The account has been created but is LOCKED.\n\
 Use 'passwd ${RUMAHL_USER}' after first boot to set it."
            fi
            mkdir -p "${target}/home/${RUMAHL_USER}" 2>/dev/null || true
            if [ -d "${target}/etc/skel" ]; then
                cp -a "${target}/etc/skel/." "${target}/home/${RUMAHL_USER}/" 2>/dev/null || true
            fi
            chown -R "${uid}:${gid}" "${target}/home/${RUMAHL_USER}" 2>/dev/null || true
            chmod 750 "${target}/home/${RUMAHL_USER}" 2>/dev/null || true
            if [ "$RUMAHL_USER_SUDO" = true ]; then
                ...
                for grp in wheel sudo; do
                    if grep -q "^${grp}:" "${target}/etc/group" 2>/dev/null; then
                        sed -i "s|^\(${grp}:[^:]*:[^:]*:\)\(.*\)$|\1\2${RUMAHL_USER},|; s|,,|,|g; s|,$||" \
                            "${target}/etc/group" 2>/dev/null || true
                    fi
                done
                mkdir -p "${target}/etc/sudoers.d" 2>/dev/null || true
                echo "${RUMAHL_USER} ALL=(ALL) ALL" > "${target}/etc/sudoers.d/10-rumahl-user"
                chmod 440 "${target}/etc/sudoers.d/10-rumahl-user" 2>/dev/null || true
            fi
        fi
    fi

    # Configure network (IPv4 + IPv6). Use 10-* so it wins over the
    # built-in 90-rumahl-wired-default.network fallback.
    if [ "$RUMAHL_NETWORK" = "dhcp" ]; then
    ...
    elif [ "$RUMAHL_NETWORK" = "static" ] && [ -n "$RUMAHL_IP" ]; then
        mkdir -p "${target}/etc/systemd/network" 2>/dev/null || true
        rm -f "${target}/etc/systemd/network/eth0.network" 2>/dev/null || true
        {
            echo "[Match]"
            ...
            echo ""
            echo "[Network]"
            echo "Address=${RUMAHL_IP}/${RUMAHL_NETMASK:-24}"
            [ -n "${RUMAHL_GATEWAY:-}" ] && echo "Gateway=${RUMAHL_GATEWAY}"
            echo "DNS=${RUMAHL_DNS:-8.8.8.8}"
            [ -n "${RUMAHL_DNS2:-}" ]    && echo "DNS=${RUMAHL_DNS2}"
            if [ -n "${RUMAHL_IP6:-}" ]; then
                echo "Address=${RUMAHL_IP6}/${RUMAHL_PREFIX6:-64}"
                [ -n "${RUMAHL_GATEWAY6:-}" ] && echo "Gateway=${RUMAHL_GATEWAY6}"
                [ -n "${RUMAHL_DNS6:-}" ]     && echo "DNS=${RUMAHL_DNS6}"
    ...
    if [ -n "$RUMAHL_LOCALE" ]; then
        mkdir -p "${target}/etc" 2>/dev/null || true
        if [ -f "${target}/etc/locale.conf" ] || [ ! -f "${target}/etc/default/locale" ]; then
            echo "LANG=${RUMAHL_LOCALE}" > "${target}/etc/locale.conf" 2>/dev/null || true
        fi
        mkdir -p "${target}/etc/default" 2>/dev/null || true
        echo "LANG=${RUMAHL_LOCALE}" > "${target}/etc/default/locale" 2>/dev/null || true
    fi

    if [ -n "$RUMAHL_KEYBOARD" ]; then
        mkdir -p "${target}/etc" 2>/dev/null || true
        echo "KEYMAP=${RUMAHL_KEYBOARD}" > "${target}/etc/vconsole.conf" 2>/dev/null || true
        mkdir -p "${target}/etc/default" 2>/dev/null || true
        cat > "${target}/etc/default/keyboard" <<KBDCONF
XKBMODEL="pc105"
XKBLAYOUT="${RUMAHL_KEYBOARD}"
XKBVARIANT=""
XKBOPTIONS=""
BACKSPACE="guess"
KBDCONF
    ...
    # UEFI removable-path BOOTX64.EFI exist, and registers the rumahl NVRAM
...
# True iff the running installer ISO is itself an rumahl OS Dev build.
...
is_rumahl_dev_iso() {
    [ -f /etc/ora/os-dev-mode ]
}

screen_welcome() {
    local _dev_warn=""
    if is_rumahl_dev_iso; then
 ...
 +------------------------------------------------------------------+
 |   *** rumahl OS DEV BUILD -- INTERNAL USE ONLY ***                 |
...
DEVWARN
    ...
    if [ -n "$DIALOG_BIN" ]; then
        local choice
        choice=$(dlg --title " rumahl OS Installer " --menu "\
${_dev_warn} Welcome to rumahl OS!\n\n\
 Choose an installation mode:\n\n\
 \Zb\Z6Quick Install\Zn  --  Recommended for most users.\n\
   Set only a password and pick a disk.\n\
   Smart defaults are applied automatically.\n\n\
 \Zb\Z6Custom Install\Zn  --  Full control for advanced users.\n\
   Configure hostname, network, locale and more.\n" 28 76 5 \
            "quick"   "Quick Install    (recommended -- minimal questions)" \
            "install" "Custom Install   (full setup wizard -- all settings)" \
            "rescue"  "Rescue / Repair  (fix an existing installation)" \
 ...
 rumahl OS will be installed using smart defaults:\n\n\
   Hostname:    ora\n\
 ...
 At any step you can press Cancel to go back,\n\
 retry, jump to another step, or drop to a shell.\n\n\
 Select OK to continue." 20 72
                return 0
                ;;
            rescue)
            ...
            shell)
                ...
                clear 2>/dev/null || true
                echo ""
                echo "  rumahl Recovery Shell.  Type 'exit' to return to the installer."
        ...
        echo ""
        if is_rumahl_dev_iso; then
            echo "  *** rumahl OS DEV BUILD -- INTERNAL USE ONLY ***"
            echo "  =============================================="
            echo "  Not for production use."
            echo ""
        fi
        echo "  rumahl OS Installer"
        echo "  ================="
        echo ""
        echo "  How would you like to install rumahl OS?"
        echo ""
        echo "    1) Quick Install   -- Smart defaults, just timezone + password + disk"
        echo "    2) Custom Install  -- Full setup wizard (hostname, network, locale, ...)"
        echo "    3) Rescue / Repair -- Fix an existing installation"
        echo "    4) Recovery Shell  -- Expert troubleshooting"
        echo ""
        printf "  Choice [1]: "
        read _ch
        case "$_ch" in
            2) INSTALLER_MODE="install" ;;
            3) INSTALLER_MODE="rescue"; run_rescue_mode; screen_welcome; return 1 ;;
            4) INSTALLER_MODE="shell"; sh -i || true; screen_welcome; return 1 ;;
            *) INSTALLER_MODE="quick" ;;
        esac
    fi
}
...
# -- Rescue mode ---------------------------------------------------------
# Finds an existing rumahl installation, re-runs repair_disk_and_bootloader
# on it, optionally resets the root password, and dumps diagnostics.
run_rescue_mode() {
    # 1) Pick a disk that contains a plausible rumahl install
 ...
 Could not locate an existing rumahl OS installation on\n\
 any connected drive.\n\n\
 ...
 know where the root filesystem lives."
    ...
    if [ -n "$DIALOG_BIN" ]; then
        ...
        target_disk=$(dlg --title " Rescue - Select Disk " \
            --menu "\n Select the disk with the rumahl installation to repair.\n" \
            18 64 6 "$@" 3>&1 1>&2 2>&3)
 ...
 Choose a repair action.\n" 20 68 8 \
                ...
                "logs"    "Show /var/log/rumahl-netinstall.log from target" \
        ...
        case "$action" in
            ...
            logs)
                ...
                if mount -o ro "$p3" "$mnt" 2>/dev/null; then
                    log_path="$mnt/var/log/rumahl-netinstall.log"
                    if [ -f "$log_path" ]; then
...
screen_hostname() {
    ...
    while true; do
        ...
        result=$(dlg --title " Hostname " --inputbox \
            ...
            12 60 "$RUMAHL_HOSTNAME" 3>&1 1>&2 2>&3)
        [ $? -ne 0 ] && return 0

        if valid_hostname "$result"; then
            RUMAHL_HOSTNAME="$result"
            return 0
        fi
...
screen_timezone() {
    ...
    tz=$(dlg --title " Timezone " --menu \
        ...
        "Europe/London"        "United Kingdom (London, Manchester)" \
        ...
        "Europe/Helsinki"      "Finland (Helsinki, Tampere)" \
        "Europe/Athens"        "Greece (Athens, Thessaloniki)" \
        "Europe/Bucharest"     "Romania / Bulgaria" \
        "Europe/Istanbul"      "Turkey (Istanbul, Ankara)" \
        "Europe/Moscow"        "Russia (Moscow, Saint Petersburg)" \
        "Asia/Dubai"           "United Arab Emirates / Oman / Qatar" \
        "Asia/Kolkata"         "India / Sri Lanka" \
        "Asia/Dhaka"           "Bangladesh / Bhutan" \
        "Asia/Bangkok"         "Thailand / Vietnam / Cambodia" \
        "Asia/Singapore"       "Singapore / Malaysia / Indonesia (West)" \
        "Asia/Shanghai"        "China / Taiwan / Hong Kong" \
        "Asia/Tokyo"           "Japan" \
        ...
        "US/Eastern"           "USA East (New York, Miami, Boston)" \
        "US/Central"           "USA Central (Chicago, Dallas, Houston)" \
        "US/Mountain"          "USA Mountain (Denver, Phoenix, Salt Lake)" \
        "US/Pacific"           "USA West (Los Angeles, Seattle, SF)" \
        "America/Anchorage"    "Alaska" \
        "Pacific/Honolulu"     "Hawaii" \
        "America/Toronto"      "Canada East (Toronto, Montreal, Ottawa)" \
        "America/Winnipeg"     "Canada Central (Winnipeg, Regina)" \
        "America/Vancouver"    "Canada West (Vancouver, Calgary)" \
        "America/Sao_Paulo"    "Brazil (East: Sao Paulo, Rio, Brasilia)" \
        "America/Manaus"       "Brazil (West: Manaus, Amazonas)" \
        "America/Buenos_Aires" "Argentina (Buenos Aires, Cordoba)" \
        "America/Santiago"     "Chile (Santiago)" \
        "America/Bogota"       "Colombia / Ecuador / Peru (Lima)" \
        "America/Mexico_City"  "Mexico (Mexico City, Guadalajara)" \
        "Africa/Cairo"         "Egypt (Cairo, Alexandria)" \
        "Africa/Nairobi"       "East Africa (Kenya, Ethiopia, Tanzania)" \
        "Africa/Lagos"         "West Africa (Nigeria, Ghana, Senegal)" \
        "Africa/Johannesburg"  "South Africa (Johannesburg, Cape Town)" \
        "UTC"                  "UTC -- Coordinated Universal Time" \
        3>&1 1>&2 2>&3)
    [ $? -eq 0 ] && [ -n "$tz" ] && RUMAHL_TIMEZONE="$tz"
}
...
screen_locale() {
    ...
    locale=$(dlg --title " Language " --menu \
        ...
        "pl_PL.UTF-8" "Polish" \
        3>&1 1>&2 2>&3)
    [ $? -eq 0 ] && [ -n "$locale" ] && RUMAHL_LOCALE="$locale"
}
...
screen_network() {
    ...
    mode=$(dlg --title " Network " --menu \
        "\n Choose how rumahl OS should configure networking.\n" 15 60 3 \
 ...
 rumahl OS will request DHCP again after installation. It now uses a\n\
 ...
 when you log in, and at http://<IP>:8080."
    ...
    if [ "$mode" = "static" ]; then
        while true; do
            RUMAHL_IP=$(dlg --title " Static IPv4 " --inputbox \
                "\n Enter the IPv4 address for this device.\n" \
                10 60 "${RUMAHL_IP:-192.168.1.100}" 3>&1 1>&2 2>&3)
            [ $? -ne 0 ] && return 0
            valid_ipv4 "$RUMAHL_IP" && break
            dlg_msg " Invalid Address " "Enter a valid IPv4 address such as 192.168.1.100."
        done

        while true; do
            RUMAHL_NETMASK=$(dlg --title " Prefix Length " --inputbox \
                "\n Enter the subnet prefix length.\n Example: 24\n" \
                10 60 "${RUMAHL_NETMASK:-24}" 3>&1 1>&2 2>&3)
            [ $? -ne 0 ] && return 0
            valid_prefix_length "$RUMAHL_NETMASK" && break
            dlg_msg " Invalid Prefix " "Enter a number between 1 and 32."
        done

        while true; do
            RUMAHL_GATEWAY=$(dlg --title " Gateway " --inputbox \
                "\n Enter the default gateway IPv4 address.\n" \
                10 60 "${RUMAHL_GATEWAY:-192.168.1.1}" 3>&1 1>&2 2>&3)
            [ $? -ne 0 ] && return 0
            valid_ipv4 "$RUMAHL_GATEWAY" && break
            dlg_msg " Invalid Gateway " "Enter a valid IPv4 address such as 192.168.1.1."
        done

        while true; do
            RUMAHL_DNS=$(dlg --title " DNS Server " --inputbox \
                "\n Enter the preferred DNS server IPv4 address.\n" \
                10 60 "${RUMAHL_DNS:-8.8.8.8}" 3>&1 1>&2 2>&3)
            [ $? -ne 0 ] && return 0
            valid_ipv4 "$RUMAHL_DNS" && break
...
screen_user() {
 ...
 User account creation skipped."
    ...
    full=$(dlg --title " Full Name " --inputbox \
        "\n Full name (GECOS) for ${RUMAHL_USER}.\n Leave empty to skip.\n" \
        10 60 "" 3>&1 1>&2 2>&3)
    [ $? -ne 0 ] && full=""
    RUMAHL_USER_FULLNAME="$full"

    # Loop user password entry until matched, empty (lock), or cancelled.
    while true; do
        pw1=$(dlg --title " User Password " --insecure --passwordbox \
            "\n Password for ${RUMAHL_USER} (min. 6 characters, empty = lock account).\n" \
            10 60 3>&1 1>&2 2>&3)
        [ $? -ne 0 ] && return 1
        if [ -z "$pw1" ]; then
            RUMAHL_USER_PW=""
 ...
 Please enter the password and the confirmation again."
    ...
    if dlg --title " Administrator " --yesno \
        "\n Grant ${RUMAHL_USER} sudo (administrator) rights?\n" \
        8 60; then
        RUMAHL_USER_SUDO=true
    else
        RUMAHL_USER_SUDO=false
    fi
}
...
screen_select_disk() {
 ...
 rumahl OS requires at least ${MIN_DISK_GB} GB.\n\n\
...
#               (classic rumahl image via dd, this is the default)
...
screen_partitioning() {
 ...
 Falling back to 'Erase entire disk'."
            ...
            KEEP_TARGET_ROOT=$(dlg --title " Pick Root Partition " \
                --menu "\n Select the partition to install rumahl OS into.\n\
...
screen_confirm() {
    ...
    summary="${summary}\nSystem Settings\n"
    summary="${summary}  Hostname:   ${RUMAHL_HOSTNAME}\n"
    summary="${summary}  Timezone:   ${RUMAHL_TIMEZONE}\n"
    summary="${summary}  Locale:     ${RUMAHL_LOCALE}\n"
    summary="${summary}  Keyboard:   ${RUMAHL_KEYBOARD}\n"
    if [ "$RUMAHL_NETWORK" = "static" ]; then
        summary="${summary}  Network:    Static (${RUMAHL_IP}/${RUMAHL_NETMASK})\n"
    else
...
#   3. Reads /etc/rumahl-crypt.conf (baked into the initramfs) for the UUIDs
#   4. Runs `cryptsetup open` on the chosen slot (from kernel cmdline
#      root=/dev/mapper/rumahl_root_a | rumahl_root_b)
...
build_luks_initramfs() {
    ...
    work_dir=$(mktemp -d 2>/dev/null || echo "/tmp/rumahl-initrd.$$")
    ...
    # -- /etc/rumahl-crypt.conf inside initramfs -----------------------
    cat > "$work_dir/etc/rumahl-crypt.conf" <<CONF
LUKS_ROOT_A_UUID=${uuid_a}
LUKS_ROOT_B_UUID=${uuid_b}
LUKS_ROOT_A_NAME=rumahl_root_a
LUKS_ROOT_B_NAME=rumahl_root_b
CONF

    # -- /init script ------------------------------------------------
    cat > "$work_dir/init" <<'LUKSINIT'
#!/bin/sh
# rumahl LUKS initramfs
export PATH=/sbin:/usr/sbin:/bin:/usr/bin
...
# Parse kernel cmdline for rumahl_slot=a|b (set by grub entry) OR
# root=/dev/mapper/rumahl_root_X
slot="a"
for word in $(cat /proc/cmdline); do
    case "$word" in
        rumahl_slot=a) slot=a ;;
        rumahl_slot=b) slot=b ;;
        root=/dev/mapper/rumahl_root_a) slot=a ;;
        root=/dev/mapper/rumahl_root_b) slot=b ;;
    esac
done

# Load UUIDs
[ -r /etc/rumahl-crypt.conf ] && . /etc/rumahl-crypt.conf

if [ "$slot" = "b" ]; then
    uuid="$LUKS_ROOT_B_UUID"; name="${LUKS_ROOT_B_NAME:-rumahl_root_b}"
else
    uuid="$LUKS_ROOT_A_UUID"; name="${LUKS_ROOT_A_NAME:-rumahl_root_a}"
fi

if [ -z "$uuid" ]; then
    echo "rumahl initramfs: missing LUKS UUID in /etc/rumahl-crypt.conf"
    exec sh
fi

# Wait for backing device
echo "rumahl initramfs: waiting for /dev/disk/by-uuid/${uuid} ..."
i=0
...
if [ ! -e "/dev/disk/by-uuid/${uuid}" ]; then
    echo "rumahl initramfs: device with UUID ${uuid} not found, dropping to shell"
    exec sh
fi
...
while [ "$attempts" -lt 3 ]; do
    echo ""
    printf "Enter passphrase for rumahl OS root (%s): " "$name"
...
if [ ! -e "/dev/mapper/${name}" ]; then
    echo "rumahl initramfs: unlock failed, dropping to shell"
    exec sh
fi
...
if [ ! -x /mnt/root/sbin/init ] && [ ! -x /mnt/root/lib/systemd/systemd ] \
   && [ ! -x /mnt/root/bin/sh ]; then
    echo "rumahl initramfs: /mnt/root has no init, dropping to shell"
    exec sh
fi
...
echo "rumahl initramfs: switch_root failed"
exec sh
LUKSINIT
...
encrypt_root_partitions() {
    ...
    _encrypt_one() {
        ...
        # partition so this is always safe for rumahl's 2 GB root partitions.
    ...
    _encrypt_one "$root_a" "rumahl_root_a" || return 1
    _encrypt_one "$root_b" "rumahl_root_b" || return 1

    # Open A, rewrite /etc/crypttab + /etc/default/grub inside, close.
    if ! printf '%s' "$LUKS_PASSPHRASE" | cryptsetup open --batch-mode "$root_a" rumahl_root_a 2>&1; then
    ...
    mkdir -p "$mnt"
    if ! mount /dev/mapper/rumahl_root_a "$mnt" 2>&1; then
        echo "cannot mount encrypted root A"
        cryptsetup close rumahl_root_a 2>/dev/null || true
        return 1
    fi
...
# Generated by rumahl installer
rumahl_root_a UUID=${uuid_a} none luks,discard
rumahl_root_b UUID=${uuid_b} none luks,discard
CRYPTTAB

    # Write /etc/rumahl-crypt.conf so the initramfs hook picks it up.
    cat > "$mnt/etc/rumahl-crypt.conf" <<CRYPTCONF
LUKS_ROOT_A_UUID=${uuid_a}
LUKS_ROOT_B_UUID=${uuid_b}
LUKS_ROOT_A_NAME=rumahl_root_a
LUKS_ROOT_B_NAME=rumahl_root_b
...
# Generated by rumahl installer (LUKS)
insmod cryptodisk
insmod luks
...
menuentry "rumahl OS (encrypted)" {
    cryptomount -u ${uuid_a}
    linux /boot/vmlinuz root=/dev/mapper/rumahl_root_a rootwait ro rootfstype=ext4 loglevel=4 systemd.show_status=true printk.devkmsg=on rumahl_slot=a cryptdevice=UUID=${uuid_a}:rumahl_root_a
${initrd_line}
}

menuentry "rumahl OS - second slot (encrypted)" {
    cryptomount -u ${uuid_b}
    linux /boot/vmlinuz root=/dev/mapper/rumahl_root_b rootwait ro rootfstype=ext4 loglevel=4 systemd.show_status=true printk.devkmsg=on rumahl_slot=b cryptdevice=UUID=${uuid_b}:rumahl_root_b
${initrd_line}
}

menuentry "rumahl OS Recovery (encrypted)" {
    cryptomount -u ${uuid_a}
    linux /boot/vmlinuz root=/dev/mapper/rumahl_root_a rootwait rw rootfstype=ext4 init=/bin/sh rumahl_slot=a cryptdevice=UUID=${uuid_a}:rumahl_root_a
${initrd_line}
}
LUKSGRUB
        sync
...
# -- Keep-mode installer -------------------------------------------------
# Installs rumahl OS into an existing partition without touching other
...
install_keep_mode() {
    ...
    # is a disk image (rumahl-os.img.xz) containing the full A/B layout. We only
    ...
    local tmp_img="/tmp/rumahl-payload.img"
    ...
    (
        ...
        mkfs.ext4 -F -L rumahl-root "$root_part" >>"$progress" 2>&1 || {
        ...
        if [ -b "${loop}p2" ] && [ -n "$esp_part" ]; then
            ...
            if mount -o ro -t vfat "${loop}p2" /mnt/keep-src-esp 2>>"$progress" \
               && mount -t vfat "$esp_part" /mnt/keep-dst-esp 2>>"$progress"; then
                # Preserve existing ESP content (other OSes); add/overwrite only rumahl files.
                [ -f /mnt/keep-src-esp/vmlinuz ] && cp -f /mnt/keep-src-esp/vmlinuz /mnt/keep-dst-esp/rumahl-vmlinuz 2>>"$progress" || true
                mkdir -p /mnt/keep-dst-esp/EFI/rumahl 2>/dev/null || true
...
screen_complete() {
    ...
    local rumahl_ip=""
    local ip_note=""
    if [ "$RUMAHL_NETWORK" = "static" ] && [ -n "$RUMAHL_IP" ]; then
        rumahl_ip="$RUMAHL_IP"
    else
        ...
        if [ -z "$rumahl_ip" ]; then
            rumahl_ip="${RUMAHL_HOSTNAME}.local"
 ...
 rumahl OS has been written to /dev/${SEL_DISK}.
...
${ip_note}
 ...
 First-boot settings:
     Hostname: ${RUMAHL_HOSTNAME}
     Timezone: ${RUMAHL_TIMEZONE}
 ...
 Remove the installation media before continuing.\n" \
        ...
        echo ""
        echo "  rumahl OS installed successfully!"
        echo ""
        echo "  After rebooting, open a browser:"
        echo "    http://${rumahl_ip}:8080"
...
wizard_step() {
    ...
    local step_name="$2"
    local step_func="$3"
    # Update backtitle with current step progress
    local _total="${WIZARD_TOTAL_STEPS:-12}"
    if [ -f /etc/ora/os-dev-mode ]; then
        BACKTITLE="rumahl OS Installer [DEV]  |  Step ${step_num}/${_total}: ${step_name}"
    else
        BACKTITLE="rumahl OS Installer  |  Step ${step_num}/${_total}: ${step_name}"
    fi
    if "$step_func"; then
...
# -- Quick (Express) Install wizard ---------------------------------
# A simplified 4-step wizard for non-technical users.
# Uses smart defaults; only asks timezone, password, disk, confirm.
run_express_wizard() {
    WIZARD_TOTAL_STEPS=4

    # -- Mount media ------------------------------------------------
    BACKTITLE="rumahl OS Installer  |  Quick Install: Loading image..."
    ...
    if [ "$mounted" = false ]; then
        ...
        [ -z "$_blkdevs" ] && _blkdevs=" (none detected)"
        dlg_msg " Media Not Found " "\
 Could not find the rumahl OS image.\n\n\
 Make sure the installer ISO or USB is connected\n\
 and contains the installer payload.\n\n\
 ...
 that cannot create a bootable installation.\n\n\
 Rebuild the image with:\n\
   sudo ./build.sh all --force-full-image"
        return 1
    fi
 ...
 Boot the installer in UEFI mode and try again."
        return 1
    fi
 ...
 Installation will not continue."
    ...
    # Apply smart defaults for quick install
    RUMAHL_INSTALL_MODE="guided"
    RUMAHL_NETWORK="dhcp"
    PARTITION_MODE="auto"

    # -- Step 1: Timezone ------------------------------------------
    BACKTITLE="rumahl OS Installer  |  Quick Install -- Step 1/4: Timezone"
    local tz
    ...
    BACKTITLE="rumahl OS Installer  |  Quick Install -- Step 2/4: Root Password"
    while true; do
        local pw1 pw2 pw_rc
 ...
 Please try again."
    ...
    BACKTITLE="rumahl OS Installer  |  Quick Install -- Step 3/4: Target Disk"
    ...
    BACKTITLE="rumahl OS Installer  |  Quick Install -- Step 4/4: Confirm"
    if ! _wizard_final_confirm; then
        dlg_msg " Cancelled " "Installation cancelled.\nNo changes were made."
        return 1
    fi

    # -- Install ---------------------------------------------------
    BACKTITLE="rumahl OS Installer  |  Quick Install: Installing..."
...
run_wizard() {
    ...
    detect_virtualization
    if [ "$RUMAHL_VIRT_TYPE" != "none" ] || [ "$RUMAHL_VIRT_CONTAINER" != "none" ]; then
        BACKTITLE="rumahl OS Installer  |  ${RUMAHL_VIRT_LABEL}  |  Tab/Arrows navigate, Enter confirms"
    fi
    ...
    # Route to Quick Install if the user chose that mode
    if [ "$INSTALLER_MODE" = "quick" ]; then
        run_express_wizard
        return $?
    fi

    # -- Full Custom Install (12-step wizard) ----------------------
...
_wizard_final_confirm() {
 ...
 Do you want to erase the entire drive and continue?"; then
    ...
    if [ "$RUMAHL_INSTALL_MODE" = "guided-safe" ]; then
...
echo ""
echo "  +--------------------------------------------------+"
echo "  |      rumahl OS Installer -- Recovery Shell         |"
echo "  +--------------------------------------------------+"
echo ""
echo "  install   -- Restart the rumahl OS installation wizard"
echo "  sysinfo   -- Display system hardware information"
echo "  netsetup  -- Configure network via DHCP"
echo "  reboot    -- Reboot the system"
echo "  poweroff  -- Shut down the system"
echo "  help      -- Show this help message"
echo ""
echo "  Tip: Type 'install' to return to the installer."
echo ""
SHEOF
...
echo "    sysinfo   -- Show system hardware information"
echo "    netsetup  -- Configure network via DHCP"
echo "    reboot    -- Reboot the system"
echo "    poweroff  -- Shut down"
echo "    help      -- Show all available commands"
echo ""
echo "  Type 'install' to return to the installer."
echo ""
...
create_bootable_installer_iso() {
    ...
    if [ -f "${FALLBACK_MARKER}" ]; then
        ...
        log_info "Rebuild with privileges and full post-image flow: sudo ./build.sh all --force-full-image --progress"
        mark_skipped "rumahl-os-installer-boot.iso (fallback payload not bootable)"
        return
    fi

    if ! command -v grub-mkrescue &> /dev/null; then
        log_warn "grub-mkrescue not available, skipping bootable installer ISO"
        mark_skipped "rumahl-os-installer-boot.iso (missing grub-mkrescue)"
        return
    fi

    if [ ! -f "${OUTPUT_DIR}/bzImage" ]; then
        log_warn "bzImage missing, skipping bootable installer ISO"
        mark_skipped "rumahl-os-installer-boot.iso (missing bzImage)"
        return
    fi
    ...
    if ! installer_initrd=$(resolve_installer_initrd); then
        ...
        mark_skipped "rumahl-os-installer-boot.iso (missing initrd source)"
        return
    fi
    ...
    if ! build_installer_runtime_initrd "${installer_initrd}" "${runtime_initrd}"; then
        ...
        mark_skipped "rumahl-os-installer-boot.iso (failed to create runtime initrd)"
        return
    fi

    if [ ! -f "${RELEASE_DIR}/rumahl-os.img.xz" ]; then
        log_warn "rumahl-os.img.xz missing in release directory, skipping bootable installer ISO"
        mark_skipped "rumahl-os-installer-boot.iso (missing rumahl-os.img.xz)"
        return
    fi
    ...
    cp "${RELEASE_DIR}/rumahl-os.img.xz" "${stage_dir}/rumahl-os.img.xz"
    write_payload_metadata "${stage_dir}"

    # Generate SHA256 checksum for integrity verification
    log_info "Generating SHA256 checksum for rumahl-os.img.xz..."
    (cd "${stage_dir}" && sha256sum rumahl-os.img.xz > rumahl-os.img.xz.sha256)

    # Copy additional release artifacts if present
    for f in "${RELEASE_DIR}/rumahl-os.qcow2.xz" \
             "${RELEASE_DIR}/rumahl-os.vdi.xz" \
             "${RELEASE_DIR}/rumahl-os.vmdk.xz" \
             "${RELEASE_DIR}/rumahl-os-update.raucb"; do
        if [ -f "$f" ]; then
    ...
    cat "${stage_dir}/rumahl-os.img.xz.sha256" >> "${stage_dir}/checksums.sha256" 2>/dev/null || true
...
Kernel:  ${kernel_ver}
Image:   rumahl-os.img.xz ($(du -h "${stage_dir}/rumahl-os.img.xz" | cut -f1))
VEOF

    cat > "${stage_dir}/README-INSTALLER.txt" <<'EOF'
rumahl OS Bootable Installer ISO

This ISO boots into an interactive installation wizard.
Follow the on-screen prompts to select a target disk and install.

Included files:
- rumahl-os.img.xz           Compressed raw disk image
- rumahl-os.img.xz.sha256    SHA256 checksum
...
Manual install from shell:
  ...
  sha256sum -c /mnt/iso/rumahl-os.img.xz.sha256
  xzcat /mnt/iso/rumahl-os.img.xz | dd of=/dev/sdX bs=4M status=progress
  sync && reboot
EOF
...
menuentry "rumahl OS Installer" {
    linux /boot/vmlinuz nomodeset
    initrd /boot/initrd.img
}

menuentry "rumahl OS Installer (serial console)" {
    linux /boot/vmlinuz console=tty0 console=ttyS0,115200 nomodeset
    initrd /boot/initrd.img
}

menuentry "rumahl OS Installer (safe mode)" {
...
EOF
    ...
    if grub-mkrescue \
        ...
        -o "${RELEASE_DIR}/rumahl-os-installer-boot.iso" \
    ...
    if [ "${grub_ok}" -eq 1 ] && command -v xorriso >/dev/null 2>&1; then
        local _xorep
        _xorep=$(xorriso -indev "${RELEASE_DIR}/rumahl-os-installer-boot.iso" \
        ...
        if [ "${iso_uefi_ok}" -eq 0 ]; then
            if xorriso -indev "${RELEASE_DIR}/rumahl-os-installer-boot.iso" \
    ...
    if [ "${grub_ok}" -eq 1 ] && [ "${iso_uefi_ok}" -eq 0 ]; then
        ...
        cp -f "${RELEASE_DIR}/rumahl-os-installer-boot.iso" \
              "${RELEASE_DIR}/rumahl-os-installer-boot.iso.bios-only" 2>/dev/null || true
        grub_ok=0
    fi
    ...
    if [ "${grub_ok}" -eq 0 ] && [ "${have_xorriso}" -eq 1 ] && \
       [ "${have_mtools}" -eq 1 ] && [ "${have_grub_efi}" -eq 1 ]; then
        ...
        if grub-mkstandalone --format=x86_64-efi \
            ...
            mformat -i "${efi_img}" -F -v "RUMAHL_EFI" :: >>"${grub_log}" 2>&1 || true
    ...
    if [ "${grub_ok}" -eq 0 ]; then
        ...
        if [ -f "${RELEASE_DIR}/rumahl-os-installer-boot.iso.bios-only" ]; then
            mv -f "${RELEASE_DIR}/rumahl-os-installer-boot.iso.bios-only" \
                  "${RELEASE_DIR}/rumahl-os-installer-boot.iso"
        ...
        rm -f "${RELEASE_DIR}/rumahl-os-installer-boot.iso.bios-only" 2>/dev/null || true
    fi
    ...
    if [ "${grub_ok}" -eq 0 ]; then
        mark_skipped "rumahl-os-installer-boot.iso (grub-mkrescue and fallback both failed)"
        return
    fi

    # ── Verify that rumahl-os.img.xz is actually embedded in the ISO ────────────
    ...
    # filename restrictions.  A missing payload causes "Could not find the rumahl
    # OS image" at install time.  Catch this at build time instead.
    local payload_ok=0
    if command -v xorriso >/dev/null 2>&1; then
        if xorriso -indev "${RELEASE_DIR}/rumahl-os-installer-boot.iso" \
                   -find / -name "rumahl-os.img.xz" 2>/dev/null | grep -q "rumahl-os.img.xz"; then
            payload_ok=1
        fi
    elif command -v isoinfo >/dev/null 2>&1; then
        if isoinfo -i "${RELEASE_DIR}/rumahl-os-installer-boot.iso" \
                   -l 2>/dev/null | grep -qi "rumahl-os.img"; then
            payload_ok=1
        fi
    else
    ...
    if [ "${payload_ok}" -eq 0 ]; then
        log_error "rumahl-os.img.xz is NOT embedded inside rumahl-os-installer-boot.iso!"
        log_error "The installer would fail with 'Could not find the rumahl OS image'."
        ...
        rm -f "${RELEASE_DIR}/rumahl-os-installer-boot.iso" 2>/dev/null || true
        mark_skipped "rumahl-os-installer-boot.iso (payload rumahl-os.img.xz missing from ISO)"
        return
    fi

    local size
    size=$(du -h "${RELEASE_DIR}/rumahl-os-installer-boot.iso" | cut -f1)
    log_success "Bootable installer ISO created: rumahl-os-installer-boot.iso (${size})"
    mark_created "rumahl-os-installer-boot.iso"
}

create_raw_image() {
    log_info "Creating raw disk image (img.xz)..."

    if [ ! -f "${OUTPUT_DIR}/rumahl-os.img" ]; then
        log_error "Base image not found at ${OUTPUT_DIR}/rumahl-os.img"
        return 1
    fi

    # Always regenerate compressed output to avoid stale artifacts.
    cd "${OUTPUT_DIR}"
    rm -f rumahl-os.img.xz
    log_info "Compressing raw image with xz..."
    xz_compress_file "rumahl-os.img" "rumahl-os.img.xz"

    cp rumahl-os.img.xz "${RELEASE_DIR}/"
    ...
    local size=$(du -h "${RELEASE_DIR}/rumahl-os.img.xz" | cut -f1)
    log_success "Raw image created: rumahl-os.img.xz (${size})"
    mark_created "rumahl-os.img.xz"
}
...
create_qcow2_image() {
    ...
    if ! require_bootable_base_image; then
        mark_skipped "rumahl-os.qcow2.xz (base image not validated bootable)"
        return
    fi

    if ! command -v qemu-img &> /dev/null; then
        log_warn "qemu-img not available, skipping qcow2"
        mark_skipped "rumahl-os.qcow2.xz (missing qemu-img)"
        return
    fi

    cd "${OUTPUT_DIR}"
    rm -f rumahl-os.qcow2 rumahl-os.qcow2.xz

    # Convert to qcow2
    log_info "Converting to qcow2 format..."
    qemu-img convert -f raw -O qcow2 -c rumahl-os.img rumahl-os.qcow2

    # Compress with xz
    log_info "Compressing qcow2 with xz..."
    xz_compress_file "rumahl-os.qcow2" "rumahl-os.qcow2.xz"

    cp rumahl-os.qcow2.xz "${RELEASE_DIR}/"
    rm -f rumahl-os.qcow2  # Clean up uncompressed intermediate file

    local size=$(du -h "${RELEASE_DIR}/rumahl-os.qcow2.xz" | cut -f1)
    log_success "QEMU image created: rumahl-os.qcow2.xz (${size})"
    mark_created "rumahl-os.qcow2.xz"
}
...
create_vdi_image() {
    ...
    if ! require_bootable_base_image; then
        mark_skipped "rumahl-os.vdi.zip (base image not validated bootable)"
        return
    fi

    if ! command -v qemu-img &> /dev/null; then
        log_warn "qemu-img not available, skipping VDI"
        mark_skipped "rumahl-os.vdi.zip (missing qemu-img)"
        return
    fi

    cd "${OUTPUT_DIR}"
    rm -f rumahl-os.vdi rumahl-os.vdi.zip

    # Convert to VDI
    log_info "Converting to VDI format..."
    qemu-img convert -f raw -O vdi rumahl-os.img rumahl-os.vdi

    # Create zip archive
    log_info "Creating VDI zip archive..."
    zip -9 rumahl-os.vdi.zip rumahl-os.vdi

    cp rumahl-os.vdi.zip "${RELEASE_DIR}/"
    rm -f rumahl-os.vdi  # Clean up intermediate file

    local size=$(du -h "${RELEASE_DIR}/rumahl-os.vdi.zip" | cut -f1)
    log_success "VirtualBox image created: rumahl-os.vdi.zip (${size})"
    mark_created "rumahl-os.vdi.zip"
}
...
create_vmdk_image() {
    ...
    if ! require_bootable_base_image; then
        mark_skipped "rumahl-os.vmdk.zip (base image not validated bootable)"
        return
    fi

    if ! command -v qemu-img &> /dev/null; then
        log_warn "qemu-img not available, skipping VMDK"
        mark_skipped "rumahl-os.vmdk.zip (missing qemu-img)"
        return
    fi

    cd "${OUTPUT_DIR}"
    rm -f rumahl-os.vmdk rumahl-os.vmdk.zip

    # Convert to VMDK
    log_info "Converting to VMDK format..."
    qemu-img convert -f raw -O vmdk rumahl-os.img rumahl-os.vmdk

    # Create zip archive
    log_info "Creating VMDK zip archive..."
    zip -9 rumahl-os.vmdk.zip rumahl-os.vmdk

    cp rumahl-os.vmdk.zip "${RELEASE_DIR}/"
    rm -f rumahl-os.vmdk  # Clean up intermediate file

    local size=$(du -h "${RELEASE_DIR}/rumahl-os.vmdk.zip" | cut -f1)
    log_success "VMware image created: rumahl-os.vmdk.zip (${size})"
    mark_created "rumahl-os.vmdk.zip"
}
...
create_ova_image() {
    ...
    if ! require_bootable_base_image; then
        mark_skipped "rumahl-os.ova (base image not validated bootable)"
        return
    fi

    if ! command -v VBoxManage &> /dev/null; then
        log_warn "VBoxManage not available, skipping OVA"
        mark_skipped "rumahl-os.ova (missing VBoxManage)"
        return
    fi
    ...
    VBoxManage storageattach "${VM_UUID}" \
        ...
        --device 0 \
        --type hdd \
        --medium "$(pwd)/rumahl-os.vdi"

    # Export to OVA
    log_info "Exporting to OVA format..."
    VBoxManage export "${VM_UUID}" \
        --output "${RELEASE_DIR}/rumahl-os.ova" \
        --vsys 0 \
        --product "${VM_NAME}" \
        --producturl "https://github.com/your-org/ora" \
        --vendor "rumahl Project" \
        --version "${VM_VERSION}" \
        --description "rumahl - Interface for Optimized Residential Autonomy"

    # Cleanup
    log_info "Cleaning up temporary VM..."
    VBoxManage unregistervm "${VM_UUID}" --delete

    local size=$(du -h "${RELEASE_DIR}/rumahl-os.ova" | cut -f1)
    log_success "OVA created: rumahl-os.ova (${size})"
    mark_created "rumahl-os.ova"
}
...
create_rauc_bundle() {
    ...
    if ! command -v rauc &> /dev/null; then
        ...
        mark_skipped "rumahl-os-YYYYMMDD.raucb (missing rauc)"
        return
    fi
    ...
    if [ ! -f "${RAUC_CERT}" ] || [ ! -f "${RAUC_KEY}" ]; then
        ...
        mark_skipped "rumahl-os-YYYYMMDD.raucb (missing signing keys)"
        return
    fi
    ...
    rauc bundle \
        ...
        --key="${RAUC_KEY}" \
        "${BUNDLE_DIR}" \
        "${RELEASE_DIR}/rumahl-os-$(date +%Y%m%d).raucb"

    # Cleanup
    rm -rf "${BUNDLE_DIR}"

    local size=$(du -h "${RELEASE_DIR}/rumahl-os-"*.raucb | cut -f1)
    log_success "RAUC bundle created: rumahl-os-$(date +%Y%m%d).raucb (${size})"
    mark_created "rumahl-os-$(date +%Y%m%d).raucb"
}
...
1. rumahl-os.img.xz
   ...
   - Usage: xzcat rumahl-os.img.xz | sudo dd of=/dev/sdX bs=4M status=progress

2. rumahl-os-installer.iso
    - Archive ISO containing rumahl-os.img.xz and install notes
    - Not directly bootable as an installer medium
    - Usage: Mount/extract the ISO, then flash rumahl-os.img.xz to your target disk

3. rumahl-os-installer-boot.iso
    - Bootable installer ISO (GRUB/UEFI, hybrid BIOS+UEFI)
    - Includes kernel, initrd, and rumahl-os.img.xz payload
...
8. rumahl-os-YYYYMMDD.raucb
   - RAUC update bundle for existing rumahl OS installations
   - Usage: rauc install rumahl-os-YYYYMMDD.raucb
...
Documentation:
- https://github.com/your-org/ora
- See ARCHITECTURE.md for system architecture
- See rumahl-os/README.md for detailed OS documentation
...
- Discussions: https://github.com/your-org/ora/discussions
EOF
...
print_summary() {
    echo ""
    log_success "============================================"
    log_success "rumahl OS Build Complete!"
    log_success "============================================"
    echo ""
...
# ── Publish to rumahl Update Server ─────────────────────────────────────────────

publish_to_update_server() {
    if [ -z "${RUMAHL_UPDATE_API_KEY:-}" ]; then
        log_info "Skipping update server publish (RUMAHL_UPDATE_API_KEY not set)"
        log_info "  To publish, set: export RUMAHL_UPDATE_API_KEY=<your-api-key>"
        return 0
    fi

    local update_server="${RUMAHL_UPDATE_SERVER:-https://update.rumahl.com}"
    local download_server="${RUMAHL_DOWNLOAD_SERVER:-https://download.rumahl.com}"
    local version
    version=$(date +%Y%m%d)
    local channel="${RUMAHL_RELEASE_CHANNEL:-stable}"
    ...
    local raucb_file="${RELEASE_DIR}/rumahl-os.raucb"
    if [ -f "$raucb_file" ]; then
        ...
        upload_resp=$(curl -sSf -X POST "${download_server}/v1/admin/upload" \
            -H "Authorization: Bearer ${RUMAHL_UPDATE_API_KEY}" \
            -F "file=@${raucb_file}" \
            -F "project=rumahl-os" \
            -F "version=${version}" \
            -F "platform=oraos-x86_64" \
            2>/dev/null) || {
            log_warn "Failed to upload RAUC bundle to download server"
        }

        # Publish OS release metadata to update server
        curl -sSf -X POST "${update_server}/v1/admin/ora/os/publish" \
            -H "Authorization: Bearer ${RUMAHL_UPDATE_API_KEY}" \
            -H "Content-Type: application/json" \
            -d "{
                \"version\": \"${version}\",
                \"channel\": \"${channel}\",
                \"platform\": \"oraos-x86_64\",
                \"file_name\": \"rumahl-os.raucb\",
                \"file_size\": ${raucb_size},
                \"sha256_checksum\": \"${raucb_sha256}\",
                \"file_path\": \"rumahl-os/${version}/oraos-x86_64/rumahl-os.raucb\",
                \"rauc_compatible\": \"rumahl-os\",
    ...
    local img_xz="${RELEASE_DIR}/rumahl-os.img.xz"
    if [ -f "$img_xz" ]; then
        local img_size
        ...
        curl -sSf -X POST "${update_server}/v1/admin/ora/installer" \
            -H "Authorization: Bearer ${RUMAHL_UPDATE_API_KEY}" \
            -H "Content-Type: application/json" \
            -d "{
                \"os_version\": \"${version}\",
                \"os_channel\": \"${channel}\",
                \"architecture\": \"x86_64\",
                \"image_url\": \"${download_server}/v1/rumahl-os/${version}/oraos-x86_64\",
...
#   dl/          →  ${SCRIPT_DIR}/.rumahl-dl-cache/  (downloaded source tarballs)
#   ccache dir   →  set via BR2_CCACHE_DIR           (compiler cache)
#   ~/.rumahl-cache/  cargo/sccache                   (Rust build cache)

RUMAHL_RAM_MIN_MB=16384     # minimum recommended RAM for RAM build (16 GB)
RUMAHL_RAM_OUTPUT_GB=30     # estimated output/ size for a full build
RUMAHL_RAM_AGGRESSIVE="${RUMAHL_RAM_AGGRESSIVE:-0}"  # --ram-aggressive: dl/ + caches in tmpfs too
...
setup_ram_build() {
    ...
    if ! mountpoint -q /tmp 2>/dev/null || ! df -t tmpfs /tmp >/dev/null 2>&1; then
        ...
        if [ "${UNATTENDED}" != true ] && [ "${_RUMAHL_RAM_TMP_WARNED:-0}" = "0" ]; then
            _RUMAHL_RAM_TMP_WARNED=1
        fi
    else
    ...
    if [ "${_total_ram_kb}" -lt $(( RUMAHL_RAM_MIN_MB * 1024 )) ]; then
        log_warn "  Less than ${RUMAHL_RAM_MIN_MB} MB RAM detected."
    ...
    if [ "${_total_ram_kb}" -lt $(( 32 * 1024 * 1024 )) ] && [ "${RUMAHL_FAST_BUILD:-0}" != "1" ]; then
        log_info "  RAM < 32 GB → auto-enabling RUMAHL_FAST_BUILD=1 (lto=off, faster compilation)"
        log_info "    This reduces peak memory usage and build time."
        RUMAHL_FAST_BUILD=1
        _RUMAHL_RAM_AUTO_FAST=1
    fi
    ...
    if [ -n "${RUMAHL_RAM_SIZE}" ]; then
        ...
        if [ "${_tmpfs_size_kb}" -le 0 ]; then
            log_error "Invalid --ram-size: ${RUMAHL_RAM_SIZE} (expected e.g. 32G, 16G, 48G)"
            exit 1
        fi
        log_info "  tmpfs size (user-specified): ${RUMAHL_RAM_SIZE}"
    else
    ...
    if ! ${_mount_cmd} -t tmpfs -o "size=${_tmpfs_size_kb}k,mode=0755" tmpfs "${_output_dir}"; then
        ...
        if [ "${_RUMAHL_RAM_HAD_PREVIOUS_OUTPUT:-0}" = "1" ]; then
    ...
    if [ "${_RUMAHL_RAM_HAD_PREVIOUS_OUTPUT:-0}" = "1" ] && [ -d "${_output_backup}/host" ]; then
    ...
    if [ "${RUMAHL_RAM_AGGRESSIVE}" = "1" ]; then
        ...
        log_info "  dl/        → ${RUMAHL_PERSISTENT_DL_DIR:-${SCRIPT_DIR}/.rumahl-dl-cache} (disk)"
    ...
    if [ "${RUMAHL_RAM_AGGRESSIVE}" = "1" ]; then
        ...
        if [ -L "${_dl_link}" ] && [ "$(readlink -f "${_dl_link}" 2>/dev/null)" = "$(readlink -f "${RUMAHL_PERSISTENT_DL_DIR}" 2>/dev/null)" ]; then
            ...
            if [ -e "${_dl_link}" ] && [ ! -L "${_dl_link}" ]; then
                ...
                cp -a "${_dl_link}/." "${RUMAHL_PERSISTENT_DL_DIR}/" 2>/dev/null || true
                rm -rf "${_dl_link}"
            fi
            if [ ! -e "${_dl_link}" ]; then
                ln -sfn "${RUMAHL_PERSISTENT_DL_DIR}" "${_dl_link}"
            fi
        fi
    fi
...
teardown_ram_build() {
    ...
    if [ "${RUMAHL_RAM_KEEP}" = "1" ]; then
        ...
        log_warn "  To persist: cp -a ${_output_dir}/.ccache ${HOME}/.rumahl-cache/ccache"
    ...
    if [ -d "${_ram_ccache}" ] && [ -n "$(ls -A "${_ram_ccache}" 2>/dev/null)" ]; then
        local _ccache_disk="${_RUMAHL_RAM_CCACHE_DISK:-${HOME}/.rumahl-cache/ccache}"
    ...
    if [ -d "${_ram_sccache}" ] && [ -n "$(ls -A "${_ram_sccache}" 2>/dev/null)" ]; then
        local _sccache_disk="${_RUMAHL_RAM_SCCACHE_DISK:-${HOME}/.rumahl-cache/sccache}"
    ...
    local _ram_cargo="${_output_dir}/.cargo-target"
    if [ -d "${_ram_cargo}" ] && [ -n "$(ls -A "${_ram_cargo}" 2>/dev/null)" ] && [ "${RUMAHL_RAM_KEEP_CARGO:-0}" = "1" ]; then
        local _cargo_disk="${HOME}/.rumahl-cache/cargo-target"
        mkdir -p "${_cargo_disk}"
    ...
    if [ -d "${_output_dir}/build" ] && [ "${RUMAHL_RAM_SAVE_BUILD:-0}" = "1" ]; then
    ...
    if [ "${_RUMAHL_RAM_MOUNTED:-0}" = "1" ]; then
        ...
        _RUMAHL_RAM_MOUNTED=0
    elif [ "${_RUMAHL_RAM_ALREADY_MOUNTED:-0}" = "1" ]; then
...
main() {
    ...
    log_info "Starting rumahl OS complete image build..."
    ...
    if [ "${RUMAHL_RAM_BUILD}" = "1" ]; then
        ...
        log_info "  RAM size limit: ${RUMAHL_RAM_SIZE:-80% of available}"
        log_info "  RAM keep: $([ "${RUMAHL_RAM_KEEP}" = "1" ] && echo yes || echo no)"
        log_info "  Aggressive: $([ "${RUMAHL_RAM_AGGRESSIVE}" = "1" ] && echo yes || echo no)"
    fi
    echo ""
    ...
    if [ "${IMAGES_ONLY}" = false ]; then
        ...
        # so rumahl-home (port 8126) can serve it instead of the embedded fallback.
        build_frontend_bundle
        # Pre-compile rumahl service binaries and embed them in the rootfs overlay
        # so the on-device self-build (rumahl-build-images.service) has binaries
        ...
        if [ ! -f "${OUTPUT_DIR}/rumahl-os.img" ]; then
            log_error "Missing base image: ${OUTPUT_DIR}/rumahl-os.img"
    ...
    if want_artifact raw; then create_raw_image; else mark_skipped "rumahl-os.img.xz (disabled by artifact selection)"; fi
    if want_artifact iso; then create_iso_image; else mark_skipped "rumahl-os-installer.iso (disabled by artifact selection)"; fi
    if want_artifact boot-iso; then create_bootable_installer_iso; else mark_skipped "rumahl-os-installer-boot.iso (disabled by artifact selection)"; fi
    if want_artifact qcow2; then create_qcow2_image; else mark_skipped "rumahl-os.qcow2.xz (disabled by artifact selection)"; fi
    if want_artifact vdi; then create_vdi_image; else mark_skipped "rumahl-os.vdi.zip (disabled by artifact selection)"; fi
    if want_artifact vmdk; then create_vmdk_image; else mark_skipped "rumahl-os.vmdk.zip (disabled by artifact selection)"; fi
    if want_artifact ova; then create_ova_image; else mark_skipped "rumahl-os.ova (disabled by artifact selection)"; fi
    if want_artifact rauc; then create_rauc_bundle; else mark_skipped "rumahl-os-YYYYMMDD.raucb (disabled by artifact selection)"; fi
    create_checksums
    create_readme
