...
# Assumes the backend tree is at /home/hermes/ora/rumahl-os/backend/
#
# Fixes:
#   1. rumahl-home/src/documentation.rs — E0733 (recursive async fn) → iterative BFS
#   2. rumahl-cli/Cargo.toml            — duplicate bin target (ora + ora same path)
#   3. rumahl-cli/Cargo.toml            — clap `env` feature
#   4. rumahl-supervisor                — futures-util + TryStreamExt (if applicable)

set -euo pipefail

BACKEND="${BACKEND:-/home/hermes/ora/rumahl-os/backend}"
if [[ ! -d "$BACKEND" ]]; then
  # fallback for WSL path layout
  if [[ -d "/home/hermes/ora/rumahl-os/backend" ]]; then
    BACKEND="/home/hermes/ora/rumahl-os/backend"
  else
...
#─────────────────────────────────────────────────────────────────────────
# 1) rumahl-home/src/documentation.rs — recursive async fn
#─────────────────────────────────────────────────────────────────────────
DOC="$BACKEND/rumahl-home/src/documentation.rs"
if [[ -f "$DOC" ]]; then
...
#─────────────────────────────────────────────────────────────────────────
# 2) rumahl-cli/Cargo.toml — make ora a thin shim so no duplicate bin target
#─────────────────────────────────────────────────────────────────────────
CLI_TOML="$BACKEND/rumahl-cli/Cargo.toml"
CLI_RUMAHL_SRC="$BACKEND/rumahl-cli/src/bin/ora.rs"
if [[ -f "$CLI_TOML" ]]; then
  if grep -q 'name = "ora"' "$CLI_TOML" && grep -q 'name = "ora"' "$CLI_TOML"; then
    echo "[fix] splitting duplicate bin targets in $CLI_TOML"
    cp "$CLI_TOML" "$CLI_TOML.bak-$(date +%s)"
    mkdir -p "$(dirname "$CLI_RUMAHL_SRC")"
    # Small shim that re-executes the main bin so behaviour stays identical.
    cat > "$CLI_RUMAHL_SRC" <<'RSEOF'
...
# [[bin]] from src/main.rs (= package name "rumahl-cli") + the new src/bin/ora.rs remain.
src2 = re.sub(
    r'(?ms)\n#[^\n]*\n\[\[bin\]\]\s*\nname\s*=\s*"ora".*?(?=\n\[|\Z)',
    '\n',
    src,
...
# Also drop the [[bin]] ora block that points at main.rs (to avoid dup with default),
# but keep it if it points at a different path.
# Actually safer: keep ora block; just rely on src/bin/ora.rs for 'ora'.
p.write_text(src2)
...
#─────────────────────────────────────────────────────────────────────────
# 3) rumahl-supervisor — add futures-util + TryStreamExt import (idempotent)
#─────────────────────────────────────────────────────────────────────────
SUP_MAIN="$BACKEND/rumahl-supervisor/src/main.rs"
SUP_TOML="$BACKEND/rumahl-supervisor/Cargo.toml"
...
echo "[done] all fixes applied. Retry: cargo build --release -p rumahl-home -p rumahl-cli -p rumahl-supervisor"
