# rumahl OS Image Formats

This directory contains scripts to build rumahl OS in multiple image formats for different deployment scenarios.
...
- **Size**: ~600-800 MB compressed
- **Deployment**:
  1. Unzip: `unzip rumahl-os.vdi.zip`
  2. Create new VM in VirtualBox
  3. Attach `rumahl-os.vdi` as disk
...
- **Size**: ~600-800 MB compressed
- **Deployment**:
  1. Unzip: `unzip rumahl-os.vmdk.zip`
  2. Create new VM in VMware
  3. Attach `rumahl-os.vmdk` as disk
  4. Configure: 2GB RAM, 2 CPUs, Bridged Network
...
- **Use case**: Updating existing rumahl OS installations
- **Size**: ~400-600 MB
- **Deployment**:
  ```bash
  # On running rumahl OS system
  rauc install rumahl-os-20240415.raucb
  reboot
  ```

### 7. **rumahl-os-installer.iso** - Installer/Archive ISO
- **Use case**: Distribution medium containing the compressed raw image
- **Contents**: `rumahl-os.img.xz` and a short install instruction file
- **Creation tools**: `xorriso` (preferred) or `genisoimage`/`mkisofs`

### 8. **rumahl-os-installer-boot.iso** - Bootable Installer ISO
- **Use case**: Bootable installer/recovery medium for manual disk installation
- **Contents**: Kernel, initrd, GRUB menu, and `rumahl-os.img.xz` payload
...
```
rumahl-os/releases/20240415-143022/
├── rumahl-os.img.xz          # Raw disk image
├── rumahl-os-installer.iso   # Installer/archive ISO
├── rumahl-os-installer-boot.iso # Bootable installer ISO
...
# Always use rootfs.ext2 fallback for rumahl-os.img
...
# Convert only from existing output/images/rumahl-os.img (no rebuild)
...
openssl req -x509 \
    ...
    -out cert.pem \
    -days 3650 \
    -subj "/CN=rumahl OS Update/O=Your Organization"
...
wget https://releases.ora.io/rumahl-os.img.xz
...
# - Use existing disk: rumahl-os.vmdk
```
...
sha256sum rumahl-os.img.xz
grep rumahl-os.img.xz SHA256SUMS
```
...
To customize rumahl OS before building:

1. **Add packages**: Edit `configs/rumahl_defconfig`
2. **Customize rootfs**: Edit `board/rumahl/rootfs-overlay/`
3. **Post-build scripts**: Edit `board/rumahl/post-build.sh`
4. **Kernel config**: Edit `board/rumahl/linux.config`
