...
# Check if rumahl-home is running
if ! curl -s -f "${RUMAHL_HOME_URL}/api/health" >/dev/null 2>&1; then
    error "rumahl-home is not running at ${RUMAHL_HOME_URL}"
    exit 1
fi
success "rumahl-home is running"

# Check if config-notify script exists
if [ ! -f /usr/lib/ora/rumahl-config-notify ]; then
    warn "rumahl-config-notify not installed, hot-reload notifications will not work"
    warn "Run: sudo ./rumahl-config-sync.sh to install"
else
    success "rumahl-config-notify is installed"
fi
...
RESPONSE=$(curl -s -X PUT "${RUMAHL_HOME_URL}/api/settings/${TEST_KEY}" \
...
READ_RESPONSE=$(curl -s -f "${RUMAHL_HOME_URL}/api/settings/${TEST_KEY}" 2>&1 || echo "")
...
UPDATE_RESPONSE=$(curl -s -X PUT "${RUMAHL_HOME_URL}/api/settings/${TEST_KEY}" \
    -H "Content-Type: application/json" \
...
VERIFY_RESPONSE=$(curl -s -f "${RUMAHL_HOME_URL}/api/settings/${TEST_KEY}" 2>&1 || echo "")
...
SIGHUP_COUNT=$(journalctl -u rumahl-home -u rumahl-core -u rumahl-assist --since "1 minute ago" 2>/dev/null | grep -ci "sighup\|reload\|config.*change" || echo "0")
...
DELETE_RESPONSE=$(curl -s -X DELETE "${RUMAHL_HOME_URL}/api/settings/${TEST_KEY}" 2>&1 || echo "")
...
log "  - Check /var/run/ora/config-notify/ for notification files"
log "  - Monitor logs: journalctl -u rumahl-* -f"
echo ""
