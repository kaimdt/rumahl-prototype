# ============================================================================
# dev-local.ps1 - rumahl OS Local Dev VM (Windows)
# ============================================================================
# Starts a Debian 12 cloud VM via QEMU. The VM mirrors the rumahl OS runtime
# layout (same /etc/ora, /opt/rumahl, /usr/bin/rumahl-*, same systemd services).
#
...
function Test-rumahlModulesLoadable {
...
if (-not (Test-rumahlModulesLoadable)) {
    $processBypass = $false
    try {
...
if ($Help) {
    ...
    Write-Host "                 like rumahl OS production; needs admin once for setup)"
    ...
    Write-Host "                 env RUMAHL_DEV_DISK overrides; dev-manager passes"
...
Write-Host ""
Write-Host "  rumahl OS - Local Dev VM (Windows / QEMU)  v$DEV_LOCAL_VERSION" -ForegroundColor Cyan
Write-Host ""
...
$RUMAHL_DEV   = Join-Path $REPO_ROOT ".rumahl-dev"
$RUMAHL_BINS  = Join-Path $RUMAHL_DEV "binaries"
$RUMAHL_SCC   = Join-Path $RUMAHL_DEV "sccache"
New-Item -ItemType Directory -Force -Path $RUMAHL_BINS, $RUMAHL_SCC | Out-Null
Write-Info "Dev shared folder: $RUMAHL_DEV"
...
# VM disk size: explicit -DiskSize wins, then RUMAHL_DEV_DISK env (set by the
# dev-manager from its dev-manager.json config), then the 40G default.
if ($DiskSize) {
    $VM_DISK_SIZE = ($DiskSize -replace 'GB$', 'G') -replace 'G+$', 'G'
} elseif ($env:RUMAHL_DEV_DISK) {
    $VM_DISK_SIZE = ($env:RUMAHL_DEV_DISK -replace 'GB$', 'G') -replace 'G+$', 'G'
} else {
    $VM_DISK_SIZE = '40G'
}
if ($CpuCount -eq 0) {
...
$VM_DISK    = Join-Path $CACHE "rumahl-dev-vm.qcow2"
$GOLDEN_DISK = Join-Path $CACHE "rumahl-dev-golden.qcow2"
$SSH_KEY    = Join-Path $CACHE "rumahl-dev-key"
$SEED_ISO   = Join-Path $CACHE "rumahl-dev-seed.iso"
...
if (Test-Path $RUNTIME_STATE_MODULE) {
    ...
    $script:RuntimeState = Read-rumahlRuntimeState -Path $RUNTIME_STATE_PATH
    $script:RuntimeState = Clear-rumahlStaleRuntimeState -State $script:RuntimeState
    if (Test-rumahlProcess -State $script:RuntimeState) {
        $qmpValid = Test-rumahlQmp -State $script:RuntimeState
        $qgaValid = Test-rumahlQga -State $script:RuntimeState
        if (-not $qmpValid) { $script:RuntimeState.lifecycle = "Starting" }
        if ($qgaValid -and $script:RuntimeState.networkMode -eq "bridge") {
            $currentIp = Get-rumahlGuestIp -State $script:RuntimeState
            if ($currentIp) { $script:RuntimeState.vmHost = $currentIp }
        }
        $connection = Get-rumahlConnection -State $script:RuntimeState
...
# -- Ports forwarded host -> VM (mirrors rumahl OS systemd unit ports) --------
$VM_HOME   = 8126
$VM_BRIDGE = 8101
...
# The VM then behaves like rumahl OS production: a normal device on the LAN
...
function Test-WatcherNeedsBuild {
    ...
    $watcherRoot = Join-Path $REPO_ROOT "rumahl-os\backend\tools\rumahl-dev-watch"
...
if ($Watcher) {
    ...
    $dashBin = Join-Path $REPO_ROOT "rumahl-os\backend\target\debug\rumahl-dev-watch.exe"
    if (Test-WatcherNeedsBuild $dashBin) {
        Write-Info "Building updated dev-watch TUI..."
        Push-Location (Join-Path $REPO_ROOT "rumahl-os\backend")
        try {
            ...
            & $env:ComSpec /d /c "cargo build -p rumahl-dev-watch 2>&1" | Select-Object -Last 5
            if ($LASTEXITCODE -ne 0) {
                Stop-WithError "Failed to build rumahl-dev-watch. Check: cd rumahl-os\backend && cargo build -p rumahl-dev-watch"
            }
            Write-Success "dev-watch TUI built"
        } finally { Pop-Location }
    }
    Write-Info "Launching rumahl Dev Watch TUI..."
...
if ($Freeze) {
    ...
    if (-not $p) { $p = Get-QemuProcess | Select-Object -First 1 }
    if ($p) {
        $check = Invoke-SSH 'test -f /etc/ora/dev-vm-provisioned && echo PROV_OK'
...
if ($Status) {
    ...
    $connection = Get-rumahlConnection -State $script:RuntimeState
    $report = Invoke-rumahlReadiness -State $script:RuntimeState -Connection $connection
    ...
    if ($report.Checks.InternalHome) { Write-Success "rumahl-home internally healthy" } else { Write-Warn "rumahl-home internally unhealthy" }
    if ($report.Checks.ExternalHome) { Write-Success "rumahl-home reachable from host" } else { Write-Warn $report.HomeDiagnosis.Summary }
...
# uses UEFI. Use -Uefi to force UEFI (e.g. for EFI-capable rumahl OS images).
...
if ($Clean -or $CleanAll) {
    ...
    Get-ChildItem -Path $CACHE -File -ErrorAction SilentlyContinue | Where-Object {
        $_.Name -notlike "debian-12-cloud-*.qcow2" -and $_.Name -ne "rumahl-dev-golden.qcow2"
    } | Remove-Item -Force -ErrorAction SilentlyContinue
    Remove-Item -Path (Join-Path $CACHE "seed") -Recurse -Force -ErrorAction SilentlyContinue
    if ($CleanAll) {
...
if (-not (Test-Path $SSH_KEY)) {
    ...
    if ($winKeygen) {
        "`n" | & $winKeygen.Source -t ed25519 -f $SSH_KEY -N "" -C "rumahl-dev-vm" 2>&1 | Out-Null
        $keygenOk = ($LASTEXITCODE -eq 0)
    }
    if (-not $keygenOk) {
        # Fallback: generate inside WSL
        $sshKeyWsl = ConvertTo-WslPath $SSH_KEY
        wsl bash -c "ssh-keygen -t ed25519 -f '$sshKeyWsl' -N '' -C 'rumahl-dev-vm'" 2>&1 | Out-Null
        $keygenOk = ($LASTEXITCODE -eq 0)
    }
...
runcmd:
  - mkdir -p /etc/ora && touch /etc/ora/ssh-ready
  - systemctl enable --now qemu-guest-agent 2>/dev/null || true
  - 'systemctl mask apt-daily.service apt-daily-upgrade.service unattended-upgrades.service 2>/dev/null || true'

final_message: "rumahl Dev VM ready."
"@
    Set-Content -Path (Join-Path $seedDir "user-data") -Value $userData -NoNewline -Encoding ASCII
    Set-Content -Path (Join-Path $seedDir "meta-data") -Value "instance-id: rumahl-dev-vm`nlocal-hostname: rumahl-dev`n" -NoNewline -Encoding ASCII
...
if ($existingProc) {
    ...
    if ($script:RuntimeState -and $script:RuntimeState.pid -ne $existingProc.Id) {
        ...
        $script:QgaPort = [int]$script:RuntimeState.qgaPort
        $script:QmpPort = [int]$script:RuntimeState.qmpPort
        if ((Test-rumahlQga -State $script:RuntimeState) -and $script:RuntimeState.networkMode -eq "bridge") {
            $discoveredIp = Get-rumahlGuestIp -State $script:RuntimeState
    ...
    if (Get-Command Test-PortConflict -ErrorAction SilentlyContinue) {
        ...
        if ($portCheck.InUse) {
            ...
            $resolveResult = Resolve-PortConflict -Port $SshPort -ServiceName "rumahl VM"
            if ($resolveResult -eq 2) {
                # Existing rumahl VM found - reuse it
                $existingProc = Get-QemuPid
                if ($existingProc) {
                    Write-Success "Reusing existing rumahl VM"
                    $qemuProc = $existingProc
                }
    ...
    if (-not $existingProc) {
        ...
        function Build-QemuArgs {
            ...
            $a = @(
                ...
                "-drive", "file=$VM_DISK,format=qcow2,if=none,id=rumahl-disk",
                "-device", "virtio-blk-pci,drive=rumahl-disk,bootindex=1",
                "-drive", "file=$SEED_ISO,format=raw,media=cdrom,if=none,id=rumahl-seed",
                "-device", "ide-cd,drive=rumahl-seed,bootindex=2",
                # Bridge mode: TAP adapter on the LAN bridge (own DHCP IP like
                # rumahl OS production); otherwise slirp user-net + hostfwd.
                $netArgs[0], $netArgs[1],
...
$serialLog = Join-Path $CACHE "qemu-serial.log"
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Write-Host 'rumahl Dev VM - Serial Console (live)' -ForegroundColor Cyan; Get-Content -Wait -Tail 0 '$serialLog'" -WindowStyle Minimized | Out-Null
...
while ($waited -lt $timeout) {
    ...
    if (($waited - $lastDiag) -ge 90) {
        ...
        Write-Warn "If the VM shows a login prompt: log in on the VM console (root / password ora) and run:"
...
if ((Test-Path $PROVISIONED_MARKER) -and (-not $Reprovision)) {
    $check = Invoke-SSH 'test -f /etc/ora/dev-vm-provisioned && test -d /opt/rumahl && echo PROV_OK'
...
if (-not $mainSyncOk) {
    ...
    try {
        ...
        foreach ($e in @('.git', 'target', 'node_modules', '.cache', 'buildroot-*', 'releases', '*.img', '*.qcow2', '*.iso', '*.tar.gz', '.rumahl-dev')) {
            $tarArgs += "--exclude=$e"
        }
    ...
    if (Test-Path $projectTar) {
        ...
        Invoke-SSH 'mkdir -p /home/ora/ora' | Out-Null
        Send-SCP -LocalPath $projectTar -RemotePath "/home/ora/ora/" | Out-Null
        Invoke-SSH 'cd /home/ora/ora && tar -xzf rumahl-project.tar.gz && rm rumahl-project.tar.gz && chown -R ora:ora /home/ora/ora' | Out-Null
...
Write-Success "Project synced (1:1 mirror at /home/ora/ora)"
...
if ($needProvision -and $Mode -eq "source") {
    ...
    $npmJob = Start-Job -ArgumentList $SSH_BIN, $SSH_OPTS, $SSH_KEY, $VM_HOST, $VM_SSH_PORT -ScriptBlock {
        ...
        # node_modules over to the `ora` user afterwards - the Vite dev
        # server runs as ora and needs write access for its .vite cache.
        $cmd = 'for i in $(seq 1 90); do command -v npm >/dev/null 2>&1 && break; sleep 10; done; cd /home/ora/ora/frontend && { [ -d node_modules ] || npm install --no-audit --no-fund 2>&1; }; chown -R ora:ora node_modules 2>/dev/null || true'
...
Write-Info "Registering rumahl OS systemd services (mode: $Mode)..."
Invoke-SSH "bash /home/ora/ora/rumahl-os/rumahl-dev-services.sh --$Mode-mode 2>&1" | Select-Object -Last 8
...
if ($needProvision) {
    ...
    $rustupJob = Start-Job -ArgumentList $SSH_BIN, $SSH_OPTS, $SSH_KEY, $VM_HOST, $VM_SSH_PORT -ScriptBlock {
        param($sshBin, $sshOpts, $sshKey, $vmHost, $port)
        $cmd = "su - ora -c 'test -x ~/.cargo/bin/rustc || curl --proto =https --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain stable --profile minimal' 2>&1"
        & $sshBin @sshOpts -i $sshKey -p $port root@$vmHost $cmd
    }
...
# (parity with the RPi4/rumahl OS target). --no-nvram: we run in BIOS mode and
...
su - postgres -c "psql -tAc \"SELECT 1 FROM pg_roles WHERE rolname='root'\"" | grep -q 1 \
    || su - postgres -c "psql -c \"CREATE ROLE root WITH LOGIN SUPERUSER PASSWORD 'ora'\""
su - postgres -c "psql -tAc \"SELECT 1 FROM pg_roles WHERE rolname='ora'\"" | grep -q 1 \
    || su - postgres -c "psql -c \"CREATE USER ora WITH PASSWORD 'ora' CREATEDB\""
mkdir -p /etc/ora && touch /etc/ora/os-dev-mode
'@
...
'@
    ...
    Invoke-SSH 'su - ora -c ''mkdir -p ~/.cargo && cp /tmp/cargo-config.toml ~/.cargo/config.toml''' | Out-Null
    ...
    Write-Info "Pre-building all services IN THE BACKGROUND (full log: /home/ora/.rumahl-prebuild.log)..."
...
if pgrep -f '/home/ora/.rumahl-prebuild.sh' >/dev/null 2>&1; then
    echo "prebuild already running - reusing it"
    exit 0
fi
rm -f /home/ora/.rumahl-prebuild.log /home/ora/.rumahl-prebuild.pid
cat > /home/ora/.rumahl-prebuild.sh <<'EOF'
#!/bin/bash
# Background workspace prebuild (warm cargo cache = fast first boot).
# Runs as root via nohup; the cargo build itself runs as `ora` (su -).
su - ora -c 'cd /home/ora/ora/rumahl-os/backend && cargo build --workspace' >> /home/ora/.rumahl-prebuild.log 2>&1
echo "PREBUILD_EXIT=$?" >> /home/ora/.rumahl-prebuild.log
rm -f /home/ora/.rumahl-prebuild.pid
EOF
chmod 755 /home/ora/.rumahl-prebuild.sh
setsid nohup /home/ora/.rumahl-prebuild.sh >/dev/null 2>&1 &
echo $! > /home/ora/.rumahl-prebuild.pid
echo "prebuild started (pid $!) - provisioning continues in parallel"
'@
    Invoke-SSHStdin $startPrebuild | Select-Object -Last 1

    Write-Info "Applying rumahl OS compat layer + improvements (one SSH session)..."
    $compatScript = @'
for s in rumahl-dev-compat.sh rumahl-dev-improvements.sh rumahl-optimize-memory.sh rumahl-config-sync.sh; do
    echo "=== $s ==="
    bash "/home/ora/ora/rumahl-os/$s" 2>&1 | tail -n 8
    echo ""
done
'@
...
if [ -d /home/ora/ora/rumahl-os/apparmor/dev-vm ]; then
    mkdir -p /etc/apparmor.d
    for p in /home/ora/ora/rumahl-os/apparmor/dev-vm/*; do
        [ -f "$p" ] && install -m 644 "$p" "/etc/apparmor.d/$(basename "$p")"
    done
    aa-status --enabled 2>/dev/null && systemctl enable --now apparmor 2>/dev/null || true
    for p in /etc/apparmor.d/rumahl-home /etc/apparmor.d/rumahl-core /etc/apparmor.d/rumahl-watchdog; do
        [ -f "$p" ] && apparmor_parser -C -r "$p" 2>/dev/null || true
    done
    echo "--- apparmor: $(aa-status 2>/dev/null | head -2 | tail -1)"
fi
# -- Dev signing key: rumahl-sign parity for the build host. The wrapper builds
#    rumahl-sign on first use and generates /etc/ora/dev-signing/rumahl-dev.key
mkdir -p /etc/ora/dev-signing
cat > /usr/local/bin/rumahl-dev-sign <<'SIGNEOF'
#!/bin/bash
# Sign a file with the rumahl dev signing key (rumahl-sign parity on the build host)
set -e
KEY=/etc/ora/dev-signing/rumahl-dev.key
if [ ! -f "$KEY" ]; then
    mkdir -p /etc/ora/dev-signing
    if [ ! -x /usr/bin/rumahl-sign ]; then
        echo "Building rumahl-sign (first use)..." >&2
        su - ora -c "cd /home/ora/ora/rumahl-os/backend && cargo build -p rumahl-sign -q" >&2
        install -m 755 /home/ora/ora/rumahl-os/backend/target/debug/rumahl-sign /usr/bin/rumahl-sign
    fi
    /usr/bin/rumahl-sign keygen --out-dir /etc/ora/dev-signing --name rumahl-dev
    echo "Dev signing key created: $KEY" >&2
fi
exec /usr/bin/rumahl-sign file --key "$KEY" --in "$1"
SIGNEOF
chmod 755 /usr/local/bin/rumahl-dev-sign
# Generate the key now (best-effort - the first rumahl-sign build takes a moment)
echo "rumahl dev VM signing key - sign plugins/apps with: rumahl-dev-sign <file>" > /etc/ora/dev-signing/README
if /usr/local/bin/rumahl-dev-sign /etc/ora/dev-signing/README 2>&1 | tail -1; then
    echo "DEV_SIGN_OK"
else
...
'@
    ...
    Invoke-SSH 'mkdir -p /etc/ora && touch /etc/ora/dev-vm-provisioned' | Out-Null
    Set-Content -Path $PROVISIONED_MARKER -Value (Get-Date -Format "o") -NoNewline
...
Invoke-SSH '(docker compose version || docker-compose version) >/dev/null 2>&1' | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-Info "Docker Compose is missing in the existing VM - installing it..."
...
'@
    Invoke-SSHStdin $composeInstallScript | Select-Object -Last 5
    if ($LASTEXITCODE -ne 0) {
        Stop-WithError "Docker Compose installation failed in VM."
    }
}
...
for db in rumahl_home rumahl_core rumahl_security rumahl_secrets rumahl_appstore; do
    su - postgres -c "psql -tAc \"SELECT 1 FROM pg_database WHERE datname='$db'\"" | grep -q 1 \
        || su - postgres -c "psql -c \"CREATE DATABASE $db OWNER ora\""
done
...
#    /etc/ora/<svc>.env points at - idempotent, runs on every dev-local run.
for envf in /etc/ora/rumahl-*.env; do
    [ -f "$envf" ] || continue
    url=$(grep '^DATABASE_URL=' "$envf" 2>/dev/null | head -1 | cut -d= -f2-)
    case "$url" in
        postgres://*)
            db=$(echo "$url" | sed -E 's|.*/([^?]*).*|\1|')
            # DB names may contain dashes (e.g. rumahl_domain-validator) - quote
...
#    both /etc/ora/<svc>.env and the priority-1 db-credentials files.
for svc in rumahl-api rumahl-connector rumahl-files rumahl-gateway rumahl-intelligence; do
    for envf in "/etc/ora/$svc.env" "/etc/ora/db-credentials/$svc.env"; do
        if [ -f "$envf" ] && grep -q '^DATABASE_URL=postgres://' "$envf" 2>/dev/null; then
            mkdir -p "/var/lib/ora/$svc"
            chown ora:ora "/var/lib/ora/$svc" 2>/dev/null || true
            sed -i "s|^DATABASE_URL=.*|DATABASE_URL=sqlite:///var/lib/ora/$svc/$svc.db?mode=rwc|" "$envf"
            echo "rumahl-db-init: fixed $svc to sqlite ($envf)"
        fi
    done
done
# -- rumahl-security needs a 32-byte hex DB encryption key (production parity)
envf=/etc/ora/rumahl-security.env
if [ -f "$envf" ] && ! grep -q '^SECURITY_DB_KEY=' "$envf"; then
    key=$(openssl rand -hex 32 2>/dev/null)
    if [ -n "$key" ]; then
        echo "SECURITY_DB_KEY=$key" >> "$envf"
        echo "rumahl-db-init: generated SECURITY_DB_KEY"
    fi
fi
# -- Port collision avoidance: rumahl-developer-app and rumahl-intelligence both
#    default to 8099 (rumahl-api's port). Pin them to free ports. Use the
...
#    EVERY service, making rumahl-home proxy /api/os/control/* and
#    /api/files/* to itself (recursive loop, 503s, FD exhaustion).
for pv in "rumahl-developer-app 8110" "rumahl-intelligence 8112"; do
    svc=${pv% *}; port=${pv#* }
    envf="/etc/ora/$svc.env"
    var=$(printf '%s' "$svc" | tr '[:lower:]-' '[:upper:]_')
    if [ -f "$envf" ] && ! grep -q "^${var}_PORT=$port" "$envf" 2>/dev/null; then
        echo "${var}_PORT=$port" >> "$envf"
        echo "rumahl-db-init: pinned $svc to port $port"
    fi
...
mkdir -p /etc/systemd/system/rumahl-home.service.d /opt/rumahl/build/rumahl-home/data /var/lib/ora/rumahl-home
chown ora:ora /var/lib/ora/rumahl-home 2>/dev/null || true
if [ "$(cat /etc/ora/dev-run-mode 2>/dev/null || echo source)" = "build" ]; then
cat > /etc/systemd/system/rumahl-home.service.d/db.conf <<CFG
[Service]
Environment=DATABASE_URL=postgres://root:ora@localhost/rumahl_home
WorkingDirectory=/opt/rumahl/build/rumahl-home
CFG
else
# Source mode: no WorkingDirectory override (unit runs cargo run from the mirror)
rm -f /etc/systemd/system/rumahl-home.service.d/db.conf
fi

# Central log viewer: rumahl-home must be able to read other services' journals.
cat > /etc/systemd/system/rumahl-home.service.d/logs.conf <<CFG
[Service]
...
mkdir -p /etc/ora
if [ ! -f /etc/ora/rumahl-home.env ]; then
cat > /etc/ora/rumahl-home.env <<'ENVEOF'
DATABASE_URL=postgres://root:ora@localhost:5432/rumahl_home
RUST_LOG=rumahl-home=debug
RUMAHL_BOOTSTRAP_ADMIN_USER=admin
RUMAHL_BOOTSTRAP_ADMIN_PASSWORD=admin1234
ENVEOF
fi
# If file already exists, ensure bootstrap vars are present
if [ -f /etc/ora/rumahl-home.env ]; then
    grep -q 'RUMAHL_BOOTSTRAP_ADMIN_USER' /etc/ora/rumahl-home.env 2>/dev/null || echo 'RUMAHL_BOOTSTRAP_ADMIN_USER=admin' >> /etc/ora/rumahl-home.env
    grep -q 'RUMAHL_BOOTSTRAP_ADMIN_PASSWORD' /etc/ora/rumahl-home.env 2>/dev/null || echo 'RUMAHL_BOOTSTRAP_ADMIN_PASSWORD=admin1234' >> /etc/ora/rumahl-home.env
fi
systemctl daemon-reload
systemctl reset-failed rumahl-db-init 2>/dev/null
...
systemctl enable --now qemu-guest-agent 2>/dev/null || true
# Bridge mode: allow the LAN subnet through the rumahl firewall (the slirp
...
case "$lan" in
    ...
    *)
        if command -v iptables >/dev/null 2>&1; then
            ...
            echo "rumahl-db-init: firewall allows LAN subnet $lan (bridge mode)"
        fi
        ;;
esac
...
start_phase() {
    label="$1"; shift
    echo "rumahl-db-init: starting $label"
    for unit in "$@"; do
        ...
        timeout 30 sh -c "until systemctl is-active --quiet '$unit.service'; do sleep 1; done" || echo "rumahl-db-init: $unit did not become active"
    done
}
systemctl start postgresql redis-server docker 2>/dev/null || true
pg_isready -q -t 30 2>/dev/null || echo "rumahl-db-init: PostgreSQL not ready"
start_phase phase-2 rumahl-secrets rumahl-security rumahl-core rumahl-gateway
start_phase phase-3 rumahl-home rumahl-api rumahl-files rumahl-connector
start_phase phase-4 rumahl-intelligence rumahl-developer-app
systemctl enable --now rumahl-hot-reload.path 2>/dev/null
systemctl enable --now rumahl-health-check.timer 2>/dev/null
'@
...
$hotReloadScript = @'
set -e
install -m 0755 /home/ora/ora/rumahl-os/rumahl-dev-hot-reload.sh /usr/local/bin/rumahl-dev-hot-reload.sh
cat > /etc/systemd/system/rumahl-hot-reload.service <<'UNIT'
[Unit]
Description=rumahl Dev VM Hot-Reload (1:1 mirror watcher)
After=multi-user.target

[Service]
Type=simple
ExecStart=/usr/local/bin/rumahl-dev-hot-reload.sh
Restart=always
RestartSec=5
...
UNIT
systemctl daemon-reload
systemctl enable --now rumahl-hot-reload.service 2>/dev/null || true
systemctl restart rumahl-hot-reload.service 2>/dev/null || true
echo "[OK] hot-reload daemon active (mode: $(cat /etc/ora/dev-run-mode 2>/dev/null || echo source))"
'@
...
if [ -f package.json ]; then
    ...
    cat > /etc/systemd/system/rumahl-frontend-dev.service <<'UNIT'
[Unit]
Description=rumahl Frontend Vite Dev Server (source mode)
...
User=ora
Group=ora
WorkingDirectory=/home/ora/ora/frontend
...
UNIT
    systemctl daemon-reload
    systemctl enable --now rumahl-frontend-dev.service 2>/dev/null || true
    grep -q RUMAHL_FRONTEND_DEV_URL /etc/ora/rumahl-home.env 2>/dev/null || echo 'RUMAHL_FRONTEND_DEV_URL=http://127.0.0.1:5173' >> /etc/ora/rumahl-home.env
    systemctl restart rumahl-home 2>/dev/null || true
    echo "[OK] Vite dev server unit created (HMR via rumahl-home proxy)"
fi
'@
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                if (($LASTEXITCODE -eq 0) -and (Test-Path (Join-Path $frontendDir "dist"))) {
                    ...
                    Invoke-SSH "mkdir -p /opt/rumahl/build/dist" | Out-Null
                    Send-SCP -LocalPath (Join-Path $frontendDir "dist") -RemotePath "/opt/rumahl/build/" -Recurse | Out-Null
...
# FULL output lives in /home/ora/.rumahl-prebuild.log. Wait for it here and
...
if ($prebuildStarted) {
    ...
    while (((Get-Date) - $prebuildWaitStart).TotalMinutes -lt 45) {
        $prebuildState = Invoke-SSH 'grep -q PREBUILD_EXIT= /home/ora/.rumahl-prebuild.log 2>/dev/null && echo DONE || echo RUNNING'
        if ("$prebuildState" -match "DONE") { $prebuildDone = $true; break }
        Start-Sleep -Seconds 15
    }
    if (-not $prebuildDone) {
        Write-Warn "Prebuild still running after 45 min - continuing. Log: /home/ora/.rumahl-prebuild.log"
    } else {
        $prebuildExit = (Invoke-SSH 'grep "PREBUILD_EXIT=" /home/ora/.rumahl-prebuild.log 2>/dev/null | tail -1').ToString().Trim()
        if ($prebuildExit -match "PREBUILD_EXIT=0") {
            ...
            (Invoke-SSH "grep -nE '^error|^warning: build failed|-->|could not compile|failed to run custom build command' /home/ora/.rumahl-prebuild.log 2>/dev/null | head -60") | ForEach-Object { Write-Dim "  $_" }
            Write-Dim "  --- last log lines ---"
            (Invoke-SSH 'tail -n 15 /home/ora/.rumahl-prebuild.log 2>/dev/null') | ForEach-Object { Write-Dim "  $_" }
            ...
            & $SCP_BIN @SSH_OPTS -i $SSH_KEY -P $VM_SSH_PORT "root@${VM_HOST}:/home/ora/.rumahl-prebuild.log" $logDest 2>&1 | Out-Null
...
Write-Info "Verifying rumahl OS services..."
$svcCount = (Invoke-SSH 'systemctl list-unit-files --type=service ''rumahl-*'' 2>/dev/null | grep -c ''^rumahl-'' || echo 0').ToString().Trim()
Write-Info "rumahl services registered: $svcCount"
...
Test-VmFile "SSL certificates"      "test -f /etc/ora/ssl/server.crt"
Test-VmFile "/etc/ora/os-dev-mode" "test -f /etc/ora/os-dev-mode"
Test-VmFile "Health monitor binary" "test -f /usr/lib/ora/rumahl-health-check"
Test-VmFile "Setup wizard binary"   "test -f /usr/lib/ora/rumahl-setup-wizard"
Test-VmFile "Centralized logging"   "test -d /var/log/ora"
Test-VmFile "Firewall script"       "test -f /usr/lib/ora/rumahl-firewall"

Write-Info "Checking rumahl-home health endpoint..."
$healthOk = $false
for ($i=0; $i -lt 12; $i++) {
    if (Test-VmHealth) { $healthOk = $true; break }
    Start-Sleep -Seconds 5
}
if ($healthOk) { Write-Success "rumahl-home OK on http://${VM_HOST}:$VM_HOME" }
else { Write-Warn "rumahl-home not responding yet. Check: ssh -i $SSH_KEY -p $VM_SSH_PORT root@$VM_HOST 'journalctl -u rumahl-home -n 50'" }
...
Write-Host ""
$runtimeReport = $null
if ($script:RuntimeState -and (Get-Command Invoke-rumahlReadiness -ErrorAction SilentlyContinue)) {
    $runtimeConnection = Get-rumahlConnection -State $script:RuntimeState
    $runtimeReport = Invoke-rumahlReadiness -State $script:RuntimeState -Connection $runtimeConnection
...
if ($runtimeReport -and $runtimeReport.Lifecycle -ne "Ready") {
    Write-Warn "rumahl Dev VM is $($runtimeReport.Lifecycle), not Ready."
    Write-Warn $runtimeReport.HomeDiagnosis.Summary
} else {
    Write-Success "rumahl Dev VM ready!"
}
Write-Host ""
$bannerTitle = if ($runtimeReport -and $runtimeReport.Lifecycle -ne "Ready") { "rumahl Dev VM $($runtimeReport.Lifecycle)" } else { "rumahl Dev VM ready" }
...
$titleLine
  ...
  |                                                                     |
  |  LOGS (100% rumahl OS compatible)                                     |
  |    All services      ssh root@$VM_HOST -p $VM_SSH_PORT 'journalctl -u rumahl-* -f'
  |    Specific service  ssh root@$VM_HOST -p $VM_SSH_PORT 'journalctl -u rumahl-home -f'
  |    Last 100 lines    ssh root@$VM_HOST -p $VM_SSH_PORT './rumahl-dev-logs.sh'
  |    Follow all logs   ssh root@$VM_HOST -p $VM_SSH_PORT './rumahl-dev-logs.sh -f'
  |                                                                     |
  |  GLOBAL CONFIG                                                      |
  |    Get setting       ssh root@$VM_HOST -p $VM_SSH_PORT 'rumahl-get-config ha.url'
  |    Service env       /etc/ora/service.env (auto-loaded)           |
  |    Per-service env   /etc/ora/<service>.env (optional)            |
  |                                                                     |
  |  CONTROL                                                            |
...
if (Get-Command Send-Notification -ErrorAction SilentlyContinue) {
    Send-Notification -Title "rumahl Dev VM Ready" -Message "Dashboard available at http://localhost:$VM_HOME" -Urgency "Normal"
}
