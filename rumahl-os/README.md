# rumahl OS - Custom Operating System for rumahl

rumahl OS is a custom Linux-based operating system built with Buildroot, designed specifically for running rumahl as a complete smart home solution. Similar to Home Assistant Operating System, rumahl OS provides a secure, minimal, and optimized environment for rumahl services.
...
### Container Platform
- **Docker Engine** - For running all rumahl components in containers
  - rumahl-supervisor manages all containers
...
- **rumahl-os.img.xz** - Raw disk image (USB/SD cards)
- **rumahl-os-installer.iso** - ISO archive containing `rumahl-os.img.xz` + install note
- **rumahl-os-installer-boot.iso** - Bootable installer ISO (GRUB/UEFI)
- **rumahl-os.qcow2.xz** - QEMU/KVM image
- **rumahl-os.vdi.zip** - VirtualBox image
- **rumahl-os.vmdk.zip** - VMware image
- **rumahl-os.ova** - OVA (universal VM format)
- **rumahl-os-YYYYMMDD.raucb** - RAUC update bundle
...
```bash
cd ~/rumahl-os/home-assistant-dashb/rumahl-os
chmod +x build.sh build-all-images.sh resume-build.sh \
  install-requirements.sh install-requirements-linux.sh install-requirements-wsl.sh
...
- `--force-fallback-image`: skips privileged full-image flow and directly creates `rumahl-os.img` from `rootfs.ext2`.
...
- `rumahl-os-installer.iso`

If `grub-mkrescue` is available, the build also creates:
- `rumahl-os-installer-boot.iso`

This ISO is bootable and contains kernel/initrd plus the `rumahl-os.img.xz` payload for manual installation.

This ISO is an installer/archive medium containing `rumahl-os.img.xz` and install notes. It is intended for easy distribution and transfer. The primary deployment artifact remains `rumahl-os.img.xz`.
...
### Why Only `rumahl-os.img` Sometimes Exists
...
# Convert only (no rebuild), requires existing output/images/rumahl-os.img
./build.sh images --unattended
```
...
# Load rumahl OS configuration
make BR2_EXTERNAL=.. rumahl_defconfig
...
# - rumahl-os.img        - Complete disk image
```
...
sudo dd if=output/images/rumahl-os.img of=/dev/sdX bs=4M status=progress
sync
```
...
1. Boot from the rumahl OS image
2. **Boot splash screen** displays rumahl logo with loading spinner
...
7. Access rumahl Home at `http://[device-ip]:8080`

### Installer Features

- **Boot Splash Screen**: Professional boot experience with rumahl logo and animated spinner
...
- **OS Login**: `root` / `ora` (change immediately!)
- **rumahl Home**: Create admin account on first access

## System Configuration

### Network Configuration

rumahl OS uses systemd-networkd:
...
```bash
# Via OTA (automatic)
# rumahl-supervisor checks for updates every 24 hours
...
Each rumahl service has a custom AppArmor profile:
...
rumahl-backup create /mnt/usb/backup.tar.gz

# Restore
rumahl-backup restore /mnt/usb/backup.tar.gz

# Automatic daily backups to external USB
systemctl enable rumahl-backup.timer
```
...
# rumahl-supervisor logs
docker logs rumahl-supervisor

# Specific service
docker logs rumahl-home
```
...
# Test connectivity
ping 8.8.8.8
curl http://rumahl-core:8090/health
```
...
rumahl OS implements comprehensive security features for smart home infrastructure:
...
- All secrets stored encrypted via rumahl-secrets
...
When installed on SD cards, USB flash, or eMMC, rumahl OS automatically:
...
rumahl OS is based on:
- Linux kernel (GPL v2)
- Buildroot (GPL v2)
- Docker (Apache 2.0)
- RAUC (LGPL 2.1)

rumahl components are licensed under MIT.
