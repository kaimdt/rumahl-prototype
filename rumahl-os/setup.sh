#!/usr/bin/env bash
# ============================================================================
# rumahl OS – unified setup entry point for Linux, WSL and ARM / Raspberry Pi
# ============================================================================
#
...
# `docker build` to pre-compile rumahl service binaries (Go/Rust) that are
# embedded into the OS image.  Without Docker on the host the service
# binaries are skipped and rumahl-build-images.service on the device has
...
# Required to build the native rumahl service binaries (rumahl-core, rumahl-home, …)
# that are embedded into the rootfs overlay by build-all-images.sh.
# Without Rust on the host, the binaries are missing and every native rumahl
...
install_rust() {
    ...
    # Older distros ship rust 1.63 or older — too old for modern rumahl deps —
...
# Pass --yes explicitly (or set RUMAHL_SETUP_CONFIRM=1) to restore the prompt.
if [ "${RUMAHL_SETUP_CONFIRM:-0}" = "1" ]; then
...
# compile the native rumahl service binaries. Without this, the rumahl OS image
# boots but all rumahl-* services stay inactive.
install_rust || warn "Rust installation failed – native rumahl services will NOT be built into the image."
...
if [ "$MODE" = "build" ] || ask_yn "Build an rumahl OS image for target '${TARGET}' now?" n; then
...
cat <<EOF
   ...
   • Network install:  curl -sSL https://download.rumahl.com/v1/ora/netinstall.sh | sudo bash
