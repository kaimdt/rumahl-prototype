...
def validate_manifest() -> None:
    ...
    if allowed != ["rumahl-security-helper"]:
    ...
    if 'account_id("/etc/passwd", "rumahl-security")' not in source:
...
def validate_phase_2_components() -> None:
    ...
    sensor = services / "rumahl-runtime-sensor"
    if sensor.exists():
    ...
    resolver = services / "rumahl-runtime-identity"
    if resolver.exists():
        ...
        for forbidden in ("/var/run/docker.sock", "Command::new", "/run/ora/security-helper.sock", "/var/lib/rumahl-security/quarantine"):
            if forbidden in source:
                fail(f"runtime identity resolver contains forbidden capability marker {forbidden}")
    policy = services / "rumahl-runtime-policy"
    if policy.exists():
    ...
    incidents = services / "rumahl-incident-engine"
    if incidents.exists():
