...
function Invoke-rumahlQmp {
    param([Parameter(Mandatory)]$State, [Parameter(Mandatory)][string]$Command)
    Invoke-rumahlTcpJson -Port ([int]$State.qmpPort) -Messages @($Command) -QmpHandshake
}

function Test-rumahlQmp {
    param([Parameter(Mandatory)]$State)
    $response = Invoke-rumahlQmp -State $State -Command '{"execute":"query-status"}'
    return [bool]($response -and $response -match '"return"')
}

function Invoke-rumahlQgaJson {
    param([Parameter(Mandatory)]$State, [Parameter(Mandatory)][string]$Json, [int]$TimeoutSeconds = 10)
    Invoke-rumahlTcpJson -Port ([int]$State.qgaPort) -Messages @($Json) -TimeoutSeconds $TimeoutSeconds
}

function Test-rumahlQga {
    param([Parameter(Mandatory)]$State)
    $response = Invoke-rumahlQgaJson -State $State -Json '{"execute":"guest-ping"}'
    return [bool]($response -and $response -match '"return"')
}

function Invoke-rumahlQgaExec {
    ...
    $start = Invoke-rumahlQgaJson -State $State -Json ('{"execute":"guest-exec","arguments":{"path":"/bin/sh","arg":["-c",' + $encodedCommand + '],"capture-output":true}}')
    try { $guestPid = ($start | ConvertFrom-Json).return.pid } catch { return $null }
    if (-not $guestPid) { return $null }
    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    while ((Get-Date) -lt $deadline) {
        Start-Sleep -Milliseconds 350
        $status = Invoke-rumahlQgaJson -State $State -Json ('{"execute":"guest-exec-status","arguments":{"pid":' + $guestPid + '}}')
...
function Get-rumahlGuestIp {
    param([Parameter(Mandatory)]$State)
    $result = Invoke-rumahlQgaExec -State $State -Command "ip -4 route get 1.1.1.1 2>/dev/null | sed -n 's/.* src \([0-9.]*\).*/\1/p' | head -1" -TimeoutSeconds 8
    if ($result -and $result.StdOut -match '(?<![0-9])((?:[0-9]{1,3}\.){3}[0-9]{1,3})(?![0-9])') { return $Matches[1] }
    return $null
}

Export-ModuleMember -Function Invoke-rumahlQmp, Test-rumahlQmp, Invoke-rumahlQgaJson, Test-rumahlQga, Invoke-rumahlQgaExec, Get-rumahlGuestIp
