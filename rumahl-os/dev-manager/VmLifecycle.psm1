...
function Get-rumahlManagerSettings {
...
function Save-rumahlManagerSettings {
...
function Start-rumahlVmBackend {
...
function Stop-rumahlVmGracefully {
    param([Parameter(Mandatory)]$State, [int]$TimeoutSeconds = 60)
    if (-not (Test-rumahlProcess -State $State)) { return $true }
    if (Test-rumahlQga -State $State) {
        [void](Invoke-rumahlQgaJson -State $State -Json '{"execute":"guest-shutdown"}')
    } elseif (Test-rumahlQmp -State $State) {
        [void](Invoke-rumahlQmp -State $State -Command '{"execute":"system_powerdown"}')
    } else { return $false }
...
function Invoke-rumahlVmQmpAction {
    ...
    $response = Invoke-rumahlQmp -State $State -Command $commands[$Action]
    return [bool]($response -and $response -match '"return"')
}

function Get-rumahlQmpStatus {
    param([Parameter(Mandatory)]$State)
    $status = Invoke-rumahlQmp -State $State -Command '{"execute":"query-status"}'
    $memory = Invoke-rumahlQmp -State $State -Command '{"execute":"query-memory-size-summary"}'
    $cpus = Invoke-rumahlQmp -State $State -Command '{"execute":"query-cpus-fast"}'
    try {
...
Export-ModuleMember -Function Get-rumahlManagerSettings, Save-rumahlManagerSettings, Start-rumahlVmBackend, Stop-rumahlVmGracefully, Stop-rumahlVmHard, Invoke-rumahlVmQmpAction, Get-rumahlQmpStatus
