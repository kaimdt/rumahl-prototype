...
Describe "rumahl runtime state" {
    It "uses a forwarded SSH port for Slirp" {
        $state = [pscustomobject](New-rumahlRuntimeState -CachePath $TestDrive)
        $state.sshPort = 2244
    ...
    It "round-trips the persisted runtime state" {
        ...
        Save-rumahlRuntimeState -State $state -Path $path
        $loaded = Read-rumahlRuntimeState -Path $path
