...
Describe "rumahl VM manager settings" {
    It "persists VM resources and network mode independently of dev-local parameters" {
        ...
        $settings = Get-rumahlManagerSettings -Path $path
        $settings.networkMode = "bridge"
        $settings.ram = "12GB"
        $settings.cpuCount = 6
        $settings.autoWatcher = $false
        $settings.autoSync = $false
        Save-rumahlManagerSettings -Settings $settings -Path $path

        $loaded = Get-rumahlManagerSettings -Path $path
