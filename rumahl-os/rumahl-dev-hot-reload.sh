#!/usr/bin/env bash
# ============================================================================
# rumahl-dev-hot-reload.sh – rumahl Dev VM Hot-Reload Daemon
# ============================================================================
# Runs INSIDE the dev VM (systemd: rumahl-hot-reload.service). Watches the 1:1
# source mirror (/home/ora/ora) that the host keeps in sync (dev-sync.sh)
# and keeps the running services up to date automatically:
#
#   Source mode (default, marker /etc/ora/dev-run-mode = "source"):
...
#       rumahl-frontend-dev (Vite picks up new dependencies)
#     - other frontend changes need NO restart (Vite HMR)
#
#   Build mode (marker = "build"):
#     - a new binary in .rumahl-dev/binaries/<svc> is deployed to
#       /usr/bin/rumahl-<svc> and the service restarted (host-built binaries
#       that travel inside the mirror)
#
# Logs go to journald:  journalctl -u rumahl-hot-reload -f
# ============================================================================
...
MIRROR="/home/ora/ora"
BACKEND="$MIRROR/rumahl-os/backend"
BIN_SRC="$MIRROR/.rumahl-dev/binaries"
HASH_DIR="/var/lib/ora/dev-binary-hashes"
...
RUN_MODE="source"
[ -f /etc/ora/dev-run-mode ] && RUN_MODE=$(cat /etc/ora/dev-run-mode)
...
# ── Helpers ────────────────────────────────────────────────────────────────
all_rumahl_services() {
    systemctl list-unit-files --type=service 'rumahl-*.service' --no-legend 2>/dev/null \
        | awk '{print $1}' | sed 's/\.service$//' | grep -v 'rumahl-hot-reload'
}

restart_service() {
    local svc="$1"
    # Source mode runs the prebuilt binaries directly (see
    # rumahl-dev-services.sh) - rebuild the affected crate before the restart
    # so source changes are picked up. Incremental, so this is fast.
    if [ "$RUN_MODE" = "source" ]; then
        su - ora -c "cd $BACKEND && /home/ora/.cargo/bin/cargo build -p ${svc#rumahl-} -q" \
...
deploy_binary() {
    ...
    if install -m 0755 "$BIN_SRC/$name" "/usr/bin/rumahl-$name" 2>/dev/null; then
        echo "$h" > "$HASH_DIR/$name"
        log "deployed binary rumahl-$name"
        if [ -f /etc/ora/.no-restart ]; then
            log "restart skipped (/etc/ora/.no-restart present)"
        else
...
BUF=/tmp/rumahl-hot-reload-events
: > "$BUF"
