// daemon.rs — long-running HTTP/WebSocket API for `rumahl-dev-deploy`.
//
// Started with `rumahl-dev-deploy daemon`. Listens on 127.0.0.1 by default
...
//     ~/.config/rumahl-dev-deploy/daemon.json    { url, token }
...
pub async fn run(bind: SocketAddr, token_override: Option<String>, no_pin: bool, no_browser: bool) -> Result<()> {
    ...
    let app = Router::new()
        ...
        // Auth via rumahl dashboard credentials (username/password → session token)
    ...
    println!("{} {}", "▶ rumahl-dev-deploy daemon".bold().cyan(), env!("CARGO_PKG_VERSION"));
...
async fn h_connect(headers: HeaderMap, State(s): State<AppState>, Json(b): Json<ConnectBody>) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    ...
    // instance (ends with `_rumahl-dev._tcp.local.`) resolve it.
    let resolved_host = if b.host.ends_with("_rumahl-dev._tcp.local.") || b.host.contains(".local") {
        ...
        if let Err(e) = cli.system_info().await {
            ...
            if msg.contains("401") || msg.contains("Unauthorized") {
                return Err((StatusCode::UNAUTHORIZED,
                    ...
                    .to_string() +
                    "1. SSH into the device and check: cat /var/lib/ora/dev-token\n" +
                    "2. Or regenerate: sudo rm -f /var/lib/ora/dev-token && sudo systemctl restart rumahl-dev-bridge\n" +
                    "3. Then copy the token shown in the bridge logs (journalctl -u rumahl-dev-bridge -n 20 --no-pager)"
                ));
            }
            last_error = Some(format!("{candidate}: {e}"));
            continue;
...
/// every rumahl service through `rumahl-core` and forwarded by the bridge.
...
fn config_dir() -> Result<PathBuf> {
    ...
    let d = base.join("rumahl-dev-deploy");
...
async fn h_ssh_fetch_token(
    ...
    let result = if !pass.is_empty() {
        ...
        tokio::process::Command::new(&sp)
            ...
            .args(ssh_args)
            .arg(&target)
            .args(["cat", "/var/lib/ora/dev-token"])
            .output().await
    } else {
    ...
    match result {
        Ok(output) if output.status.success() => {
            ...
            if token.is_empty() {
                Err((StatusCode::NOT_FOUND, "no token found at /var/lib/ora/dev-token on the device".into()))
...
// ─── Connect via rumahl dashboard credentials ───────────────────────────
...
async fn h_connect_credentials(
    ...
    let auth_resp = auth_resp.ok_or_else(|| {
        ...
        (StatusCode::BAD_GATEWAY, format!(
            "Device auth failed. Tried bridge ports {ports_str}.\n{}\n\nHints:\n  - Make sure rumahl-dev-bridge is running on the device (systemctl status rumahl-dev-bridge)\n  - Confirm the device IP and that the IDE machine can reach it\n  - Verify the rumahl dashboard username/password",
            attempts.join("\n")
        ))
    })?;
