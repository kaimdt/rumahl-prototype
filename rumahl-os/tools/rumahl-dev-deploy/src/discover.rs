...
const SERVICE: &str = "_rumahl-dev._tcp.local.";
...
/// Browse the LAN for `_rumahl-dev._tcp.local.` and collect every device that
...
pub async fn run(timeout_secs: u64) -> Result<()> {
    println!("Scanning for rumahl OS Dev devices ({SERVICE}) for {timeout_secs}s …");
    let devices = discover(timeout_secs).await?;
    if devices.is_empty() {
        bail!("no rumahl OS Dev devices found on the LAN");
    }
    println!();
    ...
    println!();
    println!(
        "Connect with:  rumahl-dev-deploy connect <ADDRESS> (Port 8101 wird automatisch verwendet) --token <hex from /etc/ora/dev-token>"
    );
    Ok(())
}
