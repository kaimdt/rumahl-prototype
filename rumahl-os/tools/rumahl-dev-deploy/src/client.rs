...
impl Client {
    ...
    fn with_auth(&self, request: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        ...
        request
            .header("X-rumahl-Dev-Token", &self.token)
    ...
    /// from `rumahl-core` heartbeats). Returns the raw JSON document so we
    ...
    /// Authenticate with rumahl dashboard credentials (username/password).
    /// Returns a session token that can be used for subsequent API calls.
    /// Requires rumahl-dev-bridge v0.2+ with /dev/auth endpoint.
    ...
    pub async fn build_replace_remote(
        ...
        if !status.capabilities.iter().any(|capability| capability == "binary.build_replace") {
            return Err(anyhow!(
                "device bridge at {} does not support remote builds yet; missing capability `binary.build_replace`. Update rumahl-dev-bridge on the device first",
                self.base,
            ));
        }
        ...
        if !status.is_success() {
            if status == reqwest::StatusCode::NOT_FOUND {
                return Err(anyhow!(
                    "device bridge at {} returned 404 for /dev/build-replace; update rumahl-dev-bridge on the device first",
                    self.base,
                ));
            }
...
fn normalize_base(host: &str) -> String {
    ...
    // Default to port 8101 (rumahl-dev-bridge). If the user only enters an IP
