...
class Dashboard {
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('oraDevDashboard', 'rumahl OS Dev — Dashboard', vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: true });
    ...
    async onMsg(msg) {
        ...
        switch (msg?.cmd) {
            case 'connect':
                await vscode.commands.executeCommand('oraDev.connect');
                break;
            case 'discover':
                await vscode.commands.executeCommand('oraDev.discover');
                break;
            case 'deploy':
                await vscode.commands.executeCommand('oraDev.deploy');
                break;
            case 'watch':
                await vscode.commands.executeCommand('oraDev.watch');
                break;
            case 'deployComponent':
                await vscode.commands.executeCommand('oraDev.deployFromTree', { name: msg.component });
                break;
            case 'watchComponent':
                await vscode.commands.executeCommand('oraDev.watchFromTree', { name: msg.component });
                break;
            case 'stopWatch':
                await vscode.commands.executeCommand('oraDev.stopWatch', msg.id);
                break;
            case 'refresh':
                this.refresh();
                break;
            case 'logs':
                await vscode.commands.executeCommand('oraDev.logs');
                break;
        }
    }
    async refresh() {
