...
import { Component, Connection, DaemonClient, DaemonUnavailable, DeviceFound, Job, ServicesStatus, WatchSession } from './api';
...
export async function activate(ctx: vscode.ExtensionContext) {
    ...
    ctx.subscriptions.push(
        output, statusItem,
        vscode.window.registerTreeDataProvider('oraDev.connection', connectionProvider),
        vscode.window.registerTreeDataProvider('oraDev.devices', devicesProvider),
        vscode.window.registerTreeDataProvider('oraDev.services', servicesProvider),
        vscode.window.registerTreeDataProvider('oraDev.components', componentsProvider),
        vscode.window.registerTreeDataProvider('oraDev.activity', activityProvider),
    );

    ctx.subscriptions.push(
        cmd('oraDev.discover', cmdDiscover),
        cmd('oraDev.connect', cmdConnect),
        cmd('oraDev.disconnect', cmdDisconnect),
        cmd('oraDev.forgetCredentials', cmdForgetCredentials),
        cmd('oraDev.deploy', cmdDeploy),
        cmd('oraDev.deployCurrent', cmdDeployCurrent),
        cmd('oraDev.watch', cmdWatchStart),
        cmd('oraDev.stopWatch', cmdWatchStop),
        cmd('oraDev.restart', cmdRestart),
        cmd('oraDev.reload', cmdReload),
        cmd('oraDev.composeReload', cmdComposeReload),
        cmd('oraDev.logs', cmdLogs),
        cmd('oraDev.dashboard', () => dashboard.show()),
        cmd('oraDev.startDaemon', cmdStartDaemon),
        cmd('oraDev.stopDaemon', cmdStopDaemon),
        cmd('oraDev.refresh', refreshAll),
        cmd('oraDev.showOutput', () => output.show()),
        cmd('oraDev.deployFromTree', (item: any) => cmdDeploy(item?.name ? [item.name] : (item?.label ? [item.label] : undefined))),
        cmd('oraDev.watchFromTree', (item: any) => cmdWatchStart(item?.name ? [item.name] : (item?.label ? [item.label] : undefined))),
        cmd('oraDev.logsFromTree', (item: any) => cmdComponentLogs(item?.name ?? item?.label ?? item)),
        cmd('oraDev.componentActions', (item: any) => cmdComponentActions(item?.name ?? item?.label ?? item)),
        cmd('oraDev.connectFromTree', (ep: string | undefined) => cmdConnect(ep)),
        cmd('oraDev.stopWatchFromTree', (item: any) => cmdWatchStop(item?.id ?? item)),
        cmd('oraDev.serviceLogs', cmdServiceLogsTail),
        cmd('oraDev.serviceLogsFromTree', (item: any) => cmdServiceLogsTail(item?.name ?? item?.label ?? item)),
        cmd('oraDev.serviceRestartFromTree', (item: any) => cmdServiceRestart(item?.name ?? item?.label ?? item)),
        cmd('oraDev.openServiceLiveLogs', (arg?: any) => cmdOpenLiveLogs(arg?.name ?? arg?.label ?? arg)),
        cmd('oraDev.stopServiceLiveLogs', () => cmdStopLiveLogs()),
        cmd('oraDev.openServiceUrl', (arg?: any) => cmdOpenServiceUrl(arg?.name ?? arg?.label ?? arg)),
        cmd('oraDev.systemInfo', cmdSystemInfo),
        cmd('oraDev.rebootDevice', cmdRebootDevice),
        cmd('oraDev.servicePalette', cmdServicePalette),
        cmd('oraDev.connectSaved', cmdConnectSaved),
    );
...
async function tryAutoConnect(host: string) {
    ...
    try {
        ...
        const daemonHost = conn.saved_host as string | undefined;
    ...
    try {
        ...
        try {
            const conn = await client.connection();
            const discovered = conn.discovered as any[] | undefined;
            const suggestedHost = conn.suggested_host as string | undefined;

            if (discovered && discovered.length > 0 && suggestedHost && token) {
                // Device IP likely changed — offer one-click reconnect.
                const suggestedName = (conn.suggested_hostname as string)
                ...
                vscode.window.showWarningMessage(
                    `rumahl Dev: Saved host ${host} is unreachable. ${discovered.length === 1 ? 'One device' : `${discovered.length} devices`} found on LAN.`,
                    'Reconnect Now', 'Show All Devices'
                ).then(async (choice) => {
                    if (choice === 'Reconnect Now') {
                        try {
                            const r = await client!.connect(suggestedHost, token) as any;
        ...
        if (msg.includes('401') || msg.includes('Unauthorized')) {
            ...
            vscode.window.showWarningMessage(
                'rumahl Dev: Saved dev-token was rejected by the device. Please reconnect.',
                'Connect'
...
async function cmdDiscover() {
    const c = await ensureClient();
    const t = vscode.workspace.getConfiguration('oraDev').get<number>('discoverTimeoutSecs') ?? 4;
    const dev = await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: `rumahl Dev: scanning LAN (${t}s)…` },
        () => c.discover(t),
    );
    devicesProvider.setDevices(dev);
    if (dev.length === 0) vscode.window.showWarningMessage('No rumahl OS Dev devices found on the LAN.');
...
async function cmdDeploy(prefilled?: string[]) {
    ...
    const target = vscode.workspace.getConfiguration('oraDev').get<string>('target') ?? 'aarch64-unknown-linux-gnu';
    const buildMode = vscode.workspace.getConfiguration('oraDev').get<string>('buildMode') ?? 'auto';
...
async function cmdWatchStart(prefilled?: string[]) {
    const c = await ensureClient();
    const automatic = (vscode.workspace.getConfiguration('oraDev').get<boolean>('automaticMode') ?? false)
        && (!prefilled || prefilled.length === 0);
    const comps = automatic ? [] : await pickComponents(prefilled);
    if (!automatic && (!comps || comps.length === 0)) return;
    const target = vscode.workspace.getConfiguration('oraDev').get<string>('target') ?? 'aarch64-unknown-linux-gnu';
    const buildMode = vscode.workspace.getConfiguration('oraDev').get<string>('buildMode') ?? 'auto';
    const debounce = vscode.workspace.getConfiguration('oraDev').get<number>('watchDebounceMs') ?? 800;
...
async function cmdRebootDevice() {
    const yes = await vscode.window.showWarningMessage(
        'Reboot the connected rumahl device? This will interrupt all running services.',
        { modal: true },
        'Reboot',
    );
