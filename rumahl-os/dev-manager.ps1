...
$rustManager = Join-Path $managerRoot "backend\target\release\rumahl-dev-manager.exe"
if (-not (Test-Path $rustManager)) { $rustManager = Join-Path $managerRoot "backend\target\debug\rumahl-dev-manager.exe" }
if (-not $Once -and -not $env:RUMAHL_DEV_MANAGER_LEGACY) {
    ...
    if (Get-Command cargo -ErrorAction SilentlyContinue) {
        & cargo run --manifest-path (Join-Path $managerRoot "backend\Cargo.toml") -p rumahl-dev-manager -- @rustArguments
        exit $LASTEXITCODE
    }
...
function Get-ValidatedState {
    $state = Read-rumahlRuntimeState -Path $script:StatePath
    $state = Clear-rumahlStaleRuntimeState -State $state
    if (Test-rumahlProcess -State $state) {
        if (Test-rumahlQga -State $state) {
            $guestIp = Get-rumahlGuestIp -State $state
            if ($state.networkMode -eq "bridge" -and $guestIp -and $guestIp -ne $state.vmHost) { $state.vmHost = $guestIp }
        }
    }
    Save-rumahlRuntimeState -State $state -Path $script:StatePath
    return $state
}
...
function Invoke-DoctorView {
    ...
    $connection = Get-rumahlConnection -State $state
    $report = Invoke-rumahlReadiness -State $state -Connection $connection
    Clear-Host
    Write-Host "rumahl Dev Doctor`n" -ForegroundColor Cyan
    ...
    Show-Check "rumahl services" $report.Checks.Services $(if ($report.FailedUnits.Count) { $report.FailedUnits -join ", " } else { "no failed units" })
    Show-Check "rumahl-home internal" $report.Checks.InternalHome
    Show-Check "rumahl-home host" $report.Checks.ExternalHome $report.HostHealth.Url
...
function Invoke-QgaCommandView { param([string]$Title, [string]$Command)
    ...
    if (-not (Test-rumahlQga -State $state)) { Write-Host "Guest Agent is unavailable." -ForegroundColor Red; return }
    $result = Invoke-rumahlQgaExec -State $state -Command $Command -TimeoutSeconds 30
...
function Start-DevVm { param([ValidateSet("slirp", "bridge")][string]$NetworkMode)
    $state = Get-ValidatedState
    if (Test-rumahlProcess -State $state) { Write-Host "The VM is already running (PID $($state.pid))." -ForegroundColor Yellow; return }
    $settings = Get-rumahlManagerSettings -Path $script:SettingsPath
    $settings.networkMode = $NetworkMode
    Save-rumahlManagerSettings -Settings $settings -Path $script:SettingsPath
    [void](Start-rumahlVmBackend -DevLocalPath $script:DevLocal -Settings $settings)
}

function Enter-QgaShell {
    $state = Get-ValidatedState
    if (-not (Test-rumahlQga -State $state)) { Write-Host "QGA is unavailable." -ForegroundColor Red; return }
    ...
    while ($true) {
        ...
        $result = Invoke-rumahlQgaExec -State $state -Command $command -TimeoutSeconds 120
...
function Open-rumahlSshOrRescue {
    param($Connection)
    if ((Test-rumahlTcpPort -HostName $Connection.Host -Port $Connection.SshPort) -and (Get-Command ssh.exe -ErrorAction SilentlyContinue)) {
        & (Get-Command ssh.exe).Source -i (Join-Path $script:Cache "rumahl-dev-key") -p $Connection.SshPort "root@$($Connection.Host)"
    } else {
...
function Show-VmControl {
    ...
    do {
        Clear-Host
        $qmp = if (Test-rumahlProcess -State $state) { Get-rumahlQmpStatus -State $state } else { $null }
        Write-Host "rumahl VM Control`n" -ForegroundColor Cyan
        Write-Host "Process: $(if (Test-rumahlProcess $state) { "PID $($state.pid)" } else { "Stopped" })"
        ...
        switch ($action) {
            ...
            "3" { [void](Invoke-rumahlVmQmpAction -State $state -Action pause) }
            "4" { [void](Invoke-rumahlVmQmpAction -State $state -Action resume) }
            "5" { [void](Invoke-rumahlVmQmpAction -State $state -Action reset) }
            "6" { if (-not (Stop-rumahlVmGracefully -State $state)) { Write-Host "Graceful shutdown timed out; hard stop remains available." -ForegroundColor Yellow; Read-Host "Press Enter" | Out-Null } }
            "7" { [void](Stop-rumahlVmHard -State $state) }
...
function Show-Settings {
    $settings = Get-rumahlManagerSettings -Path $script:SettingsPath
    Clear-Host; Write-Host "rumahl VM Settings`n" -ForegroundColor Cyan
...
function Show-ServiceControl {
    do {
        $state = Get-ValidatedState
        Clear-Host; Write-Host "rumahl Service Control`n" -ForegroundColor Cyan
        $list = Invoke-rumahlQgaExec -State $state -Command "systemctl list-units --type=service 'rumahl-*' --all --no-pager" -TimeoutSeconds 20
        ...
        if ($action -in @("R", "S", "T", "L", "H", "D")) {
            $service = Read-Host "Service name (e.g. rumahl-home)"
        ...
        switch ($action) {
            "R" { [void](Invoke-rumahlQgaExec $state "systemctl restart '$service' && systemctl is-active '$service'" 90) }
            "S" { [void](Invoke-rumahlQgaExec $state "systemctl start '$service' && systemctl is-active '$service'" 90) }
            "T" { [void](Invoke-rumahlQgaExec $state "systemctl stop '$service'; systemctl is-active '$service' || true" 90) }
...
function Start-CriticalServices {
    $state = Get-ValidatedState
    if (-not (Test-rumahlQga -State $state)) { Write-Host "QGA unavailable; cannot start services safely." -ForegroundColor Red; return }
    $phases = @(
        @("postgresql", "redis-server", "docker"),
        @("rumahl-secrets", "rumahl-security", "rumahl-core", "rumahl-gateway"),
        @("rumahl-home", "rumahl-api", "rumahl-files", "rumahl-connector"),
        @("rumahl-intelligence", "rumahl-developer-app")
    )
    for ($index = 0; $index -lt $phases.Count; $index++) {
        ...
        foreach ($service in $phases[$index]) {
            ...
            $result = Invoke-rumahlQgaExec -State $state -Command "systemctl start '$unit' && systemctl is-active --quiet '$unit'" -TimeoutSeconds 90
...
function Show-Dashboard {
    $state = Get-ValidatedState; $connection = Get-rumahlConnection -State $state
    $report = Invoke-rumahlReadiness -State $state -Connection $connection
    Clear-Host
    Write-Host "  rumahl Dev Manager" -ForegroundColor Cyan
    ...
    Write-Host ("  SSH:          {0}:{1} ({2})" -f $connection.Host, $connection.SshPort, $(if (Test-rumahlTcpPort $connection.Host $connection.SshPort) { "Connected" } else { "Unavailable" }))
...
do {
    ...
    switch ($choice) {
        ...
        "A" { Open-rumahlSshOrRescue -Connection $dashboard.Connection }
