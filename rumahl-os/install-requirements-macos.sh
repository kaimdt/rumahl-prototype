#!/usr/bin/env bash
# ============================================================================
# install-requirements-macos.sh – rumahl dependency installer (macOS)
# ============================================================================
...
brew_install() {
    ...
    for attempt in 1 2; do
        if brew install "$@" "$name" >"/tmp/rumahl-brew-${name}.log" 2>&1; then
            ok "$name installed"
            return 0
        fi
        warn "brew install $name failed (attempt $attempt/2) – retrying..."
    done
    err "brew install $name failed. See /tmp/rumahl-brew-${name}.log"
    return 1
}
...
print_recommendations() {
    ...
    ok "  Next step: ./dev-local.sh  (starts the rumahl dev VM)"
}
