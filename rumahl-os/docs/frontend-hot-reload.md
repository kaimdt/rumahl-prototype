...
> Seamless frontend development with Hot Module Replacement (HMR) during rumahl development

## Overview

rumahl supports multiple frontend development workflows:
...
- ✅ Starts rumahl-home backend (port 3001)
- ✅ Sets `RUMAHL_FRONTEND_DEV_URL` environment variable
...
**How it works:**
- Backend detects `RUMAHL_FRONTEND_DEV_URL`
...
cd ../rumahl-os/backend/services/rumahl-home && cargo run
```
...
1. **Detection**: rumahl-home checks for `RUMAHL_FRONTEND_DEV_URL` on startup
...
4. **API Proxy**: Vite proxies API calls back to rumahl-home (configured in `vite.config.ts`)
...
Browser → Vite Dev Server (localhost:5173)
        ...
        Frontend (React, instant reload)
           ↓ (/api/* requests)
        Vite Proxy → rumahl-home (localhost:3001)
           ↓
        Backend API
```
...
```json
{
  ...
  "configurations": [
    ...
    },
    {
      ...
      "type": "lldb",
      "request": "launch",
      "program": "${workspaceFolder}/rumahl-os/backend/target/debug/rumahl-home",
      "cwd": "${workspaceFolder}/rumahl-os/backend/services/rumahl-home",
      "env": {
        "RUMAHL_FRONTEND_DEV_URL": "http://localhost:5173"
      },
...
2. Is `RUMAHL_FRONTEND_DEV_URL` set correctly?
...
```
┌─────────────────────┐
│  rumahl-home          │ ← User accesses here
│  localhost:8126     │
│  ├─ /api/* → API    │
...
- [rumahl Dev Runner](../../scripts/dev/README.md) — Multi-service dev manager
...
A: Simply unset `RUMAHL_FRONTEND_DEV_URL` and ensure `frontend/dist/` exists:
```bash
unset RUMAHL_FRONTEND_DEV_URL  # Linux/macOS
$env:RUMAHL_FRONTEND_DEV_URL=""  # Windows PS
```

## Version History

- **v2.1** — Frontend dev proxy mode with RUMAHL_FRONTEND_DEV_URL
- **v2.0** — Vite migration, separate dev server
- **v1.x** — Embedded frontend build

---

**Need help?** Check [GitHub Issues](https://github.com/ora/ora/issues) or [Community Forum](https://community.ora.io)
