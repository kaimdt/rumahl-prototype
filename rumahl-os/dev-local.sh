#!/usr/bin/env bash
# ============================================================================
# dev-local.sh – rumahl OS Local Dev VM (macOS / Linux / WSL2)
# ============================================================================
# Starts a Debian 12 cloud VM via QEMU that mirrors the rumahl OS runtime
...
#   ./dev-local.sh --build-mode      Run deployed binaries (/usr/bin/rumahl-*)
...
RUMAHL_DEV="$REPO_ROOT/.rumahl-dev"
RUMAHL_BINS="$RUMAHL_DEV/binaries"
RUMAHL_SCC="$RUMAHL_DEV/sccache"
mkdir -p "$RUMAHL_BINS" "$RUMAHL_SCC"
log "Dev shared folder: $RUMAHL_DEV"
...
if [ -n "${RUMAHL_DEV_RAM:-}" ]; then
    VM_RAM="$RUMAHL_DEV_RAM"
else
...
VM_CPUS="${RUMAHL_DEV_CPUS:-$(( HOST_CPUS / 2 ))}"
[ "$VM_CPUS" -lt 2 ] && VM_CPUS=2

if [ -n "${RUMAHL_DEV_CARGO_JOBS:-}" ]; then
    CARGO_JOBS="$RUMAHL_DEV_CARGO_JOBS"
else
    VM_RAM_NUM=${VM_RAM%G}
...
# ── Ports (kept in sync with rumahl OS nginx + service config) ───────────────
VM_HOST=127.0.0.1
VM_SSH=2222
VM_HOME=8126
VM_BRIDGE=8101
# Additional ports forwarded so host browser can reach all services directly.
# Match the rumahl OS systemd unit ports.
...
VM_DISK="$CACHE/rumahl-dev-vm.qcow2"
SEED_ISO="$CACHE/rumahl-dev-seed.iso"
SSH_KEY="$CACHE/rumahl-dev-key"
...
watcher_needs_build() {
    local binary="$1"
    local watcher_root="$REPO_ROOT/rumahl-os/backend/tools/rumahl-dev-watch"
    [ ! -f "$binary" ] && return 0
...
if $DO_WATCHER; then
    ...
    DASH_BIN="$REPO_ROOT/rumahl-os/backend/target/debug/rumahl-dev-watch"
    if watcher_needs_build "$DASH_BIN"; then
        log "Building updated dev-watch TUI..."
        (cd "$REPO_ROOT/rumahl-os/backend" && cargo build -p rumahl-dev-watch 2>&1 | tail -5) || \
            die "Failed to build rumahl-dev-watch. Check: cd rumahl-os/backend && cargo build -p rumahl-dev-watch"
        ok "dev-watch TUI built"
    fi
    log "Launching rumahl Dev Watch TUI..."
    if $IS_MACOS; then
    ...
    if command -v curl >/dev/null 2>&1; then
        if curl -fsS --max-time 3 "http://127.0.0.1:${VM_HOME}/api/health" >/dev/null 2>&1 \
           || curl -fsS --max-time 3 "http://127.0.0.1:${VM_HOME}/health" >/dev/null 2>&1; then
            ok "rumahl-home health check OK (port $VM_HOME)"
        else
            warn "rumahl-home not responding on port $VM_HOME yet"
        fi
    fi
    exit 0
fi
...
choose_accel() {
    # Allow override
    if [ -n "${RUMAHL_DEV_ACCEL:-}" ]; then
        echo "$RUMAHL_DEV_ACCEL"; return
    fi
    if $IS_MACOS; then
        echo "hvf"; return
    fi
...
choose_display() {
    if [ -n "${RUMAHL_DEV_DISPLAY:-}" ]; then
        echo "$RUMAHL_DEV_DISPLAY"; return
    fi
    if $IS_MACOS; then
...
generate_seed_iso() {
    ...
    if [ ! -f "$SSH_KEY" ]; then
        ...
        ssh-keygen -t ed25519 -f "$SSH_KEY" -N "" -C "rumahl-dev-vm" >/dev/null \
...
instance-id: rumahl-dev-vm
local-hostname: rumahl-dev
METAEOF
...
if [ -n "$EXISTING_PID" ]; then
    ...
    if port_in_use "$VM_SSH"; then
        ...
        if command -v auto_resolve_port_conflict >/dev/null 2>&1; then
            auto_resolve_port_conflict "$VM_SSH" "rumahl VM"
            resolve_result=$?
            if [ $resolve_result -eq 2 ]; then
                # Existing rumahl VM found - reuse it
                EXISTING_PID=$(vm_pid)
                ok "Reusing existing rumahl VM"
                QEMU_CHILD_PID=""
    ...
    if [ -z "$EXISTING_PID" ]; then
        ...
        if ! kill -0 "$QEMU_CHILD_PID" 2>/dev/null; then
            ...
            die "Check accelerator (currently '$ACCEL'). Set RUMAHL_DEV_ACCEL=tcg to force software emulation."
        fi
    fi
fi
...
while [ $WAITED -lt $CLOUD_TIMEOUT ]; do
    ...
    if [ $((WAITED - LAST_DIAG)) -ge 90 ]; then
        ...
        warn "If the VM shows a login prompt: log in on the VM console (root / password ora) and run:"
...
if [ -f "$PROVISIONED_MARKER" ] && ! $REPROVISION; then
    ...
    if ssh_vm "test -f /etc/ora/dev-vm-provisioned && test -d /opt/rumahl && test -f /home/ora/ora/rumahl-os/rumahl-dev-compat.sh" 2>/dev/null; then
...
ssh_vm "mkdir -p /home/ora/ora" 2>/dev/null || true
...
if rsync -az --delete \
    ...
    "$REPO_ROOT/" "root@127.0.0.1:/home/ora/ora/" 2>&1 | tail -5
then
    ok "Project synced"
else
    die "rsync failed. Try: ssh -i $SSH_KEY -p $VM_SSH root@127.0.0.1"
fi
ssh_vm "chown -R ora:ora /home/ora/ora" 2>/dev/null || true
...
log "Registering rumahl OS systemd services (mode: $RUN_MODE)..."
ssh_vm "bash /home/ora/ora/rumahl-os/rumahl-dev-services.sh --${RUN_MODE}-mode 2>&1" | tail -8 \
    || warn "rumahl-dev-services.sh reported errors"
...
su - postgres -c "psql -tAc \"SELECT 1 FROM pg_roles WHERE rolname='postgres'\"" | grep -q 1 \
    && su - postgres -c "psql -c \"ALTER USER postgres WITH PASSWORD 'ora'\""

# Mark as dev mode
mkdir -p /etc/ora /etc/ora/db-credentials
touch /etc/ora/os-dev-mode
chmod 700 /etc/ora/db-credentials

# Initialize database manager config (will be used by rumahl-db-manager)
cat > /etc/ora/db-config.toml <<'DBCFG'
# rumahl Dev VM Database Configuration
...
PGEOF
    ...
    ssh_vm "su - ora -c 'test -x ~/.cargo/bin/rustc || curl --proto =https --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain stable --profile minimal'" \
        2>&1 | tail -5

    log "Configuring Cargo (mold linker + sparse registry)..."
    ssh_vm "su - ora -c 'mkdir -p ~/.cargo'"
    ssh_vm "cat > /home/ora/.cargo/config.toml" <<'CEOF'
...
CEOF
    ssh_vm "chown ora:ora /home/ora/.cargo/config.toml"

    log "Applying rumahl OS compat layer + improvements (one SSH session)..."
    ssh_vm 'for s in rumahl-dev-compat.sh rumahl-dev-improvements.sh; do echo "=== $s ==="; bash "/home/ora/ora/rumahl-os/$s" 2>&1 | tail -n 8; echo; done' \
        || warn "compat/improvements reported errors"

    # Mark as provisioned
    ssh_vm "mkdir -p /etc/ora && touch /etc/ora/dev-vm-provisioned"
...
log "Initializing databases with rumahl-db-manager..."
ssh_vm bash -s <<'DBEOF' 2>&1 | tail -10 || warn "Database initialization had warnings"
set +e

# Check if rumahl-db-manager is available (built services)
if command -v rumahl-db-manager >/dev/null 2>&1; then
    # Use the new database manager for proper security
    export POSTGRES_ADMIN_URL="postgres://postgres:ora@localhost:5432/postgres"
    rumahl-db-manager --config /etc/ora/db-config.toml init
    rumahl-db-manager --config /etc/ora/db-config.toml status
else
    # Fallback for dev VM before services are built
    echo "[INFO] rumahl-db-manager not yet available, using simple init"
    su - postgres -c "createuser -s root 2>/dev/null || true"

    # Create databases with simple permissions
    for db in rumahl_home rumahl_core rumahl_security rumahl_secrets rumahl_appstore rumahl_assist rumahl_watchdog; do
    ...
    su - postgres -c "psql -c \"GRANT ALL PRIVILEGES ON DATABASE rumahl_home TO root\" 2>/dev/null || true"
    su - postgres -c "psql -c \"GRANT ALL PRIVILEGES ON DATABASE rumahl_core TO root\" 2>/dev/null || true"

    # Create simple credential files for compatibility
    mkdir -p /etc/ora/db-credentials
    for db in rumahl_home rumahl_core rumahl_security rumahl_secrets rumahl_appstore rumahl_assist rumahl_watchdog; do
        echo "DATABASE_URL=postgres://root:ora@localhost/$db" > /etc/ora/db-credentials/rumahl-${db#rumahl_}.env
    done
    chmod 600 /etc/ora/db-credentials/*.env
    ...
    su - postgres -c "psql rumahl_home -c 'ALTER TABLE installed_themes ADD COLUMN IF NOT EXISTS widget_templates_json TEXT DEFAULT NULL'" 2>/dev/null || true
fi

# rumahl-home systemd override for dev VM
mkdir -p /etc/systemd/system/rumahl-home.service.d /opt/rumahl/build/rumahl-home/data
if [ "$(cat /etc/ora/dev-run-mode 2>/dev/null || echo source)" = "build" ]; then
cat > /etc/systemd/system/rumahl-home.service.d/db.conf <<CFG
[Service]
# Database URL will be loaded from /etc/ora/db-credentials/rumahl-home.env
...
mkdir -p /etc/ora
[ ! -f /etc/ora/rumahl-home.env ] && cat > /etc/ora/rumahl-home.env <<'ENVEOF'
...
systemctl reset-failed rumahl-db-init rumahl-migrations 2>/dev/null
systemctl restart rumahl-home 2>/dev/null || true

# Enable services and timers
systemctl enable --now rumahl-hot-reload.path 2>/dev/null || true
systemctl enable --now rumahl-health-check.timer 2>/dev/null || true
systemctl enable rumahl-db-init.service 2>/dev/null || true
systemctl enable rumahl-migrations.service 2>/dev/null || true
...
UNIT
    ...
    # rumahl-home proxies frontend requests to Vite (HMR)
    grep -q RUMAHL_FRONTEND_DEV_URL /etc/ora/rumahl-home.env 2>/dev/null || \
        echo 'RUMAHL_FRONTEND_DEV_URL=http://127.0.0.1:5173' >> /etc/ora/rumahl-home.env
    systemctl restart rumahl-home 2>/dev/null || true
...
if [ -f "$FRONTEND_DIR/package.json" ] && command -v npm >/dev/null 2>&1; then
    ...
    if (cd "$FRONTEND_DIR" && npm install --silent && npm run build) 2>&1 | tail -5; then
        if [ -d "$FRONTEND_DIR/dist" ]; then
            ...
            ssh_vm "mkdir -p /opt/rumahl/build/dist"
            rsync -az -e "ssh ${SSH_OPTS[*]} -i $SSH_KEY -p $VM_SSH" \
                "$FRONTEND_DIR/dist/" "root@127.0.0.1:/opt/rumahl/build/dist/" \
...
log "Verifying rumahl OS services..."
SERVICES_CHECK=$(ssh_vm "systemctl list-unit-files --type=service 'rumahl-*' 2>/dev/null | grep -c '^rumahl-' || echo 0")
log "rumahl services registered: ${SERVICES_CHECK//[!0-9]/}"
...
check "SSL certificates"        'test -f /etc/ora/ssl/server.crt && echo OK'
check "/etc/ora/os-dev-mode"   'test -f /etc/ora/os-dev-mode && echo OK'
check "Health monitor binary"   'test -f /usr/lib/ora/rumahl-health-check && echo OK'
check "Setup wizard binary"     'test -f /usr/lib/ora/rumahl-setup-wizard && echo OK'
check "Centralized logging"     'test -d /var/log/ora && echo OK'
check "Firewall script"         'test -f /usr/lib/ora/rumahl-firewall && echo OK'

# Final health check
log "Checking rumahl-home health endpoint..."
HEALTH_OK=false
...
if $HEALTH_OK; then
    ok "rumahl-home is responding on http://127.0.0.1:${VM_HOME}"
else
    warn "rumahl-home not yet responding – it may still be building. Check: ssh -i $SSH_KEY -p $VM_SSH root@127.0.0.1 'journalctl -u rumahl-home -n 50'"
fi
...
$(if [ "${#SKIPPED_PORTS[@]}" -gt 0 ]; then printf '  |    NOT forwarded:   %s (busy on host)                         |\n' "${SKIPPED_PORTS[*]}"; fi)
  |    Dev Watch TUI     $REPO_ROOT/rumahl-os/backend/target/debug/rumahl-dev-watch
  |                                                                     |
  |  CO-BUDDY FEATURES                                                  |
...
if command -v send_notification >/dev/null 2>&1; then
    send_notification "rumahl Dev VM Ready" "Dashboard available at http://localhost:${VM_HOME}" "normal"
fi
