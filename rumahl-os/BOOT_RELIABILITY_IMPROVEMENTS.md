# rumahl OS Boot Reliability Improvements

This document describes the comprehensive fixes applied to ensure rumahl OS boots reliably without service failures.

## Problem Analysis

The original rumahl OS implementation had several critical boot issues:

### 1. Missing Directory Initialization
- **Problem**: Services like `rumahl-stack.service` required `/mnt/data/ora` directory to exist, but it wasn't created before services started
- **Symptom**: `ConditionPathIsDirectory=/mnt/data/ora` failed, causing services to skip startup
- **Impact**: Docker stack never started, leaving rumahl non-functional
...
- **Problem**: `rumahl-stack.service` tried to run `docker compose` in `/mnt/data/ora` but no compose file existed on first boot
- **Symptom**: Service failed with "docker-compose.yml not found" error
- **Impact**: Entire rumahl container stack failed to start
...
**New Service**: `rumahl-init-data.service`

```systemd
[Unit]
Description=Initialize rumahl data directory
DefaultDependencies=no
After=mnt-data.mount
Before=rumahl-stack.service rumahl-setup.service docker.service
...
ExecStart=/bin/sh -c '\
  ...
  chmod 755 /mnt/data/ora /mnt/data/rauc /mnt/data/backups'
...
**Enhanced**: `rumahl-stack.service`

Added `ExecStartPre` step to create a minimal docker-compose.yml if none exists:

```bash
ExecStartPre=/bin/sh -c 'if [ ! -f /mnt/data/ora/docker-compose.yml ]; then \
  echo "version: '\''3.8'\''" > /mnt/data/ora/docker-compose.yml; \
  echo "services:" >> /mnt/data/ora/docker-compose.yml; \
  echo "  placeholder:" >> /mnt/data/ora/docker-compose.yml; \
  echo "    image: hello-world" >> /mnt/data/ora/docker-compose.yml; \
fi'
```
...
mnt-data.mount (data partition)
    ↓
rumahl-init-data.service (create directories)
    ↓
├── docker.service
├── rumahl-setup.service (first-boot wizard)
└── rumahl-stack.service (docker compose stack)
    ↓
rumahl-stack-watchdog.timer (periodic health check)
```

**Key Changes**:
- All services now depend on `rumahl-init-data.service` instead of directly on `mnt-data.mount`
...
**Before**:
```systemd
ConditionPathIsDirectory=/mnt/data/ora
```

**After**:
```systemd
ConditionPathIsDirectory=/mnt/data/ora
ConditionPathExists=/mnt/data/ora/docker-compose.yml
ConditionPathIsMountPoint=/mnt/data
```
...
**Enhanced**: `rumahl-stack-watchdog.service`
...
All services that depend on `/mnt/data/ora` now properly depend on `rumahl-init-data.service`:

- ✅ `rumahl-stack.service`
- ✅ `rumahl-setup.service`
- ✅ `rumahl-update-check.service`
- ✅ `rumahl-recovery.service`
- ✅ `rumahl-dev-bridge.service`

### 7. Removed Hard Dependencies Where Not Needed

**Before**:
```systemd
Requires=rumahl-verify.service
```

**After**:
```systemd
After=rumahl-verify.service
...
│  ├── mnt-data.mount (data partition)│
│  └── rumahl-init-data.service         │← NEW: Creates directories
└─────────────────────────────────────┘
    ↓
┌─────────────────────────────────────┐
...
└─────────────────────────────────────┘
    ↓
┌─────────────────────────────────────┐
│ rumahl Services (multi-user.target)   │
│  ├── rumahl-detect-virt.service       │
│  ├── rumahl-verify.service             │
│  ├── rumahl-setup.service (first boot)│← Runs before stack
│  ├── rumahl-stack.service              │← Main container stack
│  ├── rumahl-update-check.timer        │
│  └── rumahl-dev-bridge.service (dev)  │
└─────────────────────────────────────┘
    ↓
┌─────────────────────────────────────┐
│ Watchdog Services                    │
...
└─────────────────────────────────────┘
    ↓
System Ready - rumahl Home accessible at http://[ip]:8080
```
...
2. **rumahl-init-data.service**: Creates `/mnt/data/ora`, `/mnt/data/rauc`, `/mnt/data/backups`
3. **docker.service**: Starts Docker daemon
4. **rumahl-setup.service**: Starts first-boot wizard (no `.setup-complete` file exists)
5. **rumahl-stack.service**: Creates placeholder docker-compose.yml, waits for setup wizard
6. Setup wizard creates real docker-compose.yml and `.setup-complete` marker
7. **rumahl-stack.service**: Restarts with real compose file, pulls and starts containers

### Normal Subsequent Boots

1. **local-fs.target**: Mounts filesystems
2. **rumahl-init-data.service**: Validates directories exist (no-op if already present)
3. **docker.service**: Starts Docker daemon
4. **rumahl-setup.service**: Skips (`.setup-complete` exists)
5. **rumahl-stack.service**: Starts directly with existing docker-compose.yml
6. **rumahl-stack-watchdog.timer**: Activates, checks every 2 minutes

### Recovery Boot

1. Boot with kernel parameter `ora.recovery=1`
2. All normal services start
3. **rumahl-recovery.service**: Activates due to kernel parameter
...
**Recovery**:
- `rumahl-init-data.service` skips (ConditionPathIsMountPoint fails)
- `rumahl-stack.service` skips (ConditionPathIsDirectory fails)
...
- System reinstalls verified rumahl OS
...
- [ ] Boot with missing `/mnt/data/ora`
- [ ] Boot with read-only data partition
- [ ] Recovery boot (kernel param `ora.recovery=1`)
...
systemctl status rumahl-*

# Check critical path
systemctl status mnt-data.mount rumahl-init-data.service rumahl-stack.service
...
# Show critical chain
systemd-analyze critical-chain rumahl-stack.service
...
# Verify compose file
cat /mnt/data/ora/docker-compose.yml
```
...
3. **Preloaded Images**: Include common container images in rumahl OS image
