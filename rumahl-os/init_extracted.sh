...
RUMAHL_ADMIN_USER="admin"
RUMAHL_ADMIN_PASS=""
RUMAHL_LANGUAGE="de"
RUMAHL_COUNTRY="DE"
RUMAHL_UNIT_SYSTEM="metric"

# ── Modern dialog color theme ─────────────────────────────────────
# Dark, modern installer look: black canvas with the rumahl blue accent
...
# rumahl OS installer -- modern dark theme (rumahl blue accent on black)
aspect = 0
separate_widget = ""
tab_len = 4
visit_items = ON
...
rumahl OS will apply optimizations to extend its lifespan:
...
rumahl_installer_hash_password() {
    local pw="$1"
    local salt hash
...
rumahl_installer_set_password() {
    ...
    local hash
    hash=$(rumahl_installer_hash_password "$pw") || hash=""
...
apply_post_install_config() {
    ...
    if [ -n "$RUMAHL_DOMAIN" ]; then
        # Update /etc/hosts with FQDN
        if [ -f "${target}/etc/hosts" ]; then
            local hostname="${RUMAHL_HOSTNAME:-ora}"
            # Add FQDN entry
            if ! grep -q "127.0.1.1" "${target}/etc/hosts" 2>/dev/null; then
                echo "127.0.1.1 ${hostname}.${RUMAHL_DOMAIN} ${hostname}" >> "${target}/etc/hosts"
            else
                sed -i "s/^127.0.1.1.*/127.0.1.1 ${hostname}.${RUMAHL_DOMAIN} ${hostname}/" "${target}/etc/hosts" 2>/dev/null || true
            fi
        fi

        # Set search domain in resolv.conf
        mkdir -p "${target}/etc" 2>/dev/null || true
        echo "search ${RUMAHL_DOMAIN}" > "${target}/etc/resolv.conf.local" 2>/dev/null || true
    fi
    ...
    if [ -n "$RUMAHL_ROOT_PW" ]; then
        rumahl_installer_set_password "$target" "root" "$RUMAHL_ROOT_PW" || {
    ...
    # 90-rumahl-wired-default.network as fallback in case the admin never
    ...
    if [ "$RUMAHL_NETWORK" = "static" ] && [ -n "$RUMAHL_IP" ]; then
        ...
        # DHCP configuration (IPv4 + IPv6 via DHCPv6/RA).
        {
            ...
            echo "DHCP=yes"
            echo "IPv6AcceptRA=yes"
            if [ "${RUMAHL_CUSTOM_DNS:-no}" = "yes" ] && [ -n "${RUMAHL_DNS:-}" ]; then
                echo "DNS=${RUMAHL_DNS}"
                [ -n "${RUMAHL_DNS2:-}" ] && echo "DNS=${RUMAHL_DNS2}"
            fi
            echo ""
...
LOGEOF
        ...
        echo "SD_OPTIMIZED=1" > "${target}/etc/rumahl-storage.conf" 2>/dev/null || true
        echo "STORAGE_TYPE=$(get_disk_type "$disk")" >> "${target}/etc/rumahl-storage.conf" 2>/dev/null || true
    fi

    # ── First-boot headless config ────────────────────────────────────────
    # All wizard answers are written to /etc/ora/setup-config.json on the
    # target. On the first boot rumahl-setup.service detects the file and
    ...
    if [ -n "$RUMAHL_ADMIN_USER" ] && [ -n "$RUMAHL_ADMIN_PASS" ]; then
        mkdir -p "${target}/etc/ora" 2>/dev/null || true
        cat > "${target}/etc/ora/setup-config.json" <<JSONEOF
{
  "hostname": "${RUMAHL_HOSTNAME:-ora}",
  "timezone": "${RUMAHL_TIMEZONE:-Europe/Berlin}",
  "language": "${RUMAHL_LANGUAGE:-de}",
  "country": "${RUMAHL_COUNTRY:-DE}",
  "unit_system": "${RUMAHL_UNIT_SYSTEM:-metric}",
  "admin_username": "${RUMAHL_ADMIN_USER}",
  "admin_password": "${RUMAHL_ADMIN_PASS}",
  "auto_start": true
}
JSONEOF
        chmod 600 "${target}/etc/ora/setup-config.json" 2>/dev/null || true
    fi
...
screen_domain() {
    ...
    result=$(dlg --title " Domain Name " --inputbox \
        ...
        14 60 "${RUMAHL_DOMAIN:-}" 3>&1 1>&2 2>&3)
    [ $? -ne 0 ] && return 0

    if valid_domain "$result"; then
        RUMAHL_DOMAIN="$result"
        return 0
    fi
...
screen_network() {
    ...
    if [ "$mode" = "dhcp" ]; then
        ...
        if [ "$custom_dns_choice" -eq 0 ]; then
            while true; do
                RUMAHL_DNS=$(dlg --title " Custom DNS Server " --inputbox \
                    "\n Enter your preferred DNS server IPv4 address.\n" \
                    10 60 "${RUMAHL_DNS:-8.8.8.8}" 3>&1 1>&2 2>&3)
                [ $? -ne 0 ] && break
                if valid_ipv4 "$RUMAHL_DNS"; then
                    RUMAHL_CUSTOM_DNS="yes"
                    break
                fi
...
# These are handed to rumahl-home via the headless apply (setup-config.json):
...
screen_admin() {
    ...
    result=$(dlg --title " Web Admin User " --inputbox \
        ...
        11 60 "${RUMAHL_ADMIN_USER:-admin}" 3>&1 1>&2 2>&3)
    [ $? -ne 0 ] && return 1
    result="${result:-admin}"
...
screen_locale() {
    ...
    lang=$(dlg --title " Language " --menu \
        ...
        "en" "English" \
        3>&1 1>&2 2>&3)
    [ $? -eq 0 ] && [ -n "$lang" ] && RUMAHL_LANGUAGE="$lang"
    ...
    country=$(dlg --title " Country " --menu \
        ...
        "GB" "United Kingdom" \
        3>&1 1>&2 2>&3)
    [ $? -eq 0 ] && [ -n "$country" ] && RUMAHL_COUNTRY="$country"
    ...
    unit=$(dlg --title " Units " --menu \
        ...
        3>&1 1>&2 2>&3)
    [ $? -eq 0 ] && [ -n "$unit" ] && RUMAHL_UNIT_SYSTEM="$unit"
}
...
screen_confirm() {
    ...
    if [ -n "$RUMAHL_ADMIN_USER" ] && [ -n "$RUMAHL_ADMIN_PASS" ]; then
        summary="${summary}  Web Admin:  ${RUMAHL_ADMIN_USER} (password set)\n"
    else
        summary="${summary}  Web Admin:  none (register from the UI)\n"
    fi
    summary="${summary}  Locale:     ${RUMAHL_LANGUAGE}/${RUMAHL_COUNTRY} (${RUMAHL_UNIT_SYSTEM})\n"
...
screen_install() {
    ...
    if [ -n "$DIALOG_BIN" ]; then
        (
            ...
            echo "5"
            echo "XXX"
            echo "  Decompressing and writing rumahl OS to /dev/${disk}..."
            ...
            echo "XXX"
        ) | dlg --title " Installing rumahl OS " --gauge \
...
screen_complete() {
    ...
    if [ "$RUMAHL_NETWORK" = "static" ] && [ -n "$RUMAHL_IP" ]; then
        ...
        [ "$rumahl_ip" = "<IP>" ] && rumahl_ip="${RUMAHL_HOSTNAME}.local"
    fi
    ...
    if [ -n "$DIALOG_BIN" ]; then
        ...
        if [ -n "$RUMAHL_ADMIN_USER" ] && [ -n "$RUMAHL_ADMIN_PASS" ]; then
            admin_line="\
 Dashboard admin: ${RUMAHL_ADMIN_USER} (password set)\n"
        fi
...
${admin_line}
 ...
 Remove the installation media before continuing.\n" \
        ...
        echo "  Dashboard: http://${rumahl_ip}:8126"
        if [ -n "$RUMAHL_ADMIN_USER" ] && [ -n "$RUMAHL_ADMIN_PASS" ]; then
            echo "  Admin user: ${RUMAHL_ADMIN_USER} (password set)"
        fi
        echo ""
...
run_express_wizard() {
    local BACKTITLE_BASE="rumahl OS Installer  |  Quick Install"
    ...
    if [ -z "$RUMAHL_ADMIN_PASS" ]; then
        RUMAHL_ADMIN_PASS=$(head -c 18 /dev/urandom 2>/dev/null | base64 2>/dev/null | tr -dc 'A-Za-z0-9' | head -c 16)
        [ -z "$RUMAHL_ADMIN_PASS" ] && RUMAHL_ADMIN_PASS="rumahl-admin-$(date +%s)"
    fi
...
run_wizard() {
    ...
    # Mount media with retries
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Loading"
    dlg_info " Scanning " "  Searching for installation media..."
    sleep 1
 ...
 Installation will not continue."
    ...
    # Step 1: System info
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 1/${_TOTAL}: System Info"
    screen_sysinfo

    # Step 2: Hostname
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 2/${_TOTAL}: Hostname"
    screen_hostname

    # Step 3: Domain (optional)
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 3/${_TOTAL}: Domain"
    screen_domain

    # Step 4: Timezone
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 4/${_TOTAL}: Timezone"
    screen_timezone

    # Step 5: Driver detection (optional)
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 5/${_TOTAL}: Drivers"
    screen_drivers

    # Step 6: Network
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 6/${_TOTAL}: Network"
    screen_network

    # Step 7: Root password
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 7/${_TOTAL}: Password"
    while true; do
 ...
 No   -> go back and set a secure password"; then
    ...
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 8/${_TOTAL}: Web Admin"
    while true; do
 ...
 No   -> create the admin account"; then
    ...
    # Step 9: Locale
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 9/${_TOTAL}: Locale"
    screen_locale

    # Step 10: Disk selection
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 10/${_TOTAL}: Target Disk"
    if ! screen_select_disk; then
        dlg_msg " Cancelled " "Installation cancelled."
        return 1
    fi
    ...
    BACKTITLE="rumahl OS Installer  |  Custom Install -- Step 11/${_TOTAL}: Confirm & Install"
    if ! screen_confirm; then
        dlg_msg " Cancelled " "Installation cancelled.\nNo changes were made."
        return 1
    fi
    ...
    # Install
    BACKTITLE="rumahl OS Installer  |  Installing..."
    if ! screen_install; then
        return 1
    fi
...
#!/bin/sh
# Restart the rumahl OS installation wizard
AC='\033[38;5;39m'
GR='\033[38;5;245m'
RS='\033[0m'
echo ""
echo "  ${GR}+--------------------------------------------+${RS}"
echo "  ${GR}|${RS}  ${AC}Restarting rumahl OS Installation Wizard...${RS}  ${GR}|${RS}"
echo "  ${GR}+--------------------------------------------+${RS}"
echo ""
sleep 1
exec /init
...
echo ""
echo "  ${GR}+--------------------------------------------+${RS}"
echo "  ${GR}|${RS}      ${AC}rumahl OS System Information${RS}          ${GR}|${RS}"
echo "  ${GR}+--------------------------------------------+${RS}"
echo ""
...
echo ""
echo "  ${GR}+--------------------------------------------+${RS}"
echo "  ${GR}|${RS}   ${AC}rumahl OS Installer - Available Commands${RS}   ${GR}|${RS}"
echo "  ${GR}+--------------------------------------------+${RS}"
echo ""
echo "  ${CY}install${RS}     - Restart the rumahl OS installation wizard"
...
cat <<'BANNER'
  ...
  +--------------------------------------------------------------+
  |                                                              |
  |        rumahl OS Installation - Recovery Shell                |
  |                                                              |
  +--------------------------------------------------------------+
