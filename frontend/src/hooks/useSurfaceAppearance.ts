...
export const DEFAULT_SURFACE_APPEARANCE = { style: 'solid', opacity: 68, blur: 24, tint: '#8b5cf6', tintStrength: 10 }
...
export function useSurfaceAppearance() {
  ...
  const { style, opacity, blur, tint, tintStrength } = settings
  useEffect(() => {
  ...
  }, [style, opacity, blur, tint, tintStrength])
