/**
 * Minimal ZIP builder (store method, no compression) + the rumahl store
...
export interface CatalogAppDefinition {
   ...
   * guest (/api/apps/<id>/proxy). Used by the rumahl Browser (rumahl-browserd). */
  proxyOnly?: boolean
...
const astroForgeManifest = {
  id: 'rumahl-astro-forge',
  name: 'Astro Forge',
  version: '1.0.0',
  developer: 'rumahl Team',
  description: '3D-Idle-Game: Mine Asteroiden, baue deine Bohrflotte aus, erobere neue Planeten und prestigiere durch Wurmlöcher. Fortschritt wird über das rumahl App Storage gespeichert — auch offline wird weiter gefördert.',
  icon: gamepadSvg(),
  type: 'app',
  ...
  custom_pages: [
    {
      id: 'rumahl-astro-forge-main',
      title: 'Astro Forge',
      icon: 'GameController',
      url: '/apps/rumahl-astro-forge/',
      show_in_nav: true,
      order: 220,
    },
  ],
...
const nextcloudManifest = {
  ...
  // App Embedding Gateway: runs inside the rumahl desktop on its own origin.
  display: {
    mode: 'embedded',
    isolation: 'strict',
  ...
  docker: {
    ...
    environment: {
      ...
      NEXTCLOUD_ADMIN_PASSWORD: 'rumahl-admin',
...
const oraBrowserManifest = {
  ...
  version: '0.1.0',
  developer: 'rumahl OS',
  description: 'Der rumahl Browser — ein echter Browser als rumahl-Systemdienst. Die Seiten laufen in Chromium auf dem rumahl-Server (nicht als eingebettete Seite): keine X-Frame-Options-/CSP-Probleme, Tabs und Sessions gehören rumahl.',
  type: 'app',
  icon: 'browser',
...
export const STORE_CATALOG: CatalogAppDefinition[] = [
  {
    ...
    name: 'Nextcloud',
    developer: 'Nextcloud GmbH',
    description: 'Deine eigene Cloud: Dateien, Kalender, Kontakte & mehr — vollständig selbst gehostet in einem Docker-Container auf deinem rumahl OS.',
    version: '1.0.0',
    category: 'Cloud',
    permissions: ['AppStorageRead', 'AppStorageWrite'],
    openPort: 8180,
    iconUrl: cloudSvg(),
    defaultCredentials: { username: 'admin', password: 'rumahl-admin' },
  ...
  },
  {
    ...
    name: 'rumahl Browser',
    developer: 'rumahl OS',
    description: 'Der rumahl Browser — ein echter Browser als rumahl-Systemdienst. Seiten laufen in Chromium auf deinem rumahl-Server, nicht als eingebettete Seite: X-Frame-Options und CSP sind egal, Tabs & Sessions gehören rumahl.',
    version: '0.1.0',
    category: 'Browser',
    permissions: ['NetworkLocalAccess'],
    openPort: 8102,
  ...
  }),
  {
    ...
    name: "Notizen",
    developer: 'rumahl Team',
    description: "Einfache Notizen-App \u2013 jede Notiz wird \u00fcber das rumahl App Storage (KV) gespeichert. Zeigt, wie eine installierbare App das App-Framework nutzt.",
    version: '1.0.0',
  ...
  },
  {
    ...
    name: "Einkaufsliste",
    developer: 'rumahl Team',
    description: "Gemeinsame Einkaufsliste \u2013 Artikel mit Menge und Erledigt-H\u00e4kchen, gespeichert \u00fcber das rumahl App Storage.",
    version: '1.0.0',
    category: "Haushalt",
    permissions: ['AppStorageRead', 'AppStorageWrite', 'AppStorageDelete', 'AppStorageManage'],
  ...
  },
  {
    ...
    name: "Kalender",
    developer: 'rumahl Team',
    description: "Einfacher Familien-Kalender \u2013 Termine mit Datum und Uhrzeit, gespeichert \u00fcber das rumahl App Storage.",
    version: '1.0.0',
    ...
    openPort: 19003,
    buildZip: () => buildAppZip({
      'manifest.json': "{\n  \"id\": \"rumahl-calendar\",\n  \"name\": \"Kalender\",\n  \"version\": \"1.0.0\",\n  \"developer\": \"rumahl Team\",\n  \"description\": \"Einfacher Familien-Kalender \u2013 Termine mit Datum und Uhrzeit, gespeichert \u00fcber das rumahl App Storage.\",\n  \"icon\": \"https://img.icons8.com/color/256/calendar.png\",\n  \"type\": \"app\",\n  \"permissions\": [\n    \"AppStorageRead\",\n    \"AppStorageWrite\",\n    \"AppStorageDelete\",\n    \"AppStorageManage\"\n  ],\n  \"storage\": {\n    \"enabled\": true,\n    \"quota\": {\n      \"max_file_storage_bytes\": 5242880,\n      \"max_kv_entries\": 100,\n      \"max_file_size_bytes\": 1048576\n    }\n  },\n  \"custom_pages\": [\n    {\n      \"id\": \"rumahl-calendar-main\",\n      \"title\": \"Kalender\",\n      \"icon\": \"Calendar\",\n      \"url\": \"/apps/rumahl-calendar/\",\n      \"show_in_nav\": true,\n      \"order\": 212\n    }\n  ],\n  \"docker\": {\n    \"auto_build\": true,\n    \"base_image\": \"node:18-alpine\",\n    \"working_dir\": \"/app\",\n    \"install_cmd\": \"npm install\",\n    \"start_cmd\": \"node server.js\",\n    \"internal_ports\": [\n      {\n        \"port\": 3000,\n        \"external\": 19003,\n        \"protocol\": \"tcp\",\n        \"description\": \"HTTP API und Web-UI\"\n      }\n    ],\n    \"environment\": {\n      \"NODE_ENV\": \"production\"\n    },\n    \"health_check\": {\n      \"endpoint\": \"/health\",\n      \"interval\": 30,\n      \"timeout\": 10,\n      \"retries\": 3\n    }\n  },\n  \"store_metadata\": {\n    \"category\": \"Organisation\",\n    \"tags\": [\n      \"kalender\",\n      \"termin\",\n      \"familie\"\n    ],\n    \"min_rumahl_version\": \"2.0.0\",\n    \"license\": \"MIT\"\n  }\n}",
      'server.js': "/**\n * rumahl Calendar \u2014 installable lifestyle app (Package 6).\n * Persists data via the rumahl App Storage KV API using the app token.\n */\nconst express = require('express');\nconst path = require('path');\n\nconst app = express();\nconst PORT = process.env.PORT || 3000;\nconst rumahl_HOME = process.env.rumahl_HOME_URL || 'http://rumahl-home:3001';\nconst APP_ID = process.env.rumahl_APP_ID || 'rumahl-calendar';\nconst KV_KEY = 'events';\n\napp.use(express.json({ limit: '1mb' }));\napp.use(express.static(path.join(__dirname, 'public')));\n\nfunction headers() {\n  const h = { 'Content-Type': 'application/json' };\n  if (process.env.rumahl_API_KEY) h['Authorization'] = 'Bearer ' + process.env.rumahl_API_KEY;\n  return h;\n}\n\nasync function loadData() {\n  try {\n    const r = await fetch(`${rumahl_HOME}/api/apps/${APP_ID}/storage/kv/${KV_KEY}`, { headers: headers() });\n    if (!r.ok) return [];\n    const data = await r.json();\n    return Array.isArray(data.value) ? data.value : [];\n  } catch { return []; }\n}\n\nasync function saveData(data) {\n  await fetch(`${rumahl_HOME}/api/apps/${APP_ID}/storage/kv/${KV_KEY}`, {\n    method: 'PUT', headers: headers(),\n    body: JSON.stringify({ key: KV_KEY, value: data }),\n  });\n}\n\napp.get('/api/items', async (_req, res) => res.json(await loadData()));\napp.post('/api/items', async (req, res) => {\n  const items = await loadData();\n  const item = { id: Date.now().toString(36) + Math.random().toString(36).slice(2, 7), created_at: new Date().toISOString(), ...(req.body || {}) };\n  items.push(item);\n  await saveData(items);\n  res.status(201).json(item);\n});\napp.put('/api/items/:id', async (req, res) => {\n  const items = await loadData();\n  const index = items.findIndex((i) => i.id === req.params.id);\n  if (index === -1) return res.status(404).json({ error: 'not found' });\n  items[index] = { ...items[index], ...(req.body || {}), updated_at: new Date().toISOString() };\n  await saveData(items);\n  res.json(items[index]);\n});\napp.delete('/api/items/:id', async (req, res) => {\n  const items = await loadData();\n  await saveData(items.filter((i) => i.id !== req.params.id));\n  res.json({ deleted: true });\n});\napp.get('/health', (_req, res) => res.json({ status: 'ok' }));\napp.listen(PORT, () => console.log(`rumahl Calendar listening on :${PORT} (storage via ${rumahl_HOME})`));\n",
      'package.json': "{\n  \"name\": \"rumahl-calendar\",\n  \"version\": \"1.0.0\",\n  \"private\": true,\n  \"main\": \"server.js\",\n  \"scripts\": {\n    \"start\": \"node server.js\"\n  },\n  \"dependencies\": {\n    \"express\": \"^4.19.2\"\n  }\n}",
 ...
 * savegame between the browser and the rumahl App Storage KV API
 * (GET/PUT /api/apps/<app-id>/storage/kv/savegame) using the app token
 * injected by the supervisor. The app never talks to the rumahl database
...
const PORT = process.env.PORT || 3000;
const rumahl_HOME = process.env.rumahl_HOME_URL || 'http://rumahl-home:3001';
const APP_ID = process.env.rumahl_APP_ID || 'rumahl-astro-forge';
...
app.put('/api/save', async (req, res) => {
  try {
    const r = await fetch(rumahl_HOME + '/api/apps/' + APP_ID + '/storage/kv/' + KV_KEY, {
      method: 'PUT',
      headers: headers(),
...
app.listen(PORT, () => {
  console.log('Astro Forge listening on :' + PORT + ' (storage via ' + rumahl_HOME + ')');
});
`,
      'package.json': JSON.stringify({
        name: 'rumahl-astro-forge',
        version: '1.0.0',
        private: true,
        main: 'server.js',
