...
export function OsSecurityApp() {
  ...
  return <section className="min-h-full p-5 text-foreground">
    <header className="mb-5 flex items-start justify-between gap-4">
      <div><div className="flex items-center gap-2"><ShieldCheck size={28} className="text-emerald-400"/><h1 className="text-2xl font-semibold">{t('os.securityCenter.title')}</h1></div><p className="mt-1 text-sm text-foreground/55">{t('os.securityCenter.description')}</p></div>
      <div className="flex gap-2"><button onClick={() => void load()} className="rounded-xl border border-foreground/10 p-2 hover:bg-foreground/5" aria-label={t('common.refresh')}><ArrowClockwise size={18}/></button><OsWindowActions /></div>
    </header>
