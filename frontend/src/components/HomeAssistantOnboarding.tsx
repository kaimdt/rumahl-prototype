...
export function HomeAssistantOnboarding({ onComplete }: { onComplete?: () => void }) {
  ...
  return <div className="glass-card rounded-3xl border border-cyan-400/15 p-6 sm:p-8">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto]"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder={t('haOnboarding.urlPlaceholder')} className="rumahl-modal-input" /><button type="button" onClick={() => void discover()} disabled={discovering} className="rumahl-secondary-button justify-center"><MagnifyingGlass size={17} />{discovering ? t('haOnboarding.discovering') : t('haOnboarding.discover')}</button></div>
    ...
    <div className="mt-5"><label className="mb-2 flex items-center gap-2 text-xs font-semibold text-foreground/55"><ShieldCheck size={16} />{t('haOnboarding.token')}</label><input type="password" value={token} onChange={(event) => setToken(event.target.value)} placeholder={t('haOnboarding.tokenPlaceholder')} className="rumahl-modal-input w-full" /><p className="mt-2 text-xs text-foreground/40">{t('haOnboarding.tokenHelp')}</p></div>
    <button type="button" onClick={() => void connect()} disabled={!url.trim() || !token.trim() || connecting} className="rumahl-primary-button mt-5 w-full justify-center disabled:opacity-40">{connecting ? <ArrowClockwise size={17} className="animate-spin" /> : <Plug size={17} />}{connecting ? t('haOnboarding.connecting') : t('haOnboarding.connect')}</button>
  </div>
}
