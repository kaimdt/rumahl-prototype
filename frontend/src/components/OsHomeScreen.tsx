    ...
    >
      {layout === 'default' && (
        <div className="mx-auto mt-5 max-w-6xl px-1">
          <div className="rumahl-home-hero mb-4 text-left">
            <AnimatePresence mode="wait" initial={false}>
              {greetingVisible ? (
                <motion.div key="greeting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: DUR_BASE, ease: EASE_OS }}>
                  <h1 className="text-xl font-semibold tracking-tight sm:text-2xl">{t('os.greeting', { name: user?.displayName || user?.username || t('os.defaultUser') })}</h1>
                  <p className="mt-1 text-sm">{t('os.subtitle')}</p>
                </motion.div>
              ) : (
                <motion.div key="clock" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: DUR_BASE, ease: EASE_OS }}>
                  <h1 className="text-3xl font-semibold tabular-nums tracking-tight sm:text-4xl">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {STORE_CATALOG.map((def) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setCurrentPageId('app-store')}
                    className="rumahl-card flex items-center gap-3 rounded-2xl p-4 text-left transition-all  hover:border-foreground/15 "
                  >
      ...
      {layout === 'deck' && (
        <div className="mx-auto mt-7 grid max-w-7xl gap-5 px-1 lg:grid-cols-[0.85fr_1.15fr]">
          <div className="rumahl-card rounded-lg p-5 sm:p-7"><p className="text-xs uppercase tracking-[0.18em] text-accent">{t('os.launcher.intelligent')}</p><h2 className="mt-2 text-2xl font-semibold sm:text-3xl">{t('os.launcher.whatToDo')}</h2><label className="rumahl-command-search mt-6 flex min-h-14 items-center gap-3 rounded-2xl border border-foreground/12 bg-foreground/5 px-4 transition-all focus-within:border-accent/50 focus-within:shadow-[0_0_0_3px_color-mix(in_oklch,var(--accent)_14%,transparent)]"><MagnifyingGlass size={20} /><input ref={searchInput} aria-label={t('os.search')} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.nativeEvent.isComposing && visibleApps[0]) openApp(visibleApps[0]); if (event.key === 'Escape') setQuery('') }} placeholder={t('os.launcher.commandPlaceholder')} className="w-full bg-transparent text-sm outline-none" /></label><div className="mt-5 flex flex-wrap gap-2">{apps.slice(0, 4).map((app) => <button key={app.id} type="button" onClick={() => openApp(app)} className="rounded-full border border-foreground/10 bg-foreground/5 px-3 py-2 text-xs transition-colors hover:bg-foreground/10">{getName(app)}</button>)}</div></div>
          <div className="space-y-2"><p className="mb-3 px-2 text-sm font-semibold">{t('os.allApps')}</p>{visibleApps.map((app) => <button key={app.id} type="button" onClick={() => openApp(app)} className="rumahl-card group flex min-h-20 w-full touch-manipulation items-center gap-4 rounded-2xl p-3 text-left transition-all  hover:bg-foreground/10  focus-ring"><AppIcon app={app} /><span className="min-w-0 flex-1"><span className="block font-semibold">{getName(app)}</span><span className="block truncate text-xs text-foreground/45">{getDescription(app)}</span></span><ArrowRight size={18} className="text-foreground/35 transition-transform group-hover:translate-x-0.5" /></button>)}</div>
        </div>
      )}

      {layout === 'canvas' && (
        <div className="mx-auto mt-8 max-w-7xl overflow-hidden px-1"><label className="rumahl-command-search rumahl-card mx-auto mb-10 flex min-h-12 max-w-sm items-center gap-3 rounded-full px-4 transition-shadow focus-within:shadow-[0_0_0_3px_color-mix(in_oklch,var(--accent)_14%,transparent)]"><MagnifyingGlass size={19} /><input ref={searchInput} aria-label={t('os.search')} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.nativeEvent.isComposing && visibleApps[0]) openApp(visibleApps[0]); if (event.key === 'Escape') setQuery('') }} placeholder={t('os.search')} className="w-full bg-transparent text-sm outline-none" /></label><div className="flex snap-x snap-mandatory gap-4 overflow-x-auto px-[10vw] pb-6 [scrollbar-width:none]">{visibleApps.map((app, index) => <button key={app.id} type="button" onClick={() => openApp(app)} className={`rumahl-card group min-h-72 shrink-0 snap-center rounded-lg p-5 text-left transition-all duration-300   focus-ring ${index === 0 ? 'w-[min(78vw,28rem)]' : 'w-[min(68vw,20rem)]'}`}><AppIcon app={app} size="large" /><span className="mt-20 block text-2xl font-semibold">{getName(app)}</span><span className="mt-2 block text-sm text-foreground/50">{getDescription(app)}</span><span className="mt-5 inline-flex items-center gap-2 text-sm text-accent">{t('os.launcher.open')}<ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></span></button>)}</div></div>
      )}
      ...
      {layout === 'default' && (
        <div className="mx-auto max-w-6xl px-1">
          {widgetIds.length > 0 && <div className="mt-6 mb-5 grid gap-3 sm:grid-cols-2">
            {widgetIds.includes('home') && <button type="button" onClick={() => homeApp && openApp(homeApp)} className="rumahl-card group relative flex min-h-32 touch-manipulation items-center gap-4 rounded-lg p-5 text-left focus-ring sm:p-6">
            ...
            {widgetIds.includes('clock') && <div className="rumahl-card relative flex min-h-32 items-center justify-between rounded-lg p-5">
            ...
            {widgetIds.includes('system') && (
              <div className="rumahl-card relative flex min-h-32 items-center justify-between rounded-lg p-5 sm:col-span-2">
                <div>
