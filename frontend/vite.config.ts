...
// rumahl.local is only resolvable inside the dev VM — on the host the
// backend (Vite dev or nginx forward) lives on localhost:3001.
// `rumahl.local` is the host-side alias; inside the VM the name cannot be
// resolved — always normalize to localhost so the dev proxy keeps working.
const backendTarget = (process.env.VITE_RUMAHL_BACKEND_URL || 'http://localhost:3001').replace(/ora\.local/g, 'localhost')
  ...
  server: {
    proxy: {
      '/api': {
        ...
        changeOrigin: true,
        // Relay WebSocket upgrades (app proxy WS tunnel: the rumahl Browser
