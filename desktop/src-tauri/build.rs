fn main() {
    ...
    let iora_backend_url = std::env::var("IORA_BACKEND_URL")
        .unwrap_or_else(|_| "http://localhost:8092".to_string());
    println!("cargo:rustc-env=IORA_BACKEND_URL_DEFAULT={}", iora_backend_url);
    ...
    let health_poll_interval = std::env::var("HEALTH_POLL_INTERVAL_SECS")
        .unwrap_or_else(|_| "30".to_string());
    println!("cargo:rustc-env=HEALTH_POLL_INTERVAL_DEFAULT={}", health_poll_interval);

    let ha_update_interval = std::env::var("HA_UPDATE_INTERVAL_SECS")
        .unwrap_or_else(|_| "60".to_string());
    println!("cargo:rustc-env=HA_UPDATE_INTERVAL_DEFAULT={}", ha_update_interval);
