//! Tauri commands for Home Assistant integration and system control.
...
pub async fn test_ha_connection(state: State<'_, AppState>) -> Result<HaTestResult, String> {
    ...
    if ha_config.token.is_empty() {
        return Ok(HaTestResult {
            connected: false,
            error: Some("Kein Home Assistant Token konfiguriert".to_string()),
        });
    }
    ...
    match client.test_connection().await {
        ...
        Ok(false) => Ok(HaTestResult {
            connected: false,
            error: Some("Verbindung fehlgeschlagen".to_string()),
        }),
...
/// Send current metrics to Home Assistant
#[tauri::command]
...
pub async fn call_ha_service(
    ...
    let client = HaClient::new(ha_config);
    client
        .call_service(&domain, &service, entity_id.as_deref(), data)
        .await
        .map_err(|e| e.to_string())
}
