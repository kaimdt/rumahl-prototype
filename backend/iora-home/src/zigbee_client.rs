...
impl ZigbeeClient {
    ...
    pub async fn update_config(&self, config: ZigbeeConfig) {
        info!("Zigbee: Config updated (mode={:?}, enabled={})", config.mode, config.enabled);
    ...
    pub async fn refresh_from_entities(&self, entities: &[crate::EntityState], ha_integrations: &[String]) {
        // Detect mode
        let has_z2m = ha_integrations.iter().any(|i| i == "mqtt" || i == "zigbee2mqtt");
        ...
        for entity in &zigbee_entities {
            ...
            if seen.insert(device_id.clone()) {
                ...
                devices.push(ZigbeeDevice {
                    ...
                    power_source: entity.attributes.get("power_source").and_then(|v| v.as_str()).map(String::from),
                    lqi: entity.attributes.get("lqi").and_then(|v| v.as_u64()).map(|v| v as u32),
    ...
    pub async fn refresh_from_z2m_data(&self, bridge_info: Option<&str>, devices_json: Option<&str>) {
        if let Some(info) = bridge_info {
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(info) {
                let mut net = self.network.write().await;
                net.coordinator = parsed.get("coordinator").and_then(|c| c.get("type")).and_then(|v| v.as_str()).map(String::from);
                if let Some(config) = parsed.get("config").and_then(|c| c.get("advanced")) {
                    net.channel = config.get("channel").and_then(|v| v.as_u64()).map(|v| v as u32);
                    net.pan_id = config.get("pan_id").and_then(|v| v.as_str()).map(String::from);
                }
                net.permit_join = parsed.get("permit_join").and_then(|v| v.as_bool()).unwrap_or(false);
            }
        }
        ...
        if let Some(devs) = devices_json {
            if let Ok(parsed) = serde_json::from_str::<Vec<serde_json::Value>>(devs) {
                ...
                for d in &parsed {
                    ...
                    devices.push(ZigbeeDevice {
                        ieee_address: d.get("ieee_address").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                        friendly_name: d.get("friendly_name").and_then(|v| v.as_str()).unwrap_or("Unknown").to_string(),
                        device_type: d.get("type").and_then(|v| v.as_str()).unwrap_or("unknown").to_string(),
                        ...
                        power_source: d.get("power_source").and_then(|v| v.as_str()).map(String::from),
                        lqi: d.get("lqi").and_then(|v| v.as_u64()).map(|v| v as u32),
                        battery: None,
                        reachable: d.get("supported").and_then(|v| v.as_bool()).unwrap_or(true),
                        last_seen: d.get("last_seen").and_then(|v| v.as_str()).map(String::from),
                    });
                }
