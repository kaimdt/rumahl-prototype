...
--
-- channel_type values:
--   'iora'       – internal IORA dashboard notification (always on, no extra config)
--   'ha_mobile'  – Home Assistant mobile_app notify service  (target_id = notify service name, e.g. "mobile_app_pixel_8")
--   'desktop'    – IORA Desktop Client (target_id = desktop client identifier / '*' for all connected)
--
CREATE TABLE IF NOT EXISTS notification_channels (
    id          TEXT PRIMARY KEY,
    name        TEXT NOT NULL,
    channel_type TEXT NOT NULL,         -- 'iora' | 'ha_mobile' | 'desktop'
