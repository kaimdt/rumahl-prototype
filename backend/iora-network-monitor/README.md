...
The IORA Network Monitor service provides network discovery and IP activity tracking for the IORA ecosystem.
...
- `RUST_LOG`: Log level (default: `iora_network_monitor=info`)
...
```
http://your-iora-instance/api/network/
```
...
# Production
cargo build --release
./target/release/iora-network-monitor
```
...
- **iora-security**: Can use network data for threat detection
- **iora-control**: Provides UI for network device management
- **iora-nginx**: Routes API requests to this service
