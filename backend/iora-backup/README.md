...
Comprehensive backup and restore functionality for IORA OS.

## Features

### ✅ Complete System Backups
- **Databases**: All PostgreSQL databases (iora_home, iora_core, iora_security, iora_secrets, etc.)
- **Docker Volumes**: All IORA-managed Docker volumes
- **System Configuration**: /etc/iora/, docker-compose.yml, environment files
- **User Data**: /var/lib/iora/ directory with all user files
- **Apps**: Installed app data from /var/lib/iora/apps/
...
- Integrated with iora-updater service
...
```json
{
  ...
  "local_path": "/var/lib/iora/backups/backup_550e8400_20260422_143000.tar.gz",
  "remote_path": "/backups/iora/backup_550e8400_20260422_143000.tar.gz",
...
Create pre-update backup (called automatically by iora-updater).
...
| `DATABASE_URL` | `postgres://iora:changeme@postgres:5432/iora_backup` | PostgreSQL connection string |
| `POSTGRES_HOST` | `postgres` | PostgreSQL host for pg_dump |
| `POSTGRES_USER` | `iora` | PostgreSQL username |
| `POSTGRES_PASSWORD` | `changeme` | PostgreSQL password |
| `BACKUP_DIR` | `/var/lib/iora/backups` | Local backup storage directory |
| `RUST_LOG` | `info` | Log level (error, warn, info, debug, trace) |
| `IORA_OS` | `false` | Set to `true` on IORA OS for enhanced features |
...
│   └── .env
├── userdata/
│   └── (contents of /var/lib/iora/)
└── apps/
    └── (contents of /var/lib/iora/apps/)
```
...
- On IORA OS, this is protected by AppArmor profiles
...
docker logs iora-backup

# Check available disk space
df -h /var/lib/iora/backups

# Verify PostgreSQL connectivity
docker exec iora-backup pg_dump --version
```
...
docker logs iora-backup | grep -i error
```
...
tar -tzf /var/lib/iora/backups/backup_*.tar.gz | head
...
## Integration with IORA Services

### iora-updater Integration
The backup service automatically integrates with iora-updater:
1. Update check finds new version
2. iora-updater calls `/api/backup/pre-update`
...
### iora-supervisor Integration (IORA OS)
On IORA OS, the supervisor can trigger backups:
- Before major configuration changes
- Before app installations
- On user request via Control Panel

### iora-home Integration
...
Part of the IORA OS project. See main repository LICENSE file.

## Support

- Documentation: https://docs.iora.io/backup
