...
import { BACKEND, ROOT, databaseUrlFor, discoverServices } from "./iora-dev.mjs";
...
test("does not autostart development-image applications with system services", () => {
  const services = discoverServices();
  assert.equal(services.find(service => service.id === "iora-developer-app")?.autostart, false);
  assert.equal(services.find(service => service.id === "iora-dev-bridge")?.autostart, false);
});

test("uses initialized dedicated databases and the shared development database", () => {
  const env = { POSTGRES_USER: "iora", POSTGRES_PASSWORD: "secret", POSTGRES_PORT: "5544" };
  assert.equal(databaseUrlFor("iora-core", env), "postgres://iora:secret@127.0.0.1:5544/iora_core");
  assert.equal(databaseUrlFor("iora-backup", env), "postgres://iora:secret@127.0.0.1:5544/iora_home");
  assert.equal(databaseUrlFor("iora-home", { ...env, IORA_HOME_DB_URL: "postgres://custom/home" }), "postgres://custom/home");
});
