#!/usr/bin/env node
// ╔═══════════════════════════════════════════════════════════════════╗
// ║  rumahl Dev Runner — Interactive development service manager       ║
// ║  Works on Windows & Linux · No dependencies · Node.js ≥ 18      ║
// ║  Auto-discovers services from filesystem                         ║
// ╚═══════════════════════════════════════════════════════════════════╝
//
// Usage:
//   node dev/rumahl-dev.mjs              — interactive mode
//   node dev/rumahl-dev.mjs start all    — start everything
//   node dev/rumahl-dev.mjs start home   — start single service
//   node dev/rumahl-dev.mjs --watch      — enable hot-reload on start
//
...
export const BACKEND = join(ROOT, "rumahl-os", "backend");
const FRONTEND = join(ROOT, "frontend");
const LOG_DIR = join(import.meta.dirname, ".logs");
const COMPOSE_FILE = join(ROOT, "deploy", "docker-compose.yml");
const DEV_LOCAL_STATE = join(ROOT, "rumahl-os", ".cache", "runtime-state.json");
...
const SERVICE_PORTS = {
  "rumahl-home": 3001, "rumahl-core": 8090, "rumahl-control": 8091,
  "rumahl-assist": 8092, "rumahl-secrets": 8093, "rumahl-watchdog": 8094,
  "rumahl-security": 8095, "rumahl-gateway": 8096, "rumahl-supervisor": 8097,
  "rumahl-appstore": 8098, "rumahl-intelligence": 8099, "rumahl-files": 8100,
  "rumahl-api": 8101, "rumahl-connector": 8102, "rumahl-network-monitor": 8103,
  "rumahl-domain-validator": 8104, "rumahl-resource-manager": 8105,
  "rumahl-updater": 8106, "rumahl-backup": 8107, "rumahl-nginx": 8108,
};
...
export function discoverServices(root = ROOT, backend = BACKEND) {
  ...
  // 3. rumahl OS development VM (PowerShell dev-local.ps1). It is explicit-only
  // because it starts QEMU and may download/create the VM image on first use.
  if (existsSync(join(root, "rumahl-os", "dev-local.ps1"))) {
    services.push({
      id: "rumahl-dev-vm",
      name: "rumahl Dev VM",
      port: 2222,
      cwd: join(root, "rumahl-os"),
      cmd: IS_WIN ? "powershell.exe" : "pwsh",
      args: ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", join(root, "rumahl-os", "dev-local.ps1"), "-NoWatch"],
      reinstallArgs: ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", join(root, "rumahl-os", "dev-local.ps1"), "-Rebuild", "-NoWatch"],
      group: "core",
      type: "dev-vm",
      srcDir: join(root, "rumahl-os"),
      autostart: false,
    });
  }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        services.push({
          ...
          autostart: crateRoot === join(backend, "services") && pkgName !== "rumahl-nginx",
        });
      } catch {}
    }
  }
...
function formatCrateName(name) {
  return name.replace(/^rumahl-/, "rumahl ").replace(/\b\w/g, c => c.toUpperCase());
}

function categorize(name, desc) {
  const core = ["rumahl-home", "rumahl-core", "rumahl-gateway", "rumahl-security", "rumahl-secrets", "rumahl-control"];
...
export function databaseUrlFor(serviceId, env = process.env) {
  ...
  const dedicatedDatabases = {
    "rumahl-core": "rumahl_core", "rumahl-security": "rumahl_security",
    "rumahl-secrets": "rumahl_secrets", "rumahl-appstore": "rumahl_appstore",
  };
  const database = dedicatedDatabases[serviceId] || "rumahl_home";
  const user = env.POSTGRES_USER || "rumahl";
...
function serviceEnv(svc) {
  ...
  if (svc.type !== "rust") {
    env.VITE_rumahl_BACKEND_URL ||= `http://localhost:${process.env.BACKEND_PORT || SERVICE_PORTS["rumahl-home"]}`;
    return env;
  }
  env.DATABASE_URL = databaseUrlFor(svc.id, env);
  if (svc.port) env.PORT ||= String(svc.port);
  env.POSTGRES_ADMIN_URL ||= databaseUrlFor("postgres", { ...env, DATABASE_URL: "" }).replace(/\/rumahl_postgres$/, "/postgres");
  env.rumahl_FRONTEND_DEV_URL ||= `http://localhost:${process.env.VITE_PORT || 5173}`;
  env.rumahl_DEV_MODE ||= "true";
  env.rumahl_BOOTSTRAP_ADMIN_USER ||= DEV_ADMIN_USER;
  env.rumahl_BOOTSTRAP_ADMIN_PASSWORD ||= DEV_ADMIN_PASSWORD;
  env.rumahl_BOOTSTRAP_ADMIN_DISPLAY_NAME ||= "rumahl Dev Admin";
  env.rumahl_DEV_OS_USER ||= DEV_OS_USER;
  env.rumahl_DEV_OS_PASSWORD ||= DEV_OS_PASSWORD;
  return env;
}
...
function drawMain() {
  ...
  if (H >= 10) {
    ...
    row++;
    moveTo(row, 1);
    const title = `  ${A.bold}rumahl Dev Runner${A.reset}   ${A.dim}Interactive Development Manager${A.reset}`;
    ...
    write(fit(`${A.bold}${A.cyan} rumahl Dev Runner${A.reset}`, W));
    row++;
  }
  ...
  if (H >= 16) {
    moveTo(row, 1);
    write(fit(` ${A.bold}Access:${A.reset} Frontend http://127.0.0.1:${process.env.VITE_PORT || 5173}  Backend http://127.0.0.1:${process.env.BACKEND_PORT || SERVICE_PORTS["rumahl-home"]}`, W));
    row++;
    moveTo(row, 1);
    write(fit(` ${A.bold}Credentials:${A.reset} rumahl ${DEV_ADMIN_USER} / ${DEV_ADMIN_PASSWORD}  OS ${DEV_OS_USER} / ${DEV_OS_PASSWORD}`, W));
    row++;
    moveTo(row, 1);
...
function drawInfo() {
  ...
  if (svc.id === "rumahl-home") {
    fields.push(["rumahl Login", `${DEV_ADMIN_USER} / ${DEV_ADMIN_PASSWORD}`]);
...
async function main() {
  console.log(`${A.cyan}${A.bold}rumahl Dev Runner${A.reset} — discovered ${A.green}${SERVICES.length}${A.reset} services\n`);
  for (const svc of SERVICES) {
  ...
  if (cli) {
    ...
    switch (cli.cmd) {
      ...
      case "doctor":
        ...
        return;
      default:
        console.log(`Usage: node scripts/dev/rumahl-dev.mjs [start|build|list|doctor] [service|all] [--watch]`);
        process.exit(0);
    }
  }
