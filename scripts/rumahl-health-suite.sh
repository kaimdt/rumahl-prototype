...
invoke_check "Migration registration" "$SCRIPT_DIR/check-rumahl-migrations.sh" --repo-root "$REPO_ROOT"
...
invoke_check "rumahl-dev-watch build" bash -c 'cd "$1/rumahl-os/backend" && cargo build -p rumahl-dev-watch' bash "$REPO_ROOT"
invoke_check "rumahl-home build" bash -c 'cd "$1/rumahl-os/backend" && cargo build -p rumahl-home' bash "$REPO_ROOT"

if [ "$FULL_WORKSPACE" -eq 1 ]; then
  invoke_check "Rust workspace build" bash -c 'cd "$1/rumahl-os/backend" && cargo build --workspace' bash "$REPO_ROOT"
fi
