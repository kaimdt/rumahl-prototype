...
MIGRATIONS_DIR="$REPO_ROOT/iora-os/backend/services/iora-home/migrations"
DB_MOD="$REPO_ROOT/iora-os/backend/services/iora-home/src/db/mod.rs"
...
if [ ! -f "$DB_MOD" ]; then
  echo "ERROR: iora-home db/mod.rs not found: $DB_MOD" >&2
  exit 1
fi
...
if [ ! -s "$TMP_DIR/missing.txt" ] && [ ! -s "$TMP_DIR/stale.txt" ] && [ ! -s "$TMP_DIR/missing_numbers.txt" ]; then
  ...
  echo "OK: iora-home migrations are registered and sequential ($COUNT files)."
  exit 0
fi
