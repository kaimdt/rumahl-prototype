...
$migrationsDir = Join-Path $RepoRoot "iora-os/backend/services/iora-home/migrations"
$dbMod = Join-Path $RepoRoot "iora-os/backend/services/iora-home/src/db/mod.rs"

if (-not (Test-Path $migrationsDir)) {
    Write-Error "Migrations directory not found: $migrationsDir"
}
if (-not (Test-Path $dbMod)) {
    Write-Error "iora-home db/mod.rs not found: $dbMod"
}
...
if ($missing.Count -eq 0 -and $stale.Count -eq 0 -and $missingNumbers.Count -eq 0) {
    Write-Host "OK: iora-home migrations are registered and sequential ($($fileNames.Count) files)."
    exit 0
}
