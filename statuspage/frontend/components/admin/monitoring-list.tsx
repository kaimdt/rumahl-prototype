...
const DEFAULT_THEME: PageTheme = { primary: "", background: "", surface: "", text: "", muted: "", border: "", max_width: "72rem", radius: "1rem" };
...
export function MonitoringListTab({ entity }: { entity: Entity }) {
  ...
  const editStatusPage = (item: Record<string, unknown>) => {
    setForm({
      ...
      domain_enabled: dbBool(item.domain_enabled), show_disabled_components: !Object.hasOwn(item, "show_disabled_components") || dbBool(item.show_disabled_components), problem_reports_enabled: !Object.hasOwn(item, "problem_reports_enabled") || dbBool(item.problem_reports_enabled), problem_report_threshold: Number(item.problem_report_threshold ?? 3), problem_report_window_minutes: Number(item.problem_report_window_minutes ?? 60), layout_config: dbJson(item.layout_config, emptyForm.layout_config), sections: normalizeSections(item.sections), original_service_ids: Array.isArray(item.service_ids) ? item.service_ids.map(String) : [],
  ...
  return (
    <div className="space-y-5">
      ...
      {entity === "status-pages" && showForm && <div className="grid gap-3 rounded-2xl border border-border/45 bg-card/40 p-4 md:grid-cols-3">
        ...
        <div className="md:col-span-3 grid gap-3 rounded-xl border border-border/40 p-4 md:grid-cols-3">
          <h4 className="md:col-span-3 text-sm font-bold">{t("monitoring.problemReports")}</h4>
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.problem_reports_enabled} onChange={(event) => setForm({ ...form, problem_reports_enabled: event.target.checked })} />{t("monitoring.problemReportsEnabled")}</label>
          <label className="text-xs font-semibold">{t("monitoring.problemReportThreshold")}<input type="number" min={2} max={100} value={form.problem_report_threshold} onChange={(event) => setForm({ ...form, problem_report_threshold: Number(event.target.value) })} className="mt-1 w-full rounded-lg border border-border/50 bg-background px-3 py-2" /></label>
          <label className="text-xs font-semibold">{t("monitoring.problemReportWindow")}<input type="number" min={5} max={1440} value={form.problem_report_window_minutes} onChange={(event) => setForm({ ...form, problem_report_window_minutes: Number(event.target.value) })} className="mt-1 w-full rounded-lg border border-border/50 bg-background px-3 py-2" /></label>
        </div>
        </>}
        {editorSection === "structure" && <fieldset className="md:col-span-3 space-y-3 rounded-xl border border-border/40 p-3">
          ...
          <div className="rounded-xl border border-border/40 bg-background/40 p-3"><p className="text-xs font-bold">{t("monitoring.pageStructure")}</p><div className="mt-3 space-y-2">{form.layout_config.order.map((block, index) => <div key={block} className="flex items-center gap-2 rounded-lg border border-border/30 px-3 py-2"><span className="flex-1 text-sm font-semibold">{block === "maintenance" ? t("monitoring.block.maintenance") : block === "components" ? t("monitoring.block.components") : block === "incidents" ? t("monitoring.block.incidents") : t("monitoring.block.history")}</span><label className="text-xs"><input type="checkbox" checked={!form.layout_config.hidden.includes(block)} onChange={(event) => setForm({ ...form, layout_config: { ...form.layout_config, hidden: event.target.checked ? form.layout_config.hidden.filter((value) => value !== block) : [...form.layout_config.hidden, block] } })} /> {t("monitoring.visible")}</label><button type="button" disabled={index === 0} onClick={() => setForm({ ...form, layout_config: { ...form.layout_config, order: moveItem(form.layout_config.order, index, -1) } })} className="rounded border border-border/40 p-1 disabled:opacity-30"><ChevronUp className="h-3.5 w-3.5" /></button><button type="button" disabled={index === form.layout_config.order.length - 1} onClick={() => setForm({ ...form, layout_config: { ...form.layout_config, order: moveItem(form.layout_config.order, index, 1) } })} className="rounded border border-border/40 p-1 disabled:opacity-30"><ChevronDown className="h-3.5 w-3.5" /></button></div>)}</div></div>
