#!/usr/bin/env bash
# rumahl Coding Agent — container entrypoint.
#
...
# Optional env: RUMAHL_BASE_URL, RUMAHL_API_KEY, ANTHROPIC_API_KEY, OPENAI_API_KEY,
#               GEMINI_API_KEY, GIT_AUTHOR_NAME, GIT_AUTHOR_EMAIL,
#               WORKSPACE, RUMAHL_EXTENSION_PATH,
...
EXTENSION_PATH="${RUMAHL_EXTENSION_PATH:-/opt/rumahl/rumahl-provider.ts}"
COMMIT_WATCHER="${COMMIT_WATCHER:-/opt/rumahl/commit-watcher.js}"
# pi loads AGENTS.md from PI_CODING_AGENT_DIR at startup for token-efficient project instructions
export PI_CODING_AGENT_DIR="${PI_CODING_AGENT_DIR:-/opt/rumahl}"
...
fail() {
  emit "error" "$1"
  node -e 'process.stdout.write(JSON.stringify({type:"rumahl_result",changed:false,summary:process.argv[1]})+"\n")' "$1"
  exit 1
}
...
git config user.name "${GIT_AUTHOR_NAME:-rumahl-bot}" >/dev/null 2>&1
git config user.email "${GIT_AUTHOR_EMAIL:-rumahl-bot@rumahl.local}" >/dev/null 2>&1
...
# rumahl Assist extension (always loaded when using ora provider)
...
if [ "${INCREMENTAL_COMMITS:-true}" = "true" ] && [ -f "$COMMIT_WATCHER" ]; then
  ...
  if git diff --cached --quiet; then
    ...
    node -e 'process.stdout.write(JSON.stringify({type:"rumahl_result",changed:false,branch:process.argv[1],summary:"The agent finished without code changes."})+"\n")' "$WORK_BRANCH"
    exit "${PI_EXIT:-0}"
  fi
