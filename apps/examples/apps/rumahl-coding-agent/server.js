...
// ---------------------------------------------------------------------------
// Dynamic provider & model listing (proxied from rumahl Assist)
// These endpoints forward to rumahl-assist so the UI always shows the live set
...
// Optional bearer-token guard for the management API/UI (the rumahl gateway
...
// hooks). These are internal service-to-service endpoints invoked by rumahl
...
const server = app.listen(config.port, () => {
  log.info(`rumahl Coding Agent listening on :${config.port}`, {
    runnerMode: config.runnerMode,
    githubConfigured: config.isGitHubConfigured(),
  });
});
