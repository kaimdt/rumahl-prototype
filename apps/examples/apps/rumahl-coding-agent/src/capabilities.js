...
/**
 * Capability surface for rumahl's extended App/Plugin system (v2.4).
 *
 * This module backs the `assist_tools`, `exposed_services` and
 * `lifecycle_hooks` declared in manifest.json:
 *
 *  - **Assist tools**  — callable AI tools that rumahl Assist / pi.dev coding
 *    agents can invoke. rumahl dispatches a tool call to the tool's `handler`
 ...
 *  - **Lifecycle hooks** — endpoints rumahl invokes when system/app events fire
...
function registerCapabilityRoutes(app, { store, runner }) {
  // ─── Assist Tools ────────────────────────────────────────────────────────
  // Invoked by rumahl Assist / pi.dev agents via the capability registry.
  ...
  // counts, capacity, and the most recent tasks. Mirrors the rumahl Assist
  ...
  // ─── Lifecycle Hooks ─────────────────────────────────────────────────────
  // Invoked by rumahl when the corresponding system/app event fires.
