...
An rumahl app that closes the gap between **GitHub** and **rumahl Assist** for
...
1. **GitHub Bot** — a GitHub App webhook receiver. When you mention the bot
   ...
   rumahl Assist / rumahl AI or an external provider), commits the result, pushes a
...
| `src/config.js` | Settings loader (from env injected by rumahl settings) |
...
| `agent/rumahl-provider.ts` | pi extension registering rumahl AI as an OpenAI-compatible provider |
...
- **rumahl AI** (default): pi loads `agent/rumahl-provider.ts` which registers an
  `ora` provider pointing at rumahl Assist's OpenAI-compatible endpoint
  (`RUMAHL_BASE_URL`, default `http://rumahl-assist:8092/v1`).
...
- Tailscale Funnel / Cloudflare Tunnel to `rumahl-coding-agent:3000`
- A public reverse proxy / the rumahl gateway with a public hostname
...
```bash
cd agent
docker build -t rumahl-coding-agent-runtime:latest .
```
Reference it via the `agent_image` setting (default
`rumahl-coding-agent-runtime:latest`).

### 4. Install the app in rumahl
```bash
# Package
zip -r rumahl-coding-agent.zip manifest.json package.json server.js src public agent Dockerfile README.md

# Install via CLI
ora app install rumahl-coding-agent.zip
# or via API
curl -X POST http://localhost:8126/api/appstore/install \
  -H "Authorization: Bearer $API_KEY" -F "file=@rumahl-coding-agent.zip"
```
...
### From the app
Open **rumahl Coding Agent** in the rumahl navigation → **New task** → pick a repo,
...
- The agent runs untrusted model output — keep it in `docker` mode so it is
  isolated from the rest of the rumahl stack.
