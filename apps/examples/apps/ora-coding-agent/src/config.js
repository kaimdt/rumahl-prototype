...
config.getAvailableProviders = function getAvailableProviders() {
  ...
  return [
    { id: 'ora', label: 'IORA Assist (ORA AI)', enabled: true },
...
/** Fetch the live provider list from iora-assist. */
...
/** Fetch models for a specific provider from iora-assist. */
...
config.optimizePrompt = async function optimizePrompt(originalPrompt, mainProvider, mainModel) {
  ...
  const optimizerModel = config.promptOptimizerModel || 'ora-default';
