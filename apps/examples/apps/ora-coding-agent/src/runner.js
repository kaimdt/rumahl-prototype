...
class AgentRunner {
  ...
  async _runDocker(task, envForTask, onLine) {
    ...
    const container = await docker.createContainer({
      ...
      HostConfig: {
        ...
        // IORA Assist endpoint, but isolate it from other app containers.
  ...
  async _runLocal(task, envForTask, onLine) {
    const workspace = fs.mkdtempSync(path.join(os.tmpdir(), `ora-agent-${task.id}-`));
...
function renderComment(task, success, result, mention) {
  if (!success) {
    return [
      `### ORA Coding Agent — run failed`,
      '',
      `The agent could not complete the task. Check the ORA Coding Agent app logs for task \`${task.id}\`.`,
      '',
      `_Requested via @${mention}._`,
    ].join('\n');
  }
  const lines = [`### ORA Coding Agent — done`, ''];
  ...
  lines.push('', `_Powered by IORA Assist + pi. Task \`${task.id}\`._`);
  return lines.join('\n');
}
