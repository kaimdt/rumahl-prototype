...
class RuntimeManager:
    ...
    def from_env(cls) -> "RuntimeManager":
        ...
        if not app_id:
            raise RuntimeError("RUMAHL_APP_ID environment variable not set")

        rumahl_endpoint = os.getenv("RUMAHL_ENDPOINT")
        if not rumahl_endpoint:
            raise RuntimeError("RUMAHL_ENDPOINT environment variable not set")

        heartbeat_interval = int(os.getenv("RUMAHL_HEARTBEAT_INTERVAL", "5"))

        config = RuntimeConfig(
            app_id=app_id,
            heartbeat_interval=heartbeat_interval,
            rumahl_endpoint=rumahl_endpoint,
            auto_heartbeat=True,
            query_timeout=30,
        )
    ...
    async def _process_messages(self) -> None:
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"Sending to rumahl ({self.config.rumahl_endpoint}): {message}"
                )
