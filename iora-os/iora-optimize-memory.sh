#!/bin/bash
# ============================================================================
# iora-optimize-memory.sh – Dynamic Memory Allocation for IORA Services
# ============================================================================
# Automatically adjusts memory limits for IORA services based on available
# system RAM to maximize performance without causing OOM conditions.
#
# Usage: sudo ./iora-optimize-memory.sh
# ============================================================================
...
# Must run as root
if [ "$(id -u)" -ne 0 ]; then
    echo "ERROR: Must run as root: sudo ./iora-optimize-memory.sh"
    exit 1
fi
...
# ═══════════════════════════════════════════════════════════════════════════════
# Apply memory limits to IORA services
# ═══════════════════════════════════════════════════════════════════════════════
...
apply_memory_limit "iora-assist" "$ASSIST_MEM_HIGH" "$ASSIST_MEM_MAX"

# Core infrastructure services
apply_memory_limit "iora-home" "$HOME_MEM_MAX" "$HOME_MEM_MAX"
apply_memory_limit "iora-core" "$CORE_MEM_MAX" "$CORE_MEM_MAX"
apply_memory_limit "iora-supervisor" "$SUPERVISOR_MEM_MAX" "$SUPERVISOR_MEM_MAX"
apply_memory_limit "iora-appstore" "$APPSTORE_MEM_MAX" "$APPSTORE_MEM_MAX"
...
for service in iora-security iora-secrets iora-watchdog iora-gateway iora-files \
               iora-backup iora-connector iora-network-monitor iora-domain-validator \
               iora-resource-manager iora-updater iora-dev-bridge iora-control; do
    if [ -f "${SVC_DIR}/${service}.service" ]; then
...
cat > /usr/lib/iora/iora-memory-monitor <<'MEMMONEOF'
#!/bin/bash
# IORA Memory Monitor - Tracks memory usage and adjusts if needed

LOG_TAG="iora-memory"
LOG_FILE="/var/log/iora/memory.log"
...
# Check each IORA service
for service in iora-assist iora-home iora-core iora-supervisor iora-appstore; do
...
exit 0
MEMMONEOF
chmod 755 /usr/lib/iora/iora-memory-monitor

# Memory monitor timer (runs every 5 minutes)
cat > "${SVC_DIR}/iora-memory-monitor.timer" <<'EOF'
[Unit]
Description=IORA Memory Monitor Timer
...
cat > "${SVC_DIR}/iora-memory-monitor.service" <<'EOF'
[Unit]
Description=IORA Memory Monitor

[Service]
Type=oneshot
ExecStart=/usr/lib/iora/iora-memory-monitor
StandardOutput=journal
StandardError=journal
EOF

ln -sf "${SVC_DIR}/iora-memory-monitor.timer" "${SVC_DIR}/timers.target.wants/iora-memory-monitor.timer" 2>/dev/null || true
...
log "  AI (iora-assist): max ${ASSIST_MEM_MAX}"
...
log "To apply now: systemctl restart iora-*.service"
echo ""
