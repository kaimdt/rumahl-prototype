# devup.sh – IORA OS Dev Inkrementelles Update

## Das Problem

Beim Entwickeln mit IORA OS Dev schlägt der **Device-Build** (`--build-mode device`) fehl, weil das Buildroot-basierte IORA OS Dev Image **kein `cargo`/`rustc`/`apt-get`** enthält. 
...
Rebuild the Dev image with IORA_OS_DEV=1 / BR2_PACKAGE_IORA_DEV_TOOLCHAIN=y
```
...
3. Lädt die fertigen Binaries per HTTP zur `iora-dev-bridge` hoch
...
./devup.sh --service iora-home --service iora-control
...
4. **Upload via Bridge**: Sendet das Binary an `POST /dev/replace-binary` auf der iora-dev-bridge (Port 8101)
5. **Service-Restart**: Die Bridge führt `systemctl restart <service>` aus

## Konfiguration

Die Config wird in `~/.config/iora-devup/config` gespeichert:

```bash
IORA_DEVUP_HOST="192.168.1.42:8101"
IORA_DEVUP_TOKEN="abc123..."
```

Der Dev-Token wird beim OS-Build erzeugt und steht in `/var/lib/iora/dev-token` auf dem IORA OS Dev Gerät.
...
Bei einem **kompletten OS-Rebuild** mit `./build.sh all --dev` wird das Buildroot-Image jetzt mit `BR2_PACKAGE_IORA_DEV_TOOLCHAIN=y` gebaut (seit dem Fix in `build-all-images.sh`). Danach funktioniert auch der Device-Build wieder direkt auf dem Gerät.
