# IORA Service Heartbeat Protocol

Every IORA service self-announces to `iora-core` every few seconds. This
...
The previous design relied entirely on `iora-core` reverse-polling each
...
```
POST {IORA_CORE_URL}/api/core/services/heartbeat
...
```
GET {IORA_CORE_URL}/api/core/services/status
```
...
and the `iora-dev-deploy` daemon re-exposes it at `/api/v1/services` for
the VS Code extension's "Services (live)" tree view.

## Embedding the client

In Rust services using `iora-shared`:
...
|-----------------------------------|--------------------------|--------------------------------------|
| `IORA_CORE_URL`                   | `http://127.0.0.1:8090`  | Heartbeat target                     |
| `IORA_HEARTBEAT_INTERVAL_SECS`    | `5`                      | Cadence (clamped to 1–60)            |
...
* When a heartbeat is received again, the backoff resets and a recovery
  log line is emitted.
* `iora-core` flags any service stale after `HEARTBEAT_STALE_AFTER_SECS`
