# IORA Dev Automation — Co-Buddy Features
...
The IORA dev-local scripts now include intelligent auto-detection and auto-repair capabilities that:
...
**Features:**
- Detects if port is in use by IORA VM (reuses it) or another process (offers to kill it)
...
**Logs location:**
```
iora-os/.cache/health-monitor.log
```

**Manual control:**
```bash
# View health monitor logs
tail -f iora-os/.cache/health-monitor.log

# Stop health monitor
kill $(cat iora-os/.cache/health-monitor.pid)
```
...
- **VM Ready:** When the IORA Dev VM is fully started and healthy
...
mv iora-os/lib/dev-auto-repair.sh iora-os/lib/dev-auto-repair.sh.disabled

# Disable background health monitor
export IORA_NO_HEALTH_MONITOR=1
./dev-local.sh
```
...
tail -f iora-os/.cache/dev-local.log

# Health monitor log (background monitoring)
tail -f iora-os/.cache/health-monitor.log

# QEMU logs (if VM crashes)
tail -f iora-os/.cache/qemu-stderr.log
```
...
4. **Completion (Notification):**
   ...
   send_notification "IORA Dev VM Ready"
   ```
...
grep "Auto-repair enabled" iora-os/.cache/dev-local.log

# Check if health monitor process exists
ps aux | grep health-monitor

# View health monitor logs
cat iora-os/.cache/health-monitor.log
```

**Fix:**
```bash
# Ensure library exists
ls -la iora-os/lib/dev-auto-repair.sh

# Manually start health monitor
source iora-os/lib/dev-auto-repair.sh
start_health_monitor "127.0.0.1" 2222 "iora-os/.cache/iora-dev-key" \
    "iora-os/.cache/health-monitor.log" "iora-os/.cache/health-monitor.pid"
```
