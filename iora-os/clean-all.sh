#!/usr/bin/env bash
# ============================================================================
# IORA OS – Comprehensive Project Cleaner
# ============================================================================
...
#   deep      build + ALLE Caches (~/.iora-cache, .iora-dl-cache)  (~25-50 GB)
...
LEVELS:
    quick     node_modules + dist + /tmp iora logs
...
echo -e "${GREEN}${BOLD}║  IORA OS – Comprehensive Cleaner  |  Level: ${LEVEL}${NC}"
echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════════╝${NC}"
...
# /tmp logs
for _f in /tmp/iora-*.log /tmp/iora-cargo-*.log /tmp/iora-rustup-*.log; do
    [ -f "${_f}" ] && { _add_mb "${_f}"; _ITEMS_QUICK+=("${_f}|/tmp iora log"); }
done
...
_add_mb "${SCRIPT_DIR}/buildroot-2024.02/.iora-config-key"
_ITEMS_BUILD+=("${SCRIPT_DIR}/buildroot-2024.02/.iora-config-key|Buildroot config marker")

# Precompiled service binaries in overlay
for _overlay_dir in "${SCRIPT_DIR}/board/iora/rootfs-overlay/opt/iora/build"/*/bin; do
...
_add_mb "${SCRIPT_DIR}/board/iora/rootfs-overlay/opt/iora/iora-home/dist"
_ITEMS_BUILD+=("${SCRIPT_DIR}/board/iora/rootfs-overlay/opt/iora/iora-home/dist|frontend staged in overlay")
...
# ~/.iora-cache (ccache + sccache + cargo-target)
_add_mb "${HOME}/.iora-cache"
_ITEMS_DEEP+=("${HOME}/.iora-cache|~/.iora-cache/ (ccache+sccache+cargo)")

# Project dl-cache
_add_mb "${SCRIPT_DIR}/.iora-dl-cache"
_ITEMS_DEEP+=("${SCRIPT_DIR}/.iora-dl-cache|.iora-dl-cache/ (downloads)")
