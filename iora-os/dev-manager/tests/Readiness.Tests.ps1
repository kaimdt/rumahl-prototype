...
Describe "IORA readiness classification" {
    It "never reports Ready from a running process alone" {
        Get-IoraLifecycle -Checks @{ Process=$true; Qmp=$false } | Should -Be "Starting"
    }

    It "waits for QGA before considering the VM booted" {
        Get-IoraLifecycle -Checks @{ Process=$true; Qmp=$true; Qga=$false } | Should -Be "Booting"
    }
    ...
    It "reports degraded when internal health passes but host health fails" {
        ...
        Get-IoraLifecycle -Checks $checks | Should -Be "Degraded"
        (Get-IoraHomeDiagnosis -InternalHealthy $true -ExternalHealthy $false).Code | Should -Be "network-path"
    }
    ...
    It "reports Ready only after every required check passes" {
        ...
        Get-IoraLifecycle -Checks $checks | Should -Be "Ready"
    }
}
