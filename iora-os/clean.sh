...
OVERLAY_BIN_DIR="${SCRIPT_DIR}/board/iora/rootfs-overlay/opt/iora/build"
CARGO_CACHE="${HOME}/.iora-cache"
...
LEVEL (einer):
    ...
    deep          Wie build + cargo cache (~/.iora-cache).
...
if [ -z "${LEVEL}" ]; then
    ...
    echo "  deep         – build + cargo cache (~/.iora-cache)"
...
clean_build() {
    ...
    local stamp_marker="${BUILDROOT_DIR}/.iora-config-key"
...
clean_deep() {
    ...
    if [ -d "${CARGO_CACHE}" ]; then
        remove "${CARGO_CACHE}" "~/.iora-cache/ (cargo target + sccache)"
    else
        log_info "~/.iora-cache/ existiert nicht."
    fi
}
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  IORA OS Cleaner – Level: ${LEVEL}${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════╝${NC}"
