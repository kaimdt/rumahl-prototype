#!/usr/bin/env python3
"""
Patch iora-supervisor/src/main.rs to fix 9 known compile errors.
Safe to re-run (idempotent checks on each edit).

Run:
    python3 patch-iora-supervisor.py /path/to/backend/iora-supervisor/src/main.rs
"""
import re
import sys
import pathlib
...
def main():
    if len(sys.argv) != 2:
        print("Usage: patch-iora-supervisor.py <path-to-main.rs>", file=sys.stderr)
        sys.exit(2)
    ...
    src2 = re.sub(r'\n\s*target:\s*"iora-developer-app"\s*,\s*\n', '\n', src)
    if src2 != src:
