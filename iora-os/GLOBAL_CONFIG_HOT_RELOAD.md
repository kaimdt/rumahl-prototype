...
This document describes IORA's hot-reload mechanism for Global Config, which allows services to receive configuration changes immediately without restarting.
...
PUT /api/settings/:key
    ↓
iora-home updates database
    ↓
iora-home updates settings cache (update_cached_setting)
    ↓
iora-home calls iora-config-notify.sh
    ↓
Notification files created in /var/run/iora/config-notify/
    ↓
...
All services use a shared settings cache provided by `iora-shared`:
...
1. **Database Update**: iora-home writes to the settings table
2. **Cache Update**: `update_cached_setting()` updates the in-memory cache
3. **Notification**: `iora-config-notify.sh` is called
...
When iora-home receives a PUT request to update a setting:

```rust
// In iora-home/src/main.rs
async fn update_setting(
    ...
    iora_shared::system_config::update_cached_setting(key.clone(), value.clone());
...
The `iora-config-notify.sh` script:

1. Creates notification files in `/var/run/iora/config-notify/`
2. Sends SIGHUP to all active IORA services
3. Cleans up old notifications (>1 hour)

```bash
# Notify of config change
/usr/lib/iora/iora-config-notify ha.url "http://new-url:8123"
...
2. **iora-home**:
   ...
   - Calls notification: `iora-config-notify.sh ha.url "http://homeassistant.local:8123"`

3. **Notification System**:
   - Creates `/var/run/iora/config-notify/1234567890_ha_url`
   - Sends SIGHUP to all services

4. **Services** (e.g., iora-home, iora-assist):
...
```rust
use iora_shared::system_config::get_cache_updated_at;
...
watch -n 1 ls -lh /var/run/iora/config-notify/
...
Default: `/var/run/iora/config-notify/`

To change:
```bash
export IORA_CONFIG_NOTIFY_DIR=/custom/path
```
...
To change:
```bash
# In iora-config-notify.sh, change:
...
ls -la /usr/lib/iora/iora-config-notify

# Check notification directory
ls -la /var/run/iora/config-notify/

# Test manually
/usr/lib/iora/iora-config-notify test.key test.value
journalctl -u iora-home -n 20
```
...
**Solution**:
```bash
# Check if iora-home is calling update_cached_setting()
journalctl -u iora-home -n 100 | grep -i cache

# Restart iora-home to reload cache from database
...
Hot-reload requires iora-home to be running. If iora-home is down:
