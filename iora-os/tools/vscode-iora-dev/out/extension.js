...
async function activate(ctx) {
    ...
    ctx.subscriptions.push(output, statusItem, vscode.window.registerTreeDataProvider('ioraDev.connection', connectionProvider), vscode.window.registerTreeDataProvider('ioraDev.devices', devicesProvider), vscode.window.registerTreeDataProvider('ioraDev.services', servicesProvider), vscode.window.registerTreeDataProvider('ioraDev.components', componentsProvider), vscode.window.registerTreeDataProvider('ioraDev.activity', activityProvider));
    ctx.subscriptions.push(cmd('ioraDev.discover', cmdDiscover), cmd('ioraDev.connect', cmdConnect), cmd('ioraDev.disconnect', cmdDisconnect), cmd('ioraDev.forgetCredentials', cmdForgetCredentials), cmd('ioraDev.deploy', cmdDeploy), cmd('ioraDev.deployCurrent', cmdDeployCurrent), cmd('ioraDev.watch', cmdWatchStart), cmd('ioraDev.stopWatch', cmdWatchStop), cmd('ioraDev.restart', cmdRestart), cmd('ioraDev.reload', cmdReload), cmd('ioraDev.composeReload', cmdComposeReload), cmd('ioraDev.logs', cmdLogs), cmd('ioraDev.dashboard', () => dashboard.show()), cmd('ioraDev.startDaemon', cmdStartDaemon), cmd('ioraDev.stopDaemon', cmdStopDaemon), cmd('ioraDev.refresh', refreshAll), cmd('ioraDev.showOutput', () => output.show()), cmd('ioraDev.deployFromTree', (item) => cmdDeploy(item?.name ? [item.name] : (item?.label ? [item.label] : undefined))), cmd('ioraDev.watchFromTree', (item) => cmdWatchStart(item?.name ? [item.name] : (item?.label ? [item.label] : undefined))), cmd('ioraDev.logsFromTree', (item) => cmdComponentLogs(item?.name ?? item?.label ?? item)), cmd('ioraDev.componentActions', (item) => cmdComponentActions(item?.name ?? item?.label ?? item)), cmd('ioraDev.connectFromTree', (ep) => cmdConnect(ep)), cmd('ioraDev.stopWatchFromTree', (item) => cmdWatchStop(item?.id ?? item)), cmd('ioraDev.serviceLogs', cmdServiceLogsTail), cmd('ioraDev.serviceLogsFromTree', (item) => cmdServiceLogsTail(item?.name ?? item?.label ?? item)), cmd('ioraDev.serviceRestartFromTree', (item) => cmdServiceRestart(item?.name ?? item?.label ?? item)), cmd('ioraDev.openServiceLiveLogs', (arg) => cmdOpenLiveLogs(arg?.name ?? arg?.label ?? arg)), cmd('ioraDev.stopServiceLiveLogs', () => cmdStopLiveLogs()), cmd('ioraDev.openServiceUrl', (arg) => cmdOpenServiceUrl(arg?.name ?? arg?.label ?? arg)), cmd('ioraDev.systemInfo', cmdSystemInfo), cmd('ioraDev.rebootDevice', cmdRebootDevice), cmd('ioraDev.servicePalette', cmdServicePalette), cmd('ioraDev.connectSaved', cmdConnectSaved));
...
async function tryAutoConnect(host) {
    ...
    try {
        ...
        if (msg.includes('401') || msg.includes('Unauthorized')) {
            ...
            vscode.window.showWarningMessage('IORA Dev: Saved dev-token was rejected by the device. Please reconnect.', 'Connect').then(s => { if (s === 'Connect')
                cmdConnect(); });
        }
    }
}
...
async function cmdConnect(prefilled) {
    ...
    try {
        ...
        if (msg.includes('401') || msg.includes('Unauthorized')) {
            ...
            const retry = await vscode.window.showErrorMessage(`IORA Dev: Invalid dev-token.\n\n${daemonDetail}`, 'Retry with new token', 'Show Output');
...
async function cmdWatchStart(prefilled) {
    const c = await ensureClient();
    const automatic = (vscode.workspace.getConfiguration('ioraDev').get('automaticMode') ?? false)
...
async function cmdRebootDevice() {
    const yes = await vscode.window.showWarningMessage('Reboot the connected IORA device? This will interrupt all running services.', { modal: true }, 'Reboot');
    if (yes !== 'Reboot')
        return;
    const c = await ensureClient();
    try {
