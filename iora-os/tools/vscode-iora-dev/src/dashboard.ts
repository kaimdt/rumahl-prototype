...
export class Dashboard {
    ...
    setConnection(conn: Connection | null) {
        console.log('[iora-dash] setConnection: host=' + (conn?.host ?? 'null'));
    ...
    private pushConnState() {
        if (!this.panel) {
            console.log('[iora-dash] pushConnState: no panel, skipping');
            return;
        }
        console.log('[iora-dash] pushConnState: sending connState, host=' + (this.latestConn?.host ?? 'null'));
    ...
    private async onMsg(msg: any) {
        if (msg?.cmd === 'ready') {
            console.log('[iora-dash] webview READY received, latestConn=' + (this.latestConn?.host ?? 'null') + ' panel=' + (this.panel ? 'yes' : 'no'));
        ...
        switch (msg?.cmd) {
            case 'connect':         await vscode.commands.executeCommand('ioraDev.connect'); break;
            case 'discover':        await vscode.commands.executeCommand('ioraDev.discover'); break;
            case 'deploy':          await vscode.commands.executeCommand('ioraDev.deploy'); break;
            case 'watch':           await vscode.commands.executeCommand('ioraDev.watch'); break;
            case 'deployComponent': await vscode.commands.executeCommand('ioraDev.deployFromTree', { name: msg.component }); break;
            case 'watchComponent':  await vscode.commands.executeCommand('ioraDev.watchFromTree', { name: msg.component }); break;
            case 'stopWatch':       await vscode.commands.executeCommand('ioraDev.stopWatch', msg.id); break;
            case 'refresh':         this.refresh(); break;
            case 'logs':            await vscode.commands.executeCommand('ioraDev.logs'); break;
        }
    }
...
<div id="app">
  ...
  <div class="not-connected" id="noConn">
    ...
    <div class="muted" style="font-size:16px;">Not connected to an IORA device</div>
...
console.log('[iora-web] registering message listener');
...
console.log('[iora-web] dashboard loaded');
// Tell the extension that the webview is ready to receive data
vscode.postMessage({ cmd: 'ready' });
console.log('[iora-web] sent ready message');

// Initial render
render();
console.log('[iora-web] initial render done');
</script>
</body></html>`;
    }
}
