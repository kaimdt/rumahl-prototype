...
export class LiveLogManager {
    private sessions = new Map<string, Session>();

    constructor(private clientGetter: () => DaemonClient | undefined,
     ...
     */
    async open(unit: string): Promise<void> {
        const existing = this.sessions.get(unit);
        if (existing) {
            existing.channel.show(true);
            return;
        }
        const channel = vscode.window.createOutputChannel(`IORA: ${unit}`);
        ...
        channel.show(true);
        channel.appendLine(`▶ live logs for ${unit} — press Ctrl-Shift-P → "IORA Dev: Stop Live Logs" to detach`);
    ...
    private async streamLoop(session: Session): Promise<void> {
        ...
        while (!session.closed) {
            ...
            let urlInfo: { url: string; token: string };
            try {
    ...
    private async streamOnce(session: Session, url: string): Promise<void> {
        ...
        const resp = await fetch(url, {
            method: 'GET',
            signal: session.abort.signal as any,
    ...
    private appendRecord(session: Session, record: string): void {
    ...
    private async delayWithBackoff(session: Session): Promise<void> {
