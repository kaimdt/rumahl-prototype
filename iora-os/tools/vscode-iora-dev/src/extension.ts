        ...
        try {
            ...
            if (discovered && discovered.length > 0 && suggestedHost && token) {
                ...
                vscode.window.showWarningMessage(
                    `IORA Dev: Saved host ${host} is unreachable. ${discovered.length === 1 ? 'One device' : `${discovered.length} devices`} found on LAN.`,
...
async function cmdDeploy(prefilled?: string[]) {
    ...
    const buildMode = vscode.workspace.getConfiguration('ioraDev').get<string>('buildMode') ?? 'auto';
