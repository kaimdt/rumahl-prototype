# IORA OS Security Features

## Overview

IORA OS implements multiple layers of security to protect your smart home infrastructure. This document describes the security features available on different hardware platforms.
...
cryptsetup luksOpen /dev/sda5 iora-data

# Mount encrypted partition
mount /dev/mapper/iora-data /mnt
```
...
IORA OS uses RAUC for secure, atomic system updates with rollback capability.
...
When IORA OS is installed on SD cards, USB flash drives, or eMMC storage, the installer automatically applies optimizations to extend storage lifespan.
...
[network]
update-url=https://updates.iora.example.com/
auto-install=true
check-interval=3600
```
...
IORA OS includes Docker with AppArmor support. Enable container security:
...
IORA OS security features align with:
...
**IORA OS Version:** Based on Buildroot with Linux 6.6.15
