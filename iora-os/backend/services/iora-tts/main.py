#!/usr/bin/env python3
"""
iora-tts – Text-to-Speech Microservice powered by Kokoro ONNX.
Exposes a lightweight HTTP API for speech synthesis.

Usage:
    python main.py                     # uses defaults (port 8111)
    IORA_TTS_PORT=8112 python main.py
    IORA_TTS_VOICE=af_nicole python main.py
"""
import os
import io
import logging
...
logging.basicConfig(
    ...
    format="%(asctime)s [%(levelname)s] iora-tts: %(message)s",
)
log = logging.getLogger("iora-tts")

# ─── Configuration ──────────────────────────────────────────────────────────
MODEL_PATH = os.getenv("IORA_TTS_MODEL_PATH", "kokoro-v1.0.onnx")
VOICES_PATH = os.getenv("IORA_TTS_VOICES_PATH", "voices-v1.0.bin")
DEFAULT_VOICE = os.getenv("IORA_TTS_VOICE", "af_nicole")
DEFAULT_LANG = os.getenv("IORA_TTS_LANG", "en-us")
DEFAULT_SPEED = float(os.getenv("IORA_TTS_SPEED", "1.0"))
PORT = int(os.getenv("IORA_TTS_PORT", "8111"))
AUDIO_FORMAT = os.getenv("IORA_TTS_FORMAT", "wav")  # wav or raw pcm16
...
async def lifespan(app: FastAPI):
    ...
    if os.getenv("IORA_TTS_AUTO_LOAD", "1") == "1":
        load_engine()
    yield
...
if __name__ == "__main__":
    import uvicorn
    log.info(
        f"Starting IORA TTS on 0.0.0.0:{PORT} "
