...
//!
//! `iora-incident-engine` is a read-only correlation layer by contract: it
...
//! exclusively through the frozen `iora-security-helper` boundary
...
/// `iora-runtime-policy`. Unknown categories never match, so a typo cannot
/// widen automation.
