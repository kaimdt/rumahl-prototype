...
//!
//! `iora-integrity.service` (see `board/iora/post-build.sh`) verifies all
//! native IORA binaries against `/etc/iora/binary-manifest.sha256` every five
//! minutes. It never mutates the host: mismatches are logged and written as
//! structured evidence to `/run/iora/integrity-mismatch.json`.
//!
...
//! critical IORA binaries are affected. Non-critical mismatches are recorded
//! as keyed audit events without a lockdown, matching the documented
//! "automatic full lockdown only on confirmed tampering of critical IORA
...
const MISMATCH_FILE: &str = "/run/iora/integrity-mismatch.json";

/// Core binaries whose tampering is considered critical enough for an
/// automatic lockdown. App binaries under `/opt/iora/apps` are NOT critical:
...
async fn run_pass(state: &AppState, last_reaction: &mut Option<String>) -> Result<()> {
    ...
    if critical.is_empty() {
        ...
        info!(paths = %paths.join(","), "critical IORA binary tampering detected — requesting lockdown");
