...
pub async fn apply_firewall(
    ...
    if input.ruleset.contains("flush ruleset") || !input.ruleset.contains("iora_security") {
...
fn require_permission(headers: &HeaderMap, permission: &str) -> Result<()> {
    let trusted_proxy =
        headers.get("x-iora-proxy").and_then(|v| v.to_str().ok()) == Some("iora-home");
    let permissions = headers
        .get("x-iora-permissions")
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    if !trusted_proxy
