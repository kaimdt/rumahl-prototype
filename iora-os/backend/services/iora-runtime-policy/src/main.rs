...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        ...
        .init();
    let profiles = load_profiles("/etc/iora/security-profiles")?;
    let state = AppState {
    ...
    let app = Router::new()
        ...
        .with_state(state);
    let port = iora_shared_config::system_config::service_port("iora-runtime-policy", 8108);
    ...
    info!(port, "IORA runtime policy engine started");
