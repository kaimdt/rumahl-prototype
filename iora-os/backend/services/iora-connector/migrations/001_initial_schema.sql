-- IORA Connector: Schema for tunnel management and service exposure
-- Migration 001: Initial schema

-- Registered IORA instances that connect via VPN tunnel
CREATE TABLE IF NOT EXISTS tunnels (
    ...
    peer_endpoint TEXT, -- IP:port of the IORA home instance (auto-detected)
...
CREATE TABLE IF NOT EXISTS exposed_services (
    ...
    local_port INTEGER NOT NULL, -- port on the IORA instance (via VPN)
    local_protocol TEXT NOT NULL DEFAULT 'http', -- 'http', 'https', 'tcp', 'ws'
    -- Public exposure
    public_subdomain TEXT, -- e.g. 'myhouse' -> myhouse.iora-connect.example.com
...
INSERT OR IGNORE INTO connector_config (key, value) VALUES
    ...
    ('wireguard_interface', 'wg-iora'),
    ('vpn_subnet', '10.100.0.0/24'),
    ('server_public_ip', ''),
    ('default_domain', 'iora-connect.local'),
    ('max_tunnels', '50'),
