#!/usr/bin/env python3
"""
iora-stt – Speech-to-Text Microservice powered by faster-whisper.
Exposes a lightweight HTTP API for audio transcription.

Usage:
    python main.py                    # uses defaults (port 8110)
    IORA_STT_PORT=8111 python main.py
    IORA_STT_MODEL=large-v3 python main.py
"""
import os
import io
import logging
...
logging.basicConfig(
    ...
    format="%(asctime)s [%(levelname)s] iora-stt: %(message)s",
)
log = logging.getLogger("iora-stt")

# ─── Configuration ──────────────────────────────────────────────────────────
MODEL_SIZE = os.getenv("IORA_STT_MODEL", "base")  # tiny, base, small, medium, large-v3
DEVICE = os.getenv("IORA_STT_DEVICE", "cpu")       # cpu or cuda
COMPUTE_TYPE = os.getenv("IORA_STT_COMPUTE_TYPE", "int8")  # int8, float16, int8_float16
BEAM_SIZE = int(os.getenv("IORA_STT_BEAM_SIZE", "5"))
PORT = int(os.getenv("IORA_STT_PORT", "8110"))
...
async def lifespan(app: FastAPI):
    ...
    if os.getenv("IORA_STT_AUTO_LOAD", "1") == "1":
        load_model()
    yield
...
if __name__ == "__main__":
    import uvicorn
    log.info(f"Starting IORA STT on 0.0.0.0:{PORT} (model={MODEL_SIZE}, device={DEVICE})")
