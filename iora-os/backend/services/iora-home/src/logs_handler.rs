...
//! frontend can render logs for any IORA service, installed app, plugin, or
//! Docker container in a single Logs view.
//!
//! Source IDs use a `<transport>:<unit>` scheme:
//!   - `systemd:<unit>`   — a systemd unit (e.g. `iora-home.service`)
//!   - `docker:<name>`    — a docker container by name
//!   - `app:<app_id>`     — an installed IORA app (resolved to a docker container)
//!   - `plugin:sandbox`   — the shared plugin sandbox container
//!   - `self:iora-home`   — the in-process tracing buffer of this service
...
pub async fn list_log_sources(
    ...
    sources.push(LogSource {
        id: "self:iora-home".into(),
        kind: "self",
        transport: "buffer",
        name: "iora-home (this process)".into(),
        running: true,
        description: Some("Live tracing buffer of the running iora-home backend.".into()),
    });

    // 2) systemd iora-* services
    if let Ok(units) = list_systemd_iora_units().await {
        for (unit, running) in units {
            ...
            sources.push(LogSource {
                ...
                name: display,
                running,
                description: Some("systemd-managed IORA service.".into()),
            });
        }
    }

    // 3) Docker containers (named `iora-*` only — avoids leaking unrelated hosts)
    if let Ok(containers) = list_iora_docker_containers().await {
        for (name, running) in containers {
            // Skip the plugin sandbox here — it gets its own entry below.
            if name == "iora-plugin-sandbox" {
                continue;
            }
...
pub async fn get_source_logs(
    ...
    match transport {
        ...
        "app" => {
            // Apps live in `iora-app-<id>` containers.
            let container = format!("iora-app-{}", unit);
        ...
        "plugin" => {
            ...
            let container = "iora-plugin-sandbox";
            match read_docker_logs(container, lines).await {
                Ok(out) => Json(json!({
                    "source_id": source_id,
                    "transport": "docker",
...
async fn list_systemd_iora_units() -> Result<Vec<(String, bool)>, String> {
    ...
    for line in String::from_utf8_lossy(&output.stdout).lines() {
        ...
        if !unit.starts_with("iora-") || !unit.ends_with(".service") {
            continue;
        }
...
async fn list_iora_docker_containers() -> Result<Vec<(String, bool)>, String> {
    // Try the local docker CLI first (works on the dev host where the user is
    // in the `docker` group). On IORA OS the docker socket is root-only and
    // this will fail with EACCES — in that case fall back to the
    // iora-supervisor HTTP API (which runs as root and is the sole gateway
    // to dockerd).
    ...
    // Fallback: ask iora-supervisor over HTTP.
...
async fn list_containers_via_supervisor() -> Result<Vec<(String, bool)>, String> {
    ...
    Ok(list
        .into_iter()
        .filter(|c| c.name.starts_with("iora-"))
        .map(|c| {
...
async fn read_docker_logs(container: &str, lines: usize) -> Result<Vec<String>, String> {
    ...
    // API on IORA OS where the docker socket is root-only.
    let cli = tokio::time::timeout(
        Duration::from_secs(5),
        Command::new("docker")
            .args([
    ...
    // Fallback: ask iora-supervisor (returns up to 100 lines).
