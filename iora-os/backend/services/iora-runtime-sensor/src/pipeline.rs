...
//!
//! ```text
//! normalized event -> iora-runtime-identity (resolve)
//!                   -> iora-runtime-policy (evaluate)
//!                   -> iora-incident-engine (correlate)
//! ```
//!
...
// ─── Identity snapshot (subset of iora-runtime-identity) ───────────────────
...
// ─── Policy evaluation (iora-runtime-policy contract) ──────────────────────
...
// ─── Incident correlation (iora-incident-engine contract) ──────────────────
...
pub async fn run(client: &reqwest::Client, event: RuntimeEvent) -> Result<()> {
    let identity_port =
        iora_shared_config::system_config::service_port_discovery("iora-runtime-identity", 8107);
    let policy_port =
        iora_shared_config::system_config::service_port_discovery("iora-runtime-policy", 8108);
    let incident_port =
