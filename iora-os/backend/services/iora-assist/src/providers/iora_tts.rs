...
pub struct IoraTtsProvider {
    client: Client,
    config: ProviderConfig,
}

impl IoraTtsProvider {
...
#[async_trait]
impl AIProvider for IoraTtsProvider {
    fn name(&self) -> &str {
        "IORA TTS (Kokoro)"
    }
    ...
    async fn chat(
        ...
        Err("IORA TTS provider does not support chat. Use for speech synthesis only.".into())
    }
    ...
    async fn transcribe_audio(
        ...
        Err("IORA TTS provider does not support STT. Use IORA STT for transcription.".into())
    }
