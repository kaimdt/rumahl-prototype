...
**ORA AI** is the AI platform for IORA, providing intelligent assistance through multiple AI providers with full voice capability support.
...
- **Smart Home Integration**: Context-aware AI that understands your IORA system
...
| **DesktopAI** | ✅ | ✅ | ✅ | Optional | Via IORA Desktop |
...
curl -X POST http://localhost:8092/api/assist/voice/synthesize \
  -H "Content-Type: application/json" \
  -d '{"text": "Hello, this is IORA Assist", "voice": "alloy"}' \
  --output speech.mp3
```
...
ORA_AI_API_KEY=sk-proj-your-key-here
...
ORA_AI_API_KEY=sk-ant-your-key-here
ORA_AI_MODEL=claude-3-5-sonnet-20241022
ORA_AI_API_VERSION=2023-06-01
```
...

{
  ...
  "system_prompt": "You are IORA Assist...",
  "context": {}
}
```
...
## Integration with IORA

ORA AI integrates with the IORA ecosystem:

1. **Smart Home Context**: Access to entity states via `iora-home`
...
5. **Desktop Integration**: Offline AI via IORA Desktop
...
|----------|----------|---------|-------------|
| `ORA_AI_PROVIDER` | No | `local` | AI provider (openai/anthropic/local/desktop) |
| `ORA_AI_API_KEY` | Sometimes | - | API key for OpenAI/Anthropic |
| `ORA_AI_BASE_URL` | Sometimes | - | Base URL for Local/Desktop |
| `ORA_AI_MODEL` | No | Provider-specific | Model name to use |
| `ORA_AI_API_VERSION` | No | - | API version (Anthropic) |
...
- [IORA Architecture](../../ARCHITECTURE.md)
