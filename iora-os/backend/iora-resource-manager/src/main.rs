...
fn calculate_cpu_percent(stats: &Stats) -> f64 {
    if let (Some(cpu_stats), Some(precpu_stats)) = (&stats.cpu_stats, &stats.precpu_stats) {
        if let (Some(cpu_usage), Some(precpu_usage), Some(system_usage), Some(presystem_usage)) = (
            &cpu_stats.cpu_usage,
            ...
            if system_delta > 0.0 && cpu_delta > 0.0 {
                let num_cpus = cpu_usage.percpu_usage.as_ref().map(|v| v.len()).unwrap_or(1) as f64;
...
async fn monitor_containers(state: &AppState) -> Result<()> {
    ...
    for container in containers {
        ...
        // Skip if not an IORA app container
        if !container_name.starts_with("iora-app-") && !container_name.starts_with("iora-") {
            continue;
        }
        ...
        match get_container_stats(docker, &container_id).await {
            Ok(stats) => {
                ...
                let app_id = if container_name.starts_with("iora-app-") {
                    Some(container_name.trim_start_matches("iora-app-").to_string())
                } else {
                    None
                };
...
async fn get_allocation_history(State(state): State<AppState>) -> impl IntoResponse {
    match sqlx::query_as::<_, ResourceAllocation>(
    ...
    .await
    {
        Ok(allocations) => (StatusCode::OK, Json(allocations)).into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({
...
async fn main() -> Result<()> {
    ...
    info!("Starting IORA Resource Manager service...");
