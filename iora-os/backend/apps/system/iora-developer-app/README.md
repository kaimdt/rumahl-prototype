...
Official development tool for the IORA ecosystem providing exclusive hot-reload capabilities, IDE integration, and advanced development workflows.

## Overview

The IORA Developer App is a special system application that enables:
...
**Important**: The Developer App is **NOT** pulled from a container registry. Instead, it is **built locally** as part of the IORA system build process.
...
4. **Bundled Updates**: Updated automatically with IORA Core/Supervisor
...
The image is tagged as `iora-developer-app:local` and used by the supervisor when Developer Mode is enabled.

## Integration with IORA System
...
3. **Runtime Stage**: Dedicated `iora-developer-app` target in Dockerfile
...
- Updated when IORA Core or Supervisor is updated
...
```
iora.app.installation_source=developer_app
```
...
- **Source**: `backend/iora-developer-app/src/main.rs`
- **Manifest**: `backend/iora-developer-app/manifest.json`
- **Dependencies**: `backend/iora-developer-app/Cargo.toml`

To modify the Developer App:
1. Edit source files in `backend/iora-developer-app/`
