...
//!
//! 1. **Assist Tools** (`assist_tools`): callable AI tools/functions that IORA
...
/// A callable AI tool that an app/plugin exposes to IORA Assist and pi.dev
/// coding agents. When the model decides to call the tool, IORA dispatches the
...
/// A single lifecycle hook: when `event` fires, IORA invokes `handler`.
#[derive(Debug, Clone, Serialize, Deserialize)]
