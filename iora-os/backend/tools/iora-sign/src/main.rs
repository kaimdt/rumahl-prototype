// iora-sign — build-host helper.  This does NOT ship on the device.
//
// Subcommands:
//   iora-sign keygen  --out-dir keys/                Generate key pair
//   iora-sign manifest --root <TARGET_DIR>          \
//                      --key keys/iora-release.key  \
//                      --out <TARGET_DIR>/etc/iora/manifest.json
//   iora-sign file    --key keys/iora-release.key --in <path>
//
// The manifest format matches what iora-verify consumes.
...
enum Cmd {
    ...
    Keygen {
        ...
        out_dir: PathBuf,
        #[arg(long, default_value = "iora-release")]
        name: String,
    },
...
fn cmd_manifest(root: &Path, key: &Path, out: &Path, extra: &str) -> Result<()> {
    ...
    let mut roots = vec![
        "opt/iora".to_string(),
        "etc/iora-version".to_string(),
        "etc/iora/build-info.json".to_string(),
        "etc/iora/iora-release.pub".to_string(),
        "etc/systemd/system/iora-stack.service".to_string(),
        "etc/systemd/system/iora-stack-watchdog.service".to_string(),
        "etc/systemd/system/iora-update-check.service".to_string(),
        "etc/systemd/system/iora-update-check.timer".to_string(),
        "etc/systemd/system/iora-verify.service".to_string(),
        "etc/systemd/system/iora-tamper.target".to_string(),
        "etc/systemd/system/iora-tamper-screen.service".to_string(),
        "etc/rauc/system.conf".to_string(),
        "usr/bin/iora-verify".to_string(),
        "usr/bin/iora-updater".to_string(),
    ];
    ...
    for rel in &roots {
        ...
        if !abs.exists() {
            eprintln!("[iora-sign] skip missing: /{rel}");
            continue;
        }
        if abs.is_file() {
    ...
    let manifest = Manifest {
        version: env_or("IORA_VERSION", "dev"),
        created: env_or(
            "SOURCE_DATE_EPOCH",
