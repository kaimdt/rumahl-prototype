...
//
// The IORA dashboard stores its global, system-wide settings in the
// `system_preferences` table inside the `iora_home` PostgreSQL DB.
// This module exposes a thin REST surface so an IDE / IORA Studio can
// read, write and delete those entries directly without going through
// iora-home auth (which is dashboard-user scoped).
//
...
pub async fn config_put(
    ...
    // Mirror iora-home's pattern: try UPDATE, fall back to INSERT.
