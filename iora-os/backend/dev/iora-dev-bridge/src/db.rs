...
//     EVERY IORA DATABASE, INCLUDING THE POSTGRES SUPERUSER ROLE.
//     It is gated by the dev-bridge auth middleware and only ever
//     ships on `IORA_OS_DEV=1` images. Production images do not
//     contain the iora-dev-bridge binary at all.
//
...
//
// Authentication to PG: we read the password from /etc/iora/db.password
// (the same file that first-boot writes; mode 0640, group iora). The
// dev bridge runs as root so it can read it. As a fallback we honour
// `$IORA_DB_PASSWORD` and `$DATABASE_URL` for local dev outside IORA OS.
...
const DB_PASSWORD_FILE: &str = "/etc/iora/db.password";
const DEFAULT_PG_USER: &str = "iora";
...
fn pg_options_for(database: &str) -> Result<PgConnectOptions> {
    ...
    let user = std::env::var("IORA_DB_USER").unwrap_or_else(|_| DEFAULT_PG_USER.to_string());
    let host = std::env::var("IORA_DB_HOST").unwrap_or_else(|_| DEFAULT_PG_HOST.to_string());
    let port: u16 = std::env::var("IORA_DB_PORT")
        .ok()
